...
function x888889_FindScriptTypeMail( sceneId, selfId, scripttype)
	...
	if scripttype == MAIL_COMMISIONSHOP then
		x888889_NotifyFailBox( sceneId, selfId, -1, "    ÄúµÄÔªŒšºÍ½ðŽÅÒÑ½›³É¹¦ÊÕÈ¡£¬Õˆ²éÊÕà]¼þ¡£" )
	elseif scripttype == MAIL_SHITUPRIZE then
...
function x888889_Mail_ShiTuPrize( sceneId, selfId, param0, param1, param2, param3 )
	...
	if 40 == plevel then
		if 10 < c40+1 then
			x888889_NotifySystemMsg(sceneId,selfId,"îIÈ¡ÃûŽŸª„È¯Ê§”¡£¬ÒÑ½›ß_µ½ÉÏÏÞ¡£")
			return
		end
	elseif 50 == plevel then
		if 10 < c50+1 then
			x888889_NotifySystemMsg(sceneId,selfId,"îIÈ¡ÌØ¼‰ÃûŽŸª„È¯Ê§”¡£¬ÒÑ½›ß_µ½ÉÏÏÞ¡£")
			return
		end
	end
	...
	if 1 == ret then
		...
		x888889_NotifySystemMsg(sceneId,selfId,"îIÈ¡ª„È¯³É¹¦")
		--ÔÚ³é½±ÈÕÖ¾ÖÐ¼ÇÂ¼....
		...
		x888889_NotifySystemMsg(sceneId,selfId,"›]ÓÐ×ã‰òµÄÈÎ„ÕµÀ¾ß™Ú¿Õég£¬îIÈ¡Ê§”¡")
		return
	end
