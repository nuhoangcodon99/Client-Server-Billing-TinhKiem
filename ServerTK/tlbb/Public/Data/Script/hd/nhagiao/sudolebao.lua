...
x000591_g_scriptId = 000591
x000591_g_isActive = 0

x000591_limit_times =30
x000591_g_rate_YQ = 99
x000591_g_rate_quatang = 40
x000591_g_rate_random = 80


x000591_g_quatang = { -- 8 item
...
x000591_g_LucHopLenhBai ={ --6 item
30504101,
30504102,
...
function x000591_OnDefaultEvent( sceneId, selfId, bagIndex )

end

function x000591_IsSkillLikeScript( sceneId, selfId)
	return 1; 
end

function x000591_CancelImpacts( sceneId, selfId )
	return 0; 
end

function x000591_OnConditionCheck( sceneId, selfId )
...
function x000591_OnDeplete( sceneId, selfId )
...
function x000591_OnActivateOnce( sceneId, selfId )
	if x000591_g_isActive == 0 then
	...
	if LuaFnIsItemAvailable( sceneId, selfId, bagpos ) ~= 1 then
		x000591_NotifyTip( sceneId, selfId, "V§t ph¦m b¸ khóa")
		return 0
	end
	local delok = LuaFnEraseItem( sceneId, selfId, bagpos )
	--x000591_NotifyTip( sceneId, selfId,"delok: "..delok)
	...
	x000591_NotifyTip( sceneId, selfId,  "Ði¬m hoÕt ðµng tích luÛ hi®n tÕi : "..trungthu2013_point+pointadd.." ði¬m hoÕt ðµng!" )
	...
	if curDayTime == floor( dayCount / 100) then
		if dayCount - curDayTime * 100 >= x000591_limit_times then --gi¾i hÕn 10 l¥n t£ng quà
			x000591_NotifyTip( sceneId, selfId,"Ðã hªt s¯ lßþt t£ng quà trong ngày nên các hÕ s¨ không nh§n ðßþc ph¥n thß·ng!" )
			return 1
		end
	end
	...
	if  randValue > x000591_g_rate_YQ then --ty le 3%
		local gift_id = x000591_g_yeuquyet_bt8[random(1,getn(x000591_g_yeuquyet_bt8))];
		local BagIndex = TryRecieveItem( sceneId, selfId, gift_id, QUALITY_MUST_BE_CHANGE )
		
				x000591_NotifyTip( sceneId, selfId,"Chúc m×ng, nh§n ðßþc quà t£ng:  #Y#{_ITEM" ..gift_id.. "}" )
	...
	elseif randValue > x000591_g_rate_quatang then
		local gift_id = x000591_g_quatang[random(1,8)];
		local BagIndex = TryRecieveItem( sceneId, selfId, gift_id, QUALITY_MUST_BE_CHANGE )
		if BagIndex ~= -1 then
			local	bindidx	=LuaFnItemBind( sceneId, selfId, BagIndex)
			if bindidx ~= 1 then
				x000591_NotifyTip( sceneId, selfId, "C¯ ð¸nh th¤t bÕi !" )
			else
				x000591_NotifyTip( sceneId, selfId,"Chúc m×ng, nh§n ðßþc quà t£ng:  #Y#{_ITEM" ..gift_id.. "}" )
	...
	local addthem_itemid = 30504004;
	local i = random(100);
	if i > x000591_g_rate_random then
		...
		if BagIndex ~= -1 then
			...
			if bindidx ~= 1 then
				...
				x000591_NotifyTip( sceneId,selfId, "Chúc m×ng, nh§n ðßþc quà t£ng:   #Y#{_ITEM" ..addthem_itemid.. "}" )
			end
		end
	end
...
function x000591_OnActivateEachTick( sceneId, selfId)
	return 1; 
end

function x000591_NotifyTip( sceneId, selfId, Msg )
