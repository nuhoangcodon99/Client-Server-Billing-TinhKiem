...
function  x016008_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"Ta s¨ chï cho b¢ng hæu cách ği«u khi¬n 1 con Trâu Tây TÕng !!")
		if	GetMenPai( sceneId, selfId) == 5 then
			if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 452))	then
				AddNumText(sceneId, x016008_g_ScriptId,"H÷c kî nång: cßŞi Mao Ngßu",12,40)
			end
			if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 461))	then
				AddNumText(sceneId, x016008_g_ScriptId,"H÷c kî nång: cßŞi BÕch Ngßu",12,60)
			end
		end
		AddNumText( sceneId, x016008_g_ScriptId, "V« cßŞi Trâu Tây TÕng", 11, 100 )
	EndEvent(sceneId)
...
function  x016008_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 40 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x016008_g_ScriptId,"Ğ°ng ı",-1,0)
      			AddNumText(sceneId, x016008_g_ScriptId,"TÕm bi®t",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if GetNumText() == 60 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x016008_g_ScriptId,"Ğ°ng ı",-1,1)
    			  AddNumText(sceneId, x016008_g_ScriptId,"TÕm bi®t",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
