...
function x002075_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif key == x002075_g_Key["stu"]	then
		...
		x002075_OnAsk( sceneId, selfId, targetId, x002075_g_Key["stu"], "  Ngß½i h÷c kÛ nång này c¥n phäi tiêu #{_MONEY" .. demandMoney .. "}, có tiªp tøc hay không?" )
	...
	elseif key == x002075_g_Key["upy"] then
		...
		if LuaFnGetMoney( sceneId, selfId )+GetMoneyJZ( sceneId, selfId ) < demandMoney then
			x002075_MsgBox( sceneId, selfId, targetId, "  C¥n #{_MONEY" .. demandMoney .. "} m¾i có th¬ nâng c¤p ðßþc tiªp" )
			return 0
		end
