...
function x002016_UpdateEventList( sceneId, selfId,targetId )
	...
	BeginEvent( sceneId )
		...
		if missionIndex ~= -1 then
			...
			if missionName ~= "段延慶任務" and IsMissionHaveDone( sceneId, selfId, x002016_g_ActivateMissionId ) > 0 then
				AddNumText( sceneId, x002016_g_scriptId, "段延慶任務", 3, 1 )
			end
...
function x002016_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- 段延庆任务
		if IsHaveMission( sceneId, selfId, x002016_g_RSMissionId ) > 0 then
			x002016_NotifyFailBox( sceneId, selfId, targetId, "    哦，你已有其他英雄的關係任務（劇情循環任務）尚未完成，請完成之後再來找我吧。" )
			return
		end
