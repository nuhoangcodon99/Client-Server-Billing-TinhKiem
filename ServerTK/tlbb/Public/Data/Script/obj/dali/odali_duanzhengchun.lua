...
x002004_g_ActivateMissionId = 8			-- ¼¤»îÌõ¼ş
x002004_g_SongXinScriptId = 006668		-- ËÍĞÅ
...
function x002004_UpdateEventList( sceneId, selfId,targetId )
	...
	BeginEvent(sceneId)	
		...
		if missionIndex ~= -1 then
			...
			if missionName ~= "¶Î×uÈÎ„Õ" and IsMissionHaveDone( sceneId, selfId, x002004_g_ActivateMissionId ) > 0 then
				AddNumText( sceneId, x002004_g_scriptId, "¶Î×uÈÎ„Õ", 3, 1 )
			end
...
function x002004_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- ¶ÎÓşÈÎÎñ
		if IsHaveMission( sceneId, selfId, x002004_g_RSMissionId ) > 0 then
			x002004_NotifyFailBox( sceneId, selfId, targetId, "    Å¶£¬ÄãÒÑÓĞÆäËûÓ¢ĞÛµÄêP‚SÈÎ„Õ£¨„¡ÇéÑ­­hÈÎ„Õ£©ÉĞÎ´Íê³É£¬ÕˆÍê³ÉÖ®ááÔÙíÕÒÎÒ°É¡£" )
			return
		end
		...
		--¿´ÏÂ½ñÌìµÄ¶ÎÓşÈÎÎñÊÇ²»ÊÇ×öÍê50´ÎÁË
