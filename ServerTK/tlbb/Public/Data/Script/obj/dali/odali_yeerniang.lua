...
function x002003_UpdateEventList( sceneId, selfId,targetId )
	...
	BeginEvent( sceneId )
		AddText(sceneId,"  "..PlayerName..PlayerSex..", ta l£ng l¨ nhìn ngß½i ği ği lÕi lÕi trong thành ĞÕi Lı, vµi vµi vàng vàng. Lúc nào cûng không nén n±i lòng mình mu¯n höi ngß½i, nhæng vi®c ngß½i làm, ğó là vì bän thân ngß½i hay là vì ngß¶i khác?");

		local missionIndex = GetScriptIDByMissionID( sceneId, selfId, x002003_g_RSMissionId )
		if missionIndex ~= -1 then
			local missionName = TGetMissionName( missionIndex )
			if missionName ~= "Ì“ÖñÈÎ„Õ" and IsMissionHaveDone( sceneId, selfId, x002003_g_ActivateMissionId ) > 0 then
				AddNumText( sceneId, x002003_g_scriptId, "Ì“ÖñÈÎ„Õ", 3, 1 )
			end
...
function x002003_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if num == 1 then										-- ĞéÖñÈÎÎñ
		if IsHaveMission( sceneId, selfId, x002003_g_RSMissionId ) > 0 then
			x002003_NotifyFailBox( sceneId, selfId, targetId, "    Å¶£¬ÄãÒÑÓĞÆäËûÓ¢ĞÛµÄêP‚SÈÎ„Õ£¨„¡ÇéÑ­­hÈÎ„Õ£©ÉĞÎ´Íê³É£¬ÕˆÍê³ÉÖ®ááÔÙíÕÒÎÒ°É¡£" )
			return
		end
		...
		--¿´ÏÂ½ñÌìµÄĞéÖñÈÎÎñÊÇ²»ÊÇ×öÍê50´ÎÁË
		...
		if nDayTime == CurDaytime  then -- µ±Ìì
			if nCount >= 50  then
				BeginEvent( sceneId )
					AddText( sceneId, "  ½ñÌìÒÑ½›ÂéŸ©ÄãÌ«¶àµÄÊÂÇéÁË£¬ŒÔÚÊÇß^Òâ²»È¥£¬Ã÷ÌìÔÙÂéŸ©Äã°É£¡" )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
				return
			end
