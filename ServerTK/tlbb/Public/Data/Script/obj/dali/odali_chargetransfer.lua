...
x400958_g_transfer_target =
{
	...
	[2] = {x = 332, z = 253, scene_num = 1}, 		--ËÕÖÝ
...
function x400958_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetLevel(sceneId, selfId) < 10 then
		BeginEvent( sceneId )
			AddText( sceneId, "  Các hÕ chßa ðü ðÆng c¤p 10!" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )	
		return	
	end
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		BeginEvent( sceneId )
			AddText( sceneId, "  ·Ç³£±§Ç¸£¬ÓÉì¶Â·Í¾Ì«ß^ßbßh£¬ÎÒ‚ƒß@Ñeß\\Ý”ÄÜÁ¦ÓÐÏÞ£¬ËùÒÔ²»½ÓÊÜ½Mê ‚÷ËÍ£¬ÕˆÄúëxé_ê Îé†ÎªšÇ°í°É£¡" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		BeginEvent(sceneId)
			AddText(sceneId, "  ÄãÓÐäîß\\Ø›Å“ÔÚÉí£¬ÎÒ‚ƒóAÕ¾²»ÄÜžéÄãÌá¹©‚÷ËÍ·þ„Õ¡£")
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId)
		return
	end
	...
	if id >=1 and id <= 6 then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x400958_g_ScriptId, "Không", 0, 100);
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId);
	elseif id == 100 then
		...
		if index == 3 then
			if GetLevel(sceneId, selfId) < 75 then
				BeginEvent(sceneId)
					AddText(sceneId, "  Các hÕ chßa ðü ðÆng c¤p 75!")
				EndEvent()
				DispatchEventList(sceneId, selfId, targetId)
				return
			end
