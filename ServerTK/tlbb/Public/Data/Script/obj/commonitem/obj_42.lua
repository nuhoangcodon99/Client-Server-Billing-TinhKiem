...
x331042_g_scriptId = 331042 
x331042_g_Pos = {{left={182,122},right={282,222}}}
x331042_g_tyle_thu_vip = 10 --ty le thu cuoi vip
...
x331042_g_Impact1 = 3003 
x331042_g_Impact2 = -1 
x331042_g_SpecailObj = 42

--**********************************

function x331042_OnDefaultEvent( sceneId, selfId, bagIndex )

end

--**********************************

function x331042_IsSkillLikeScript( sceneId, selfId)
	return 1;
end

--**********************************

function x331042_CancelImpacts( sceneId, selfId )
	return 0; 
end

--**********************************

function x331042_OnConditionCheck( sceneId, selfId )
...
function x331042_OnDeplete( sceneId, selfId )
...
function x331042_OnActivateOnce( sceneId, selfId )
	if(-1~=x331042_g_Impact1) then
		local posX,posZ = GetWorldPos(sceneId, selfId)

		CreateSpecialObjByDataIndex(sceneId, selfId, x331042_g_SpecailObj, posX, posZ, 0)
			
		--event ban phao hoa tet giap ngo
		local daytime = GetDayTime()
		local hour = GetHour()
		--x331042_Tips(sceneId, selfId, "sceneid:  "..sceneId..", daytime: "..daytime..", hour: "..hour)
		
		if sceneId == 121 and daytime < 14039 and hour >= 18 and hour <22 then
			for i = 1, getn( x331042_g_Pos ) do
				if x331042_g_Pos[i].left[1] <= posX and posX <= x331042_g_Pos[i].right[1] and x331042_g_Pos[i].left[2] <= posZ and posZ <= x331042_g_Pos[i].right[2] then  
					if LuaFnGetPropertyBagSpace( sceneId, selfId ) < 2  then
						x331042_Tips(sceneId, selfId, "C¥n 1 không gian túi ð° ô ðÕo cø  ð¬ nh§n ðßþc ph¥n thß·ng may m¡n" )
						return 1
					end
					local idx = random( getn(x331042_g_GiftTbl) )
					local ItemId = x331042_g_GiftTbl[idx]
					...
					if ran < x331042_g_tyle_thu_vip then
						local idx = random( getn(x331042_ItemList_ThuCuoi_VIP) )
						local ItemId = x331042_ItemList_ThuCuoi_VIP[idx]
						local BagIndex = TryRecieveItem( sceneId, selfId, ItemId, QUALITY_MUST_BE_CHANGE )
						if BagIndex ~= -1 then
							local playerName = GetName(sceneId, selfId)
							--LuaFnItemBind( sceneId, selfId, BagIndex) --co dinh item
...
function x331042_OnActivateEachTick( sceneId, selfId)
	return 1; 
end
function x331042_Tips(sceneId, selfId,Tips)
	BeginEvent(sceneId)
