...
function x332002_OnConditionCheck( sceneId, selfId )
    ...
    if PlayerLevel < 65 then    --ÁúÅÆÊ¹ÓÃµÈ¼¶×îµÍ65
        return 0
    end
    
    if TryCreatePet(sceneId,selfId,1) <= 0 then
        BeginEvent(sceneId)
			strText = "Xin l²i, cµt thú nuôi cüa các hÕ không ğü ch², xin hãy s¡p xªp và sØ døng lÕi"
...
function x332002_OnDeplete( sceneId, selfId )
	...
	if(0<LuaFnDepletingUsedItem(sceneId, selfId)) then
		--BroadMsgByChatPipe(sceneId, selfId, "#{_INFOUSR"..PlayerName.."}#ccc33ccÊ¹ÓÃÁË#{_INFOMSG"..szTransferItem.."}", 4)
		return 1;
	end
	return 0;
end
