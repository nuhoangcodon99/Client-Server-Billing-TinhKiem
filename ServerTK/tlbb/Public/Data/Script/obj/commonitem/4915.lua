...
--½Å±¾ºÅ
x334915_g_scriptId = 334915 --ÁÙÊ±Ğ´Õâ¸ö,ÕæÕıÓÃµÄÊ±ºòÒ»¶¨Òª¸Ä.
...
--AEµÄÄ¿±ê¹ØÏµ±ê¼Ç
x334915_g_standFlag = 1 -- 2:¶ÓÓÑ£¬ 1£ºÓÑ¾ü£¬ -1£ºµĞ¾ü
--AEÓ°ÏìÊıÄ¿ÏŞÖÆ
x334915_g_effectCount = 4 -- -1:²»ÏŞÖÆ
--Ğ§¹ûµÄID
x334915_g_Impact1 = 4915 --ÁÙÊ±Ğ´Õâ¸ö
...
--		LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x334915_g_Impact1, 0);
		...
		if(0<=targetId) then
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				if x334915_g_active_e ==1 then
					...
					if slot >=1 then
						...
						x334915_FailTips(sceneId, selfId,"Túi ğ° ğã ğ¥y")
					end
					...
					if slot >=1 then
						...
						x334915_FailTips(sceneId, selfId,"Túi ğ° cüa ğ¯i phß½ng ğã ğ¥y")
					end
				end
				--end hd add
			...
			BeginEvent(sceneId)
			    AddText(sceneId, "Các hÕ và ğ¯i phß½ng có ğµ thân thi®n tång 5");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			  
			  local	namSelf		= GetName( sceneId, selfId )
			  local	namTarget	= GetName( sceneId, targetId )
				local	str	= format( "#B#{_INFOUSR"..namSelf.."}#cffffff tñ tay ğßa#W [1 bông hoa h°ng]#cffffffcho #B#{_INFOUSR"..namTarget.."}#cffffff, ğ¡m ğu¯i nhìn #B#{_INFOUSR"..namTarget.."}#cffffff." )
