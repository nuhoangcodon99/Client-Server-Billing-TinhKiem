		...
		if selfGUID and targetGUID then
			BeginEvent(sceneId);
				if selfGUID == targetGUID then
					AddText(sceneId, "此為你自已的結婚請帖。憑此可以參加你的婚禮，快發給你的朋友吧。");
				else
					local ret, targetName = LuaFnGetItemCreator(sceneId, selfId, bagIndex);
					if ret and targetName and ret == 1 then
						AddText(sceneId, "此為"..targetName.."發送給您的請帖。可以憑此請帖去#G洛陽（177，94）#R喜來樂#W處參加婚禮。#r在婚禮結束前，可以在婚禮會場內找喜來樂用請帖兌換禮品哦。");
					else
						AddText(sceneId, "此為未知角色發送給您的請帖。可以憑此參加婚禮，記得婚禮結束之後可以用請帖兌換。");
					end
				end
			EndEvent(sceneId);
