...
--½Å±¾ºÅ
x334916_g_scriptId = 334916 --ÁÙÊ±Ğ´Õâ¸ö,ÕæÕıÓÃµÄÊ±ºòÒ»¶¨Òª¸Ä.

x334916_g_active_e =0 
...
--AEµÄÄ¿±ê¹ØÏµ±ê¼Ç
x334916_g_standFlag = 1 -- 2:¶ÓÓÑ£¬ 1£ºÓÑ¾ü£¬ -1£ºµĞ¾ü
--AEÓ°ÏìÊıÄ¿ÏŞÖÆ
x334916_g_effectCount = 4 -- -1:²»ÏŞÖÆ
--Ğ§¹ûµÄID
x334916_g_Impact1 = 4916 --ÁÙÊ±Ğ´Õâ¸ö
x334916_g_Impact2 = -1 --²»ÓÃ
			...
			if LuaFnIsFriend(sceneId, targetId, selfId) > 0 then
				...
				if x334916_g_active_e ==1 then
					...
					if slot >=1 then
						...
						x334916_FailTips(sceneId, selfId,"Túi ğ° ğã ğ¥y")
					end
					...
					if slot >=1 then
						...
						x334916_FailTips(sceneId, selfId,"Túi ğ° cüa ğ¯i phß½ng ğã ğ¥y")
					end
				end
				--end hd add
				...
				BeginEvent(sceneId)
			    AddText(sceneId, "Ğµ hæu häo giæa ngß½i và ğ¯i phß½ng ğã tång lên 50");
			  EndEvent(sceneId)
...
function x334916_FailTips(sceneId, selfId,Tips)
	BeginEvent(sceneId)
