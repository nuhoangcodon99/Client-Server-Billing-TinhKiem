	...
	if eventId == x000111_g_ScriptId then
		...
		elseif GetNumText() == 123 then
			...
			if HumanLevel < 30 then--Ð¡ÓÚ45¼¶
				x000111_MyMsgBox( sceneId, selfId, targetId, "C¥n ðÕt c¤p ðµ 30 tr· lên" );
				return
			end
			--¼ì²âÍæ¼Ò²Î¼Ó»î¶¯µÄÊ±¼ä
			...
			if nDayTime == nDay then
				x000111_MyMsgBox(sceneId, selfId, targetId, "Xin l²i, ðã qua th¶i gian hoÕt ðµng");
				return
			end
			...
			-- ÌáÊ¾ÐÅÏ¢
			x000111_MyMsgBox(sceneId, selfId, targetId, "Các hÕ nh§n ðßþc Sß ð° ðÕi l­ bao !");
			BeginEvent(sceneId)
				strText = "Các hÕ nh§n ðßþc Sß ð° ðÕi l­ bao !";
				AddText(sceneId, strText)
			EndEvent(sceneId)
