...
--ËùÓµÓÐµÄÊÂ¼þIDÁÐ±í
x000118_g_eventList={400918, 400963}	--	È¥ÐþÎäµº¡¢È¥Ê¥ÊÞÉ½
...
function x000118_UpdateEventList( sceneId, selfId,targetId )
	...
	--AddNumText( sceneId, x000118_g_scriptId, "Ãâ·ÑÎÞµÐ", 6, 10 )	--È¥³ýÃâ·ÑÎÞµÐ¹¦ÄÜ--del by Heanqi
	...
	--AddNumText(sceneId,x000118_g_scriptId,"Ki¬m tra ðµ trß·ng thành trân thú",6,3)
	AddNumText(sceneId,x000118_g_scriptId,"#{XXWD_8916_07}",11,5)
	AddNumText(sceneId,x000118_g_scriptId,"Linh Thú Ðan Hþp Thành",6,6)
	AddNumText(sceneId,x000118_g_scriptId,"Gi¾i thi®u v« Linh Thú Ðan Hþp Thành",11,7)
--	AddNumText(sceneId,x000118_g_scriptId,"Làm thª nào ð¬ hu¤n luy®n trân thú nhanh",11,8)
	EndEvent(sceneId)
...
--		BeginEvent( sceneId )
--		AddText( sceneId, "  ²éÑ¯ÕäÊÞµÄ³É³¤ÂÊ£¬²éÑ¯Ò»´ÎÐèÒªÊÕÈ¡#{_MONEY100}µÄ·ÑÓÃ¡£" )
...
--	elseif GetNumText() == 4 then
	...
	elseif GetNumText() == 6 then
		BeginUICommand( sceneId )
			UICommand_AddInt( sceneId, targetId )
		EndUICommand( sceneId )
		DispatchUICommand( sceneId, selfId, 19824 )	--Linh Thú Ðan Hþp Thành
...
--×¢Òâ£¬ÕâÀïÕâÑùÐ´Ê¡ÂÔÁËÔÚeventlistÖÐËÑË÷scriptIdÕâÒ»²¿·Ö£¬¿ÉÄÜ»áÓÐÎÊÌâ
...
function x000118_ChangeItem(sceneId, selfId, targetId, key)
	...
	--ÓµÓÐÊýÁ¿³¬¹ý¶ø¿ÉÓÃÊýÁ¿²»×ãËµÃ÷ÊÇÓÐÒÑ¾­¼ÓËøµÄ¡£Õâ¸öµØ·½ÊÇÌØÀýÅÐ¶Ï£¬²ß»®¼á³ÖÒª¼ÓÕâ¸öÌáÊ¾£¬ÈÏÎªÔª±¦°ÚÌ¯ÎïÆ·ÌáÊ¾Òª×¼È·¡£add by hukai#40690
	...
	if LuaFnDelAvailableItem( sceneId, selfId, x000118_g_change_needitem.itemindex, x000118_g_change_needitem.itennum ) == 1 then
		...
		AddItem( sceneId, nTargetitem, 1 )
			...
			if Ret > 0 then
				...
				local strNotice = format("Hoán ð±i thành công, các hÕ nh§n ðßþc #H#{_ITEM%d}#W", nTargetitem)
    	...
    	x000118_ShowNotice( sceneId, selfId, "Kh¤u tr× v§t ph¦m th¤t bÕi")
		return
	end
end
...
function x000118_ShowRandomSystemNotice( sceneId, selfId, strItemInfo )
	...
	local PlayerName = GetName(sceneId,selfId)
	local str = format( "#{_INFOUSR%s}#PL¸ch tân thiên tân vÕn kh±, cu¯i cùng tìm ðßþc 200 cái #Y#{_ITEM30501318}#P ðªn #GLÕc Dß½ng (182,155)#Y Vân Hàm Nhi #Phoán ð±i thành công#{_INFOMSG%s}#P.", PlayerName, strItemInfo )
