...
--	PrintStr("World_Time="..World_Time.." world scene_id="..x311009_g_Monger_Region[World_Time+1].Scene_Id.." sceneId="..sceneId)
	...
	if x311009_g_Monger_Region[World_Time+1].Scene_Id ~= sceneId then
		BeginEvent(sceneId)
			AddText(sceneId,"  Xin l²i, hi®n ta không th¬ cung c¤p phøc vø này cho ngß½i")
		EndEvent(sceneId)
	...
	BeginEvent(sceneId)
	AddText(sceneId,"  Xin chào, ta kinh doanh các loÕi hàng hóa ch· Tào v§n · ðây")

	if IsHaveMission(sceneId,selfId,4021) > 0 then
	
		if	EVENT_NOW == 1 then
			AddNumText(sceneId,x311009_g_scriptId,"Vào chþ ðen",7,255)
		elseif	EVENT_NOW == 2 then
			AddNumText(sceneId,x311009_g_scriptId,"Giúp ðµi giá",6,EVENT_NOW)
		elseif	EVENT_NOW == 3 then
			AddNumText(sceneId,x311009_g_scriptId,"Giúp chª ngñ th¸ trß¶ng",6,EVENT_NOW)
		end
