...
--x000082_g_mpInfo[9]	= { "Mµ Dung", 435,  29, 137, 9}

x000082_g_Yinpiao = 40002000 

x000082_g_Impact_NotTransportList = { 5929, 5944 } -- ½ûÖ¹´«ËÍµÄImpact
x000082_g_TalkInfo_NotTransportList = { "#{GodFire_Info_062}", "#{XSHCD_20080418_099}" } -- ½ûÖ¹´«ËÍµÄImpactÌáÊ¾ÐÅÏ¢
...
function x000082_OnDefaultEvent( sceneId, selfId,targetId )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x000082_g_ScriptId, "Thành th¸ - Tô Châu - Thiªt tß¶ng ph¯", 9, 1007 )
		...
		--AddNumText( sceneId, x000082_g_ScriptId, "³ÇÊÐ - ÂåÑô - ¾ÅÖÝÉÌ»á", 9, 1006 )
			AddNumText( sceneId, x000082_g_ScriptId, "Ði ðªn các môn phái", 9, 1012 )
		--for i, mp in x000082_g_mpInfo do
			--AddNumText( sceneId, x000082_g_ScriptId, "ÃÅÅÉ - "..mp[1], 9, i )
		--end
...
function x000082_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x000082_MsgBox( sceneId, selfId, targetId, "  Thành viên trong nhóm ngß½i có ngß¶i có nhi®m vø ch· hàng b¢ng ðß¶ng thuÖ, d¸ch trÕm chúng ta không th¬ cung c¤p d¸ch vø cho ngß½i" )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x000082_MsgBox( sceneId, selfId, targetId, "  Ngß½i có nhi®m vø ch· hàng b¢ng ðß¶ng thuÖ, d¸ch trÕm chúng ta không th¬ cung c¤p d¸ch vø cho ngß½i" )
		return
	end
	...
	if arg == 1000 then		--·µ»ØÃÅÅÉ
		if id == 9 and  GetHumanMenpaiPoint(sceneId, selfId) == 0 then
			x000082_MsgBox( sceneId, selfId, targetId, "  Ngß½i vçn chßa gia nh§p b¤t kÏ môn phái nào!" )
		else
