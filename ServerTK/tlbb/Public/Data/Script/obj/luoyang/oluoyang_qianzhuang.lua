...
function x000076_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText(sceneId, 102, "Gi¾i thi®u Bäo hi¬m sß½ng",11,3)
...
function x000076_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif eventId == 102 then
		...
		BeginUICommand(sceneId)
			...
			UICommand_AddString(sceneId,"Nªu mu¯n mua rß½ng ğñng ğ°, ngß½i c¥n phäi t¯n #{_EXCHG"..x000076_g_Box[BoxNum].Cost.."}.")			
...
function x000076_MoneyChange ( sceneId, selfId, targetId, Cost, Money, MoneyJZ )
  ...
  local nMoneyJZ = 0
	...
	if (MoneyJZ >= Cost) then
		...
		x000076_MsgBox( sceneId, selfId, "Các hÕ ğã bö ra  #{_EXCHG"..nMoneyJZ.."} ğ¬ mua mµt rß½ng ğ° m¾i" )
	...
	elseif (MoneyJZ > 0) and (Money > 0) and (MoneyJZ + Money) >= Cost then
		...
		x000076_MsgBox ( sceneId, selfId, "Các hÕ ğã bö ra  #{_EXCHG"..nMoneyJZ.."}" )
		x000076_MsgBox ( sceneId, selfId, "Các hÕ ğã bö ra  #{_MONEY"..nMoney.."}"  )
		x000076_MsgBox ( sceneId, selfId, "Các hÕ nh§n ğßşc mµt rß½ng ğ° m¾i" )
	...
	elseif (MoneyJZ == 0) and (Money >= Cost) then
		...
		x000076_MsgBox ( sceneId, selfId, "Các hÕ ğã bö ra  #{_MONEY"..nMoney.."} ğ¬ mua mµt rß½ng ğ° m¾i" )
