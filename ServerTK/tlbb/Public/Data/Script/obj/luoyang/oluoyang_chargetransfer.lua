-- created by ×ó´ºÎ°

x400956_g_ScriptId = 400956; --½Å±¾ºÅ
x400956_g_name	="Ü‡‚÷Áx";

--´«ËÍÄ¿±ê
x400956_g_transfer_target =
{
	[1] = {x = 332, z = 253, scene_num = 1}, 		--ËÕÖİ
	[2] = {x = 259, z = 86, scene_num = 2}, 		--´óÀí
	[3] = {x = 294, z = 90, scene_num = 186}, 	--Â¥À¼
	[4] = {x = 206, z = 266, scene_num = 34}, 	--ÄÏº£
	[5] = {x = 186, z = 43, scene_num = 28},		--ÄÏÚ¯
	[6] = {x = 158, z = 113, scene_num = 22}, 	--³¤°×É½
}

-- ÊÕ·Ñ½ğ¶î
x400956_g_transfer_cost = 5000; -- 50Òø½»×Ó

--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x400956_OnDefaultEvent( sceneId, selfId, targetId )
	x400956_UpdateEventList( sceneId, selfId, targetId );
end

--**********************************
--ÊÂ¼şÁĞ±í
--**********************************
function x400956_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent(sceneId);
		AddText( sceneId, "#{SFCS_80828_01}" );
		AddNumText(sceneId, x400956_g_ScriptId, "Tô Châu", 9, 1);
		AddNumText(sceneId, x400956_g_ScriptId, "ĞÕi Lı", 9, 2);
		AddNumText(sceneId, x400956_g_ScriptId, "Lâu Lan", 9, 3);
		AddNumText(sceneId, x400956_g_ScriptId, "Nam Häi", 9, 4);
		AddNumText(sceneId, x400956_g_ScriptId, "Nam Chiêu", 9, 5);
		AddNumText(sceneId, x400956_g_ScriptId, "Trß¶ng BÕch S½n", 9, 6);
	EndEvent()
	DispatchEventList(sceneId, selfId, targetId);
end

--**********************************
--ÊÂ¼şÁĞ±íÑ¡ÖĞÒ»Ïî
--**********************************
function x400956_OnEventRequest( sceneId, selfId, targetId, eventId )
	--ÅÜÉÌÏà¹Ø
	if GetItemCount(sceneId, selfId, 40002000)>=1  then
		BeginEvent( sceneId )
			AddText( sceneId, "  ÄãÉíÉÏÓĞãyÆ±£¬ÕıÔÚÅÜÉÌ£¡ÎÒ²»ÄÜÍÖúÄã¡£" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	--¶ÓÎéÏà¹Ø
	if GetTeamId(sceneId,selfId)>=0 and 
		IsTeamFollow(sceneId, selfId)==1 and
		LuaFnIsTeamLeader(sceneId,selfId)==1 then
		BeginEvent( sceneId )
			AddText( sceneId, "  ·Ç³£±§Ç¸£¬ÓÉì¶Â·Í¾Ì«ß^ßbßh£¬ÎÒ‚ƒß@Ñeß\\İ”ÄÜÁ¦ÓĞÏŞ£¬ËùÒÔ²»½ÓÊÜ½Mê ‚÷ËÍ£¬ÕˆÄúëxé_ê Îé†ÎªšÇ°í°É£¡" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	--äîÔËÏà¹Ø
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		BeginEvent(sceneId)
			AddText(sceneId, "  ÄãÓĞäîß\\Ø›Å“ÔÚÉí£¬ÎÒ‚ƒóAÕ¾²»ÄÜéÄãÌá¹©‚÷ËÍ·ş„Õ¡£")
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId)
		return
	end
	--
	local id = GetNumText();
	if id >=1 and id <= 6 then
		BeginEvent(sceneId)
			--"±¾´Î´«ËÍÊÕÈ¡50Òø½»×Ó£¬ÄãÒª´«ËÍÂğ£¿"
			AddText(sceneId, "#{FFCS_081210_1}")
			if id == 1 then	
				AddNumText(sceneId, x400956_g_ScriptId, "Duy®t", 0, 11);
			elseif id == 2 then
				AddNumText(sceneId, x400956_g_ScriptId, "Duy®t", 0, 21);
			elseif id == 3 then
				AddNumText(sceneId, x400956_g_ScriptId, "Duy®t", 0, 31);
			elseif id == 4 then
				AddNumText(sceneId, x400956_g_ScriptId, "Duy®t", 0, 41);
			elseif id == 5 then
				AddNumText(sceneId, x400956_g_ScriptId, "Duy®t", 0, 51);
			elseif id == 6 then
				AddNumText(sceneId, x400956_g_ScriptId, "Duy®t", 0, 61);
			end
			AddNumText(sceneId, x400956_g_ScriptId, "Không", 0, 100);
		EndEvent()
		DispatchEventList(sceneId, selfId, targetId);
	elseif id == 100 then
		BeginUICommand( sceneId )
		UICommand_AddInt( sceneId, targetId )
		EndUICommand( sceneId )
		DispatchUICommand( sceneId, selfId, 1000 )		
	else
		local index = floor(id/10);
		--Â¥À¼75¼¶ÏŞÖÆ
		if index == 3 then
			if GetLevel(sceneId, selfId) < 75 then
				BeginEvent(sceneId)
					AddText(sceneId, "  Các hÕ chßa ğü ğÆng c¤p 75!")
				EndEvent()
				DispatchEventList(sceneId, selfId, targetId)
				return
			end
			--Í¬Æï´øĞ¡ºÅÏŞÖÆ
			if LuaFnGetDRideFlag(sceneId, selfId) == 1 then
				local objId = LuaFnGetDRideTargetID(sceneId, selfId);
				if objId ~= -1 and GetLevel(sceneId, objId) < 75 then
					BeginEvent(sceneId)
						AddText(sceneId, "#{SRCS_090203_1}")
					EndEvent()
					DispatchEventList(sceneId, selfId, targetId)
					return					
				end
			end
		end
		--[tx44121]bugÕâÀïÃ»ÓĞÉèÖÃ×îµÍ¼¶±ğ£¬µ¼ÖÂµÍ¼¶±ğ¿ÉÍ¨¹ıË«Æï³Ë½ø¸ß¼¶±ğ³¡¾°
		local minLevel = 10
		if index == 3 then
			minLevel = 75
		end
		if LuaFnGetDRideFlag(sceneId, selfId) == 1 then
			local objId = LuaFnGetDRideTargetID(sceneId, selfId);
			if objId ~= -1 and GetLevel(sceneId, objId) < minLevel then
				local Tip = format("#{CQTS_90227_1}%d#{CQTS_90227_2}", minLevel)
				x400956_NotifyFailTips( sceneId, selfId, targetId, Tip )
				return					
			end
		end	
		--[/tx44121]	
			
		local pos_x = x400956_g_transfer_target[index]["x"];
		local pos_z = x400956_g_transfer_target[index]["z"];
		local scene_num = x400956_g_transfer_target[index]["scene_num"];
			
		local nMoneyJZ = GetMoneyJZ (sceneId, selfId)
		local nMoney = GetMoney (sceneId, selfId)

		--Ç®¹»Âğ£¿
		if (nMoneyJZ + nMoney) >= x400956_g_transfer_cost then 
				
			-- ÊÕ·Ñ
			-- Ê¹ÓÃ´øÓÅÏÈ¼¶µÄ½ğÇ®ÏûºÄº¯Êı
			if LuaFnCostMoneyWithPriority (sceneId, selfId, x400956_g_transfer_cost) == -1 then
				BeginEvent(sceneId)
					AddText(sceneId, "ÊÕÙMÊ§”¡£¡");
				EndEvent()
				DispatchMissionTips(sceneId, selfId)
				return
			else
				CallScriptFunction((400900), "TransferFunc",sceneId, selfId, scene_num, pos_x, pos_z, minLevel); 
			end

		-- Ç®²»¹»
		else
			BeginEvent(sceneId)
				AddText(sceneId, "½ğåX²»×ã");
			EndEvent()
			DispatchEventList(sceneId, selfId, targetId)
			return

		end
	end	
end
--**********************************
-- ÆÁÄ»ÖĞ¼äĞÅÏ¢ÌáÊ¾
--**********************************
function x400956_NotifyFailTips( sceneId, selfId, targetId, Tip )
	BeginEvent( sceneId )
		AddText( sceneId, Tip )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
end
