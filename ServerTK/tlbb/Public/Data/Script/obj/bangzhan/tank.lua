...
function x402302_UpdateEventList( sceneId, selfId, targetId )
	...
	if isAguild == isNPCAguild then --Íæ¼Ò°ïÅÉºÍÕ½³µ°ïÅÉÊÇ·ñÏàÍ¬
		BeginEvent(sceneId);
			...
			AddNumText( sceneId, x402302_g_ScriptId, "Xác ð¸nh", 9, x402302_g_eventId_yes);
			AddNumText( sceneId, x402302_g_ScriptId, "Bö qua", 8, x402302_g_eventId_no);
		EndEvent(sceneId);
...
function x402302_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if isAguild ~= isNPCAguild or index == -1 then --Íæ¼Ò°ïÅÉºÍÕ½³µ°ïÅÉÊÇ·ñÏàÍ¬£¬ÊÇ·ñÕÒµ½Æô¶¯Õ½³µÀàÐÍ
		return
	end
