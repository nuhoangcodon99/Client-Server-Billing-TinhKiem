...
--		AddItemToBox(sceneId, itemBoxId, QUALITY_MUST_BE_CHANGE, 1, 40004414);
	...
	if sceneId == 123 or sceneId == 126 or sceneId == 148 or sceneId == 149 then
		...
		if randMessage == 1 then
	   		message = format("#G%s#P tÕi #Y%s#P#{BroadMsg_YanWangGuMu_00}#W#{_INFOUSR%s}#P#{BroadMsg_YanWangGuMu_01}#Y%s#P#{BroadMsg_YanWangGuMu_02}", namScene, namMonster, namHuman, namMonster );
		elseif randMessage == 2 then		
	...
	if sceneId >= 159 and sceneId <= 167 then
		...
		if randMessage == 1 then
	   		message = format("#{_INFOUSR%s}#P tÕi #Y%s#P tÕi #G%s#P#{BroadMsg_15}#Y%s#P#{BroadMsg_16}", namHuman, namMonster, namScene, namMonster);
		elseif randMessage == 2 then		
			message = format("#{_INFOUSR%s}#P tÕi #G%s#P#{BroadMsg_11}#Y%s.", namHuman, namScene, namMonster);
		else
