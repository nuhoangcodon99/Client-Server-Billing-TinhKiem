...
function x501000_OnDie(sceneId, objId, killerId)
	...
	--hd add drop 26/5/2015
	if petDropInfo == 11353 then --check chi la Long qui
		if MAP_ACTIVE==1 then
			...
			if LuaFnHasTeam( sceneId, killerId ) == 0 then
				if Rate <= MAP_RATE then
					AddMonsterDropItem( sceneId, objId, killerId, DROP_BOSS_ALLMAP[rand] )
				end
			else
				...
				for i=0, TeammateCount-1  do
					...
					if Rate <= MAP_RATE then
						AddMonsterDropItem( sceneId, objId, nPlayerId, DROP_BOSS_ALLMAP[rand] )
					end
				end
			end
		end
	end
	--end hd add drop 26/5/2015
