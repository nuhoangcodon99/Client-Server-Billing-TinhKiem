...
function x713550_OnDefaultEvent( sceneId, selfId, targetId, zhiwuId )
	...
	if num == 0 then
		BeginEvent(sceneId)
			AddText(sceneId, "Nß¾c và ð¤t s¨ hªt, ð« ngh¸ hãy bäo v® tñ nhiên ".."|| "..PlantFlag_X.." || "..PlantFlag_Z)
		EndEvent(sceneId)
	...
	if x713550_g_energy < EnergyCost then
		BeginEvent(sceneId)
			AddText(sceneId, "Các hÕ không ðü hoÕt lñc!")
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		return
	end
	...
	SetItemBoxMaxGrowTime(sceneId,ItemBoxId02,45000)--45000
