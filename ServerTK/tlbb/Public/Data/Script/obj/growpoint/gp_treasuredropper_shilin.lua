--Éú³¤µã
--¶ÔÓ¦Éú»î¼¼ÄÜ£º-1
--Ê¯ÁÖ±¦ÎïµôÂäÆ÷
--½Å±¾ºÅ******(712515)

--Ò»¸ö±¦ÏäÀïº¬ÓĞ¼¸¸öÎïÆ·
x712515_g_DropNumTable  = {
	{num = 1, odd = 0.2},	--1¸öµÄ¼¸ÂÊ
	{num = 2, odd = 0.4},	--2¸öµÄ¼¸ÂÊ
	{num = 3, odd = 0.2},	--3¸öµÄ¼¸ÂÊ
	{num = 4, odd = 0.15},	--4¸öµÄ¼¸ÂÊ
	{num = 5, odd = 0.05},	--5¸öµÄ¼¸ÂÊ
}

x712515_g_TickCreate_Msg = ""

--Ã¿ÖÖÎïÆ·ÊÇÄÄÀàÎïÆ·£¬ÇÒ´ËÀàÎïÆ·¶ÑµşÊıÁ¿ÊÇ¼¸¸ö
x712515_g_DropTable = {
	--ÑªÒ©Àà
	{
		itemType = 1, 	--ÑªÒ©ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.293, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.2},				--1¸öµÄ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2¸öµÄ¼¸ÂÊ
							{num = 3, odd = 0.2},				--3¸öµÄ¼¸ÂÊ
							{num = 4, odd = 0.15},			--4¸öµÄ¼¸ÂÊ
							{num = 5, odd = 0.05},			--5¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30001004,30003006,30101036,
					},
	},
	--À¶Ò©Àà
	{
		itemType = 2, 	--À¶Ò©ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.294, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.2},				--1¸öµÄ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2¸öµÄ¼¸ÂÊ
							{num = 3, odd = 0.2},				--3¸öµÄ¼¸ÂÊ
							{num = 4, odd = 0.15},			--4¸öµÄ¼¸ÂÊ
							{num = 5, odd = 0.05},			--5¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30002004,30101046,
					},
	},
	--¿ÚÁ¸
	{
		itemType = 3, 	--¿ÚÁ¸µÄÀàĞÍ±êÊ¶
	 	odd = 0.2, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.2},				--1¸öµÄ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2¸öµÄ¼¸ÂÊ
							{num = 3, odd = 0.2},				--3¸öµÄ¼¸ÂÊ
							{num = 4, odd = 0.15},			--4¸öµÄ¼¸ÂÊ
							{num = 5, odd = 0.05},			--5¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30602006,30603006,30604006,30605006,
					},
	},
	--²ÄÁÏÀà
	{
		itemType = 4, 	--²ÄÁÏÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.1, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 2, odd = 0.2},				--2¸öµÄ¼¸ÂÊ
							{num = 4, odd = 0.4},				--4¸öµÄ¼¸ÂÊ
							{num = 6, odd = 0.2},				--6¸öµÄ¼¸ÂÊ
							{num = 8, odd = 0.15},			--8¸öµÄ¼¸ÂÊ
							{num = 10, odd = 0.05},			--10¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						20101014,20101015,20101016,20102006,20102018,20103006,20104006,20105006,
					},
	},
	--°éÉúÀà
	{
		itemType = 12, 	--°éÉúÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.02, 			--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.3},				--1¸öµÄ¼¸ÂÊ
							{num = 2, odd = 0.4},				--2¸öµÄ¼¸ÂÊ
							{num = 3, odd = 0.3},				--3¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						20102030,20103018,20103030,20103042,20103054,20105018,
					},
	},
	--°×É«ÎäÆ÷Àà
	{
		itemType = 5, 	--°×É«ÎäÆ÷ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.045, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						10100006,10101006,10102006,10103006,10104006,10105006,
					},
	},
	--À¶É«ÎäÆ÷Àà
	{
		itemType = 10, 	--À¶É«ÎäÆ÷ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						 10200006,10201006,10202006,10203006,10204006,10205006,
					},
	},
	--°×É«×°±¸Àà
	{
		itemType = 6, 	--°×É«×°±¸ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.045, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						10110015,10110016,10110017,10111015,10111016,10111017,10112015,10112016,10112017,10113015,10113016,10113017,10120006,10121006,10122006,
					},
	},
	--À¶É«×°±¸Àà
	{
		itemType = 11, 	--À¶É«×°±¸ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						 10210011,10211011,10212011,10213011,10220006,10221006,10222006,
					},
	},
	--±¦Ê¯Àà
	{
		itemType = 7, 	--±¦Ê¯ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.001, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						50101001,50101002,50102001,50102002,50102003,50102004,50103001,50104002,50111001,50111002,50112001,50112002,50112003,50112004,50113001,50113002,50113003,50113004,50113005,50114001,
					},
	},
	--Åä·½Àà
	{
		itemType = 8, 	--Åä·½ÀàµÄÀàĞÍ±êÊ¶
	 	odd = 0.002, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 1.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÎïÆ·ĞòÁĞºÅ±í
						30303055,30303056,30303057,30303058,30304045,30304046,30304047,30304048,30304049,30304050,30304051,30304052,30304053,
					},
	},
	--ÕäÊŞ
	{
		itemType = 9, 	--ÕäÊŞµÄÀàĞÍ±êÊ¶
	 	odd = -1.0, 		--³öÏÖ¼¸ÂÊ
	 	numOdd = {
							{num = 1, odd = 0.0},				--1¸öµÄ¼¸ÂÊ
						 },
		idx = {	--ÕäÊŞĞòÁĞºÅ±í
						3009,3019,3029,
					},
	},
}

--±¦Îï»ØÊÕÊ±¼ä2006-8-21 17:33 ĞŞ¸ÄÎªÓÉ¹ÜÀíÆ÷´«µİ½øÀ´

--¸ù¾İ±¾´ÎËæ»úÊıÀ´ÕÒµ½±íÖĞ¶ÔÓ¦µÄË÷Òı
function x712515_GetTableIndexByOdd(tb)
	local oddNow = random();
	
	local base = 0.0;
	for i = 1, getn(tb) do
		if(tb[i].odd + base >= oddNow) then
			return i;
		end
		base = base + tb[i].odd;
	end
	
	return nil;
end

--Éú³Éº¯Êı¿ªÊ¼************************************************************************
--Ã¿¸öItemBoxÖĞ×î¶à10¸öÎïÆ·£¬·µ»ØÖµ -1 Ö±½Ó»ØÊÕÉú³¤µã£¬²»Õ¼ÓÃ
function x712515_OnCreate( sceneId, growPointType, x, y, dur)
	local ItemBoxId = nil
	local delDur = dur - 60000;	--±¦Ïä»òÕäÊŞµÄÏûÊ§Ê±¼ä
	
	-- ±¦ÏäÀïÓĞ¼¸¸öÎïÆ·
	local numIdx = x712515_GetTableIndexByOdd(x712515_g_DropNumTable)
	if(numIdx) then
		for i = 1, x712515_g_DropNumTable[numIdx].num do
			--Õâ¸öÎïÆ·ÊÇÊ²Ã´ÀàĞÍµÄÎïÆ·
			local showIdx = x712515_GetTableIndexByOdd(x712515_g_DropTable)
			local dropNumIdx = nil
			local itemId = nil
			if(showIdx) then
				--Õâ¸öÀàĞÍµÄÎïÆ·ÊıÁ¿ÊÇ¼¸¸ö
				dropNumIdx = x712515_GetTableIndexByOdd(x712515_g_DropTable[showIdx].numOdd)
				if(dropNumIdx and 9 ~= x712515_g_DropTable[showIdx].itemType) then
					local itemNum = getn(x712515_g_DropTable[showIdx].idx)
					if(itemNum and 1<=itemNum) then
						local itemIdx = floor(random(1, itemNum))
						itemId = x712515_g_DropTable[showIdx].idx[itemIdx]
						--Ìí¼ÓµôÂäÏäºÍÎïÆ·µ½³¡¾°
						for k = 1, x712515_g_DropTable[showIdx].numOdd[dropNumIdx].num do
							if(nil == ItemBoxId) then
								ItemBoxId = ItemBoxEnterScene( x, y, growPointType, sceneId, QUALITY_MUST_BE_CHANGE,1, itemId )
								print( sceneId .. "  (b¯i) cänh s¯ (" .. x .. ", " .. y .. ") ch² ğó xu¤t hi®n 1 rß½ng v§t ph¦m"..
											 "("..x712515_g_DropNumTable[numIdx].num..")" )
							else
								AddItemToBox( sceneId, ItemBoxId, QUALITY_MUST_BE_CHANGE,1, itemId )
							end
						end
					end
				elseif(dropNumIdx and 9 == x712515_g_DropTable[showIdx].itemType) then
					local petIdx = floor(random(1, getn(x712515_g_DropTable[showIdx].idx)))
					local petId = x712515_g_DropTable[showIdx].idx[petIdx]
					--Ìí¼ÓÕäÊŞµ½³¡¾°
					for k = 1, x712515_g_DropTable[showIdx].numOdd[dropNumIdx].num do
						local PetObjId = nil
						PetObjId = CreatePetOnScene( sceneId, petId, x, y )
						SetCharacterDieTime( sceneId, PetObjId, delDur )
						print( sceneId .. "  (b¯i) cänh s¯ (" .. x .. ", " .. y .. ") ch² ğó xu¤t hi®n 1 con trân thú "..
									 GetName( sceneId, PetObjId ) .. "." )
					end 
				end
			end --end of if(showIdx) then
			
			--¼ÇÂ¼ÎïÆ·µôÂä°üËù°üº¬µÄÄÚÈİ
			if(showIdx and dropNumIdx and itemId ~= nil) then
				if(9 ~= x712515_g_DropTable[showIdx].itemType) then
					local itemName;
					_,itemName,_ = GetItemInfoByItemId(itemId)
					print("Trong rß½ng v§t ph¦m có (loÕi"..x712515_g_DropTable[showIdx].itemType..")"..
								x712515_g_DropTable[showIdx].numOdd[dropNumIdx].num..
								"Cái (con) ["..itemName.."]")
					itemNum = x712515_g_DropTable[showIdx].numOdd[dropNumIdx].num;
					LuaFnAuditItemCreate(sceneId, -1, itemNum, itemId, itemName, "Hình thành t× Bäo rß½ng");
				end
			end
			
		end -- end of for i = 1, x712515_g_DropNumTable[numIdx].num do
		
		--Éè¶¨»ØÊÕÊ±¼ä
		if(ItemBoxId) then
			SetItemBoxMaxGrowTime( sceneId, ItemBoxId, delDur )
			return 0	--Õâ¸ö×ø±ê±»±¦ÏäÕ¼ÓÃÁË
		end
	end
	
	return -1	--Õâ¸ö×ø±êÃ»ÓĞ±»Õ¼ÓÃ
end
--Éú³Éº¯Êı½áÊø**********************************************************************

--Ò»´Î´´½¨¶à¸ö±¦ÏäµÄÍê³Éº¯Êı¿ªÊ¼****************************************************
function x712515_OnTickCreateFinish( sceneId, growPointType, tickCount )
	if(strlen(x712515_g_TickCreate_Msg) > 0) then
		--2006-8-22 14:37 µÈ´ıÏş½¡µÄserver¶Ô»°Æ½Ì¨
		print( sceneId .. " Hi®u cänh "..x712515_g_TickCreate_Msg)
	end
end
--Ò»´Î´´½¨¶à¸ö±¦ÏäµÄÍê³Éº¯Êı½áÊø****************************************************

--´ò¿ªÇ°º¯Êı¿ªÊ¼
function x712515_OnOpen( sceneId, selfId, targetId )
--·µ»ØÀàĞÍ
-- 0 ±íÊ¾´ò¿ª³É¹¦
end
--´ò¿ªÇ°º¯Êı½áÊø


--»ØÊÕº¯Êı¿ªÊ¼
function x712515_OnRecycle( sceneId, selfId, targetId )
--·µ»Ø1£¬Éú³¤µã»ØÊÕ
	return 1
end
--»ØÊÕº¯Êı½áÊø

--´ò¿ªºóº¯Êı¿ªÊ¼
function x712515_OnProcOver( sceneId, selfId, targetId )
end

--´ò¿ªºóº¯Êı½áÊø
function x712515_OpenCheck( sceneId, selfId, AbilityId, AblityLevel )
	return OR_OK
end
