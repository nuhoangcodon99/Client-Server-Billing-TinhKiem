...
function	x701601_CheckAbilityLevel(sceneId,selfId,AbilityID)
	...
	if Flag>0 then
		...
		AddText(sceneId, selfId, 0, "Evolved successfully!")
	end
...
function	x701601_ReomveProc(sceneId,selfId, EquipIndex, GemIndex, MatIndex)
	...
	if gem_index == 30900012 then  --低级宝石摘除符
		return 0
	elseif (gem_index >= 30900036 and gem_index <= 30900044) or (gem_index==30503136) then  --高级宝石摘除符
		--BroadMsgByChatPipe( sceneId, selfId, gem_index, 4 )
		return 9
	else
...
function	x701601_SplitGemSucceed(sceneId,selfId, EquipPos, GemPos, MatPos)
    ...
    if (CharmId >= 30900038 and CharmId <= 30900044) then  --高级宝石摘除符
	     ...
	     str = format( "#H#{_INFOUSR%s} took #{_INFOMSG%s} and recieved #{_INFOMSG%s}.", PlayerName, MatInfo, GemInfo)
