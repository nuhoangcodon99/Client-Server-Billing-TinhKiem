...
function	 x712030_OnOpen(sceneId,selfId,targetId)
	...
	--获得随机数,设置一次钓到鱼的时间
	x712030_g_FishTime = random(80000)+20000	--获得一个20-100秒之间的随机数
	--x712030_g_FishTime = random(8000)+2000	--获得一个2-10秒之间的随机数
	SetAbilityOperaTime(sceneId,selfId,x712030_g_FishTime)	--设置一次钓到鱼的时间
...
function	x712030_OnProcOver(sceneId,selfId,targetId)
	...
	if ret_1 > 0 then					-->0表示物品成功放入背包中
		Msg2Player(sceneId,selfId,"你釣到一條秋白鮭。",MSG2PLAYER_PARA)
		-- 增加熟练度
		ABilityID	=	GetItemBoxRequireAbilityID(sceneId,targetId)
