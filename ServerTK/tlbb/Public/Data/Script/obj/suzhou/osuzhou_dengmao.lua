...
--x001081_g_mpInfo[9]	= { "Mµ Dung", 435,  29, 137, 9}
...
function x001081_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if GetItemCount(sceneId, selfId, x001081_g_Yinpiao)>=1  then
		...
		AddNumText( sceneId, x001081_g_ScriptId, "Làm thª nào ta có th¬ ði ðªn Ðôn Hoàng và Tung S½n ?", 11, 2000 )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x001081_g_ScriptId, "Ði ðªn các môn phái", 9, 1012 )
...
function x001081_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x001081_MsgBox( sceneId, selfId, targetId, "  Thành viên trong nhóm ngß½i có ngß¶i có nhi®m vø ch· hàng b¢ng ðß¶ng thuÖ, d¸ch trÕm chúng ta không th¬ cung c¤p d¸ch vø cho ngß½i" )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x001081_MsgBox( sceneId, selfId, targetId, "  Ngß½i có nhi®m vø ch· hàng b¢ng ðß¶ng thuÖ, d¸ch trÕm chúng ta không th¬ cung c¤p d¸ch vø cho ngß½i" )
		return
	end
	...
	if arg == 1000 then		--·µ»ØÃÅÅÉ
		if id == 9 and  GetHumanMenpaiPoint(sceneId, selfId) == 0 then
			x001081_MsgBox( sceneId, selfId, targetId, "  Ngß½i vçn chßa gia nh§p b¤t kÏ môn phái nào!" )
		else
  ...
  if arg == 1007 then		--Â¥À¼
	...
	if GetNumText()== 1010 then		--ÊøºÓ¹ÅÕò
		...
		BeginUICommand(sceneId)
			...
			UICommand_AddString(sceneId, "Thúc Hà C± Tr¤n là n½i PK không tång sát khí, các hÕ có mu¯n ðªn không?");
