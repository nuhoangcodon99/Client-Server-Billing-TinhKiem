...
function x011006_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==0	then
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x011006_g_scriptId, "Ta mu¯n bái nh§p Minh Giáo",6,3)
	...
	if GetNumText()==3	then
		...
		elseif GetLevel( sceneId, selfId ) < 10 then
			...
			elseif x011006_g_MenPai==9	then
				...
				x011006_MsgBox( sceneId, selfId, "L¤y ðßþc 20 t¤m Chiêu t§p l®nh cüa môn phái" )
	...
	if GetNumText()==6 then
		...
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, 108, 59, "ý‹Èf´º" )
		return
	end
end
