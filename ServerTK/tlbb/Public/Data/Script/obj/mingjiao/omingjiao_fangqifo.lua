...
x011003_g_mpInfo		= {}
x011003_g_mpInfo[0]	= { "Tinh Túc", 16,  96, 152, MP_XINGSU }
x011003_g_mpInfo[1]	= { "Tiêu Dao", 14,  67, 145, MP_XIAOYAO }
x011003_g_mpInfo[2]	= { "Thiªu Lâm",  9,  95, 137, MP_SHAOLIN }
x011003_g_mpInfo[3]	= { "Thiên S½n", 17,  95, 120, MP_TIANSHAN }
x011003_g_mpInfo[4]	= { "Thiên Long", 13,  96, 120, MP_DALI }
x011003_g_mpInfo[5]	= { "Nga My", 15,  89, 144, MP_EMEI }
x011003_g_mpInfo[6]	= { "Võ Ðang", 12, 103, 140, MP_WUDANG }
x011003_g_mpInfo[7]	= { "Minh Giáo", 11,  98, 167, MP_MINGJIAO }
x011003_g_mpInfo[8]	= { "Cái Bang", 10,  91, 116, MP_GAIBANG }
...
function x011003_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if	GetLevel( sceneId, selfId)>=10  then	
			AddNumText(sceneId,x011003_g_scriptId,"LÕc Dß½ng",9,0)
			AddNumText(sceneId,x011003_g_scriptId,"Tô Châu",9,1)
			AddNumText( sceneId, x011003_g_scriptId, "LÕc Dß½ng- CØu Châu thß½ng hµi", 9, 3 )
			AddNumText( sceneId, x011003_g_scriptId, "Tô Châu - Thiªt tß¶ng ph¯", 9, 4 )
		end
		
		--add by WTT
		if	GetLevel( sceneId, selfId)>=20  then	
			AddNumText( sceneId, x011003_g_scriptId, "Thúc Hà C± Tr¤n", 9, 6 )
		end
		
		if	GetLevel( sceneId, selfId)>=75  then	
			AddNumText( sceneId, x011003_g_scriptId, "#{MPCSLL_80925_01}", 9, 5 )
		end
		AddNumText(sceneId,x011003_g_scriptId,"ÐÕi Lý",9,2)
		
    --[tx42913]
    CallScriptFunction( x011003_g_xuanWuDaoId, "OnEnumerate",sceneId, selfId, targetId )
		--[/tx42913]
		
		AddNumText(sceneId,x011003_g_scriptId,"Dçn ta t¾i các môn phái khác",9,11)

		-- ÎÒÔõÑù²ÅÄÜÈ¥¶Ø»ÍºÍáÔÉ½
		AddNumText( sceneId, x011003_g_scriptId, "Ðªn Ðôn Hoàng và Tung S½n?", 11, 2000 )
	EndEvent(sceneId)
...
------------------------------------------------------------------------------------------
--[tx42913]
	if eventId == x011003_g_xuanWuDaoId then --È¥ÐþÎäµº
...
--[/tx42913]		
    ...
    if  GetNumText() == 11 then
		...
		BeginEvent(sceneId)
		    ...
		    for i=0, 8 do
				AddNumText( sceneId, x011003_g_scriptId, "Môn phái - "..x011003_g_mpInfo[i][1], 9, i+12 )
			end			
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)	
		return 
