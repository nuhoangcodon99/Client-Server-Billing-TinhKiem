	...
	BeginEvent(sceneId)
		...
		if	GetMenPai( sceneId, selfId) == 7 then
			...
			if	(0==LuaFnHaveSpecificEquitation(  sceneId, selfId, 463))	then
				AddNumText(sceneId, x017009_g_ScriptId,"ŒH÷c kÜ thu§t: BÕch ğiêu",12,60)
			end
		end
...
function  x017009_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 40 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x017009_g_ScriptId,"ÊÇ",-1,0)
      AddNumText(sceneId, x017009_g_ScriptId,"·ñ",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if GetNumText() == 60 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x017009_g_ScriptId,"ÊÇ",-1,1)
      AddNumText(sceneId, x017009_g_ScriptId,"·ñ",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
