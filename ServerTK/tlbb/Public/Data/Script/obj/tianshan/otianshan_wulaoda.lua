...
x017005_g_mpInfo		= {}
x017005_g_mpInfo[0]	= { "Tinh Túc", 16,  96, 152, MP_XINGSU }
x017005_g_mpInfo[1]	= { "Tiêu Dao", 14,  67, 145, MP_XIAOYAO }
x017005_g_mpInfo[2]	= { "Thiªu Lâm",  9,  95, 137, MP_SHAOLIN }
x017005_g_mpInfo[3]	= { "Thiên S½n", 17,  95, 120, MP_TIANSHAN }
x017005_g_mpInfo[4]	= { "Thiên Long", 13,  96, 120, MP_DALI }
x017005_g_mpInfo[5]	= { "Nga My", 15,  89, 144, MP_EMEI }
x017005_g_mpInfo[6]	= { "Võ Ðang", 12, 103, 140, MP_WUDANG }
x017005_g_mpInfo[7]	= { "Minh Giáo", 11,  98, 167, MP_MINGJIAO }
x017005_g_mpInfo[8]	= { "Cái Bang", 10,  91, 116, MP_GAIBANG }
...
function x017005_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if	GetLevel( sceneId, selfId)>=10  then	
			AddNumText(sceneId,x017005_g_ScriptId,"LÕc Dß½ng",9,0)
			AddNumText(sceneId,x017005_g_ScriptId,"Tô Châu",9,1)
			AddNumText( sceneId, x017005_g_ScriptId, "LÕc Dß½ng - CØu Châu thß½ng hµi", 9, 3 )
			AddNumText( sceneId, x017005_g_ScriptId, "Tô Châu -  Thiªt tß¶ng ph¯", 9, 4 )
		end
		
		--add by WTT
		if	GetLevel( sceneId, selfId)>=20  then	
			AddNumText( sceneId, x017005_g_ScriptId, "Thúc Hà C± Tr¤n", 9, 6 )
		end
		
		if	GetLevel( sceneId, selfId)>=75  then	
			AddNumText( sceneId, x017005_g_ScriptId, "#{MPCSLL_80925_01}", 9, 5 )
		end
		AddNumText(sceneId,x017005_g_ScriptId,"ÐÕi Lý",9,2)		
		
    --[tx42913]
    CallScriptFunction( x017005_g_xuanWuDaoId, "OnEnumerate",sceneId, selfId, targetId )
		--[/tx42913]	
		
		AddNumText(sceneId,x017005_g_ScriptId,"Dçn ta t¾i các môn phái khác",9,11)
		
		-- ÎÒÔõÑù²ÅÄÜÈ¥¶Ø»ÍºÍáÔÉ½
		AddNumText( sceneId, x017005_g_ScriptId, "Ðªn Ðôn Hoàng và Tung S½n?", 11, 2000 )
	EndEvent(sceneId)
...
------------------------------------------------------------------------------------------
--[tx42913]
	if eventId == x017005_g_xuanWuDaoId then --È¥ÐþÎäµº
...
--[/tx42913]
    ...
    if  GetNumText() == 11 then
		...
		BeginEvent(sceneId)
		    ...
		    for i=0, 8 do
				AddNumText( sceneId, x017005_g_ScriptId, "Môn phái - "..x017005_g_mpInfo[i][1], 9, i+12 )
			end			
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)	
		return 
...
------------------------------------------------------------------------------------------ 
	...
	if GetNumText()== 6 then					--ÊøºÓ¹ÅÕò		
		BeginUICommand(sceneId)
			...
			UICommand_AddString(sceneId, "Ðªn Thúc Hà C± Tr¤n, PK s¨ không b¸ sát khí, các hÕ xác nh§n mu¯n ðªn không?");
