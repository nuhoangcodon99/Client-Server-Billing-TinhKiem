...
--AddNumText( sceneId, x800116_g_ScriptId, "ÎÒÏë¹ºÂò½ð±Ò", 5, 1 )       --Del by Houzf in 20080811 because of TT 37697
	...
	--		AddNumText( sceneId, x800116_g_ScriptId, "ÎÒÏë¼ÄÊÛ½ð±Ò", 5, 4 )
	--	end
	end
...
function x800116_Sell( sceneId, selfId, targetId, Grade, Price )
	...
	if ret == 1 then
		if x800116_g_CShopInfo[shopId] and x800116_g_CShopInfo[shopId].type[type] and x800116_g_CShopInfo[shopId].name then
			...
			local len = strlen(strGUID)
			strGUID = strsub( strGUID, len-3 )
			local str = format( "#IHi®n [ID:****%s] · #R%s#I ðang gØi bán #Y%s, #I giá bán #Y%s, #I nªu có ý mu¯n mua hãy mau chóng v« #G LÕc Dß½ng#I nhé.",
				strGUID,
				nameInfo,
				typeInfo.buyFmtValueStr,
				typeInfo.buyFmtPriceStr,
