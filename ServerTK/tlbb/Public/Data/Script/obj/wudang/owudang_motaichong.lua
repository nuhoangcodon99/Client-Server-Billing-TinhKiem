...
function x012009_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if	GetLevel( sceneId, selfId)>=10  then	
			...
			AddNumText( sceneId, x012009_g_ScriptId, "Tô Châu -  Thiªt thß½ng ph¯", 9, 4 )
		end

		--add by WTT
		if	GetLevel( sceneId, selfId)>=20  then	
			AddNumText( sceneId, x012009_g_ScriptId, "Thúc Hà C± Tr¤n", 9, 6 )
		end
    ...
    CallScriptFunction( x012009_g_xuanWuDaoId, "OnEnumerate",sceneId, selfId, targetId )
		...
		AddNumText(sceneId,x012009_g_ScriptId,"Dçn ta t¾i các môn phái khác",9,11)
...
------------------------------------------------------------------------------------------ 
	...
	if GetNumText()== 6 then					--ÊøºÓ¹ÅÕò		
		BeginUICommand(sceneId)
			...
			UICommand_AddString(sceneId, "Khi ðªn Thúc Hà C± Tr¤n, PK s¨ không b¸ sát khí. Các hÕ xác nh§n có mu¯n ðªn không?");
