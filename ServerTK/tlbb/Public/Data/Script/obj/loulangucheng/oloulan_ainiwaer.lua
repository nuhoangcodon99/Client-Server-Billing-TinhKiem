...
function x001100_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if GetItemCount(sceneId, selfId, x001100_g_Yinpiao)>=1  then
		BeginEvent( sceneId )
			AddText( sceneId, "  TrÍn ngﬂ∂i c·c h’ cÛ ng‚n phi™u, ang ch’y trØn n˛! Ta khÙng th¨ gi˙p c·c h’" )
		EndEvent( sceneId )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x001100_g_ScriptId, "Th‡nh th∏ - L’c DﬂΩng - Cÿu Ch‚u thﬂΩng hµi", 9, 1002 )
		AddNumText( sceneId, x001100_g_ScriptId, "Th‡nh th∏ - TÙ Ch‚u", 9, 1003 )
		AddNumText( sceneId, x001100_g_ScriptId, "Th‡nh th∏ - TÙ Ch‚u - Thi™t tﬂ∂ng phØ", 9, 1004 )
		...
		AddNumText( sceneId, x001100_g_ScriptId, "–i ™n c·c mÙn ph·i", 9, 1011 )
...
function x001100_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetTeamId(sceneId,selfId)>=0 and 
		...
		for	i=0,num-1 do
			...
			if IsHaveMission(sceneId,mems[i],4021) > 0 then
				x001100_MsgBox( sceneId, selfId, targetId, "  Th‡nh viÍn trong nhÛm ngﬂΩi cÛ ngﬂ∂i cÛ nhiÆm v¯ ch∑ h‡ng b¢ng ﬂ∂ng thu÷, d∏ch tr’m ch˙ng ta khÙng th¨ cung c§p d∏ch v¯ cho ngﬂΩi" )
				return
			end
		end
	end
	...
	if IsHaveMission(sceneId,selfId,4021) > 0 then
		x001100_MsgBox( sceneId, selfId, targetId, "  NgﬂΩi cÛ nhiÆm v¯ ch∑ h‡ng b¢ng ﬂ∂ng thu÷, d∏ch tr’m ch˙ng ta khÙng th¨ cung c§p d∏ch v¯ cho ngﬂΩi" )
		return
	end
	...
	if arg == 1000 then		--∑µªÿ√≈≈…
		if id < 0 or id >= 9 then
			x001100_MsgBox( sceneId, selfId, targetId, "  NgﬂΩi vÁn chﬂa gia nhßp b§t kœ mÙn ph·i n‡o!" )
		else
	...
	if arg == 1011 then		
		BeginEvent( sceneId )
			for i, mp in x001100_g_mpInfo do
				AddNumText( sceneId, x001100_g_ScriptId, "MÙn ph·i - "..mp[1], 9, i )
			end
	...
	if arg == 1016 then		-- ¯∫”π≈’Ú
		...
		BeginUICommand(sceneId)
			...
			UICommand_AddString(sceneId, "NgﬂΩi cÛ muØn ™n Th˙c H‡ C± Tr§n khÙng? NΩi n‡y PK khÙng b∏ s·t khÌ.");
