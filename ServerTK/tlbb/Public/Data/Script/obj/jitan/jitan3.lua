...
function x900012_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x900012_g_scriptId, "放置10个九黎饰物", 6, 10)
		AddNumText( sceneId, x900012_g_scriptId, "关于鏖战九黎", 11, 11)
		AddNumText( sceneId, x900012_g_ScriptId, "关于追寻女娲石", 11, 12 )
		AddNumText( sceneId, x900012_g_scriptId, "关于追寻女娲神石", 11, 13)
        EndEvent(sceneId)
...
function x900012_OnEventRequest( sceneId, selfId, targetId, eventId )
  ...
  if GetNumText()  == 10 then
	...
	LuaFnDelAvailableItem(sceneId,selfId,x900012_g_ItemId,10)	--删除物品
	--删除该物品
	...
	if LuaFnLockCheck( sceneId, selfId, bagpos, 0 ) < 1 then
		...
		if nItemNum <= 0 then
			x900012_MsgBox( sceneId, selfId, targetId, "此物品已被锁定！" )
			return 0;
		end
	end
...
--开始执行随机事件
		...
		local strText = format("#b#cff00f0荒日九黎头领:“是谁，把我从沉睡中给吵醒了？竟敢打扰我睡觉，你们会为此付出代价的”", playername)		
