...
x900011_g_ItemId = 20309019

--**********************************
--事件列表
--**********************************
function x900011_OnDefaultEvent( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		AddText(sceneId,"    #W此处为九黎前线标记信标，用于#G九黎部族前线指挥官#W在此发号施令。#G注：在此提交10个九黎饰物可吸引九黎头领前来") 
		AddNumText( sceneId, x900011_g_scriptId, "放置10个九黎饰物", 6, 10)
		AddNumText( sceneId, x900011_g_scriptId, "关于鏖战九黎", 11, 11)
		AddNumText( sceneId, x900011_g_ScriptId, "关于追寻女娲石", 11, 12 )
		AddNumText( sceneId, x900011_g_scriptId, "关于追寻女娲神石", 11, 13)
        EndEvent(sceneId)
...
--事件列表选中一项
--**********************************
function x900011_OnEventRequest( sceneId, selfId, targetId, eventId )
  
  if GetNumText()  == 10 then
	  --检查包内是否有位置
	if GetItemCount(sceneId, selfId, x900011_g_ItemId) < 10  then
	BeginEvent( sceneId )
	...
	LuaFnDelAvailableItem(sceneId,selfId,x900011_g_ItemId,10)	--删除物品
	--删除该物品
	...
	if LuaFnLockCheck( sceneId, selfId, bagpos, 0 ) < 1 then
		...
		if nItemNum <= 0 then
			x900011_MsgBox( sceneId, selfId, targetId, "此物品已被锁定！" )
			return 0;
		end
	end
...
--开始执行随机事件
		BeginEvent( sceneId )
		LuaFnCreateMonster(549, 15704, 185, 88, 17, 0, 402030)
		local playername = GetName(sceneId, selfId)
		local strText = format("#b#cff00f0古月九黎头领:“是谁，把我从沉睡中给吵醒了？竟敢打扰我睡觉，你们会为此付出代价的”", playername)		
