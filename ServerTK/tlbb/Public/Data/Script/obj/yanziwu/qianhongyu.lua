...
function x402246_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if nStep == 10  then
			AddNumText( sceneId, x402246_g_scriptId, "Chiªn ð¤u !", 10 ,1  )
		end
			
		if nStep < 14  then
			--AddNumText( sceneId, x402246_g_scriptId, "Trñc tiªp chiªn ð¤u", 10 ,2  )
		end
...
function x402246_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1  then
		-- ¼ì²âµ±Ç°ÊÇ²»ÊÇÔÚÏà¹Ø¸±±¾£¬ÕâÀï±ØÐëÒª×ö°²È«¼ì²é£¬ÏÖÔÚÏÈ²»×ö
	...
	if GetNumText() == 2  then
		...
		BeginEvent(sceneId)
			AddText(sceneId, "  Cuµc chiªn ð¤u v¾i 3 ác nhân b¡t ð¥u")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
