...
function x402241_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		if LuaFnGetCopySceneData_Param(sceneId, 8) == 1  then
			AddNumText( sceneId, x402241_g_scriptId, "Chiªn ğ¤u !", 10 ,1  )
			--AddNumText( sceneId, x402241_g_scriptId, "Trñc tiªp khiêu ğ¤u ğ® nh¸ quan", 10 ,2  )
		end
...
function x402241_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1  then
		-- ¼ì²âµ±Ç°ÊÇ²»ÊÇÔÚÏà¹Ø¸±±¾£¬ÕâÀï±ØĞëÒª×ö°²È«¼ì²é£¬ÏÖÔÚÏÖ²»×ö
	...
	if GetNumText() == 2  then
		...
		BeginEvent(sceneId)
			AddText(sceneId, "Ğ® nh¸ quan dî kinh khai khäi")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
