...
function x402284_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 5 giây næa  s¨ chiªn ð¤u" )
		return
	end

	if 6 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 4 giây næa  s¨ chiªn ð¤u" )
		return
	end

	if 5 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 3 giây næa  s¨ chiªn ð¤u" )
		return
	end

	if 4 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 2 giây næa  s¨ chiªn ð¤u" )
		return
	end

	if 3 == step then
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 1 giây næa  s¨ chiªn ð¤u" )
		return
	end

	if 2 == step then
		--ÌáÊ¾Õ½¶·¿ªÊ¼....
		CallScriptFunction( x402284_g_FuBenScriptId, "TipAllHuman", sceneId, "B¡t ð¥u chiªn ð¤u" )
		--É¾³ýNPC....
