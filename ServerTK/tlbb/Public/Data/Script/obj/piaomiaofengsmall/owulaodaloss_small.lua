...
function x402288_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		if 1 == CallScriptFunction( x402288_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "LiQiuShui" ) then
			AddNumText( sceneId, x402288_g_ScriptId, "決戰李秋水？", 10, 1 )
		end
...
function x402288_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		CallScriptFunction( x402288_g_FuBenScriptId, "TipAllHuman", sceneId, "戰鬥5秒鐘後開始" )
		return
	end

	if 6 == step then
		CallScriptFunction( x402288_g_FuBenScriptId, "TipAllHuman", sceneId, "戰鬥4秒鐘後開始" )
		return
	end

	if 5 == step then
		CallScriptFunction( x402288_g_FuBenScriptId, "TipAllHuman", sceneId, "戰鬥3秒鐘後開始" )
		return
	end

	if 4 == step then
		CallScriptFunction( x402288_g_FuBenScriptId, "TipAllHuman", sceneId, "戰鬥2秒鐘後開始" )
		return
	end

	if 3 == step then
		CallScriptFunction( x402288_g_FuBenScriptId, "TipAllHuman", sceneId, "戰鬥1秒鐘後開始" )
		return
	end

	if 2 == step then
		--提示战斗开始....
		CallScriptFunction( x402288_g_FuBenScriptId, "TipAllHuman", sceneId, "戰鬥開始" )
		return
	end
