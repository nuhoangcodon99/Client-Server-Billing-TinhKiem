...
function x009003_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==0	then
		...
		if x009003_g_MenPai == 0 then
			BeginEvent(sceneId)
				AddText(sceneId, "Ngß½i lÕi t¾i ðòi bái phái Tiêu Dao làm sß phø, ngß½i ðã là ð® tØ Thiªu Lâm, còn mu¯n bái sß gì næa")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
			return
		end
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x009003_g_scriptId, "Ta mu¯n bái nh§p phái Thiªu Lâm",6,3)
	...
	if GetNumText()==3	then
		...
		elseif GetLevel( sceneId, selfId ) < 10 then
			...
			elseif x009003_g_MenPai==9	then
				...
				x009003_MsgBox( sceneId, selfId, "L¤y ðßþc 20 t¤m Chiêu t§p l®nh cüa môn phái" )
	...
	if GetNumText()==6 then
		...
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		CallScriptFunction( SCENE_SCRIPT_ID, "AskTheWay", sceneId, selfId, sceneId, 91, 71, "Ðþëy" )
		return
	end
end
