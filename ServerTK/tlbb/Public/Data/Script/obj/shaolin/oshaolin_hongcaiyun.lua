...
function x009018_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText(sceneId, x009018_g_scriptId, "H÷c khinh công Thiªu Lâm",12,0)
	EndEvent(sceneId)
...
function x009018_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText()==0	then
		...
		elseif GetMenPai(sceneId, selfId) == 9 then
			BeginEvent(sceneId)
				AddText(sceneId,"H÷c khinh công cüa Thiªu Lâm trß¾c tiên c¥n gia nh§p phái Thiªu Lâm!")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		else
			BeginEvent(sceneId)
				AddText(sceneId,"Ngß½i không phäi là ð® tØ cüa b±n môn, ta không th¬ dÕy ngß½i khinh công cüa Thiªu Lâm")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end	
