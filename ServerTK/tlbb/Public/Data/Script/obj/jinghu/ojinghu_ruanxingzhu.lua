...
function x005001_NotifyMessage(sceneId, selfId,targetId )		
		...
		if(nCount == 10 or nCount == 20 or nCount == 30 or nCount == 40 )	then				
				local msg = format("[%s]: #W#{_INFOUSR%s} #GChâu#P#{JINGHU_RUANXINZHU}", GetSceneName(sceneId), GetName(sceneId, selfId) )
				LuaFnNpcChat(sceneId, targetId, 1, msg)				
		end
		--Èç¹ûÃ¿Íê³É50´Î·¢²¼ÊÀ½çÏûÏ¢
		if(nCount == 50)	then								
				local msg = format("#W#{_INFOUSR%s} #GChâu#P#{JINGHU_RUANXINZHU}", GetName(sceneId, selfId) )
...
function x005001_GiveRabbitEar( sceneId, selfId )
	...
	if ret and ret > 0 then
		...
		if itemTransfer and selfName then
			local msg = "#YNguy­n Tinh Trúc: #W#{_INFOUSR"..selfName.."}#P#{tuerduo}#{_INFOMSG"..itemTransfer.."}#P ðái tÕi #W#{_INFOUSR"..selfName.."}#P ðích ð¥u thßþng th¸ cäm tÕ";
...
function x005001_GiveSmallRabbitEar( sceneId, selfId )
	...
	if ret and ret > 0 then
		...
		if itemTransfer and selfName then
			local msg = "#YNguy­n Tinh Trúc: #W#{_INFOUSR"..selfName.."}#P#{tuerduo2}#{_INFOMSG"..itemTransfer.."}#P t¯ng c¤p #W#{_INFOUSR"..selfName.."}#P#{tuerduo3}";
			BroadMsgByChatPipe(sceneId, selfId, msg, 4);
		end
	end
