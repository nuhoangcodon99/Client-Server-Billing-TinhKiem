...
function x013011_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		...
		AddNumText(sceneId, x013011_g_ScriptId, "H÷c khinh công phái ÐÕi Lý",12,0)
	EndEvent(sceneId)
...
function x013011_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText()==0	then
		...
		elseif GetMenPai(sceneId, selfId) == 9 then
			...
			BeginEvent(sceneId)
				AddText(sceneId,"Ngß½i không phäi là ð® tØ cüa b±n môn phái, ta không th¬ dÕy ngß½i khinh công Thiên Long")
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
		end
	end	
