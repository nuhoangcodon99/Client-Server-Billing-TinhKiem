...
function x013001_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"  Lão nÕp B±n Nh½n, thí chü tìm lão nÕp có chuy®n gì?")
...
function x013001_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText()==0	then
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x013001_g_scriptId, "Ta mu¯n bái nh§p Thiên Long",6,3)
	...
	if	GetNumText()==3	then
			...
			elseif GetLevel( sceneId, selfId ) < 10 then
				...
				elseif x013001_g_MenPai==9	then
					...
					BeginEvent(sceneId)
						AddText(sceneId,"Các hÕ ðã gia nh§p Thiên Long Tñ!");
					EndEvent(sceneId)
					...
					x013001_MsgBox( sceneId, selfId, "L¤y ðßþc 20 t¤m Chiêu t§p l®nh cüa môn phái" )
					...
					if	LuaFnGetSex( sceneId, selfId)==0	then
						LuaFnMsg2Player( sceneId, selfId,"Các hÕ ðã gia nh§p Thiên Long Tñ!",MSG2PLAYER_PARA)
	...
	if GetNumText()==6 then
		BeginEvent(sceneId)
			AddText(sceneId, "B±n quan (97, 67) có th¬ dÕy ngß½i kÛ nång chiªn ð¤u cüa môn phái ta. Ông · ngay bên ta")
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
