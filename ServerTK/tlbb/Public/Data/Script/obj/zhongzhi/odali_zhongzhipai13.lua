...
--½Å±¾ºÅ
x714069_g_ScriptId = 714069

event_xuanzezhiwu = 713550

--Ö²Îï²úÆ·±àºÅÁÐ±í
x714069_g_eventList={20104001,20104002,20104005,20104007,20104009,20104012,
			20105001,20105004,20105007,20105012} --Ö²ÎïµÄ±àºÅ,²»ÊÇeventId
--**********************************
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
function x714069_OnDefaultEvent( sceneId, selfId,targetId )
	--PLANTFLAG[1] =0
	--PLANTFLAG[2] =0
	...
	if AbilityLevel ~= 0	then
		BeginEvent(sceneId)
			...
			--Í¨¹ýx714069_g_eventListºÍscriptglobalÖÐµÄÖ²ÎïÁÐ±í¶Ô±È£¬²¢¸ù¾ÝÍæ¼ÒÖÖÖ²¼¼ÄÜµÈÀ´ÏÔÊ¾ÏàÓ¦Ö²Îï
			for i, eventId in x714069_g_eventList do	--±éÀúÕâ¸öµ¾²ÝÈË¿ÉÒÔÖÖÖ²µÄÖ²ÎïÁÐ±í
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--±éÀúscriptglobalÖÐµÄËùÓÐÖ²ÎïÁÐ±í
					if eventId == g_ZhiWuId then
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --Èç¹ûÍæ¼ÒÖÖÖ²¼¼ÄÜµÈ>=¸ÃÖ²ÎïÒªÇó¼¼ÄÜµÈ]
							AddNumText(sceneId, x714069_g_eventList[i], V_ZHONGZHI_NAME[j],6,-1)
							break
						end
					end
				end
...
--ÊÂ¼þÁÐ±íÑ¡ÖÐÒ»Ïî
--**********************************
function x714069_OnEventRequest( sceneId, selfId, targetId, eventId )
	zhiwuId = eventId
	for i, findId in x714069_g_eventList do
...
--**********************************
function x714069_OnMissionSubmit( sceneId, selfId, targetId, scriptId )
	for i, findId in x714069_g_eventList do
...
--½ÓÊÜ´ËNPCµÄÈÎÎñ
--**********************************
function x714069_OnMissionAccept( sceneId, selfId, targetId, missionScriptId )
	for i, findId in x714069_g_eventList do
