...
function x714093_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if AbilityLevel ~= 0	then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x714093_g_ScriptId, "Tr°ng loÕi thu hoÕch s¾m",6,254)
			AddNumText(sceneId, x714093_g_ScriptId, "Tr°ng loÕi thu hoÕch muµn",6,255)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
function x714093_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--Í¨¹ýx714093_g_eventListºÍscriptglobalÖÐµÄÖ²ÎïÁÐ±í¶Ô±È£¬²¢¸ù¾ÝÍæ¼ÒÖÖÖ²¼¼ÄÜµÈÀ´ÏÔÊ¾ÏàÓ¦Ö²Îï
			for i, eventId in x714093_g_eventList do	--±éÀúÕâ¸öµ¾²ÝÈË¿ÉÒÔÖÖÖ²µÄÖ²ÎïÁÐ±í
