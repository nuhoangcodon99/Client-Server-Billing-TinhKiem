...
function x714057_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if AbilityLevel ~= 0	then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x714057_g_ScriptId, "Tr°ng loÕi thu hoÕch s¾m",6,254)
			AddNumText(sceneId, x714057_g_ScriptId, "Tr°ng loÕi thu hoÕch muµn",6,255)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
function x714057_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--Í¨¹ýx714057_g_eventListºÍscriptglobalÖÐµÄÖ²ÎïÁÐ±í¶Ô±È£¬²¢¸ù¾ÝÍæ¼ÒÖÖÖ²¼¼ÄÜµÈÀ´ÏÔÊ¾ÏàÓ¦Ö²Îï
			for i, eventId in x714057_g_eventList do	--±éÀúÕâ¸öµ¾²ÝÈË¿ÉÒÔÖÖÖ²µÄÖ²ÎïÁÐ±í
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--±éÀúscriptglobalÖÐµÄËùÓÐÖ²ÎïÁÐ±í
					if eventId == g_ZhiWuId then
						...
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --Èç¹ûÍæ¼ÒÖÖÖ²¼¼ÄÜµÈ>=¸ÃÖ²ÎïÒªÇó¼¼ÄÜµÈ]
							...
							AddNumText(sceneId, x714057_g_ScriptId, V_ZHONGZHI_NAME[name_Index].."(C¤p "..V_ZHONGZHI_NEEDLEVEL[j]..")",6,name_Index)
							break
						end
					end
				end
