...
--Ö²Îï²úÆ·±àºÅÁÐ±í
x714201_g_eventList={20104007,20104008, 20104009, 20104010, 20105007,20105008,20105009, 20105010,} --Ö²ÎïµÄ±àºÅ,²»ÊÇscriptId
--**********************************
--ÊÂ¼þ½»»¥Èë¿Ú
--**********************************
function x714201_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if AbilityLevel >= 7	then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x714201_g_ScriptId, "Tr°ng loÕi thu hoÕch s¾m",6,254)
			AddNumText(sceneId, x714201_g_ScriptId, "Tr°ng loÕi thu hoÕch s¾m",6,255)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	else
		BeginEvent(sceneId)
			AddText(sceneId, "Ngß½i trß¾c m¡t c¥n gieo tr°ng ðÕt c¤p 7 r°i hãy ðªn.")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	end
end
...
function x714201_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if NumText == 254 or NumText == 255 then
		BeginEvent(sceneId)
			...
			--Í¨¹ýx714201_g_eventListºÍscriptglobalÖÐµÄÖ²ÎïÁÐ±í¶Ô±È£¬²¢¸ù¾ÝÍæ¼ÒÖÖÖ²¼¼ÄÜµÈÀ´ÏÔÊ¾ÏàÓ¦Ö²Îï
			for i, eventId in x714201_g_eventList do	--±éÀúÕâ¸öµ¾²ÝÈË¿ÉÒÔÖÖÖ²µÄÖ²ÎïÁÐ±í
				for j,g_ZhiWuId in V_ZHONGZHI_ID do		--±éÀúscriptglobalÖÐµÄËùÓÐÖ²ÎïÁÐ±í
					if eventId == g_ZhiWuId then
						...
						if AbilityLevel >= V_ZHONGZHI_NEEDLEVEL[j] then --Èç¹ûÍæ¼ÒÖÖÖ²¼¼ÄÜµÈ>=¸ÃÖ²ÎïÒªÇó¼¼ÄÜµÈ]
							...
							AddNumText(sceneId, x714201_g_ScriptId, V_ZHONGZHI_NAME[name_Index].."(C¤p "..V_ZHONGZHI_NEEDLEVEL[j]..")",6,name_Index)
							break
						end
					end
				end
