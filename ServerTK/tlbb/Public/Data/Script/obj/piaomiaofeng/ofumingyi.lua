...
function x402273_OnDefaultEvent( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		if 1 == CallScriptFunction( x402273_g_FuBenScriptId, "GetBossBattleFlag", sceneId, "ShuangZi" ) then
			AddNumText( sceneId, x402273_g_ScriptId, "Khiêu chiªn B¤t Bình ÐÕo Nhân Trác B¤t Phàm", 10, 1 )
		end
...
function x402273_OnPMFTimer( sceneId, step, data1, data2 )
	...
	if 7 == step then
		...
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 5 giây næa s¨ chiªn ð¤u" )
		return
	end

	if 6 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 4 giây næa s¨ chiªn ð¤u" )
		return
	end

	if 5 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 3 giây næa s¨ chiªn ð¤u" )
		return
	end
	...
	if 4 == step then
		...
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 2 giây næa s¨ chiªn ð¤u" )
		return
	end

	if 3 == step then
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "Còn 1 giây næa s¨ chiªn ð¤u" )
		return
	end

	if 2 == step then
		--ÌáÊ¾Õ½¶·¿ªÊ¼....
		CallScriptFunction( x402273_g_FuBenScriptId, "TipAllHuman", sceneId, "B¡t ð¥u chiªn ð¤u" )
		return
	end
