...
x125013_g_Goto = {
			{name="Khu¤t Bình Nguyên ",scene=2,x=177,z=135,scname="ÐÕi Lí"},
...
function x125013_UpdateEventList( sceneId, selfId, targetId )
	BeginEvent(sceneId)
		...
		if szName == "T¯ng Tri Ng÷c " and level < 20 then
			...
			str = "  R¤t xin l²i, ðÆng c¤p cüa bÕn dß¾i 20 không th¬ ðªn Thúc Hà C± Tr¤n"
			AddText(sceneId, str);
		else
			...
			for i, scene in x125013_g_Goto  do
				if scene.name == GetName(sceneId, targetId)  then
					local str = ""
					str = "  Các hÕ có mu¯n ðªn " .. scene.scname .. " không ?"
					AddText(sceneId, str);
					AddNumText( sceneId, x125013_g_scriptId, "Hãy ðßa tôi ðªn " .. scene.scname ,9 ,1  )
				end
			end
		end
			...
			if Scene.name == GetName(sceneId, targetId)  then
				...
				if Scene.scname == "Thúc Hà C± Tr¤n" then
					BeginUICommand(sceneId)
						...
						UICommand_AddString(sceneId, "Thúc Hà C± Tr¤n là n½i nguy hi¬m, bÕn có ch¡c mu¯n vào?");
