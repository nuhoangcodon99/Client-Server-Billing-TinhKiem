...
function x125012_UpdateEventList( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"Các hÕ mu¯n thiªt l§p ði¬m phøc sinh v¾i tÕi hÕ ?");
		for i, eventId in x125012_g_eventList do
			CallScriptFunction( eventId, "OnEnumerate",sceneId, selfId, targetId )
		end
		
		AddNumText( sceneId, x125012_g_scriptId, "Ði¬m phøc sinh cüa tÕi hÕ n¢m · ðây" ,9 ,1  )
...
function x125012_OnEventRequest( sceneId, selfId, targetId, eventId )
	if GetNumText()==1  then
		...
		BeginEvent(sceneId)
			AddText(sceneId,"Ði¬m phøc sinh cüa các hÕ ðã ðßþc ð£t · ðây");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
