...
x300065_g_GiftTbl = {
	...
	{ itemId=30008018, odds=5000 },--Thanh Tâm Ğan
	{ itemId=30008019, odds=5000 },--Ğ£c Xá L®nh
	{ itemId=30008014, odds=5000 },--Thông Thiên Linh Ğan
	{ itemId=30900016, odds=5000 },--cao c¤p hşp thành phù
...
function x300065_OnActivateOnce( sceneId, selfId )
	...
	--·¢¹§Ï²ÓÊ¼ş....
	if GiftId ~= 0 then
		LuaFnSendSystemMail(sceneId, GetName(sceneId, selfId), "    Xin chúc m×ng, các hÕ nh§n ğßşc quà t£ng Giáng Sinh #Y#{_ITEM" ..GiftId.. "}#W.#r    Thiên Long Bát Bµ chúc các hÕ Giáng Sinh an lành hÕnh phúc và nåm m¾i vui vë !")
	end
	
	if GiftId >= 30008014 and GiftId <= 30505107 or  GiftId == 30900016 then
		local message =format("#HGiáng sinh an lành ğªn kh¡p m÷i n½i, trong lúc dÕo ch½i Giáng Sinh #{_INFOUSR%s}#H nh£t ğßşc #YL­ V§t Giáng Sinh#H t× #ITuyªt Nhân#H, chßa k¸p hªt vui m×ng, v×a m· ra lÕi phát hi®n #W#{_ITEM%d}#P.", GetName(sceneId, selfId), GiftId )
