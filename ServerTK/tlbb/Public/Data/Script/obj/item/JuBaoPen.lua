...
x332204_g_strGongGaoInfo = {
  "#{_INFOUSR%s}#H開啟#G#{_ITEM30008038}#H時，竟然真的如願獲得了#{_INFOMSG%s}#H，真是令人羨慕啊！",
  "#{_INFOUSR%s}#H正在埋怨#G#{_ITEM30008038}#H太小時，竟然在裡面發現了一個更大的#{_INFOMSG%s}！",
  "#{_INFOUSR%s}#H平日裡勤儉節約，今日在#G#{_ITEM30008038}#H裡得到了一個#{_INFOMSG%s}#H，終於不用在物品多的時候為扔掉哪個而發愁了！",
}
...
function x332204_OnConditionCheck( sceneId, selfId )
	...
	--hd add
	if (1) then
		x332204_ShowNotice( sceneId, selfId, "V ph ch遖 k韈h ho誸");
		return 0;
	end
	--end hd add
...
function x332204_OnTest( sceneId, selfId )
    ...
    local sysstr = "=====================#r"
    local TotalItem = 0
    sysstr = sysstr.."總共物品種類"..ListSize..",具體數據:"
