...
function x335136_OnConditionCheck( sceneId, selfId )
	...
	if LuaFnIsStalling(sceneId, selfId) == 1  then
	  if itemIndex == x335136_g_YuRenBaoZhuID then
			...
			x335136_MsgBox( sceneId, selfId, "Ğ¯i phß½ng ğang · trÕng  thái không th¬ sØ døng v§t ph¦m này");
		end
		return 0
	end
	...
	if(LuaFnGetDRide(sceneId, selfId, targetId) == 1)then
		if itemIndex == x335136_g_YuRenBaoZhuID then
			...
			x335136_MsgBox( sceneId, selfId, "Ğ¯i phß½ng ğang · trÕng  thái cùng cßŞi không th¬ sØ døng v§t ph¦m này");
		end
		return 0
	end
  ...
  if flag == 0 then --0±íÊ¾Íæ¼Ò²»ÔÚËÄ´óÖ÷³ÇÖĞ
		if itemIndex == x335136_g_YuRenBaoZhuID then
			...
			x335136_MsgBox( sceneId, selfId, "Chï có th¬ · ĞÕi Lı, LÕc Dß½ng, Tô Châu và Lâu Lan")
		end
		return 0
	end
...
function x335136_OnActivateOnce( sceneId, selfId )
  ...
  local itemIndex = LuaFnGetItemIndexOfUsedItem(sceneId, selfId);
	...
	if (itemIndex == x335136_g_YuRenBaoZhuID) or (itemIndex == 30501162) then
	...
	if (bRet == 1) then	--ÂäÂí¼ı³É¹¦
	  ...
	  if (itemIndex == x335136_g_YuRenBaoZhuID) then
		...
		elseif (itemIndex == 30504315) then --QuÖ Trang ThuÖ Thái
			local ImpactIndex = x335136_g_ImpactTable[3][random(x335136_g_ImpactCount)]	--Ëæ»úÑ¡ÔñĞ§¹û
			bRet = LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, targetId, ImpactIndex, 0)		--ÔÙÌí¼Ó±äÉíĞ§¹û
		elseif (itemIndex == 30504316) then --K©o Halloween
			local ImpactIndex = x335136_g_ImpactTable[4][random(x335136_g_ImpactCount)]	--Ëæ»úÑ¡ÔñĞ§¹û
			bRet = LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, targetId, ImpactIndex, 0)		--ÔÙÌí¼Ó±äÉíĞ§¹û
		end
	end
	...
	if(bRet == 1) then --±äÉí³É¹¦
		--zchw
		if (itemIndex == x335136_g_YuRenBaoZhuID) then
			 szMsg = format("Các hÕ ğã b¸ %s sØ døng Th¢ng B¶m Bäo Châu", GetName(sceneId,selfId))
		elseif (itemIndex == 30501162) then --Ê¦Í½±¦Öé
			 szMsg = format("Các hÕ ğã b¸ %s sØ døng Sß Ğ° Bäo Châu", GetName(sceneId,selfId))
		end	
		szMsg =  format("Các hÕ ğã b¸ %s t¤n công b¢ng phß½ng thÑc ğ£c thù", GetName(sceneId,selfId))
