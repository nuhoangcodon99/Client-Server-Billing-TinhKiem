...
x300085_g_scenePosInfoList = { {sceneId=7,  scenename="劍閣", PosName="劍門疊翠", PosX=130, PosY=140,  PosR=10, Area=711 },
															 {sceneId=8,  scenename="敦煌", PosName="瀚海求佛", PosX=267, PosY=251,  PosR=10, Area=811 },
															 {sceneId=5,  scenename="鏡湖", PosName="玉帶臨風", PosX=35 , PosY=265,  PosR=10, Area=511 },
															 {sceneId=4,  scenename="太湖", PosName="舞榭歌臺", PosX=155, PosY=255,  PosR=10, Area=411 },
															 {sceneId=3,  scenename="嵩山", PosName="江山多嬌", PosX=280, PosY=80,   PosR=10, Area=311 },
															 {sceneId=30, scenename="西湖", PosName="一望虎跑", PosX=170, PosY=230,  PosR=10, Area=3011},															 
															 {sceneId=31, scenename="龍泉", PosName="飛流直下", PosX=270, PosY=280,  PosR=10, Area=3111},
															 {sceneId=25, scenename="蒼山", PosName="似水流年", PosX=260, PosY=110,  PosR=10, Area=2512},															 
															 {sceneId=32, scenename="武夷", PosName="煙鎖二喬", PosX=50 , PosY=180,  PosR=10, Area=3211},															 
															 {sceneId=0,  scenename="洛陽", PosName="金城湯池", PosX=50,  PosY=220,  PosR=10, Area=11  } }
...
function x300085_OnConditionCheck( sceneId, selfId )
	...
	if LuaFnLockCheck( sceneId, selfId, bagId, 0 ) < 0 then
		x300085_MsgBox( sceneId, selfId, "此物品已被鎖定！" )
		return 0
	end
	...
	local str = format("千尋任務第%d環: 去#G%s#W探索#G%s#{_INFOAIM%d,%d,%d,%s}#W，一覽江湖美景吧。抵達後請使用#Y尋路之心#W。", nRoundNum, ScintInfo.scenename, ScintInfo.PosName, ScintInfo.PosX, ScintInfo.PosY, ScintInfo.sceneId, ScintInfo.scenename)		
	...
	if Distance > ScintInfo.PosR then
		BeginEvent(sceneId)
			AddText(sceneId,str)
			AddText(sceneId,"距離施放尋路之心的地方還有"..Distance.."米")
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,-1)
	...
	elseif Distance <= ScintInfo.PosR then  		
		...
		elseif nMisType == x300085_g_ShiTuMission_IDX then
			...
			x300085_NotifyTip( sceneId, selfId, "使用物品失敗" )		
			return 0
		end  		
