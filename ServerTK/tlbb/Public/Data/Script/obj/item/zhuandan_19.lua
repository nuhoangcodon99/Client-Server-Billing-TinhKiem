...
function x889018_OnActivateOnce( sceneId, selfId )
    ...
    if( RandomBase > 0 ) then    
        ...
        local PrizeNum = 0
        if (RandomID == x889018_g_SockPrize[4] or RandomID == x889018_g_SockPrize[6] or
        	RandomID == x889018_g_SockPrize[49]) then
          PrizeNum = 2
        else
          PrizeNum = 1
        end
			     ...
			     if Ret > 0 then
			       	...
			       	if 1 == Notice then
			       	   local szItemTransfer = GetItemTransfer(sceneId,selfId,0)
					       x889018_ShowRandomSystemNotice( sceneId, selfId, szItemTransfer )
				      end
...
function x889018_ShowRandomSystemNotice( sceneId, selfId, strItemInfo )
	
	local PlayerName = GetName(sceneId,selfId)
	local nMsgIndex = random( 1, 3 )
	local str
	if nMsgIndex == 1 then
		str = format( x889018_g_strGongGaoInfo[1], PlayerName, strItemInfo)
	elseif nMsgIndex == 2 then
		str = format( x889018_g_strGongGaoInfo[2], PlayerName, strItemInfo )
	else
		str = format( x889018_g_strGongGaoInfo[3], PlayerName, strItemInfo)
	end
