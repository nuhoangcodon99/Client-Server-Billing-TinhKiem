...
function x300022_DiscoverItem(sceneId, selfId)
		...
		strText = format("#W#{_INFOUSR%s}#cff99cc ·#G%s#cff99cc ðang ðào bäo v§t thì may m¡n nh£t ðßþc#W#{_INFOMSG%s}", PlayName, sceneName, ItemName)
		--PrintNum(bBroadCast)
...
function x300022_DiscoverCorpseMonster(sceneId, selfId)
		...
		local strText = format("#W#{_INFOUSR%s}#cff99cc Trong lúc ðào bäo v§t không mai b¸ lµ thông tin, tÕi #G%s#cff99cc dçn ðªn 1 tên ÐoÕt bäo Mã t£c, Giang h° chi sî chï c¥n ði tiêu di®t b÷n Mã t£c, s¨ có th¬ nh§n ðßþc Bäo tàng cüa Mã t£c!", 
...
function x300022_OnDefaultEvent( sceneId, selfId, BagPos)
	...
	--PrintStr(sceneName)
	local strText = format("Các hÕ phäi ðªn %s toÕ ðµ [%d,%d] m¾i có th¬ sØ døng", sceneName, targetX, targetZ)
