...
function x300067_OnConditionCheck( sceneId, selfId )
	...
	if 30700201 == BookId then
		if PlayerLevel < 90 then
			x300067_ShowTips( sceneId, selfId, "此要訣只適合90級以上（包括90級）的玩家修煉" )
			return 0
		end
	elseif 30700202 == BookId then
		if PlayerLevel >= 90 then
			x300067_ShowTips( sceneId, selfId, "此要訣只適合90級以下（不含90級）的玩家修煉" )
			return 0
		end
	...
	--此要诀在本场景使用无效....
	if sceneId ~= 194 then
		x300067_ShowTips( sceneId, selfId, "此要訣在本場景使用無效" )
		return 0
	end
	...
	if CurDayTime <= lastDayTime then
		x300067_ShowTips( sceneId, selfId, "您今天已經使用過寒玉谷行功要訣" )
		return 0
	end

	--当前是否已有挂机buff....
	if LuaFnHaveImpactOfSpecificDataIndex( sceneId, selfId, x300067_g_NoRMBBuffID ) == 1 then
		x300067_ShowTips( sceneId, selfId, "你已經在修煉中" )
		return 0
	end
