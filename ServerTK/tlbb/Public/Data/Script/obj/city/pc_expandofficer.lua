...
function x805013_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			AddNumText( sceneId, x805013_g_scriptId, "#G#bÐóng góp 100 ði¬m #Ykhuyªch trß½ng#G cho Bang", 6, 100 )
		else
...
function x805013_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 100 then
		...
		local sMessage = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#cffff00 tÕi #c00FFFFÐông Phù Dung#cffff00, ðã c¯ng hiªn cho Bang phái 100 ði¬m khuyªch trß½ng! Các bang chúng khác nên h÷c t§p!", PlayerName);	
		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
		
		BeginEvent(sceneId)
		AddText( sceneId, "#WÐã ðóng góp thành công #Y100 #Wði¬m khuyªn trß½ng cho #GBang Hµi" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	end
