...
function x805008_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			AddNumText( sceneId, x805008_g_scriptId, "#G#bÐóng góp 100 ði¬m #Ycông nghi®p#G cho Bang", 6, 100 )
...
function x805008_SetFavorOfGuild( sceneId, selfId )
	...
	if( Point > 0 ) then
	    ...
	    LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 7800 + BuffLevel, 0)   --ÉèÖÃbuff
	end
...
function x805008_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 100 then
		...
		local sMessage = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#cffff00 tÕi #c00FFFFMã ºng Hùng#cffff00, ðã c¯ng hiªn cho Bang phái 100 ði¬m công nghi®p! Các bang chúng khác nên h÷c t§p!", PlayerName);	
		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
		
		BeginEvent(sceneId)
		AddText( sceneId, "#WÐã ðóng góp thành công #Y100 #Wði¬m công nghi®p cho #GBang Hµi" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
