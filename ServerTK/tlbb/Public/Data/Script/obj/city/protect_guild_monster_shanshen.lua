...
function x805045_OnDie( sceneId, selfId, killerId )
	...
	local message;
	message = format("#cff99cc#{_INFOUSR%s}面對強敵，率領隊伍展開了艱苦卓絕的%s保衛戰，山神損兵折將，無法攻克，最後終於放棄了摧毀城市的計畫。", nam_ply, cityName );
...
function x805045_OnInit(sceneId, selfId)
	...
	LuaFnNpcChat(sceneId, selfId, 1, "山神的威嚴！豈容冒犯！")
end
...
function x805045_CreateNpc(sceneId, NpcId, x, y, Ai, AiFile, Script)
	...
	if (strNpcName == "大山鬼") or (strNpcName == "山神") then
			SetCharacterTitle(sceneId, nNpcId, "“石頭怪人”")
	end
