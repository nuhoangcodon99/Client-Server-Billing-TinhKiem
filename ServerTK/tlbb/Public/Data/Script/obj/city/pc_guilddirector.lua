...
x805009_g_NoMissionInfo = "    Trong bang hình nhß không có xây dñng cái gì. "
x805009_g_NoMissionInfo1 = "    Trong bang hình nhß không có nghiên cÑu cái gì. "
--**********************************
--ÊÂ¼þÁÐ±í
--**********************************
...
--		if CityGetSelfCityID( sceneId, selfId ) == sceneId then
		...
		local strGUID = LuaFnGetGUID( sceneId, selfId )
			...
			AddNumText( sceneId, x805009_g_scriptId, "#b#cff99ccTiªn hành xây dñng", 6, 100 )
			AddNumText( sceneId, x805009_g_scriptId, "#b#cff99ccTiªn hành nghiên cÑu", 6, 101 )
			...
			for i, titleItem in x805009_g_GuildContriTitle do
				...
				if currGuildContriTitle == guildName..titleItem.currName or currGuildContriTitle == guildName.." "..titleItem.currName then
					AddNumText( sceneId, x805009_g_scriptId, "Tñ l§p chÑc v¸ "..titleItem.name, 6, 30 )
					break;
				end
			end
...
function x805009_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 100 then
		...
		if number == 0  then
			BeginEvent(sceneId)
				AddText(sceneId, "  #WCác hÕ c¥n mang theo #G#{_ITEM30505205} #Wm¾i có th¬ sØ døng chÑc nång này." );
			EndEvent(sceneId)
			DispatchEventList(sceneId,selfId,targetId)
			return
		end
 ...
 --»Øµ½´óÀí
	elseif GetNumText() == 27 then
		if GetItemCount(sceneId, selfId, x805009_g_Yinpiao)>=1  then
			BeginEvent( sceneId )
				AddText( sceneId, "  Các hÕ ðang thñc hi®n nhi®m vø v§n chuy¬n, ta không th¬ giúp các hÕ !" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
	...
	elseif GetNumText() == 29 then
		...
		AddNumText( sceneId, x805009_g_scriptId, "V« LÕc Dß½ng", 9, 11 )
		...
		for i, titleItem in x805009_g_GuildContriTitle do
			...
			if currGuildContriTitle == guildName..titleItem.currName or currGuildContriTitle == guildName.." "..titleItem.currName then	
				...
				if currGuildContriSum < titleItem.needContri then
					local strTip = "#R"..playerName.."#W mu¯n tñ l§p chÑc v¸ cüa Bang #G"..guildName..titleItem.name.."#W c¥n phäi có #G"..titleItem.needContri.."#W ði¬m c¯ng hiªn, hãy c¯ng hiªn cho bang hµi nhi«u h½n næa !";
					BeginEvent( sceneId )
				...
				local strTip = "Xin chúc m×ng #R"..playerName.."#W ðã nh§n thành công danh hi®u #G"..guildName..titleItem.name.."#W cüa Bang Hµi, hãy vì danh hi®u mà phát dß½ng quang ðÕi cho Bang Hµi."
				
				--Ôö¼ÓÌØÐ§
				LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, 49, 0);
		
				local sMessage = format("@*;SrvMsg;GLD:#WNh¶ sñ giúp ðÞ cüa nhi«u ngß¶i #R#{_INFOUSR%s}#W ðã ðÕt thành công danh hi®u thành viên cüa Bang Hµi #G%s#W Hy v÷ng #R#{_INFOUSR%s}#W ngày càng tång thêm n² lñc.", playerName, guildName..titleItem.name ,playerName);	
	    		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
			
			
				sMessage = format("#W#{_INFOUSR%s} thuµc Bang #G%s#W ðÕt thành công danh hi®u #G%s#W.", playerName,guildName, guildName..titleItem.name );
...
function x805009_BanggongExchange( sceneId, selfId, nvalue ) --Õâ¸öÊÇÍæ¼ÒÊäÈëÖµ£¬Êµ¼ÊµÄ¿Û³ýÖµ»¹ÒªÔÚ´Ë»ù´¡ÉÏ+10%µÄË°ÂÊ
	...
	if ret == 1 then
		...
		if BagIndex ~= -1 then
			...
			x805009_NotifyFailTips( sceneId, selfId, "TÕo v§t ph¦m th¤t bÕi")
		end
	else
		x805009_NotifyFailTips( sceneId, selfId, "Kh¤u tr× bang c¯ng th¤t bÕi")
	end
