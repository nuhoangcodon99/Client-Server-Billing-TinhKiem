...
function x805014_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			AddNumText( sceneId, x805014_g_scriptId, "#G#bÐóng góp 100 ði¬m #Ynông nghi®p#G cho Bang", 6, 100 )
...
function x805014_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 100 then
		...
		local sMessage = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#cffff00 tÕi #c00FFFFChu Thª Hæu#cffff00, ðã c¯ng hiªn cho Bang phái 100 ði¬m nông nghi®p! Các bang chúng khác nên h÷c t§p!", PlayerName);	
		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
		
		BeginEvent(sceneId)
		AddText( sceneId, "#WÐã ðóng góp thành công #Y100 #Wði¬m nông nghi®p cho #GBang Hµi" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
...
function x805014_BingZhenXiGua(sceneId, selfId, targetId)
	...
	local strChatMessage = "#{_INFOUSR"..selfName.."}#P h¾n h· t× trong tay#G Chu Thª hæu[129, 100]#P l¤y 1 miªng#Y#{_INFOMSG" .. szTransferItem .. "}#cff99cc, ån 1 miªng vào mát t× ð¥u xu¯ng ðªn bàn chân!";
