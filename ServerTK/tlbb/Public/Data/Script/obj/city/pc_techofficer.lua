...
function x805015_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			AddNumText( sceneId, x805015_g_scriptId, "SØa chæa danh xßng bang hµi", 6, 3 )
			...
			AddNumText( sceneId, x805015_g_scriptId, "#G#bÐóng góp 100 ði¬m #Ykhoa h÷c#G cho Bang", 6, 100 )
...
function x805015_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 20 then																--ÁìÈ¡ÌÌ½ðÃûÌû	--add by xindefeng
		...
		BeginEvent(sceneId)
			...
			--È·¶¨¶Ò»»°´Å¥
			AddNumText(sceneId, x805015_g_scriptId, "Có, tôi ð°ng ý", 6, 201)				
			--·µ»ØÉÏÒ»Ò³°´Å¥
			AddNumText(sceneId, x805015_g_scriptId, "Không, tôi nghî lÕi", 6, 202)		
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
	...
	elseif GetNumText() == 100 then
		...
		local sMessage = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#cffff00 tÕi #c00FFFFHµi H÷a#cffff00, ðã c¯ng hiªn cho Bang phái 100 ði¬m khoa h÷c! Các bang chúng khác nên h÷c t§p!", PlayerName);	
		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
		
		BeginEvent(sceneId)
		AddText( sceneId, "#WÐã ðóng góp thành công #Y100 #Wði¬m khoa h÷c cho #GBang Hµi" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
...
function x805015_GetTangJinMinTie(sceneId, selfId, targetId)
  ...
  if nBangGongPoint < x805015_g_MingTieNeedBangGong then
  	szMsg = format("    Ði¬m c¯ng hiªn ít h½n so v¾i Bang Hµi#G%d#W, không th¬ thñc hi®n ðßþc.", x805015_g_MingTieNeedBangGong)
  ...
  local ret = CityChangeAttr(sceneId, selfId, 6, -x805015_g_MingTieNeedBangGong)
	if not ret or ret ~= 1 then		
		x805015_NotifyFailTips(sceneId, selfId, "    Kh¤u tr× Bang C¯ng th¤t bÕi, vui lòng thØ lÕi")	--ÐÑÄ¿ÌáÊ¾"ÎÞ·¨É¾³ý°ï¹±"
		return
	end
	...
	BeginEvent( sceneId )
		AddText( sceneId, "   Ðây là mµt nâng kim danh thiªp, hãy sØ døng nó th§t t¯t" )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
end
