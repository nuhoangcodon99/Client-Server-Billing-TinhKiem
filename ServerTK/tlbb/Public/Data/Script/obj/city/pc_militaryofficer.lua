...
function x805028_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x805028_g_scriptId, "#G#bÐóng góp 100 ði¬m #Yqu¯c phòng#G cho Bang", 6, 100 )

		-- ³ÇÊÐÁìË«
		AddNumText( sceneId, x805028_g_scriptId, "Lînh 2 l¥n kinh nghi®m", 6, 6 )
		
		AddNumText( sceneId, x805028_g_scriptId, "Nh§n 1 miªng Ngân Bì L®nh Ti­n", 6, 7 )
...
function x805028_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 100 then
		...
		--add ði¬m khuyªch trß½ng
		CityChangeAttr( sceneId, selfId, GUILD_DEF_LEVEL, 100 )
		local  PlayerName=GetName(sceneId,selfId);
		local sMessage = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#cffff00 tÕi #c00FFFFVõ ÐÕi Uy#cffff00, ðã c¯ng hiªn cho Bang phái 100 ði¬m qu¯c phòng! Các bang chúng khác nên h÷c t§p!", PlayerName);	
		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
		
		BeginEvent(sceneId)
		AddText( sceneId, "#WÐã ðóng góp thành công #Y100 #Wði¬m qu¯c phòng cho #GBang Hµi" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )	
	...
	elseif GetNumText() == 7 then
		...
		if GetItemCount(sceneId, selfId, x805028_g_Item) >= 1  then
			BeginEvent(sceneId)
				AddText( sceneId, " Các hÕ ðã có Ngân Bì L®nh Ti­n r°i. Còn không nhanh chóng ði tr× phï, còn ðþi ðªn khi nào ?" )
			EndEvent(sceneId)
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
		...
		if CityGetAttr(sceneId, selfId, 6) < 50   then
			BeginEvent(sceneId)
				AddText( sceneId, " Nh§n lînh 1 miªng Ngân Bì L®nh Ti­n c¥n tiêu hao 50 ði¬m c¯ng hiªn bang hµi. Các hÕ hãy vì bang hµi làm thêm tý vi®c r°i m¾i bàn tiªp." )
			EndEvent(sceneId)
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
		...
		BeginEvent(sceneId)
			AddText( sceneId, " Nh§n lînh 1 miªng Ngân Bì L®nh Ti­n c¥n tiêu hao 50 ði¬m c¯ng hiªn bang hµi. Các hÕ xác ð¸nh nh§n lînh không ?" )
			AddNumText( sceneId, x805028_g_scriptId, "Ðúng, ta xác nh§n.", 6, 8 )
			AddNumText( sceneId, x805028_g_scriptId, "Không, ta vçn không mu¯n.", 6, 9 )
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
		return
	...
	elseif GetNumText() == 8 then
		...
		if ret ~= 1  then
			BeginEvent(sceneId)
				AddText( sceneId, " Túi ðñng ð° không ðü." )
			EndEvent(sceneId)
			DispatchEventList( sceneId, selfId, targetId )
			return
		end
		...
		BeginEvent(sceneId)
			AddText( sceneId, " Các hÕ ðã nh§n ðßþc Ngân Bì L®nh Ti­n r°i. Mau chóng ði Kính H° tr× phï ði!" )
		EndEvent(sceneId)
		DispatchEventList( sceneId, selfId, targetId )
		return
