...
function x805017_UpdateEventList( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		if Humanguildid == cityguildid then
			...
			AddNumText( sceneId, x805017_g_scriptId, "#G#bÐóng góp 100 ði¬m #Ythß½ng nghi®p#G cho Bang", 6, 100 )
...
function x805017_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	elseif GetNumText() == 100 then
		...
		local  PlayerName=GetName(sceneId,selfId);
		local sMessage = format("@*;SrvMsg;GLD:#{_INFOUSR%s}#cffff00 tÕi #c00FFFFMµc D¸ch#cffff00, ðã c¯ng hiªn cho Bang phái 100 ði¬m thß½ng nghi®p! Các bang chúng khác nên h÷c t§p!", PlayerName);	
		BroadMsgByChatPipe(sceneId, selfId, sMessage, 6);
		
		BeginEvent(sceneId)
		AddText( sceneId, "#WÐã ðóng góp thành công #Y100 #Wði¬m thß½ng nghi®p cho #GBang Hµi" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	end
end
