...
function x014001_OnDefaultEvent( sceneId, selfId,targetId )
	...
	if x014001_g_MenPai == 8 then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x014001_g_scriptId, "H÷c tâm pháp Tiêu Dao",12,0)
			AddNumText(sceneId, x014001_g_scriptId, "V« h÷c tâm pháp cüa Tiêu Dao phái",11,10)
			--AddNumText(sceneId, x014001_g_scriptId, "#{JZBZ_081031_02}",11,11)
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
	else
			BeginEvent(sceneId)
			AddText(sceneId,"Ngß½i không phäi ð® tØ Tiêu Dao. Ngß½i ðªn do thám b±n phái hä?")
