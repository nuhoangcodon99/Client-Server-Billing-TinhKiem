...
function  x014004_OnDefaultEvent( sceneId, selfId,targetId )
	BeginEvent(sceneId)
		AddText(sceneId,"Ta là sñ phø cüa nhæng con Tu¥n lµc · Tiêu Dao này. Ngß½i mu¯n h÷c gì nào ?")
		...
		AddNumText( sceneId, x014004_g_ScriptId, "V« h÷c kÜ thu§t", 11, 100 )
	EndEvent(sceneId)
...
function  x014004_OnEventRequest( sceneId, selfId, targetId, eventId )
  ...
  if GetNumText() == 40 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x014004_g_ScriptId,"Ğ°ng ı",-1,0)
      			AddNumText(sceneId, x014004_g_ScriptId,"Không, cäm ½n",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
	end
	...
	if GetNumText() == 60 then
		BeginEvent( sceneId )
			...
			AddNumText(sceneId, x014004_g_ScriptId,"Ğ°ng ı",-1,1)
      			AddNumText(sceneId, x014004_g_ScriptId,"Không, cäm ½n",-1,999)			
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
		return
