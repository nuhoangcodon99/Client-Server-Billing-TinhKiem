...
x806001_g_msg_swear["nmm"]	= " SØa danh hi®u kªt bái c¥n có #{_EXCHG%d}, ngân lßþng hi®n có trên ngß¶i các hÕ không ðü"
...
x806001_g_msg_swear["wait"]	= " Ðang ch¶ phúc ðáp"
...
function x806001_CheckAccept( sceneId, selfId, targetId )
	...
	while i < TeamSizeSameScene do
		...
		if IsInDist( sceneId, theID, targetId, 6 ) ~= 1 then
			...
			x806001_MessageBox( sceneId, selfId, targetId, "  trong nhóm cüa các hÕ #R" .. theName .. "#Wcách ta quá xa, nên ta không th¬ kªt bái cho các ngß½i" )
...
function x806001_OnSubmit( sceneId, selfId, targetId )
	BeginEvent( sceneId )
		...
		AddNumText( sceneId, x806001_g_ScriptId, "HuÖ", 8, x806001_g_key["unall"] )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
end
