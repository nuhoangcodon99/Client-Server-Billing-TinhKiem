		...
		BeginEvent( sceneId )
			if LuaFnHaveMaster( sceneId, selfId ) == 0 then
				...
				if GetMissionFlag(sceneId, selfId, MF_ShiTu_ChuShi_Flag) == 1 then --±íÊ¾ÒÑ¾­³öÊ¦
					AddText( sceneId, "  Các hÕ ğã xu¤t sß r°i, không th¬ xu¤t sß næa." )
				else
					AddText( sceneId, "  Ta mu¯n xu¤t sß" )
				end
			else
				AddText( sceneId, str )
				AddNumText( sceneId, x806006_g_ScriptId, "Ğ°ng ı", 6, 1 )
				AddNumText( sceneId, x806006_g_ScriptId, "Bö qua", 6, 2 )	--add by xindefeng
			end
		EndEvent( sceneId )
...
function x806006_OnAccept( sceneId, selfId, targetId )
	...
	--Èç¹ûÖ»¿Û³ı½ğ±Ò
	if nMoneyJZ == 0 then
		str	= format( "Giäi tr× quan h® sß ğ°, c¥n #{_MONEY%d}.", x806006_g_Gold )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	--Èç¹ûÖ»¿Û³ı½»×Ó
	if nMoneyJB == 0 then
		str	= format( "Giäi tr× quan h® sß ğ°, c¥n#{_EXCHG%d}.", x806006_g_Gold )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	--Èç¹û¼ÈÓĞ½»×ÓÓÖÓĞ½ğ±Ò
	if nMoneyJB ~= 0 and nMoneyJZ ~= 0 then
		str	= format( "Giäi tr× quan h® sß ğ°, c¥n#{_EXCHG%d}.".."#{_MONEY%d}.", nMoneyJZ, nMoneyJB )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	x806006_MsgBox( sceneId, selfId, targetId, "  Giäi tr× quan h® v¾i "..MasterName.." ." )

	--·¢ÆÕÍ¨ÓÊ¼ş¸øÊ¦¸µ
	LuaFnSendSystemMail( sceneId, MasterName, "Ğ° ğ® cüa các hÕ " .. selfName .. " ğã xu¤t sß, giäi tr× quan h® sß ğ° v¾i các hÕ" )
