	...
	if key == -1 then
		...
		for i=0, 7 do
			...
			if guid ~= -1 then
				...
				AddNumText( sceneId, x806009_g_ScriptId, "S® "..PrenticeName.." B∏ u±i ra khˆi sﬂ mÙn", 6, i )
			end
		end
...
function x806009_OnAccept( sceneId, selfId, targetId, nIndex )
	...
	--str	= format( "Gi‰i tr◊ quan hÆ sﬂ ∞, c•n#{_MONEY%d}°£", x806009_g_Gold )
	...
	--»Áπ˚÷ªø€≥˝Ωª◊”
	if nMoneyJB == 0 then
		str	= format( "Gi‰i tr◊ quan hÆ sﬂ ∞, c•n#{_EXCHG%d}.", x806009_g_Gold )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	--»Áπ˚º»”–Ωª◊””÷”–Ω±“
	if nMoneyJB ~= 0 and nMoneyJZ ~= 0 then
		str	= format( "Gi‰i tr◊ quan hÆ sﬂ ∞, c•n#{_EXCHG%d}, {_MONEY%d}.", nMoneyJZ, nMoneyJB )
		Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
	end
	x806009_MsgBox( sceneId, selfId, targetId, "   Gi‰i tr◊ quan hÆ væi  "..PrenticeName..". " )

	--∑¢∆’Õ®” º˛∏¯ÕΩµ‹
	LuaFnSendSystemMail( sceneId, PrenticeName, "Sﬂ ph¯ c¸a c·c h’ " .. selfName .. " „ gi‰i tr◊ quan hÆ sﬂ ∞ væi c·c h’" )
