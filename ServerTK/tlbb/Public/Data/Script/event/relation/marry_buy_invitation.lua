...
function x806017_OnEnumerate(sceneId, selfId, targetId)
	...
	--if isMarried and isMarried == 0 then
		AddNumText(sceneId, x806017_g_ScriptId, "Mua thïnh thi®p", 7, x806017_g_eventId_update);
	--end
end
...
function x806017_OnSelectCount(sceneId, selfId, targetId, itemLevel, itemCount)
	if itemLevel and itemCount then
		BeginEvent(sceneId);
			...
			AddNumText(sceneId, x806017_g_ScriptId, "HuÖ", 8, itemLevel * 100 + x806017_g_eventId_select_level);
		EndEvent(sceneId);
		DispatchEventList(sceneId, selfId, targetId);
	end
end
