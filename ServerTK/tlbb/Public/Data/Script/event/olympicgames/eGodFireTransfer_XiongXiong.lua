...
x808099_g_AccomplishNPC_Name="刀堯"
...
x808099_g_Custom	= { {id="已將火炬交給洱海的刀堯",num=1} }
--MisDescEnd

--任务是否完成
x808099_g_Mission_IsComplete = 0		--任务参数的第0位
--圣火交给刀尧的标记
x808099_g_RecordIdx 				 = 1		--任务参数的第1位
--任务脚本号记录
x808099_g_MissScriptID_Idx	 = 2		--任务参数的第2位
--任务发布NPC标记
x808099_g_AcceptNPC_Idx			 = 3		--任务参数的第3位 25.洱海NPC


x808099_g_AcceptMission_IDX		= 965	--熊熊的圣火接收任务索引
x808099_g_CompleteMission_IDX	= 966	--熊熊的圣火提交任务索引
x808099_g_MissionInfo_IDX			= 967	--任务说明索引

--所拥有的事件ID列表
x808099_g_EventList	= {}

x808099_g_Impact_Accept_Mission 	 = 47		-- 接受任务时的特效ID
x808099_g_Impact_Transport_Mission = 113 	-- 运输状态特效
x808099_g_Impact_GodOfFireMan_Mission  = 5942 -- 圣火传递男变身特效
x808099_g_Impact_GodOfFireGirl_Mission = 5943 -- 圣火传递女变身特效
x808099_g_Impact_DelGodOfFire_Mission  = 5944 -- 删除圣火特效的特效
x808099_g_PlayerSlow_LVL					 = 10		-- 接受任务的最低等级

x808099_g_Mission_StepNum					 = 3		-- 本任务在任务链的第几步

x808099_g_ItemId = { id=40004447,num=1 }	-- 任务物品
...
function x808099_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x808099_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x808099_g_Name then
			x808099_NotifyTip( sceneId, selfId, "接受任務失敗" )					
			return 0
		end
		-- 任务是否已满
	...
	elseif key == x808099_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x808099_g_AccomplishNPC_Name then
			x808099_NotifyTip( sceneId, selfId, "提交任務失敗" )					
			return 0
		end
		-- 如果已经接了任务熊熊的圣火
	...
	if LuaFnGetName( sceneId, targetId ) == x808099_g_Name
		 ...
		 if IsHaveMission( sceneId, selfId, x808099_g_MissionId ) <= 0 then
				...
				if nStepNum == x808099_g_Mission_StepNum then
					AddNumText( sceneId, x808099_g_ScriptId, "我要參加：熊熊的聖火", 7, x808099_g_AcceptMission_IDX )
				end
		 end
		-- AddNumText( sceneId, x808099_g_ScriptId, "熊熊的圣火介绍", 11, x808099_g_MissionInfo_IDX )
	elseif LuaFnGetName( sceneId, targetId ) == x808099_g_AccomplishNPC_Name
		...
		if IsHaveMission( sceneId, selfId, x808099_g_MissionId ) > 0 then
			...
			if GetMissionParam(sceneId, selfId, misIndex, x808099_g_Mission_IsComplete) <= 0 then
				...
				AddNumText( sceneId, x808099_g_ScriptId, "我已經完成：熊熊的聖火", 7, x808099_g_CompleteMission_IDX )
			end
		end
	else
		return 0
...
function x808099_OnAbandon( sceneId, selfId )
  ...
  if HaveItem(sceneId, selfId, x808099_g_ItemId.id) > 0 then
  	if LuaFnGetAvailableItemCount(sceneId, selfId, x808099_g_ItemId.id) >= x808099_g_ItemId.num then
	  	...
	  	x808099_NotifyTip( sceneId, selfId, "您的物品現在不可用或已被鎖定。" )			
			return 0
  	end
  end
...
--杀死怪物或玩家
--**********************************
function x808099_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
end
