...
x808104_g_AccomplishNPC_Name="周無畏"
...
x808104_g_Custom	= { {id="已擊敗足球",num=5}, {id="已擊敗小齊齊",num=1} }
--MisDescEnd

--任务是否完成
x808104_g_Mission_IsComplete = 0		--任务参数的第0位
--击败足球的标记
x808104_g_KillBallIdx				 = 1		--任务参数的第1位
--击败小齐齐的标记
x808104_g_KillSunMMIdx 			 = 2		--任务参数的第2位

x808104_g_CompleteGengQiang_IDX	=	775	--圣火传递更强提交任务索引
x808104_g_MissionInfo_IDX				= 776	--更强副本任务说明索引

--所拥有的事件ID列表
x808104_g_EventList	= {}

x808104_g_Impact_Accept_Mission 	 = 47		-- 接受任务时的特效ID
...
x808104_g_Impact_DelGodOfFire_Mission  = 5944 -- 删除圣火特效的特效
x808104_g_PlayerSlow_LVL					 = 10		-- 接受任务的最低等级

x808104_g_Mission_StepNum					 = 7		-- 本任务在任务链的第几步
...
function x808104_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnGetName( sceneId, targetId ) ~= x808104_g_Name then
		x808104_NotifyTip( sceneId, selfId, "接受任務失敗" )					
		return 0
	end
...
function x808104_OnEnumerate( sceneId, selfId, targetId )
	...
	if IsHaveMission( sceneId, selfId, x808104_g_MissionId ) > 0 then
		...
		if GetMissionParam(sceneId, selfId, misIndex, x808104_g_Mission_IsComplete) > 0 then
			AddNumText( sceneId, x808104_g_ScriptId, "我已經完成：聖火傳遞更強", 7, x808104_g_CompleteGengQiang_IDX )
...
function x808104_OnAbandon( sceneId, selfId )
  ...
  if HaveItem(sceneId, selfId, x808104_g_ItemId.id) > 0 then
  	if LuaFnGetAvailableItemCount(sceneId, selfId, x808104_g_ItemId.id) >= x808104_g_ItemId.num then
	  	...
	  	x808104_NotifyTip( sceneId, selfId, "您的物品現在不可用或已被鎖定。" )			
			return 0
  	end
  end
...
function x808104_CheckSubmit( sceneId, selfId, targetId )
	...
	--判断该npc是否是对应任务的npc
	if LuaFnGetName( sceneId, targetId ) ~= x808104_g_AccomplishNPC_Name then
		x808104_NotifyTip( sceneId, selfId, "提交任務失敗" )					
		return 0
	end
