...
x808102_g_Custom	= { {id="已連續答對燕青的5個問題",num=1} }
--MisDescEnd

--任务是否完成
x808102_g_Mission_IsComplete = 0		--任务参数的第0位
--答题标记
x808102_g_AnswerCntIdx			 = 1		--任务参数的第1位

x808102_g_AcceptMission_IDX		= 575	--运动的梦想接收任务索引
x808102_g_CompleteGengGao_IDX	=	576	--运动的梦想提交任务索引
x808102_g_MissionInfo_IDX			= 577	--运动的梦想任务说明索引

--所拥有的事件ID列表
x808102_g_EventList	= {}

x808102_g_Impact_Accept_Mission 	 = 47		-- 接受任务时的特效ID
x808102_g_Impact_Transport_Mission = 113 	-- 运输状态特效
x808102_g_Impact_GodOfFireMan_Mission  = 5942 -- 圣火传递男变身特效
x808102_g_Impact_GodOfFireGirl_Mission = 5943 -- 圣火传递女变身特效
x808102_g_Impact_DelGodOfFire_Mission  = 5944 -- 删除圣火特效的特效
x808102_g_PlayerSlow_LVL					 = 10		-- 接受任务的最低等级

x808102_g_Mission_StepNum					 = 5		-- 本任务在任务链的第几步

x808102_g_ItemId = { id=40004447,num=1 }	-- 任务物品
...
function x808102_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnGetName( sceneId, targetId ) ~= x808102_g_Name then
		x808102_NotifyTip( sceneId, selfId, "接受任務失敗" )					
		return 0
	end
...
function x808102_OnEnumerate( sceneId, selfId, targetId )
	...
	if IsHaveMission( sceneId, selfId, x808102_g_MissionId ) <= 0 then
		...
		if nStepNum == x808102_g_Mission_StepNum then
			AddNumText( sceneId, x808102_g_ScriptId, "我要參加：運\動的夢想", 7, x808102_g_AcceptMission_IDX )
		end
	else
		...
		if GetMissionParam(sceneId, selfId, misIndex, x808102_g_Mission_IsComplete) > 0 then
			AddNumText( sceneId, x808102_g_ScriptId, "我已經完成：運\動的夢想", 7, x808102_g_CompleteGengGao_IDX )
		end
	end
	-- AddNumText( sceneId, x808102_g_ScriptId, "运动的梦想介绍", 11, x808102_g_MissionInfo_IDX )
...
function x808102_OnAbandon( sceneId, selfId )
  ...
  if HaveItem(sceneId, selfId, x808102_g_ItemId.id) > 0 then
  	if LuaFnGetAvailableItemCount(sceneId, selfId, x808102_g_ItemId.id) >= x808102_g_ItemId.num then
	  	...
	  	x808102_NotifyTip( sceneId, selfId, "您的物品現在不可用或已被鎖定。" )			
			return 0
  	end
  end
...
function x808102_CheckSubmit( sceneId, selfId, targetId )
	...
	--判断该npc是否是对应任务的npc
	if LuaFnGetName( sceneId, targetId ) ~= x808102_g_AccomplishNPC_Name then
		x808102_NotifyTip( sceneId, selfId, "提交任務失敗" )					
		return 0
	end
...
function x808102_OnKillObject( sceneId, selfId, objdataId ,objId)--参数意思：场景号、玩家objId、怪物表位置号、怪物
end
