--µç»ğÌûÅä·½½Å±¾
--¶ÔÓ¦Éú»î¼¼ÄÜ åĞÒ£ ÆæÃÅ¶İ¼×

--½Å±¾ºÅ
x700354_g_scriptId = 700354

--ĞèÒªµÄµÈ¼¶
x700354_g_AbilityNeedLevel = 2
--Åä·½µÄID
x700354_g_RecipeId = 354
--Ë³·¢ÌØĞ§ID
x700354_g_SpecialEffectID = 18;
--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700354_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_PENGREN)

--	if AbilityLevel < x700354_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700354_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700354_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
--**********************************
--Ö±½ÓÈ¡ÏûĞ§¹û£º
...
--**********************************
function x700354_CancelImpacts( sceneId, selfId )
	return 0;
end
--**********************************
--Ìõ¼ş¼ì²âÈë¿Ú£º
...
--**********************************
function x700354_OnConditionCheck( sceneId, selfId )
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700354_g_RecipeId)) then
		BeginEvent(sceneId)
...
--**********************************
function x700354_OnDeplete( sceneId, selfId )
...
--**********************************
function x700354_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700354_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700354_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700354_OnActivateEachTick( sceneId, selfId)
	return 1;
end
