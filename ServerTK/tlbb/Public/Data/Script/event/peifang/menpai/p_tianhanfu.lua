...
--½Å±¾ºÅ
x700341_g_scriptId = 700341

--ĞèÒªµÄµÈ¼¶
x700341_g_AbilityNeedLevel = 8
--Åä·½µÄID
x700341_g_RecipeId = 341
--Ë³·¢ÌØĞ§ID
x700341_g_SpecialEffectID = 18;
--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700341_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_PENGREN)

--	if AbilityLevel < x700341_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700341_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700341_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
--**********************************
--Ö±½ÓÈ¡ÏûĞ§¹û£º
...
--**********************************
function x700341_CancelImpacts( sceneId, selfId )
	return 0;
end
--**********************************
--Ìõ¼ş¼ì²âÈë¿Ú£º
...
--**********************************
function x700341_OnConditionCheck( sceneId, selfId )
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700341_g_RecipeId)) then
		BeginEvent(sceneId)
...
--**********************************
function x700341_OnDeplete( sceneId, selfId )
...
--**********************************
function x700341_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700341_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700341_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700341_OnActivateEachTick( sceneId, selfId)
	return 1;
end
