--ÃÙÈË¾§Åä·½½Å±¾
--¶ÔÓ¦Éú»î¼¼ÄÜ ÌìÉ½ Ğş±ùÊõ

--½Å±¾ºÅ
x700369_g_scriptId = 700369

--ĞèÒªµÄµÈ¼¶
x700369_g_AbilityNeedLevel = 2
--Åä·½µÄID
x700369_g_RecipeId = 369
--Ë³·¢ÌØĞ§ID
x700369_g_SpecialEffectID = 18;
--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700369_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_PENGREN)

--	if AbilityLevel < x700369_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
--			strText = "Không ğü c¤p ğµ kÛ nång"
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700369_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700369_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
--**********************************
--Ö±½ÓÈ¡ÏûĞ§¹û£º
...
--**********************************
function x700369_CancelImpacts( sceneId, selfId )
	return 0;
end
--**********************************
--Ìõ¼ş¼ì²âÈë¿Ú£º
...
--**********************************
function x700369_OnConditionCheck( sceneId, selfId )
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700369_g_RecipeId)) then
		BeginEvent(sceneId)
			strText = "Ğã h÷c ğßşc ph¯i phß½ng loÕi này!"
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId);
		return 0;		
...
--**********************************
function x700369_OnDeplete( sceneId, selfId )
...
--**********************************
function x700369_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700369_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700369_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700369_OnActivateEachTick( sceneId, selfId)
	return 1;
end
