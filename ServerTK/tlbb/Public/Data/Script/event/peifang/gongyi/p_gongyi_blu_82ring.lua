...
--½Å±¾ºÅ
x700445_g_scriptId = 700445
--ĞèÒªµÄµÈ¼¶
x700445_g_AbilityNeedLevel = 9
--Åä·½µÄID
x700445_g_RecipeId = 445
--Ë³·¢ÌØĞ§ID
x700445_g_SpecialEffectID = 18

--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700445_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_GONGYI)

--	if AbilityLevel < x700445_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700445_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700445_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
...
--**********************************
function x700445_CancelImpacts( sceneId, selfId )
	return 0;
end
...
--**********************************
function x700445_OnConditionCheck( sceneId, selfId )
	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_GONGYI)
	--Ğ£ÑéÊ¹ÓÃµÄÎïÆ·
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700445_g_RecipeId)) then
		BeginEvent(sceneId)
...
--**********************************
function x700445_OnDeplete( sceneId, selfId )
...
--**********************************
function x700445_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700445_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700445_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700445_OnActivateEachTick( sceneId, selfId)
	return 1;
end
