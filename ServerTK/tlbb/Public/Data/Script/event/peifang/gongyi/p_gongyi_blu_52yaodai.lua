...
--½Å±¾ºÅ
x700436_g_scriptId = 700436
--ĞèÒªµÄµÈ¼¶
x700436_g_AbilityNeedLevel = 6
--Åä·½µÄID
x700436_g_RecipeId = 436
--Ë³·¢ÌØĞ§ID
x700436_g_SpecialEffectID = 18

--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700436_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_GONGYI)

--	if AbilityLevel < x700436_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700436_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700436_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
...
--**********************************
function x700436_CancelImpacts( sceneId, selfId )
	return 0;
end
...
--**********************************
function x700436_OnConditionCheck( sceneId, selfId )
	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_GONGYI)
	--Ğ£ÑéÊ¹ÓÃµÄÎïÆ·
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700436_g_RecipeId)) then
		BeginEvent(sceneId)
...
--**********************************
function x700436_OnDeplete( sceneId, selfId )
...
--**********************************
function x700436_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700436_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700436_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700436_OnActivateEachTick( sceneId, selfId)
	return 1;
end
