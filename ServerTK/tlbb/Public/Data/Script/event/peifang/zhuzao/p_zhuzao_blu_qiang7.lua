...
--½Å±¾ºÅ
x700406_g_scriptId = 700406
--ĞèÒªµÄµÈ¼¶
x700406_g_AbilityNeedLevel = 7
--Åä·½µÄID
x700406_g_RecipeId = 406
--Ë³·¢ÌØĞ§ID
x700406_g_SpecialEffectID = 18

--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700406_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_ZHUZAO)

--	if AbilityLevel < x700406_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700406_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700406_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
...
--**********************************
function x700406_CancelImpacts( sceneId, selfId )
	return 0;
end
...
--**********************************
function x700406_OnConditionCheck( sceneId, selfId )
	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_ZHUZAO)
	--Ğ£ÑéÊ¹ÓÃµÄÎïÆ·
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700406_g_RecipeId)) then
		BeginEvent(sceneId)
...
--**********************************
function x700406_OnDeplete( sceneId, selfId )
...
--**********************************
function x700406_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700406_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700406_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700406_OnActivateEachTick( sceneId, selfId)
	return 1;
end
