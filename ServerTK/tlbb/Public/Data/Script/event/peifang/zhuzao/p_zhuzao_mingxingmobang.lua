...
--ĞèÒªµÄµÈ¼¶
x700019_g_AbilityNeedLevel = 9
--Åä·½µÄID
x700019_g_RecipeId = 19
--Ë³·¢ÌØĞ§ID
x700019_g_SpecialEffectID = 18;
--**********************************
--ÊÂ¼ş½»»¥Èë¿Ú
--**********************************
function x700019_OnDefaultEvent( sceneId, selfId, bagIndex )
--	--»ñµÃÍæ¼ÒÉú»î¼¼ÄÜµÈ¼¶
--	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_ZHUZAO)

--	if AbilityLevel < x700019_g_AbilityNeedLevel then
--		BeginEvent(sceneId)
...
--	end
--	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
--	SelfDestructFlag = CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700019_g_RecipeId )
--
--	-- Ñ§ÍêÒÔºó×ÔÎÒ´İ»Ù
...
--**********************************
function x700019_IsSkillLikeScript( sceneId, selfId)
	return 1;
end
--**********************************
--Ö±½ÓÈ¡ÏûĞ§¹û£º
...
--**********************************
function x700019_CancelImpacts( sceneId, selfId )
	return 0;
end
--**********************************
--Ìõ¼ş¼ì²âÈë¿Ú£º
...
--**********************************
function x700019_OnConditionCheck( sceneId, selfId )
	AbilityLevel = QueryHumanAbilityLevel(sceneId,selfId ,ABILITY_ZHUZAO)
	--Ğ£ÑéÊ¹ÓÃµÄÎïÆ·
	...
	if (0<LuaFnIsPrescrLearned( sceneId, selfId, x700019_g_RecipeId)) then
		BeginEvent(sceneId)
...
--**********************************
function x700019_OnDeplete( sceneId, selfId )
...
--**********************************
function x700019_OnActivateOnce( sceneId, selfId )
	-- µ÷ÓÃÍ¨ÓÃÅä·½Ñ§Ï°
	CallScriptFunction( USERECIPE_SCRIPT, "ReadRecipe", sceneId, selfId, x700019_g_RecipeId )
	LuaFnSendSpecificImpactToUnit(sceneId, selfId, selfId, selfId, x700019_g_SpecialEffectID, 0);
	return 1;
end
...
--×¢£ºÕâÀïÊÇ¼¼ÄÜÉúĞ§Ò»´ÎµÄÈë¿Ú
--**********************************
function x700019_OnActivateEachTick( sceneId, selfId)
	return 1;
end
