...
x713500_g_AbilityName[2] = "Ğông y"
...
x713500_g_AbilityDescription[2] = "Ğông y không th¬ h÷c trong 1 s¾m 1 chi«u"
...
function x713500_OnDefaultEvent( sceneId, selfId, targetId, AbilityID )
	...
	elseif GetMoney(sceneId, selfId) < x713500_g_StudyCost[AbilityID] then
		BeginEvent(sceneId)
			strText = format("Ti«n cüa các hÕ không ğü")
			AddText(sceneId,strText);
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	else
...
function x713500_OnEnumerate( sceneId, selfId, targetId )
		AddNumText(sceneId,x713500_g_ScriptId,"TÕi hÕ mu¯n h÷c kÛ nång",12,-1);
end
