...
x600009_g_MissionTarget = "    Bäo hµ %n ğªn g¥n %s %s. "						--ÈÎÎñÄ¿±ê
...
x600009_g_CopySceneType = FUBEN_CONVOYPET	--¸±±¾ÀàĞÍ£¬¶¨ÒåÔÚScriptGlobal.luaÀïÃæ

x600009_g_CopySceneMap = "muchang.nav"
x600009_g_Exit = "muchang_area.ini"
x600009_g_TickTime = 5					--»Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x600009_g_LimitTotalHoldTime = 360		--¸±±¾¿ÉÒÔ´æ»îµÄÊ±¼ä£¨µ¥Î»£º´ÎÊı£©,Èç¹û´ËÊ±¼äµ½ÁË£¬ÔòÈÎÎñ½«»áÊ§°Ü
x600009_g_LimitTimeSuccess = 500		--¸±±¾Ê±¼äÏŞÖÆ£¨µ¥Î»£º´ÎÊı£©£¬Èç¹û´ËÊ±¼äµ½ÁË£¬ÈÎÎñÍê³É
x600009_g_CloseTick = 3					--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊı£©
x600009_g_NoUserTime = 300				--¸±±¾ÖĞÃ»ÓĞÈËºó¿ÉÒÔ¼ÌĞø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
x600009_g_DeadTrans = 0					--ËÀÍö×ªÒÆÄ£Ê½£¬0£ºËÀÍöºó»¹¿ÉÒÔ¼ÌĞøÔÚ¸±±¾£¬1£ºËÀÍöºó±»Ç¿ÖÆÒÆ³ö¸±±¾
x600009_g_Fuben_X = 70					--½øÈë¸±±¾µÄÎ»ÖÃX
x600009_g_Fuben_Z = 113					--½øÈë¸±±¾µÄÎ»ÖÃZ
...
function x600009_OnDefaultEvent( sceneId, selfId, targetId )
	if GetName( sceneId, targetId ) ~= x600009_g_Name then		--ÅĞ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x600009_AcceptEnterCopyScene( sceneId, selfId )
	if IsHaveMission( sceneId, selfId, x600009_g_MissionId ) > 0 then					--ÓĞÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail, 0 )	--½«ÈÎÎñµÄµÚ0ºÅÊı¾İÉèÖÃÎª0,±íÊ¾Î´Íê³ÉµÄÈÎÎñ
		SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_Param_sceneid, -1 )		--½«ÈÎÎñµÄµÚ2ºÅÊı¾İÉèÖÃÎª-1, ÓÃÓÚ±£´æ¸±±¾µÄ³¡¾°ºÅ
...
function x600009_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map( sceneId, x600009_g_CopySceneMap )						--µØÍ¼ÊÇ±ØĞëÑ¡È¡µÄ£¬¶øÇÒ±ØĞëÔÚConfig/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param( sceneId, 0, x600009_g_CopySceneType )				--ÉèÖÃ¸±±¾Êı¾İ£¬ÕâÀï½«0ºÅË÷ÒıµÄÊı¾İÉèÖÃÎª999£¬ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êı×Ö×Ô¶¨Òå)
...
function x600009_OnPlayerEnter( sceneId, selfId )
	...
	--×£¿­ 2008-03-06
	--²»Ó¦¸Ã³öÏÖÕâÖÖÇé¿ö....ĞèÒª¼ì²âÊÇ·ñÊÇ±¾ÈÎÎñ....
	...
	if petFlag == 0 then
		...
		SetLevel( sceneId, monsterId, GetLevel( sceneId, selfId ) )			-- µ÷Õû¼¶±ğ
...
function x600009_OnHumanDie( sceneId, selfId, killerId )
	...
	SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail, 2 )			-- ÈÎÎñÊ§°Ü
	LuaFnSetCopySceneData_Param( sceneId, 4, 1 )

	if x600009_g_DeadTrans == 1 then														--ËÀÍöºóĞèÒª±»Ç¿ÖÆÌß³ö³¡¾°
		local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, 3 )				--È¡µÃ¸±±¾Èë¿Ú³¡¾°ºÅ
		NewWorld( sceneId, selfId, oldsceneId, x600009_g_Back_X, x600009_g_Back_Z )
	end
end
...
function x600009_BackToCity( sceneId, selfId )
	--¿ÉÒÔÓÃÔ­À´µÄ·½·¨,µ«ÊÇ»áÌáÊ¾Á½±é"ÈÎÎñÊ§°Ü",ËùÒÔĞŞ¸Ä --changed by xindefeng
	...
	if IsHaveMission( sceneId, selfId, x600009_g_MissionId ) > 0 then
		...
		if GetMissionParam( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail ) ~= 1 then	--Èç¹ûÈÎÎñÎ´³É¹¦
			SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail, 2 )	--ÉèÖÃÈÎÎñÊ§°Ü
		end
	end
...
function x600009_OnLockedTarget( sceneId, selfId, objId )
	...
	if GetMonsterDataID( sceneId, objId ) == petMonsterId then				-- ÕäÊŞ
		...
		------------------Å²µ½x600009_OnPlayerEnter()È¥ÁË,Íæ¼ÒÒ»½øÈë¸±±¾¾ÍÆô¶¯Ê±ÖÓ changed by xindefeng----------------
		--Æô¶¯¸ÃÈÎÎñµÄÈÎÎñÊ±ÖÓÆ÷
		--StartMissionTimer( sceneId, selfId, x600009_g_MissionId )
		--SetMissionEvent( sceneId, selfId, x600009_g_MissionId, 5 )
		---------------------------------------------------------------------------------------------------------------
		
		SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_StrForePart + 4, 1 )	--ÉèÖÃ±êÖ¾:ÊÇ·ñÓë»¤ËÍ³èÎï¶Ô»°(¼´ÔÚ¸±±¾ÊÇ·ñÒÑ¾­¿ªÊ¼»¤ËÍ?)
...
function x600009_OnTimer( sceneId, selfId )
	...
	--changed by xindefeng
	--¸Ã³ÉÍæ¼Ò½øÈë¸±±¾¾ÍÆô¶¯¼ÆÊ±Æ÷,µ«²»ÉèÖÃ(x600009_g_StrForePart + 4)Mission ParaÕâÑù,Íæ¼Ò½øÈë¸±±¾ºó,Ã»ÓĞÓë»¤ËÍ³èÎï¶Ô»°¾Í³öÈ¥ÁË,ÄÇÃ´Ò²¿ÉÒÔÌáÊ¾ÉèÖÃÊ§°Ü!	
	--Íæ¼ÒÓë»¤ËÍ³èÎï¶Ô»°ºóÓÉx600009_OnLockedTarget()ÉèÖÃ(x600009_g_StrForePart + 4)Mission Para,ÉèÖÃÂ·¾¶±êÊ¶Íæ¼ÒÔÚ¸±±¾ÖĞ¿ªÊ¼»¤ËÍ
	local sceneType = LuaFnGetSceneType( sceneId )	--ÊÇ·ñÊÇ¸±±¾ÀàĞÍ
	if sceneType ~= 1 then		
		--»¤ËÍÈÎÎñ,ÈÎÎñÃ»Íê³É¾Í³ö¸±±¾,¾ÍËãÈÎÎñÊ§°Ü!
		if GetMissionParam( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail ) ~= 1 then	--ÈÎÎñ³É¹¦·ñ?
			SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail, 2 )	--ÈÎÎñ»¹Ã»³É¹¦¾Í³ö¸±±¾,ÒòÎªÊÇ»¤ËÍÈÎÎñ¾ÍÉèÖÃÈÎÎñÊ§°Ü
			ResetMissionEvent( sceneId, selfId, x600009_g_MissionId, 5 )									--Àë¿ª¸±±¾ºóÒªÍ£Ö¹¶¨Ê±µ÷ÓÃ±¾º¯Êı
			SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_StrForePart + 4, 0 )	--ÉèÖÃ±êÖ¾:ÊÇ·ñÓë»¤ËÍ³èÎï¶Ô»°(¼´ÔÚ¸±±¾ÊÇ·ñÒÑ¾­¿ªÊ¼»¤ËÍ?)
			LuaFnSetCopySceneData_Param( sceneId, 4, 1 )																	--ÉèÖÃ¸±±¾¹Ø±Õ±êÖ¾
			x600009_NotifyFailTips( sceneId, selfId, "Nhi®m vø th¤t bÕi!" )												--ÌáÊ¾Ê§°Ü
		end
		return
	end
	...
	if GetMissionParam( sceneId, selfId, misIndex, x600009_g_StrForePart + 4 ) == 1 then
		...
		if fubentype == x600009_g_CopySceneType then			
			...
			for i = 0, monstercount - 1 do
				monsterId = GetMonsterObjID( sceneId, i )
				if GetMonsterDataID( sceneId, monsterId ) == petMonsterId then		-- ÅĞ¶ÏÊÇ·ñ³É¹¦
					local x, z = GetLastPatrolPoint( sceneId, 0 )
					local petX, petZ = GetWorldPos( sceneId, monsterId )

					if (x - petX) * (x - petX) + (z - petZ) * (z - petZ) < 4 then	-- ÀëÖÕµã²»µ½ 2 Ã×
					...
					return	--»¤ËÍµÄ³èÎï»¹ÔÚ¾ÍÖ±½Ó·µ»Ø
				end
			end
		end

		--»¤ËÍ¹ı³ÌÖĞÕäÊŞËÀÍöËãÈÎÎñÊ§°Ü
		x600009_NotifyFailTips( sceneId, selfId, "Nhi®m vø th¤t bÕi!" )
		SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_IsMissionOkFail, 2 )		-- ÈÎÎñÊ§°Ü
		LuaFnSetCopySceneData_Param( sceneId, 4, 1 )		--ÉèÖÃ¸±±¾¹Ø±Õ±êÖ¾
		ResetMissionEvent( sceneId, selfId, x600009_g_MissionId, 5 )						-- ²»ÔÙ×ß´ËĞÄÌø
		SetMissionByIndex( sceneId, selfId, misIndex, x600009_g_StrForePart + 4, 0 )	--ÉèÖÃ±êÖ¾:ÊÇ·ñÓë»¤ËÍ³èÎï¶Ô»°(¼´ÔÚ¸±±¾ÊÇ·ñÒÑ¾­¿ªÊ¼»¤ËÍ?)	--add by xindefeng
	end
end
