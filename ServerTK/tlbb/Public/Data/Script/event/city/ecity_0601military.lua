...
x600031_g_IsMissionOkFail = 0							-- »ŒŒÒÕÍ≥…±Íº«

--“‘…œ «∂ØÃ¨**************************************************************

--»ŒŒÒ±‰¡øµ⁄“ªŒª”√¿¥¥Ê¥¢ÀÊª˙µ√µΩµƒΩ≈±æ∫≈
--»ŒŒÒŒƒ±æ√Ë ˆ
x600031_g_MissionName = "NhiÆm v¯ quØc phÚng"
x600031_g_MissionInfo = ""													--»ŒŒÒ√Ë ˆ
x600031_g_MissionTarget = "    Sÿ d¯ng lÆnh i tu•n, b°t giÊ ngﬂ∂i kh‰ nghi. "	--»ŒŒÒƒø±Í
x600031_g_ContinueInfo = "    SÒ tÏnh ti™n tri¨n nhﬂ th™ n‡o r∞i?"						--Œ¥ÕÍ≥…»ŒŒÒµƒnpc∂‘ª∞
x600031_g_MissionComplete = "    L‡m tØt l°m, r§t tØt r§t tØt. "						--ÕÍ≥…»ŒŒÒnpcÀµª∞µƒª∞
...
--»ŒŒÒ»Îø⁄∫Ø ˝
--**********************************
function x600031_OnDefaultEvent( sceneId, selfId, targetId )	--µ„ª˜∏√»ŒŒÒ∫Û÷¥––¥ÀΩ≈±æ

	if GetName( sceneId, targetId ) ~= x600031_g_Name then		--≈–∂œ∏√npc «∑Ò «∂‘”¶»ŒŒÒµƒnpc
		return
	end
...
function x600031_OnAccept( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		local missionInfo
		if random(2) == 1 then
			missionInfo = format( "    G•n ‚y trong th‡nh phØ to‡n th§y nhÊng kÎ lÈn lÈn l˙t l˙t, c·c h’ c•m lÆnh tu•n tra n‡y, i tu•n bØn xung quanh th‡nh phØ" )
		else
			missionInfo = format( "    GiÊ an to‡n cho th‡nh phØ l‡ tr·ch nhiÆm c¸a ch˙ng ta, lÆnh tu•n tra n‡y giao cho c·c h’, lßp mµt v‡i th‡nh tÌch nhÈ" )
		end

		AddText( sceneId, "C·c h’ „ nhßn nhiÆm v¯: " .. x600031_g_MissionName )
		AddText( sceneId, missionInfo )
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
	
	-- ∏¯’‚∏ˆ»ŒŒÒ∑÷≈‰“ª–©π÷ŒÔœÚπ€µƒ±ÿ“™ ˝æ›
	-- æﬂÃÂ «∏˘æ›ÕÍº“µƒµ»º∂£´ÀÊª˙“ª∏ˆ√≈≈…¿¥ªÒµ√
...
function x600031_OnKillObject( sceneId, selfId, objdataId ,objId )
	...
	local misIndex = GetMissionIndexByID( sceneId, selfId, x600031_g_MissionId )		--µ√µΩ»ŒŒÒ‘⁄20∏ˆ»ŒŒÒ÷–µƒ–Ú¡–∫≈
