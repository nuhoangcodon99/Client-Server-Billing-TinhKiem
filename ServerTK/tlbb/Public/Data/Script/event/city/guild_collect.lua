...
x600046_g_mapping = {
	[1140] = {name = "#YL¸ch thÕch#W", product = "C±n thÕch", place1 = "#GÐôn Hoàng Minh Sa S½n#W", place2 = "#GTung S½n, KÏ S½n Pha Nam#W", itemId = 40004464},
	[1141] = {name = "#YNhÕn Linh#W", product = "Vû Ti­n", place1 = "#GThái H° Nguyên Ð¥u Tñ#W", place2 = "#GKính H° Yên Chi Ðäo Dß HoÕ Mi Ðäo#W", itemId = 40004462},
...
function x600046_ConfirmGetPrize(sceneId, selfId, targetId)
	local misData = GetMissionData(sceneId, selfId, MD_GUILD_COLLECT_DATA); --MD_GUILD_COLLECT_DATA £¨248£© µÍËÄÎ»±£´æ±¾´Î»î¶¯µÄÖÜÊý£¬µÚ5Î»Îª1±íÊ¾ÒÑÁì½±	
	local weekTime = mod(misData, 10000);
	--ÅÐ¶ÏÍæ¼ÒÊÇ·ñ½»¸¶ÁËÊÕ¼¯Ä¿±êµÀ¾ßµÄÈÎÎñ
	if weekTime ~= GetWeekTime() then
		x600046_ReturnDlg(sceneId, selfId, targetId, "#{BHSJ_081014_27}") --¶Ô²»Æð£¬ÄãÃ»ÓÐÁìÈ¡»î¶¯½±ÀøµÄ×Ê¸ñ¡£
		return
	end
	--ÅÐ¶ÏÍæ¼ÒÊÇ·ñÁìÈ¡¹ý±¾´Î»î¶¯µÄ½±Àø
	if floor(misData/10000) == 1 then
		x600046_ReturnDlg(sceneId, selfId, targetId, "#{BHSJ_081014_28}") --¶Ô²»Æð£¬ÄãÒÑ¾­ÁìÈ¡¹ý±¾´Î»î¶¯µÄ½±Àø
		return			
	end
	--Âú×ãÁì½±Ìõ¼þ	
	local order = CityGetAttr(sceneId, selfId, 17);		--µÃµ½°ïÅÉÊÕ¼¯ÊýÄ¿ÅÅÃû
	if order == 0 then
...
function x600046_OnSubmit( sceneId, selfId, targetId, type )
	...
	--¾­Ñé½±ÀøÖµÊÇÉý¼¶¾­ÑéµÄ10% Óëµ¥´ÎÈÎÎñ½»¸¶¾­ÑéÖÐµÄÐ¡Öµ
