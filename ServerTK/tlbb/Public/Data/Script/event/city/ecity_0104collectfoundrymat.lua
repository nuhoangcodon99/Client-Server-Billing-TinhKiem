...
x600006_g_MissionTarget = "    T× %n tìm %s cái %i. "								--ÈÎÎñÄ¿±ê
...
x600006_g_ItemCountIdx = x600006_g_StrForePart + 1									-- ÈÎÎñÎïÆ·ÊýÁ¿µÄ´æ´¢Î»ÖÃ
x600006_g_ItemIdx = x600006_g_StrForePart + 2										-- ÈÎÎñÎïÆ·µÄ´æ´¢Î»ÖÃ
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x600006_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x600006_g_Name then		--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x600006_OnAccept( sceneId, selfId, targetId )
	...
	-- ÔÙËæ»úÌô³öÒ»¸öÊÊÓÃµôÂä¸ÃÎïÆ·µÄ¹ÖÎï NpcId ºÅ
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x600006_OnKillObject( sceneId, selfId, objdataId, objId )				--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
	...
	for i = 0, allOwnersCount - 1 do
		...
		if IsHaveMission( sceneId, curOwner, x600006_g_MissionId ) > 0 and	--Èç¹ûÕâ¸öÈËÓµÓÐÈÎÎñ
		   GetScriptIDByMissionID( sceneId, curOwner, x600006_g_MissionId ) == x600006_g_ScriptId then		--²¢ÇÒÊÇ±¾½Å±¾µÄÈÎÎñ
...
function x600006_OnItemChanged( sceneId, selfId, itemdataId )
	...
	if demandItemId == itemdataId then
		...
		local strText = format( "Ðã l¤y ðßþc ðü s¯ %s %d/%d", strItemName, nItemNum, demandItemCount )
		...
		if nItemNum >= demandItemCount then
			strText = format( "L¤y ðßþc %s %d/%d", strItemName, demandItemCount, demandItemCount )
