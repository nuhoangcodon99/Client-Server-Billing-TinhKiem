...
--ÈÎÎñÄ¿±ê
x805042_g_MissionTarget		= "  Giªt hªt t¤t cä quái v§t ð¬ hoàn thành nhi®m vø"
--Î´Íê³ÉÈÎÎñµÄnpc¶Ô»°
...
x805042_g_Param_killmonstercount	= 1					--1ºÅ£ºÉ±ËÀÈÎÎñÐ¡¹ÖµÄÊýÁ¿
x805042_g_Param_killbosscount	= 2							--2ºÅ£ºÉ±ËÀÈÎÎñboss¹ÖµÄÊýÁ¿
...
x805042_g_Param_time			= 5									--5ºÅ£ºÍê³É¸±±¾ËùÓÃÊ±¼ä(µ¥Î»£ºÃë)
																							--6ºÅ£º¾ßÌå¸±±¾ÊÂ¼þ½Å±¾Õ¼ÓÃ
																							--7ºÅ£º¾ßÌå¸±±¾ÊÂ¼þ½Å±¾Õ¼ÓÃ
--MisDescEnd
--************************************************************************

x805042_g_CopySceneType			= FUBEN_PROTECTGUILD	--¸±±¾ÀàÐÍ£¬¶¨ÒåÔÚScriptGlobal.luaÀïÃæ
x805042_g_LimitMembers			= 3		--¿ÉÒÔ½ø¸±±¾µÄ×îÐ¡¶ÓÎéÈËÊý
x805042_g_TickTime					= 5		--»Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x805042_g_LimitTotalHoldTime= 360	--¸±±¾¿ÉÒÔ´æ»îµÄÊ±¼ä£¨µ¥Î»£º´ÎÊý£©,Èç¹û´ËÊ±¼äµ½ÁË£¬ÔòÈÎÎñ½«»áÊ§°Ü
x805042_g_LimitTimeSuccess	= 500	--¸±±¾Ê±¼äÏÞÖÆ£¨µ¥Î»£º´ÎÊý£©£¬Èç¹û´ËÊ±¼äµ½ÁË£¬ÈÎÎñÍê³É
x805042_g_CloseTick					= 7		--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊý£© zchw 3-->7
x805042_g_NoUserTime				= 300	--¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
x805042_g_Fuben_X						= 99	--½øÈë¸±±¾µÄÎ»ÖÃX
x805042_g_Fuben_Z						= 152	--½øÈë¸±±¾µÄÎ»ÖÃZ
x805042_g_Back_X						= 99	--Ô´³¡¾°Î»ÖÃX
x805042_g_Back_Z						= 152	--Ô´³¡¾°Î»ÖÃZ
x805042_g_Totalkillmonstercount	= 30	--ÐèÒªÉ±ËÀmonsterÊýÁ¿
x805042_g_Totalkillbosscount	= 1	--ÐèÒªÉ±ËÀBossÊýÁ¿

x805042_g_StartTick				=	3	--ÈÎÎñ¿ªÊ¼Ê±¼ä
x805042_g_CurrentStage			=	0	--µ±Ç°Ëù´¦µÄ½×¶Î	0--15Ãë¿ªÊ¼½×¶Î  1--8·ÖÖÓÇåÀíÐ¡¹Ö½×¶Î 2--Boss½×¶Î
x805042_g_FirstStage			=	183	--µÚÒ»½×¶Î£¬15·ÖÖÓÏÞÖÆ
x805042_g_FirstStage_msg		=	{4,9,15,21,27,33,39,45,51,57,63,69,75,81,87,93,99,105,111,117,123,129,135,141,147,153,159,165,171,175,177,178,179,180,181,182,}	--µÚÒ»½×¶Î£¬Ê£ÓàÊ±¼äÌáÊ¾
...
x805042_g_keySD["x"]			= 8			--ÈËÎïÔÚÈë¿Ú³¡¾°ÖÐµÄxÎ»ÖÃ
x805042_g_keySD["z"]			= 9			--ÈËÎïÔÚÈë¿Ú³¡¾°ÖÐµÄzÎ»ÖÃ
x805042_g_keySD["killMonsterNum"]	= 10		--É±ËÀ¹ÖÎïÊýÁ¿
x805042_g_keySD["genMonsterNum"]	= 11			--Éú³É¹ÖÎïÊýÁ¿
x805042_g_keySD["playerLevel"]	= 12		--¼ÇÂ¼µ±Ç°¸±±¾µÄ¼¶±ð
...
function x805042_CheckEnter( sceneId, selfId, targetId )
	...
	if LuaFnHasTeam( sceneId, selfId ) == 0 then
		x805042_MsgBox_With_Help( sceneId, selfId, targetId, "C¥n t± ðµi m¾i có th¬ tiªn vào." )
		return 0
	end
	...
	for i=0, nPlayerNum-1  do
		...
		if GetLevel(sceneId,nPlayerId) < x805042_g_minLevel  then
			x805042_MsgBox_With_Help( sceneId, selfId, targetId, "T± ðµi không ðü c¤p ðµ b¡t buµc." )
			return 0
		end
	end
	...
	if GetTeamSize( sceneId, selfId ) < x805042_g_LimitMembers then
	  x805042_MsgBox_With_Help( sceneId, selfId, targetId, "T± ðµi không ðü ngß¶i" )
	  return 0
	end


	if LuaFnIsTeamLeader( sceneId, selfId ) == 0 then
		x805042_MsgBox_With_Help( sceneId, selfId, targetId, "Các hÕ không phäi ðµi trß·ng, g÷i ðµi trß·ng ðªn g£p ta." )
		return 0
	end
	...
	if nearteammembercount ~= LuaFnGetTeamSize( sceneId, selfId ) then
		x805042_MsgBox_With_Help( sceneId, selfId, targetId, "Ðµi hæu không · g¥n ðây" )
		return 0
	end
...
function x805042_OnEnumerate( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		AddText( sceneId, "S½n th¥n dî kinh ð¯i giá cá bang hµi thành th¸ nhçn vô khä nhçn, tâm tu phát uy li­u ! B¤t tri tØ hoÕt ðích phàm nhân, thß¶ng thß¶ng s½n th¥n ðích lñc lßþng !" )
		AddNumText( sceneId, x805042_g_ScriptId, "Khiêu chiªn s½n th¥n" ,10 ,1)
		AddNumText( sceneId, x805042_g_ScriptId, "Gi¾i thi®u bäo v® Bang Hµi" ,11 ,2)
	EndEvent( sceneId )
...
function x805042_MakeCopyScene( sceneId, selfId, nearmembercount )
	...
	if bRetSceneID > 0 then
		...
		x805042_NotifyTip( sceneId, selfId, "Bän sao ðã quá hÕn, hãy thØ lÕi sau !" )
		return 0
	end
...
function x805042_OnCopySceneTimer( sceneId, nowTime )
	...
	if leaveFlag == 1 then
		...
		if leaveTickCount >= x805042_g_CloseTick then										--µ¹¼ÆÊ±¼äµ½£¬´ó¼Ò¶¼³öÈ¥°É
			local oldsceneId = LuaFnGetCopySceneData_Param( sceneId, x805042_g_keySD["scn"] )	--È¡µÃ¸±±¾Èë¿Ú³¡¾°ºÅ
		...
		elseif leaveTickCount < x805042_g_CloseTick then
			...
			local strText = format( "Các hÕ s¨ r¶i khöi ðây sau %d giây !", (x805042_g_CloseTick-leaveTickCount) * x805042_g_TickTime )
	...
	if x805042_g_CurrentStage == 0 then
		...
		if TickCount >= x805042_g_StartTick then
			...
			strText = "Chiªn ð¤u b¡t ð¥u !";
			
			x805042_TipAllHuman( sceneId,  strText)
			x805042_TipAllHuman( sceneId,  "15 phút ð¬ tiêu di®t các s½n th¥n")
			
		else
			strText = format("Tr§n chiªn s¨ b¡t ð¥u trong %d giây !", (x805042_g_StartTick - TickCount) * x805042_g_TickTime);
			x805042_TipAllHuman( sceneId,  strText)
		end
	end
	...
	if x805042_g_CurrentStage == 1 then
		...
		if TickCount >= x805042_g_FirstStage then
			...
			x805042_TipAllHuman( sceneId,  "Nhi®m vø th¤t bÕi, hªt gi¶ !")
		...
		for msgTick = 1, getn(x805042_g_FirstStage_msg) do
			if TickCount == x805042_g_FirstStage_msg[msgTick] then
				local tickMsg = format("Ðã giªt ðßþc S½n Th¥n, th¶i gian còn lÕi %d giây", (x805042_g_FirstStage-TickCount)*x805042_g_TickTime)
...
function x805042_MsgBox_With_Help( sceneId, selfId, targetId, msg )
	...
	BeginEvent( sceneId )
		AddText( sceneId, msg )
		AddNumText( sceneId, x805042_g_ScriptId, "Liên quan bäo v® Bang Hµi" ,11 ,2)
	EndEvent( sceneId )
	DispatchEventList( sceneId, selfId, targetId )
