...
x600013_g_MissionTarget = "    Trä l¶i ðúng 1 câu ð¯i câu th½ c±. "						--ÈÎÎñÄ¿±ê
...
x600013_g_Offset = 140000												-- Suppose to 150000, ±íÀïµÚ¼¸¸öÎïÆ·ÁÐµÄÆ«ÒÆÁ¿
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x600013_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x600013_g_Name then		--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x600013_IssueQuiz( sceneId, selfId )
	...
	for i = 1, x600013_g_Answer do
		AddNumText( sceneId, x600013_g_ScriptId, Opt[RandomOrder[i]], -1, i )		-- 9 + i ÖÆ¶¨¿Í»§¶ËÌØ¶¨µÄÍ¼±ê (Ñ¡ÏîÍ¼±êÎªÊý×ÖÏî)
	end
...
function x600013_CheckAnswer( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		...
		if GetNumText() == GetMissionParam( sceneId, selfId, misIndex, x600013_g_MissionParam_AnswerIndex ) then
			...
			SetMissionByIndex( sceneId, selfId, misIndex, x600013_g_MissionParam_IsFailed, 1 )	-- Ê§°Ü
		end
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x600013_OnKillObject( sceneId, selfId, objdataId, objId )	--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
