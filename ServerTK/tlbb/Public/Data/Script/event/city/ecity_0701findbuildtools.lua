...
x600036_g_MissionParam_Phase		=2	--2 ½×¶ÎºÅ ´ËºÅÓÃÓÚÇø·Öµ±Ç°ÈÎÎñUIµÄÃèÊöÐÅÏ¢
...
--Ñ­»·ÈÎÎñµÄÊý¾ÝË÷Òý£¬ÀïÃæ´æ×ÅÒÑ×öµÄ»·Êý
...
x600036_g_MissionComplete = "    R¤t t¯t r¤t t¯t, tiªn ðµ xây dñng lÕi tång nhanh r°i. "			--Íê³ÉÈÎÎñnpcËµ»°µÄ»°
...
x600036_g_ItemOffset = 29				-- Suppose to 29, ±íÀïµÚ¼¸¸öÎïÆ·ÁÐµÄÆ«ÒÆÁ¿
x600036_g_NPCOffset = 34				-- Suppose to 34, ±íÀïµÚ¼¸ÁÐ NPC µÄÆ«ÒÆÁ¿
x600036_g_NPCOffsetEx	= 257 		--±íÀïµÚ¼¸ÁÐ NPC µÄÆ«ÒÆÁ¿À©³ä modi:lby20071126
x600036_g_MonsterOffset = 43		-- Suppose to 43, ±íÀïµÚ¼¸ÁÐ Monster µÄÆ«ÒÆÁ¿
...
function x600036_OnDefaultEvent( sceneId, selfId, targetId )
	if GetName( sceneId, targetId ) ~= x600036_g_Name then		--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x600036_OnLockedTarget( sceneId, selfId, objId )
	...
	if Phase == 1 then				-- ¸Õ½ÓÁËÈÎÎñÀ´ÕÒ NPC
		-- 8 ³É»ú»áÖ±½Ó¸øÓëÈÎÎñÎïÆ·£¬ÈÎÎñÍê³É£¬1 ³É»ú»á¸ø¸±±¾ÈÎÎñ£¬1 ³É»ú»á¸øÉ±¹ÖÈÎÎñ
		local rand = random(100)	-- Ö±½ÓÍê³ÉÈÎÎñ
		if rand <= 80 then
			...
			SetMissionByIndex( sceneId, selfId, misIndex, x600036_g_IsMissionOkFail, 1 )		-- ÈÎÎñÍê³É
		...
		elseif rand <= 90 then		-- ·ÖÅÉ¸±±¾ÈÎÎñ
			...
			SetMissionByIndex( sceneId, selfId, subMisIndex, 2, 10 )						--ÐèÒªÉ±ËÀµÄ¹ÖÎïÊýÁ¿£¬½ö¹©¿Í»§¶ËÊ¹ÓÃ
...
--		PrintStr( "ÎÒµÄ¼¶±ð: " .. myLevel .. "£¬¹ÖµÄ¼¶±ð: " .. nLevel )
	...
	elseif Phase == 3 then			-- ·ÖÅäÁË¸±±¾ÈÎÎñ
		...
		CallScriptFunction( x600036_g_subScriptId, "OnEnumerate", sceneId, selfId, objId )
		return
	elseif Phase == 4 then			-- ¸±±¾ÈÎÎñÍê³ÉÁË
		-- É¾³ý¸±±¾ÈÎÎñ£¬¸øÓëÈÎÎñÎïÆ·
		local pos = LuaFnTryRecieveItem( sceneId, selfId, itemId, QUALITY_MUST_BE_CHANGE)
		if pos == -1 then
			CallScriptFunction( x600036_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "Ch² tr¯ng trong túi xách không ðü" )
...
function x600036_OnKillObject( sceneId, selfId, objdataId, objId )
	local allOwnersCount = GetMonsterOwnerCount( sceneId, objId )		--È¡µÃÕâ¸ö¹ÖÎïËÀºóÓµÓÐ·ÖÅäÈ¨µÄÈËÊý
	local i, curOwner, misIndex, nNpcId, strNpcName
	local MonsterName = GetName( sceneId, objId )
	local demandItemId, demandItemCount

	for i = 0, allOwnersCount - 1 do
		curOwner = GetMonsterOwnerID( sceneId, objId, i )				--È¡µÃÓµÓÐ·ÖÅäÈ¨µÄÈËµÄobjId
		if IsHaveMission( sceneId, curOwner, x600036_g_MissionId ) > 0 and
		   GetScriptIDByMissionID( sceneId, curOwner, x600036_g_MissionId ) == x600036_g_ScriptId then		--Èç¹ûÕâ¸öÈËÓµÓÐÈÎÎñ
