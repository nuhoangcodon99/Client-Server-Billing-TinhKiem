...
x600008_g_MissionTarget = "    Tìm kiªm %i, ðßa cho bang hµi thành th¸ Châu Thª Hæu (129, 99). "	--ÈÎÎñÄ¿±ê
...
x600008_g_Offset = 10													-- Suppose to 10, ±íÀïµÚ¼¸¸öÎïÆ·ÁÐµÄÆ«ÒÆÁ¿
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x600008_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x600008_g_Name then		--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x600008_OnAccept( sceneId, selfId, targetId )
	...
	if GetLevel( sceneId, selfId ) < 30 then
		CallScriptFunction( x600008_g_CityMissionScript, "NotifyFailTips", sceneId, selfId, "µÈ¼‰²»‰ò" )
		return
	end
	...
	BeginEvent( sceneId )
		local missionInfo
		if random(2) == 1 then
			missionInfo = format( "    ±¾³ÇÕýÔÚÑÐ¾¿¸÷·NÞr®aÆ·ºÍÊ³Æ·µÈ£¬¬FÔÚß€È±ÉÙ#Y%s#W£¬Ï£ÍûÄãÄÜŽÍÎÒÕÒí¡£", strItemName )
		else
			missionInfo = format( "    Â ÕfºÜ¶àŽÍ•þ¶¼ÔÚÊÕ¼¯#Y%s#W£¬Ò»¶¨ÓÐÔ­Òò£¬„ÚŸ©ÄãŽÍÎÒŒ¤í£¬¶¨ÓÐ³êÖx¡£", strItemName )
		end

		AddText( sceneId, missionInfo )
		AddText( sceneId, "#r    Äã½ÓÊÜÁËÈÎ„Õ£º" .. x600008_g_MissionName )
	EndEvent( sceneId )
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x600008_OnKillObject( sceneId, selfId, objdataId, objId )	--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
