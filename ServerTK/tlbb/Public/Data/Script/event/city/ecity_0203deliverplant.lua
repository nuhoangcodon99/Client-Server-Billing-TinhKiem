...
x600010_g_MissionTarget = "    Ðem %i ðßa cho %n"							--ÈÎÎñÄ¿±ê
...
x600010_g_ItemOffset = 19												-- Suppose to 19, ±íÀïµÚ¼¸¸öÎïÆ·ÁÐµÄÆ«ÒÆÁ¿
x600010_g_NPCOffset = 12												-- Suppose to 12, ±íÀïµÚ¼¸ÁÐ NPC µÄÆ«ÒÆÁ¿
x600010_g_NPCOffsetEx = 247											-- Suppose to 12, ±íÀïµÚ¼¸ÁÐ NPC µÄÆ«ÒÆÁ¿
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x600010_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x600010_g_Name then		--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
...
function x600010_OnAccept( sceneId, selfId, targetId )
	...
	BeginEvent( sceneId )
		...
		AddText( sceneId, "#r    Các hÕ ðã nh§n : " .. x600010_g_MissionName )
	EndEvent( sceneId )
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x600010_OnKillObject( sceneId, selfId, objdataId, objId )	--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
...
function x600010_OnLockedTarget( sceneId, selfId, objId )
	...
	local misIndex = GetMissionIndexByID( sceneId, selfId, x600010_g_MissionId )		--µÃµ½ÈÎÎñÔÚ20¸öÈÎÎñÖÐµÄÐòÁÐºÅ
