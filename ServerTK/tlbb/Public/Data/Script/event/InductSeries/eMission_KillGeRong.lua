...
x500611_g_SceneID=1
x500611_g_AccomplishNPC_Name="Ti«n H°ng Vû"
...
--ÈÎÎñĞúng·ñÒÑ¾­Íê³É
x500611_g_IsMissionOkFail	= 0		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ0Î»
...
--ÈÎÎñĞúng·ñÍê³É
x500611_g_Mission_IsComplete = 0		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ0Î»
--Ğúng·ñÉ±¹ÖtoÕ ğµ ±ê¼Ç
x500611_g_RecordIdx 				 = 1		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ1Î»
--ÈÎÎñ½Å±¾ºÅ¼ÇÂ¼
x500611_g_MissScriptID_Idx	 = 2		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ2Î»
--ÈÎÎñ·¢²¼NPC±ê¼Ç
x500611_g_AcceptNPC_Idx			 = 3		--ÈÎÎñ²ÎÊıtoÕ ğµ µÚ3Î» 2.Tô ChâuNPC

x500611_g_AcceptMission_IDX		= 765	--½ÓÊÕÈÎÎñË÷Òı
x500611_g_CompleteMission_IDX	= 766	--Ìá½»ÈÎÎñË÷Òı

--ËùÓµÓĞtoÕ ğµ ÊÂ¼şIDÁĞ±í
x500611_g_EventList	= {}

x500611_g_PlayerSlow_LVL					 = 38		-- Tiªp thøÈÎÎñtoÕ ğµ ×îµÍµÈc¤p
...
--¹ÖÎïÃû³Æ
x500611_g_MonsterName					=	"S½n TrÕi ĞÕi Vß½ng"
--ÒªÇótoÕ ğµ É±¹ÖÊıÁ¿
...
function x500611_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if key == x500611_g_AcceptMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500611_g_Name then
			x500611_NotifyTip( sceneId, selfId, "Nh§n nhi®m vø th¤t bÕi" )					
			return 0
		end
		-- ÈÎÎñĞúng·ñÒÑÂú
	...
	elseif key == x500611_g_CompleteMission_IDX then
		...
		if LuaFnGetName( sceneId, targetId ) ~= x500611_g_AccomplishNPC_Name then
			x500611_NotifyTip( sceneId, selfId, "Trä nhi®m vø th¤t bÕi" )					
			return 0
		end
		-- Èç¹ûÒÑ¾­½ÓÁËÈÎÎñ
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x500611_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊıÒâË¼: ³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎï
	...
	for i=0,num-1  do
		...
		if IsHaveMission(sceneId, humanObjId, x500611_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, x500611_g_Mission_IsComplete) <=0  then
				...
				BeginEvent(sceneId)
					local str = format("Ğã giªt chªt %s%d/%d", x500611_g_MonsterName, killedCount, x500611_g_KillMonsterCnt )						
					AddText(sceneId, str)
				EndEvent(sceneId)
