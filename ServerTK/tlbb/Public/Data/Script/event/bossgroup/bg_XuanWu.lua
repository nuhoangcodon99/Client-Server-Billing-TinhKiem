...
function x810000_OnDefaultEvent( sceneId, actId, iNoticeType, param2, param3, param4, param5 )
	...
	for _, BossData in x810000_g_BossData do
		if BossData.NeedCreate == 1 then
			...
			SetCharacterTitle(sceneId, MonsterID, "Huy«n Vû Ðäo Thü Hµ Thú")
		end
	end
...
function x810000_OnDie( sceneId, selfId, killerId )

	
end


function x810000_PickupItem( sceneId, selfId, itemId, bagidx )
	local transfer = GetBagItemTransfer(sceneId, selfId, bagidx)
	local str = format("#cff99cc#{_INFOUSR%s} cùng ð°ng ðµi tä xung hæu ðµt v¾i #GMãng C± Ðµc Chu Cáp Vß½ng cùng ð°ng b÷n #cff99cctÕi #YHuy«n Vû Ðäo #cff99cc. Cä ðµi toàn th¡ng tr· v« ð°ng th¶i còn nh§n ðßþc chiªn lþi ph¦m #{_INFOMSG%s}",GetName(sceneId,selfId), transfer)
