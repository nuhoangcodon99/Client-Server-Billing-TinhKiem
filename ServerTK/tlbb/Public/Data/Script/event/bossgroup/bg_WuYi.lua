	...
	for _, BossData in x810002_g_BossData do
		if BossData.NeedCreate == 1 then
			...
			if (BossData.ID == 9120) then
				SetCharacterTitle(sceneId, MonsterID, "Chung Cñc Bµc Phá")
			else
				SetCharacterTitle(sceneId, MonsterID, "T¯i Giai Ph¯i Giác")
			end
		end
	end
...
function x810002_OnDie( sceneId, selfId, killerId )

	
end

function x810002_PickupItem( sceneId, selfId,itemId, bagidx )
	
	local transfer = GetBagItemTransfer(sceneId, selfId, bagidx)
	local str = format("#cff99cc#{_INFOUSR%s} cùng ð°ng ðµi tä xung hæu ðµt v¾i #GBång Yêu cùng ð°ng b÷n #cff99cctÕi #YVõ Di #cff99cc. Cä ðµi toàn th¡ng tr· v« ð°ng th¶i còn nh§n ðßþc chiªn lþi ph¦m #{_INFOMSG%s}",GetName(sceneId,selfId), transfer)
