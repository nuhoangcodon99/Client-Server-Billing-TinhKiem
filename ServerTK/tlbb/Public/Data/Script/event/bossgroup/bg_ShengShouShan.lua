...
function x810110_OnDefaultEvent( sceneId, actId, iNoticeType, param2, param3, param4, param5 )
	...
	for _, BossData in x810110_g_BossData do
		if BossData.NeedCreate == 1 then
			...
			SetCharacterTitle(sceneId, MonsterID, "Thiên Niên Thiên Thánh Thú")
		end
	end
...
function x810110_OnDie( sceneId, selfId, killerId )

	
end

function x810110_PickupItem( sceneId, selfId,itemId, bagidx )

	local transfer = GetBagItemTransfer(sceneId, selfId, bagidx)
	local str = format("#cff99cc#{_INFOUSR%s} cùng ð°ng ðµi tä xung hæu ðµt v¾i #GLong Quy cùng ð°ng b÷n #cff99cctÕi #YThánh Thú S½n #cff99cc. Cä ðµi toàn th¡ng tr· v« ð°ng th¶i còn nh§n ðßþc chiªn lþi ph¦m #{_INFOMSG%s}",GetName(sceneId,selfId), transfer)
