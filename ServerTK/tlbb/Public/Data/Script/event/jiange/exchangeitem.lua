...
function x212221_OnEnumerate( sceneId, selfId, targetId )
	AddNumText( sceneId, x212221_g_scriptId, "Hoán ð±i bµ trang b¸ nµi công c¤p 75", 6, 750 )
	AddNumText( sceneId, x212221_g_scriptId, "Hoán ð±i bµ trang b¸ ngoÕi công c¤p 75", 6, 751 )
	AddNumText( sceneId, x212221_g_scriptId, "Hoán ð±i bµ trang b¸ nµi công c¤p 85", 6, 850 )	
	AddNumText( sceneId, x212221_g_scriptId, "Hoán ð±i bµ trang b¸ ngoÕi công c¤p 85", 6, 851 )
end
...
function x212221_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if stone.num > selfStoneCount   then
		BeginEvent(sceneId)
			strText = format("Ð±i th¤t bÕi, c¥n chu¦n b¸ ðü %s t¥n hoàn châu", tostring(stone.num))
	...
	if (bindNum >= stone.num) then --°ó¶¨µÄÊ¯Í·×ã¹»
		if LuaFnDelAvailableItem(sceneId, selfId, stone.id2, stone.num) == 0 then
			BeginEvent(sceneId)
				strText = "Kh¤u tr× v§t ph¦m th¤t bÕi !";
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
  ...
  message = format("#W#{_INFOUSR%s}#P thành công mang ðªn #YDoanh Vô Doanh#P tÕi #GT¥n Hoàng Ð¸a Cung#P mang ðªn %s cái #Y T¥n Hoàng Châu#P th§t là cäm kích ð°ng th¶i ban t£ng #Y#{_INFOMSG%s}#P. Chúc m×ng !", 
