	...
	BeginEvent(sceneId)
		if LuaFnHaveImpactOfSpecificDataIndex(sceneId,targetId,x910226_g_Impact)>=1 then
			...
			if GetName(sceneId,selfId)==nName then
				AddNumText(sceneId,x910226_g_ScriptId,"Chåm sóc",6,0)
			end
		end
	EndEvent(senceId)
...
local key = GetNumText()
	...
	if key == 0 then
		...
		SetCharacterName(sceneId,MstId,"Tiên hoa c¤p 4 cüa "..GetName(sceneId,selfId))
		...
		BeginEvent(sceneId)
			AddText(sceneId,"Cây ðã l¾n r°i!")
		EndEvent(sceneId)
