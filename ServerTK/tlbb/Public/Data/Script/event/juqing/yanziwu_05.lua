...
function x200015_OnKillObject( sceneId, selfId, objdataId ,objId)
	...
	elseif szMonsterName == "Ð® tØ H± Träo môn"	  then
		-- È¡ ði¬mCái này ¹ÖÎïËÀºóÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËÊý
		local num = GetMonsterOwnerCount(sceneId,objId)
		for j=0,num-1  do
			-- È¡ ði¬mÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËtoÕ ðµ objId
			...
			if IsHaveMission(sceneId, humanObjId, x200015_g_MissionId) > 0 then
				...
				BeginEvent(sceneId)
				AddText(sceneId,"Ðã giªt chªt Ð® tØ H± Träo môn: 1/1");
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,humanObjId)
			end
		end

	elseif szMonsterName == "Cù nhi­m võ sî"	  then
		-- È¡ ði¬mCái này ¹ÖÎïËÀºóÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËÊý
		local num = GetMonsterOwnerCount(sceneId,objId)
		for j=0,num-1  do
			-- È¡ ði¬mÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËtoÕ ðµ objId
			...
			if IsHaveMission(sceneId, humanObjId, x200015_g_MissionId) > 0 then
				...
				BeginEvent(sceneId)
				AddText(sceneId,"Ðã giªt chªt Cù nhi­m võ sî: 1/1");
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,humanObjId)
			end
		end
