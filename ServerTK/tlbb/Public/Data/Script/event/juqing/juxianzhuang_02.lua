...
function x200021_OnAccept( sceneId, selfId )
	...
	for ii=0, nMonsterNum-1 do
		...
		if GetName(sceneId, nMonsterId)  == "Bäo Thiên Linh"  then
		...
		elseif GetName(sceneId, nMonsterId)  == "KÏ Løc"  then
			SetUnitReputationID(sceneId, selfId, nMonsterId, 29)
			SetMonsterFightWithNpcFlag(sceneId, nMonsterId, 1)
		elseif GetName(sceneId, nMonsterId)  == "HÑa V÷ng Häi"  then
			SetUnitReputationID(sceneId, selfId, nMonsterId, 29)
			SetMonsterFightWithNpcFlag(sceneId, nMonsterId, 1)
		end
	end
...
function x200021_OnKillObject( sceneId, selfId, objdataId ,objId)
	...
	if GetName(sceneId,objId) == "Bäo Thiên Linh"	  then
		...
		for j=0,num-1  do
			...
			if IsHaveMission(sceneId, humanObjId, x200021_g_MissionId) > 0 then
				...
				BeginEvent(sceneId)
				AddText(sceneId,"Ðã giªt chªt Bäo Thiên Linh 1/1");
				EndEvent(sceneId)
	...
	if GetName(sceneId,objId) == "HÑa V÷ng Häi"	  then
		-- È¡ ði¬mCái này ¹ÖÎïËÀºóÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËÊý
		local num = GetMonsterOwnerCount(sceneId,objId)
		for j=0,num-1  do
			-- È¡ ði¬mÓµÓÐ·ÖÅäÈ¨toÕ ðµ ÈËtoÕ ðµ objId
			...
			if IsHaveMission(sceneId, humanObjId, x200021_g_MissionId) > 0 then
				...
				BeginEvent(sceneId)
				AddText(sceneId,"Ðã giªt chªt HÑa V÷ng Häi 1/1");
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,humanObjId)
