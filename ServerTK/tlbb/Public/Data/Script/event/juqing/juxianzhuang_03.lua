-- ¾ÛÏÍ×¯Ö® LÕc Dß½ng³¡¾°»¤ËÍ£¬ÖØµã²¿·ÖÅ¶
-- ´¥·¢Ìõ¼ş£¬Íê³ÉÈÎÎñ21£¬È»ºó½øÈëLÕc Dß½ngµÄÊÂ¼şÇøÓò
...
function x200022_OnAccept( sceneId, selfId, targetId )
	...
	if x200022_g_Step ~= 0  and x200022_g_Step ~= 2   then
		BeginEvent(sceneId)
			AddText(sceneId, "Møc tiêu ğang b§n, xin ğşi mµt lát!")
		EndEvent()
	...
	if x200022_g_Step == 2   then
		...
		if x200022_g_PlayerNum >= 6   then
			...
			Msg2Player(  sceneId, selfId,"#YS¯ ngß¶i tham gia l¥n hµ t¯ng này ğã ğ¥y, xin ch¶ l¥n sau!" , MSG2PLAYER_PARA )
			return
		end
...
function x200022_OnSceneTimer( sceneId, selfId )
	...
	-- ¼ì²âÖ»ÒªKi«u PhongºÍ°¢ÖéÖĞµÄÈÎºÎÒ»¸öÈË²»ÔÚ³¡¾°ÖĞÁË£¬¾ÍÇå¿ÕËùÓĞµÄÈ«¾Ö±äÁ¿
...
--		PrintNum(x200022_g_PlayerNum)
	...
	elseif x200022_g_Step == 4   then
		...
		-- ¼ì²â»¤ËÍµÄÍê³ÉÌõ¼ş£¬Ki«u Phong ×ßµ½ÁËÄ¿±êµã
		local nMonsterNum = GetMonsterCount(sceneId)
		local jj = 0
		local bHaveMonster = 0
		for jj=0, nMonsterNum-1 do
