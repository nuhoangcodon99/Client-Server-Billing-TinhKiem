...
-- TÕi »ªºÕôŞtoÕ ğµ °ïÖúÏÂ½øÈëÍò½Ù¹È,TÕi Íò½Ù¹ÈÖĞÕÒÈË´òÌıĞoàn DñtoÕ ğµ ÏûÏ¢.
-- ¶Ó³¤¿ÉÒÔ´ø×Ô¼ºtoÕ ğµ Ğ¡¶ÓÈ¥Íê³ÉCái này ÈÎÎñ,Ö»Òª¶Ó³¤ÓĞCái này ÈÎÎñ¾Í¿ÉÒÔ,
-- Ğµi viên Ö»ÄÜ¶À×Ô½øÈë¸±±¾,µ¥¶ÀÍê³ÉCái này ÈÎÎñ
...
x200001_g_CopySceneType = FUBEN_JUQING	--¸±±¾ÀàĞÍ,¶¨ÒåTÕi ScriptGlobal.luaÀïÃæ

x200001_g_CopySceneMap = "wanjie_1.nav"
x200001_g_Exit = "wanjie_1.ini"
x200001_g_LimitMembers = 1				--¿ÉÒÔ½ø¸±±¾toÕ ğµ ×îĞ¡¶ÓÎéÈËÊı
x200001_g_TickTime = 5					--»Øµ÷½Å±¾toÕ ğµ Ê±ÖÓÊ±¼ä(µ¥Î»:  giây/´Î)
x200001_g_LimitTotalHoldTime = 360		--¸±±¾¿ÉÒÔ´æ»îtoÕ ğµ Ê±¼ä(µ¥Î»: ´ÎÊı),Èç¹û´ËÊ±¼äµ½ÁË,ÔòÈÎÎñ½«»áth¤t bÕi
x200001_g_LimitTimeSuccess = 500		--¸±±¾Ê±¼äÏŞÖÆ(µ¥Î»: ´ÎÊı),Èç¹û´ËÊ±¼äµ½ÁË,ÈÎÎñÍê³É
x200001_g_CloseTick = 3					--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±(µ¥Î»: ´ÎÊı)
x200001_g_NoUserTime = 300				--¸±±¾ÖĞÃ»ÓĞÈËºó¿ÉÒÔ¼ÌĞø±£´ætoÕ ğµ Ê±¼ä(µ¥Î»:  giây)
x200001_g_DeadTrans = 0					--ËÀÍö×ªÒÆÄ£Ê½,0: ËÀÍöºó»¹¿ÉÒÔ¼ÌĞøTÕi ¸±±¾,1: ËÀÍöºó±»Ç¿ÖÆÒÆ³ö¸±±¾
x200001_g_Fuben_X = 108					--½øÈë¸±±¾toÕ ğµ Î»ÖÃX
x200001_g_Fuben_Z = 115					--½øÈë¸±±¾toÕ ğµ Î»ÖÃZ
...
function x200001_OnEnumerate( sceneId, selfId, targetId )
	...
	-- 1,ÓµÓĞÈÎÎñºÅÎª2toÕ ğµ ÈÎÎñ,
	if IsHaveMission( sceneId, selfId, 2 ) > 0 then
		AddNumText( sceneId, x200001_g_ScriptId, "Hãy ğßa ta t¾i VÕn Kiªp C¯c (ğêm)", 10 ,-1  )
		return 0
	end

	-- 2,Ö»ÒªÍæ¼ÒÓĞ3ÈÎÎñ,¾ÍÄÜ½øÈë
	if IsHaveMission( sceneId, selfId, 3 ) > 0 then
		AddNumText( sceneId, x200001_g_ScriptId, "Hãy ğßa ta t¾i VÕn Kiªp C¯c (ğêm)", 10 ,-1  )
		return 0
	end
	
	-- 3,Èç¹ûÍæ¼ÒÍê³ÉÁË2,Ã»ÓĞ3,Ò²ÄÜ½øÈë
	...
	-- 4,Èç¹ûÍæ¼ÒÍê³ÉÁË3,µ«ĞúngÃ»ÓĞ4,Ò²Ğúng¿ÉÒÔ½øÈë
...
function x200001_MakeCopyScene( sceneId, selfId )
	...
	LuaFnSetSceneLoad_Map(sceneId, "wanjie_1.nav"); --µØÍ¼Ğúng±ØĞëÑ¡È¡toÕ ğµ ,¶øÇÒ±ØĞëTÕi Config/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x200001_g_CopySceneType);--ÉèÖÃ¸±±¾Êı¾İ,ÕâÀï½«0ºÅË÷ÒıtoÕ ğµ Êı¾İÉèÖÃÎª999,ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êı×Ö×Ô¶¨Òå)
	LuaFnSetCopySceneData_Param(sceneId, 1, x200001_g_ScriptId);--½«1ºÅÊı¾İÉèÖÃÎª¸±±¾³¡¾°ÊÂ¼ş½Å±¾ºÅ
...
--PrintNum(111)
	...
	if nJuqingStep == 1  then
		...
		for i=0, 	nHumanNum-1  do
			...
			-- µ÷ÓÃ½ÓÈÎÎñtoÕ ğµ Óï¾ä
			CallScriptFunction((200003), "OnDefaultEvent",sceneId, nHumanId, targetId )
		end
		LuaFnSetCopySceneData_Param(sceneId, 9, nowTime)

		-- TÕi ÕâÀïÌí¼Ómµt Ğ©NPCtoÕ ğµ paopao,Ê¹ ği¬m¾çÇé¿´ÉÏÈ¥¸ü¼ÓÏÎ½Ómµt Ğ©
	...
	elseif nJuqingStep == 6  then  --¼ì²â»¤ËÍ¶ÔÏóĞúng²»Ğúngµ½ÁË
		...
		if distance2 <= 1 then
			...
			for i=0, 	nHumanNum-1  do
			  ...
			  -- ¸øÓĞCái này ÈÎÎñtoÕ ğµ ÈË¼ÓÉÏÍê³É±êÖ¾
