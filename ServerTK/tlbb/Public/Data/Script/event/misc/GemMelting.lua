...
function x800118_OnGemMelting( sceneId, selfId, GemItemPos1, GemItemPos2, GemItemPos3, NeedItemPos )
	...
	if GemItemID1 ~= GemItemID2 or GemItemID1 ~= GemItemID3 then
		x800118_NotifyTip( sceneId, selfId, "Bäo thÕch không ð°ng nh¤t, dung luy®n th¤t bÕi" )
		return
	end
	...
	if  -1 == ProductID then
		x800118_NotifyTip( sceneId, selfId, "Bäo thÕch không th¬ dung luy®n, dung luy®n th¤t bÕi !" )
		return
	end

	--¼ì²âÊÇ·ñÓÐËùÐèÎïÆ·....
	if NeedID ~= NeedItemID then
		x800118_NotifyTip( sceneId, selfId, "V§t ph¦m dung luy®n c¥n thiªt th¤t bÕi, dung luy®n th¤t bÕi" )
		return
	end
	...
	if PlayerMoney < NeedMoney then
		x800118_NotifyTip( sceneId, selfId, "Không ðü ngân lßþng, dung luy®n th¤t bÕi" )
		return
	end
	...
	if ret ~= 1 then
		x800118_NotifyTip( sceneId, selfId, "SØ døng nguyên li®u th¤t bÕi, dung luy®n th¤t bÕi !" )
		return
	end
	...
	if LuaFnEraseItem( sceneId, selfId, GemItemPos1 ) == 0 then
		x800118_NotifyTip( sceneId, selfId, "Kh¤u tr× v§t ph¦m th¤t bÕi, dung luy®n th¤t bÕi !" )
		return
	end
	...
	if BagIndex == -1 then
		x800118_NotifyTip( sceneId, selfId, "Tai nãi ðã ð¥y, dung luy®n th¤t bÕi" )
		return
	end
	if bNeedBind == 1 then
	...
	--ÐÑÄ¿ÌáÊ¾....
	x800118_NotifyTip( sceneId, selfId, "3 viên #{_ITEM"..GemItemID1.."}, dung luy®n thành công thành #{_ITEM"..ProductID.."}." )
	...
	if LuaFnGetItemQuality(ProductID) >= 3 then
		local MeltingNPCTbl =
		{
			...
			[420] = "Thúc Hà C± Tr¤n (134,84) Kim Khäm Thñc",
