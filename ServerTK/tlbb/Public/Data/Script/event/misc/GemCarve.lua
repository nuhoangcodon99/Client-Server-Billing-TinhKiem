...
function x800117_OnGemCarve( sceneId, selfId, GemItemPos, NeedItemPos,TargetID )
	...
	if ret ~= 1 then
		x800117_NotifyTip( sceneId, selfId, "Bäo thÕch không th¬ ðiêu trác, ðiêu trác th¤t bÕi !" )
		return
	end
	ret = LuaFnIsItemAvailable( sceneId, selfId, NeedItemPos )
	if ret ~= 1 then
		x800117_NotifyTip( sceneId, selfId, "Không sØ døng ðßþc v§t ph¦m c¥n thiªt, ðiêu trác th¤t bÕi !" )
		return
	end
	...
	if ret < 0 then
		x800117_NotifyTip( sceneId, selfId, "Không ðü ngân lßþng, ðiêu trác th¤t bÕi !" )
		return
	end
	...
	if BagIndex == -1 then
		x800117_NotifyTip( sceneId, selfId, "Tai nãi ðã ð¥y, ðiêu trác th¤t bÕi !" )
	end
	...
	if (ProductNeedBind == 1) then
	  ...
	  if bindidx ~= 1 then
		  local bindmsg = "Không th¬ kªt hþp"													
		  BeginEvent( sceneId )
	...
	--ÐÑÄ¿ÌáÊ¾Íæ¼Òµñ×Á³É¹¦....
	x800117_NotifyTip( sceneId, selfId, "#{_ITEM"..GemItemID.."} ðßþc ðiêu trác thành  #{_ITEM"..ProductID.."}" )
	...
	local strText = format("#{_INFOUSR%s}#I tÕi #G%s #R%s #I  tay c¥m #{_INFOMSG%s}#I ðiêu trác thành công #{_INFOMSG%s} #I tÕi %s  giúp bäo thÕch tång thêm sÑc mÕnh.", Name, SceneName,NPCName,NeedItemInfo, GemItemInfo, SceneName)
