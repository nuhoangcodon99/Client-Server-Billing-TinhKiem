...
function x713625_OnDefaultEvent( sceneId, selfId, targetId, nNum, npcScriptId, bid )
	if bid then
		if 0 == nNum then	--Ñ§Ï°
			...
			local studyMsg = format("Nªu các hÕ ğÕt t¾i c¤p %d, phäi tiêu t¯n #{_MONEY%d} và %d ği¬m bang hµi m¾i có th¬ h÷c kÛ nång ki®n th¬. Các hÕ có quyªt ğ¸nh h÷c không?", lv, money, con)
			AddText(sceneId,studyMsg)
			--È·¶¨Ñ§Ï°°´Å¥
					AddNumText(sceneId,x713625_g_ScriptId,"TÕi hÕ xác ğ¸nh mu¯n h÷c", 6, 3)
			--È¡ÏûÑ§Ï°°´Å¥
					AddNumText(sceneId,x713625_g_ScriptId,"TÕi hÕ chï mu¯n coi", 8, 4)
			EndEvent(sceneId)
		...
		-- Ä¿Ç°½¡Ìå¼¼ÄÜ±ØĞëÔÚ³ÇÊĞÀï²ÅÄÜÑ§Ï°¡¢Éı¼¶¡¢ÁË½â
	end
end
...
function x713625_OnEnumerate( sceneId, selfId, targetId, bid, nNum)
	if bid then
		if 1 == nNum then
			...
			if ret > 0 then AddNumText(sceneId,x713625_g_ScriptId,"H÷c "..x713625_g_AbilityName.." kÛ nång", 12, 0) end
			return
		elseif 2 == nNum then
			local ret = CallScriptFunction( CITY_BUILDING_ABILITY_SCRIPT, "OnCityCheck",sceneId, selfId, x713625_g_AbilityID, bid, 6)
			if ret > 0 then AddNumText(sceneId,x713625_g_ScriptId,"Thång c¤p"..x713625_g_AbilityName.." kÛ nång", 12, 1) end
			return
		elseif 3 == nNum then
			AddNumText(sceneId,x713625_g_ScriptId,"TÕi hÕ mu¯n tìm hi¬u ki®n th¬",11,2)
			AddNumText(sceneId,x713625_g_ScriptId,"Quay lÕi",8,5)
		end
	else
		-- Ä¿Ç°½¡Ìå¼¼ÄÜ±ØĞëÔÚ³ÇÊĞÀï²ÅÄÜÑ§Ï°¡¢Éı¼¶¡¢ÁË½â
	end
