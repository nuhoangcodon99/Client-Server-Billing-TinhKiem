	...
	elseif ButtomNum == PET_FULL_OK then
		...
		if (nMoneyJZ + nMoney >= MoneyCost) then
			...
			if (moneyJZ == MoneyCost) then
				-- Ö»¿Û½»×Ó
				BeginEvent( sceneId )
					AddText( sceneId, "  Các hÕ ğã sØ døng #G#{_EXCHG"..moneyJZ.."}#W, ğ¬ chæa tr¸ cho trân thú. " )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			...
			elseif (moneyJZ > 0) and (moneyJZ + money) == MoneyCost then
				-- ¿Û³ı½»×ÓºÍ½ğÇ®
				BeginEvent( sceneId )
					AddText( sceneId, "  Các hÕ tiêu t¯n #G#{_EXCHG"..moneyJZ.."}#W, " )
					AddText( sceneId, "  Các hÕ tiêu t¯n #G#{_MONEY"..money.."}#W, " )
					AddText( sceneId, "  ğ¬ chæa tr¸ cho trân thú. " )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			...
			elseif (moneyJZ == 0) and ( money == MoneyCost) then
				-- ¿Û³ı½ğÇ®
				BeginEvent( sceneId )
					AddText( sceneId, "  Các hÕn t¯n #G#{_MONEY"..money.."}#W, ğ¬ chæa tr¸ cho trân thú." )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )

			else			
				BeginEvent( sceneId )
					AddText( sceneId, "  H°i phøc th¤t bÕi. " )
				EndEvent( sceneId )
				DispatchEventList( sceneId, selfId, targetId )
			end
