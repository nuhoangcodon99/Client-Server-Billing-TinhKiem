  ...
  if MatBagIndex1 ~= -1 then
	...
	-- Èç¹ûÊ§°Ü
		...
		if gemQual - qualDec > 0 then
			...
			x701614_MsgBox( sceneId, selfId, "#YTh§t không may, khäm nÕm th¤t bÕi bäo thÕch còn lÕi "..qualDec.." c¤p !" )
		else
			x701614_MsgBox( sceneId, selfId, "#YTh§t không may, khäm nÕm th¤t bÕi bäo thÕch b¸ t±n hÕi !" )
		end
		--Add Log
		LogInfo	= format( "[EMBED]: sceneId=%d, GUID=%0X, bSuc=%d, idOldGem=%d, idEqp=%d, idMat0=%d, idMat1=%d, idNewGem=%d",
			sceneId,
  ...
  if MatBagIndex1 ~= -1 then
	...
	-- Èç¹ûÊ§°Ü
		...
		if gemQual - qualDec > 0 then
			...
			x701614_MsgBox( sceneId, selfId, "#YTh§t không may m¡n, khäm nÕm th¤t bÕi "..qualDec.." c¤p" )
		else
			x701614_MsgBox( sceneId, selfId, "#YKhông may m¡n, tß½ng khäm th¤t bÕi, bäo thÕch b¸ suy giäm." )
		end
		--Add Log
		LogInfo	= format( "[EMBED]: sceneId=%d, GUID=%0X, bSuc=%d, idOldGem=%d, idEqp=%d, idMat0=%d, idMat1=%d, idNewGem=%d",
			sceneId,
