...
x701602_g_Pos_normal= {{left={269,316},right={275,326}}}
x701602_g_Pos_fashion= {{left={340,268},right={344,273}}}-- ½Å±¾Ãû³Æ
...
x701602_g_CompoundRule = {
	{
		...
		[9] = { SpecialStuff = 30503134, MoneyCost = 12000, CountTable = { [3] = { SuccOdds = 25, SuccOddsWithSpecStuff = 50, }, [4] = { SuccOdds = 50, SuccOddsWithSpecStuff = 75, }, [5] = { SuccOdds = 75, SuccOddsWithSpecStuff = 100, }, }, },
...
function x701602_GemCompound( sceneId, selfId, bagIndex1, bagIndex2, bagIndex3, bagIndex4, bagIndex5, bagIndex6 )
	...
	for i = 1, 5 do
		if bagIndexList[i] ~= -1 then
			...
			local Typegem = mod( CompoundType, 100000 )

				if Typegem > 41000 or Typegem > 42000 or Typegem > 43000 or Typegem > 44000 then
					BeginEvent( sceneId )
						AddText( sceneId, "Level of gems now were maximum !")
					EndEvent( sceneId )
					DispatchMissionTips( sceneId, selfId )
					return
				elseif  Typegem > 31000 or Typegem > 32000 or Typegem > 33000 or Typegem > 34000 then
					for i=1,getn( x701602_g_Pos_normal ) do

						if x701602_g_Pos_normal[i].left[1] <= posX and posX <= x701602_g_Pos_normal[i].right[1] and x701602_g_Pos_normal[i].left[2] <= posZ and posZ <= x701602_g_Pos_normal[i].right[2] then  
							BeginEvent( sceneId )
								AddText( sceneId, "Ng÷c th¶i trang không th¬ tiªn hành hþp thành tÕi ðây !!!")
							EndEvent( sceneId )
							DispatchMissionTips( sceneId, selfId )
							return
						else
							if bagIndex6 ~= 30503137 then
								BeginEvent( sceneId )
									AddText( sceneId, "Hãy cho vào nguyên li®u ð£c thù ð¬ hþp thành ðßþc ng÷c th¶i trang !!")
								EndEvent( sceneId )
								DispatchMissionTips( sceneId, selfId )
								return
							end
				...
				elseif  Typegem < 31000 or Typegem < 32000 or Typegem < 33000 or Typegem < 34000 then
					for i=1,getn( x701602_g_Pos_fashion ) do

						if x701602_g_Pos_fashion[i].left[1] <= posX and posX <= x701602_g_Pos_fashion[i].right[1] and x701602_g_Pos_fashion[i].left[2] <= posZ and posZ <= x701602_g_Pos_fashion[i].right[2] then  

							BeginEvent( sceneId )
								AddText( sceneId, "Ta chï có th¬ giúp nhà ngß½i h½p thành ng÷c th¶i trang thôi !  ")
							EndEvent( sceneId )
							DispatchMissionTips( sceneId, selfId )
							return
						end
