...
--ÈÎÎñ±äÁ¿
--µÚÒ»Î»´æµÄÊÇ(Âô)ÈÈÏúµÄÖµ  						µÚ0Î»		Price_Up
--µÚ¶þÎ»´æµÄÊÇ(Âò)É±¼ÛµÄÖµ							µÚ1Î»		Price_Down
--µÚÈýÎ»´æµÄÊÇÉíÉÏÑÎ¡¢Ìú¡¢Ã×µÄÊýÁ¿			µÚ2Î»   ÑÎ*100+Ìú*10+Ã×
--			µÚËÄÎ»´æµÄÊÇÈÈÏúµÄCDÉÏ´ÎÊ¹ÓÃÊ±¼ä			µÚ3Î»		
--			µÚÎåÎ»´æµÄÊÇÉ±¼ÛµÄCDÉÏ´ÎÊ¹ÓÃÊ±¼ä			µÚ4Î»		
--µÚËÄÎ»´æµÄÊÇÍæ¼Òµ±Ç°µÄ×´Ì¬£¬±íÊ¾½øÐÐµ½µÄ²½Öè£¬×ßµ½µÄÁ÷³Ì¡£		µÚ3Î»
--µÚÎåÎ»´æµÄÊÇÍæ¼Òµ±Ç°µÄÌØÊâ×´Ì¬£¬Èç¸ß¼Ûäî»õµÈ				µÚ4Î»
--µÚÁùÎ»´æµÄÊÇ¹ÙÆ±ÉÏµÄÃæ¶î  						µÚ5Î»		Balance
--µÚÆßÎ»´æµÄÊÇäîÔËÊ¹µÄTargetId£¬1:´ú±íÂåÑôµÄäîÔËÊ¹£¬2:´ú±íËÕÖÝ£¬3´ú±í´óÀí£¬4´ú±í¿ÉÒÔ³öÊÛäî»õµÄ		µÚ6Î» TransNPC
--µÚ°ËÎ»´æµÄÊÇ»õÉÌÉÏ´Î·¢ÉúÊÂ¼þµÄÊ±¼ä£¬Ò»¶¨µÄÊ±¼äÄÚ£¬Ö»ÄÜÖ´ÐÐÒ»´Î»õÉÌµÄÌØÊâÊÂ¼þ¡£
--
--¹ÙÆ±µÄÎïÆ·±àºÅ40002109
...
x311010_g_Transportation_Exit[3] = {
	...
	[2] = { Exit_Name="Tung S½n ðªn Thái H°",   Exit_Reply_Number = 16, Exit_Position = {x=281,y=250} } ,
	}
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x311010_OnDefaultEvent( sceneId, selfId, BagIndex )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	--Í¶»úÈ¡ÇÉµÄ·½·¨£¬Ö»´ËÒ»´Î£¬ÎðÍÆ¹ã¡£
...
--ÆÁ±Î³ÆºÅ²¿·Ö
--			find = -1
--Ôö¼ÓÒ»¸ö²éÑ¯Íæ¼ÒÊÇ·ñÒÑµÃµ½¸Ã³ÆºÅµÄ½Ó¿Ú£¬ËïÓê
...
--			PrintNum(New_Title)
			...
			--client ÒªÔÚUI ÉÏÏÔÊ¾"ÂòÂôäî»õ"ºÍ"¶Ò»»¹ÙÆ±"Õâ2¸öbutton
			--²¢ÇÒ°ÑNPCµÄID´«¸øclient£¬ÒÔ±¸ÏÂÒ»²½µ÷ÓÃ¡£
    ...
    elseif 1 > 0 then
	    local TransNPC = x311010_TransPortNPC(sceneId,targetId)
			...
			--client ÒªÔÚUI ÉÏÏÔÊ¾"»ñµÃ¹ÙÆ±"Õâ¸öbutton
			--²¢ÇÒ°ÑNPCµÄID´«¸øclient£¬ÒÔ±¸ÏÂÒ»²½µ÷ÓÃ¡£
			BeginUICommand(sceneId)
...
function x311010_CheckAccept( sceneId, selfId )
	...
	elseif GetLevel( sceneId, selfId ) < 40 then
		...
		local msg = "c¤p b§c cüa các hÕ vçn chßa ðªn 30, hãy ðþi ðªn khi ðÕt ðßþc c¤p 30 r°i hÆn ðªn tìm ta nhé"
 		local cmsg = "Ta s¨ lên c¤p 30 nhanh thôi"
		x311010_Client_Show_Message( sceneId, selfId, targetId, msg, cmsg,0)
...
--ÒÔºó²ß»®¸ÄÖ÷ÒâÁË¾Í°ÑÕâ¶Î´ò¿ª¡£
	elseif GetMoney(sceneId,selfId)+GetMoneyJZ(sceneId, selfId) < Corpus then
		BeginEvent(sceneId)
			AddText(sceneId,"Các hÕ ít nh¤t c¥n có #{_MONEY"..Corpus.."}, m¾i có th¬ l¤y ðßþc quan phiªu");
			EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return 0
...
function x311010_OnHaggleUp( sceneId, selfId )	--µã»÷¸ÃÈÎÎñ"ÈÈÏú"ºóÖ´ÐÐ´Ë½Å±¾

--ÅÐ¶ÏËûÊÇ·ñ¿ÉÈÈÏú
--±ÈÈçÊ±¼äµÄÅÐ¶Ï£¬ÅÐ¶ÏÕâ´ÎºÍÉÏ´ÎÖ®¼äÊ±¼äµÄ²î¾à
...
--ÈÈÏúºó¼Û¸ñ±äÎªPrice_Up
			UICommand_AddInt(sceneId,misIndex)
--´ËÊ±ÈÈÏúµÄÀäÈ´Ê±¼ä±äÎª10·ÖÖÓ£¬²¢¸æÖªclient
	
--µÚ2Î»±íÊ¾µ±Ç°»¹Ê£µÄPriceUpÊ±¼ä
			Pre_Time = GetMissionData(sceneId,selfId,MD_CAOYUN_BARGAINUP)
...
function x311010_OnHaggleDown( sceneId, selfId )	--µã»÷¸ÃÈÎÎñ"É±¼Û"ºóÖ´ÐÐ´Ë½Å±¾
...
--´ËÊ±É±¼ÛµÄÀäÈ´Ê±¼ä±äÎª15·ÖÖÓ£¬²¢¸æÖªclient
--µÚ2Î»±íÊ¾µ±Ç°»¹Ê£µÄPriceUpÊ±¼ä
		Pre_Time =  GetMissionData(sceneId,selfId,MD_CAOYUN_BARGAINUP)
		Now_Time = LuaFnGetCurrentTime();
		if Now_Time - Pre_Time < 10*60 then
...
function x311010_OnAcceptMission( sceneId, selfId )
  ...
  --ÊÇ·ñÔÚäîÔË
	...
	if ItemNum >= x311010_g_CashItem_count then
		...
		if ret < 1  then
--			BeginEvent(sceneId)
--				strText = "#YÄãµÄÈÎÎñÈÕÖ¾ÒÑ¾­ÂúÁË£¬ÎÞ·¨½øÐÐäîÔË¡£"
--				AddText(sceneId,strText);
--			EndEvent(sceneId)
--			DispatchMissionTips(sceneId,selfId)
			return
		...
		local misIndex = GetMissionIndexByID(sceneId,selfId,x311010_g_MissionId)			--µÃµ½ÈÎÎñµÄÐòÁÐºÅ
		...
		if (nDelJZ == -1) then
			BeginEvent(sceneId)
			AddText(sceneId,"Các hÕ c¥n có #{_EXCHG"..Corpus.."} m¾i có th¬ có quan phiªu");  
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return 0;
		end
		...
		if (nDelMoney <= 0) then
			BeginEvent(sceneId)
				AddText(sceneId,"Các hÕ c¥n có thª ch¤p #{_EXCHG"..nDelJZ.."} trß¾c khi thñc hi®n tàu v§n.");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		elseif (nDelJZ <= 0) then
			BeginEvent(sceneId)
				AddText(sceneId,"Các hÕ c¥n có thª ch¤p #{_MONEY"..nDelMoney.."} trß¾c khi thñc hi®n tàu v§n." );
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		else
			BeginEvent(sceneId)
					AddText(sceneId,"Các hÕ c¥n thª ch¤p #{_EXCHG"..nDelJZ.."}");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			BeginEvent(sceneId)
				AddText(sceneId,"Các hÕ c¥n thª ch¤p  #{_MONEY"..nDelMoney.."} trß¾c khi thñc hi®n tàu v§n.");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end
...
--			AddItemListToHuman(sceneId,selfId)
	...
	elseif x311010_CheckAccept(sceneId,selfId) > 0 then
		...
		if ret > 0 then 	
			...
			if (nDelJZ == -1) then
				BeginEvent(sceneId)
				AddText(sceneId,"Các hÕ ðã thª ch¤p #{_EXCHG"..Corpus.."} l¤y mµt quan phiªu Tàu V§n.");  
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
				return 0;
			end
			...
			if (nDelMoney <= 0) then
				BeginEvent(sceneId)
					AddText(sceneId,"Các hÕ ðã thª ch¤p #{_EXCHG"..nDelJZ.."} l¤y mµt quan phiªu Tàu V§n.");
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
			elseif (nDelJZ <= 0) then
				BeginEvent(sceneId)
					AddText(sceneId,"Các hÕ ðã thª ch¤p #{_MONEY"..nDelMoney.."} l¤y mµt quan phiªu Tàu V§n." );
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
			else
				BeginEvent(sceneId)
...
--				AddItemListToHuman(sceneId,selfId)
			...
			--°ÑÇ°Ãæ¼ÓµÄÈÎÎñÉ¾µô£¬add by hukai#39191
...
--**********************************
--µ¯³ö½»Ò×½çÃæ
--¾ÍÊÇÔÚäîÔËÊ¹´¦µã»÷ÁË¡°ÂòÂôäî»õ¡±»òÕßµã»÷»õÉÌ£¬²¢Ëæ»úµ¯³öÁË½»Ò×½çÃæ
--**********************************
...
--client ÒªÏÔÊ¾³ö½»Ò×½çÃæ£¬¸ù¾ÝÉÏÃæµÄµÚÒ»¸ö²ÎÊý£¬À´·ÅÑÎ¡¢Ìú¡¢Ã×µÄbuttonºÍÏÔÊ¾É±¼ÛÈÈÏúbutton
				UICommand_AddInt(sceneId,TransNpc)
...
--			local TransNPC = GetMissionParam(sceneId,selfId,misIndex,6)
	...
	-- hongyu  
	-- Èç¹ûÍæ¼Ò´«¹ýÀ´Ò»¸öÎÞ¹ØµÄÖµ£¬Ö±½Ó¶ªÆú
	if ManipulateID~=1 and ManipulateID~=2 and ManipulateID~=3  then
		return
	end
	-- ¿Í»§¶Ë·¢¹ýÀ´µÄ ManipulateID ÓÐ1£¬2£¬3Õâ3¸öÖµ£¬·Ö±ð±íÊ¾
	--  		1		ÑÎ	ÂåÑô	0
	-- 			2		Ìú	´óÀí	2
	--			3		Ã×	ËÕÖÝ	1
	--
	...
	if ManipulateID == 1  then
		if sceneId == 0   then
			...
			-- Íæ¼ÒÇëÇóÖ´ÐÐÂòÈë²Ù×÷£¬¼ì²âÍæ¼ÒÊÇ²»ÊÇÒÑ¾­Âò¹ýÁËÕâ¸ö¶«Î÷
			if nData1==0  then
				return
			end
		end
	...
	if ManipulateID == 2  then
		if sceneId == 2   then
			-- Íæ¼ÒÇëÇóÖ´ÐÐÂô³ö²Ù×÷£¬¼ì²âÍæ¼ÒÊÇ²»ÊÇÓÐ¶«Î÷¿ÉÂô³ö
			if nData2~=0  then
				return
			end
		else
	...
	if ManipulateID == 3  then
		if sceneId == 1   then
			-- Íæ¼ÒÇëÇóÖ´ÐÐÂòÈë²Ù×÷£¬¼ì²âÍæ¼ÒÊÇ²»ÊÇÓÐ¿Õ¼ä¿ÉÂòÈë
			if nData3~=0  then
				return
			end
		else
...
--¸ù¾ÝÕâ¸öNPCºÍPlayerÉíÉÏµÄÇé¿öÀ´ÅÐ¶Ï£¬¶ÔÑÎ£¬Ìú£¬Ã×Õâ¸ö¶«Î÷ÊÇÂò»¹ÊÇÂô
			if ManipulateID == TransNPC then

--ÕâÊ±ÊÇ¹ºÂòÑÎ
--ÏÈÅÐ¶ÏÕâ¸öPlayerÊÇ·ñÒÑ¾­¹ºÂòÁËÑÎ
...
--						BeginEvent(sceneId)
--							strText="Äã¿ìÈ¥¶Ò»»¹ÙÆ±°É£¬±¾´Î½»Ò×ÒÑ¾­³¬ÏÞ¡£"
--							AddText(sceneId,strText);
--						EndEvent(sceneId)
--						DispatchMissionTips(sceneId,selfId)
--						return
...
--***					--ºÍÉÏÃæÄ³²¿·ÖÏàÍ¬¾Í¿ÉÒÔ
					...
					SetMissionByIndex(sceneId,selfId,misIndex,2,Cargo)
						...
						EndEvent(sceneId)
						DispatchMissionTips(sceneId,selfId)
					--²¢°ÑÉíÉÏµÄ±ê¼ÇÎ»ÖÃÎªÂòÈëÁËÑÎ
					--¸æËßclientÔÚUIÉÏÎÒµÄ»õ²ÕÄÚ£¬ÏÔÊ¾ÑÎ
					BeginUICommand(sceneId)
							UICommand_AddInt(sceneId,5)
...
--²¢È¡ÈËÎïÉíÉÏÕâ¸ö¼Û¸ñÊý¾ÝÀ´Ö´ÐÐ²Ù×÷¡£
--Âô¾ÍÊÇ½áÊøÕâ»·£¬»·ÊýÒª¸Ä±ä
		if trade == 1 then
--»·Êý+1
			misIndex = GetMissionIndexByID(sceneId,selfId,x311010_g_MissionId)
			local Mission_Round =	GetMissionData(sceneId,selfId,MD_CAOYUN_HUAN)
--missiondataÖÐµÚ2Î»¼ÇÂ¼µÄÊÇäîÔËµÄ»·Êý
...
function x311010_OnRedeemUI( sceneId, selfId )
    ...
    if IsHaveMission(sceneId,selfId,x311010_g_MissionId) > 0 then
			...
			--½áÊø´ËÈÎÎñ£¬²¢¸ù¾Ý¹ÙÆ±ÉÏµÄÃæ¶îÀ´¸øÓèÏà¹Ø½±Àø
			--Ç®ºÍ¾­ÑéµÈ
			--¿ÉÄÜ½±Àø»¹¸ú»·ÊýÓÐ¹ØÏµ
			--begin modified by zhangguoxin 090207
...
--ÕâÀï²»¼ì²ébalance£¬ÒòÎªËùÓÐÂòµÄµØ·½¶¼ÅÅ³ýÁËÊÇ·ñ»á³öÏÖ¸ºÊýµÄÇé¿ö
			local Quotiety = x311010_Calculate_Modulus( sceneId, selfId )
			Quotiety = 2
			local Corpus = 10000 * Quotiety
...
--				BeginEvent(sceneId)	
--					strText = format("ÄãµÃµ½#{_MONEY"..Margin.."}µÄäîÔË»¨ºì¡£")
--					AddText(sceneId,strText);
--				EndEvent(sceneId)
--				DispatchMissionTips(sceneId,selfId)

				local targetId = GetMissionData(sceneId,selfId,MD_CAOYUN_TARGETID)
				--local msg = "¹§Ï²Äú½áÊøäîÔË£¬±¾ÂÖäîÔËÄú»ñµÃ»¨ºì#{_EXCHG"..Balance.."}£¬¼ÓÉÏ20%¶îÍâÊÕÒæ£¬Ò»¹²Îª#{_EXCHG"..1.2*Balance.."}¡£¿Û³ýÑº½ðºó£¬Äú»ñµÃ»¨ºì#{_EXCHG"..1.2*Balance-Corpus.."}¡£"
			...
			elseif Balance > 0 then
				...
				BeginEvent(sceneId)	
					strText = format("Các hÕ kinh doanh không lß½ng thi®n, không nh§n ðßþc hoa h°ng, chï nh§n ðßþc ti«n Tào v§n l¥n này #{_MONEY"..Balance.."}" )
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
			else
...
--**********************************
--ÅÐ¶ÏÓ¦¸Ã½áÊøËûµÄ´òÑ¹ÊÐ³¡µÄcool down
--**********************************
...
--**********************************
--ÅÐ¶ÏÓ¦¸Ã½áÊøËûµÄºåÌ§Îï¼ÛµÄcool down
--**********************************
...
--
--			--°´ÕÕ×îÐÂµÄ¼ÆËã·½·¨¼ÆËã³öµÄµôÂäµ½ÒøÆ±ÖÐµÄ½ðÇ®
--				--ÒøÆ±µÄ¼ÛÖµ£½Íæ¼ÒäîÔËÖ¤ÉÏËùÓÐµÄ½ðÇ®¡ÁµÈ¼¶Ó°ÏìÏµÊý¡£
--				--µÈ¼¶Ó°ÏìÏµÊý¼ÆËã¹æÔòÈçÏÂ£º
--				--µ±Íæ¼ÒµÈ¼¶<=30¼¶Ê±£¬µÈ¼¶Ó°ÏìÏµÊý = 25%
--				--µ±Íæ¼ÒµÈ¼¶>30¼¶²¢ÇÒ<=73¼¶Ê±£¬µÈ¼¶Ó°ÏìÏµÊý = 25%*(Íæ¼ÒµÈ¼¶/30)^1.27
--				--µ±Íæ¼ÒµÈ¼¶>73¼¶Ê±£¬µÈ¼¶Ó°ÏìÏµÊý = 80%
--				
--			local nLevel = GetLevel(sceneId, selfId)
--			local nQuotiety = 0
--			if nLevel <= 30  then
...
function x311010_OnAccept_Necessary( sceneId, selfId )
	...
	--Î¤Çà5ÔÂ4ÈÕÔÚÎÄµµÀïËµÒªÏÞÖÆ³É20´Î
  ...
  --
	...
	local iDayHuan = GetMissionData(sceneId,selfId,MD_CAOYUN_DAYCOUNT); --µ±ÌìÄÚÍê³ÉµÄÈÎÎñ´ÎÊý
	
	local iTime = GetMissionData(sceneId,selfId,MD_CAOYUN_DAYTIME);			--ÉÏ´ÎÍê³ÉÈÎÎñÊ±¼ä
	local iDayTime = floor(iTime/100)																		--ÉÏÒ»´Î½»ÈÎÎñµÄÊ±¼ä(ÌìÊý)
	local iQuarterTime = mod(iTime,100)																	--ÉÏÒ»´Î½»ÈÎÎñµÄÊ±¼ä(¿Ì)
...
--			BeginEvent(sceneId)
--				strText = format("Õâ¸öÈÎÎñÒ»Ìì×î¶à×ö%d´Î¡£", Max_Time_EveryDay )
--				AddText(sceneId,strText)
--			EndEvent(sceneId)
--			DispatchMissionTips(sceneId,selfId)
...
--			DispatchMissionTips(sceneId,selfId)
			local targetId = GetMissionData(sceneId,selfId,MD_CAOYUN_TARGETID)
--			local msg = "¶Ô²»Æð£¬Äú½ñÌìËù´ÓÊÂµÄäîÔËÈÎÎñÒÑ¾­Âú"..Max_Time_EveryDay.."´ÎÉÏÏÞ£¬ÇëÄúÃ÷ÌìÔÙÀ´°É¡£"
...
--*****************************************************
--Íæ¼ÒÁìÈ¡¹ÙÆ±²¢¹ºÂò±¾µØ»õÎïºó¸øÍæ¼ÒËæ»ú¼ÓÈýÖÖºÃ´¦
--Client\Interface\Carriage\Carriage.lua
