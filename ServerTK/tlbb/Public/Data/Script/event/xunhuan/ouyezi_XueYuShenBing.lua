...
--ÈÎÎñÐèÒªÉ±ËÀµÄ¹Ö
--±£Ö¤¿ÉÒÔ¸ù¾ÝµÈ¼¶¶¯Ì¬µÄ±ä»»É±¹ÖÊýÁ¿
x500503_g_RandomCustom = {{ id = "S¯ quái v§t", numNeeded = 3, numComplete = 1 }}

--ÈÎÎñÎÄ±¾ÃèÊö
x500503_g_MissionName			= "Huyªn Døc Th¥n Binh"
--ÈÎÎñÃèÊö
x500503_g_MissionInfo			= "C¥n giªt chªt quái v§t có ðÆng c¤p tß½ng Ñng 10 c¤p so v¾i ngß¶i ch½i, ðªn #{_INFOAIM266,140,1, Âu Dã TØ}  ð¬ chÑng minh thñc lñc"
--ÈÎÎñÄ¿±ê
x500503_g_MissionTarget		= "#{XYSB_20070928_010}"
--Î´Íê³ÉÈÎÎñµÄnpc¶Ô»°
x500503_g_ContinueInfo		= "#{XYSB_20070928_009}"
--Íê³ÉÈÎÎñnpcËµµÄ»°
x500503_g_MissionComplete	= "Chúc m×ng các hÕ ðã hoàn thành nhi®m vø Anh Hùng: Huyªt Døc Th¥n Binh"
--»·ÊýÉÏÏÞ
...
--ÈÎÎñ½Å±¾ºÅ
x500503_g_MissionScriptID	= 2			--±äÁ¿µÄµÚ2Î»
--É±¹Ö×ÜÊý
x500503_g_KillMonsterCount	= 3		--±äÁ¿µÄµÚ3Î»
--µ±Ç°É±¹ÖÊý
x500503_g_KillCountIng	= 4				--±äÁ¿µÄµÚ4Î»

--ËùÓµÓÐµÄÊÂ¼þIDÁÐ±í
x500503_g_EventList	= {}

x500503_g_XueXiZhuZaoInfo			= "H÷c ðúc th¥n khí"
...
x500503_g_ITEM_APT_LEVEL = 16				-- ¿ÉÒÔ½ÓÊÜÈÎÎñµÄÎäÆ÷×ÊÖÊ¼¶±ð
x500503_g_KillMONSTER_COUNT = 5000	-- ÒªÇóÉ±ËÀµÄ¹ÖÎï×ÜÊý
x500503_g_WeaponLevelMin		= 40		-- ÎäÆ÷×îµÍµÈ¼¶
x500503_g_WeaponLevelMax		= 150		-- ÎäÆ÷×î¸ßµÈ¼¶
x500503_g_AcceptLowLevel		= 40		-- ½ÓÊÜÈÎÎñµÄ×îµÍµÈ¼¶

x500503_g_Impact_Accept_Mission = 18		-- ½ÓÊÜÈÎÎñÊ±µÄÌØÐ§ID
x500503_g_Impact_Complete_Mission = 48	-- Íê³ÉÈÎÎñÊ±µÄÌØÐ§ID

x500503_g_ItemBonus={{id=30505700,num=1,sqlvl=42},{id=30505701,num=1,sqlvl=52},{id=30505702,num=1,sqlvl=62},{id=30505703,num=1,sqlvl=72},{id=30505704,num=1,sqlvl=82},{id=30505705,num=1,sqlvl=92}}   -- ²ÐÈ±µÄÉñ½Ú
...
function x500503_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnGetName( sceneId, targetId ) ~= x500503_g_Name then
		x500503_NotifyTip( sceneId, selfId, "Kªt n¯i v¾i máy chü l²i.." )					
		return 0
	end
	...
	if IsHaveMission( sceneId, selfId, x500503_g_MissionId ) > 0 then				
		...
		local ItemLevel = GetMissionData( sceneId, selfId, MD_MISSHENBING_WEAPONLEVEL )		-- »ñµÃ¿Û³ýµÄÎäÆ÷µÄµÈ¼¶
		local Errorlvl = GetMissionData( sceneId, selfId, 0 )															-- ¼ÇÂ¼´íÎóµÄÎäÆ÷µÄµÈ¼¶
		...
		BeginEvent(sceneId)
			...
			AddText( sceneId, "#r#Nhi®m vø này tß½ng Ñng v¾i vû khí ðÆng c¤p: "..shenqilvl )
		EndEvent( )
...
function x500503_OnAccept( sceneId, selfId, targetId, scriptId,iItemLevel )
	...
	--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ0Î»ÖÃ0 (ÈÎÎñÍê³ÉÇé¿ö)
	SetMissionByIndex( sceneId, selfId, misIndex, 0, 0 )
	--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ1Î»ÖÃÎªÈÎÎñ½Å±¾ºÅ
...
function x500503_OnContinue( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		for i, item in x500503_g_RadioItemBonus do
			AddRadioItemBonus( sceneId, item.id, item.num )					-- ½±Àø¶àÑ¡Ò»ÎïÆ·
		end
...
--			ret2 = EndAddItem( sceneId, selfId )
			...
			if ret1 > 0 then
				...
				TryRecieveItem( sceneId, selfId, selectRadioId, QUALITY_MUST_BE_CHANGE )	-- ·Å²»ÏÂ¾ÍÃ»ÓÐÁË
					...
					BeginEvent(sceneId)
					strText = "Túi ð° ðã ð¥y, không th¬ nh§n ph¥n thß·ng"
			...
			--ÏÔÊ¾¶Ô»°¿ò
			BeginEvent( sceneId )	
				AddText( sceneId, "  Các hÕ ðã hoàn thành công vi®c, xem nhß ðây là ph¥n thß·ng nhö cho các hÕ" )
			EndEvent( sceneId )
			DispatchEventList( sceneId, selfId, targetId )			
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x500503_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎï
		...
		if IsHaveMission(sceneId, humanObjId, x500503_g_MissionId) > 0 then
			...
			if GetMissionParam(sceneId, humanObjId, misIndex, 0) <=0  then
				...
				if ((PlayerLevel - MonsterLevel >= 0) and (PlayerLevel - MonsterLevel < 10))
					...
					--¸ù¾ÝË«ÍêÃÀ±øÆ÷µÄµÈ¼¶»ñµÃÍæ¼ÒÐèÒªÉ±¶àÉÙ¸ö¹Ö£¬
					--¿ÉÄÜÒÔÇ°40-60¼¶µÄÍæ¼ÒÒÑ¾­ÁË½ÓÊÜÈÎÎñµÄÉ±5000µÄ¹Ö£¬ÐèÒª¾ÀÕý -zz
					...
					BeginEvent(sceneId)
						local str = format("Ðã giªt chªt %s%d/%d", " quái v§t", killedCount, demandKillCount )						
						AddText(sceneId, str)
					EndEvent(sceneId)
...
function x500503_AcceptMission( sceneId, selfId, targetId )
	...
	BeginEvent(sceneId)
		...
		-- AddItemDemand( sceneId, x500503_g_ItemBonus[1].id, x500503_g_ItemBonus[1].num )  -- ½±ÀøÒ»ÖÖÎïÆ·
...
function x500503_OnMissionCheck( sceneId, selfId, npcid, scriptId, index1, index2, index3, petindex )
	...
	-- ÑªÓûÉñ±øÈÎÎñÌá½»½çÃæ
	-- Ìá½»ÎäÆ÷µÄÈÎÎñÊÇ²»ÊÇÒª½ÓÊÜµÄÈÎÎñ
	...
	for i=1, 3 do
		...
		if index < 100 then
			...
			-- ¼ì²âÎäÆ÷ÊÇ·ñ·ûºÏÈÎÎñ½ÓÊÜÒªÇó  level >= x500503_g_WeaponLevelMin ÄÚ¹¦¹¥»÷ºÍÍâ¹¦¹¥»÷ >= x500503_g_ITEM_APT_LEVEL ÎäÆ÷µÄµÈ¼¶²»ÄÜ³¬¹ýÍæ¼ÒµÄµÈ¼¶ ÒÑ¼ø¶¨¹ý×ÊÖÊ
			...
			if accres > 0 then
				...
				if	ret > 0 then
					SetMissionData(sceneId, selfId, MD_MISSHENBING_WEAPONLEVEL, ItemLevel )		--¼ÇÂ¼¿Û³ýµÄÎäÆ÷µÄµÈ¼¶
					...
					BeginEvent(sceneId)			
						local strText = format("HuÖ bö vû khí th¤t bÕi")
						AddText(sceneId,strText)									
					EndEvent(sceneId)
	...
	x500503_NotifyTip( sceneId, selfId, "Không nh§n ðßþc nhi®m vø, vû khí th¤t bÕi" )
...
function x500503_GetItemDetailInfo(itemId)
	...
	if itemId == -1 then
		local strText = format("%s không tìm th¤y · 'EquipBase.txt'!!", itemName)		
	end
