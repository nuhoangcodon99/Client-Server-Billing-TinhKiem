...
x801016_g_Examinant_Name = {"Khäo quan 1","Khäo quan 2","Khäo quan 3","Khäo quan 4","Khäo quan 5","Khäo quan 6","Khäo quan 7","Khäo quan 8","Khäo quan 9","Khäo quan 10","Khäo quan 11","Khäo quan 12","Khäo quan 13","Khäo quan 14","Khäo quan 15","Khäo quan 16","Khäo quan 17","Khäo quan 18","Khäo quan 19","Khäo quan 20"}
...
--	local str = "µÚÒ»ÂÖÖÝÊÔÒÑ¾­½áÊø£¬½øÈëµÚ¶þÂÖÊ¡ÊÔµÄÍæ¼ÒÓÐ:"
...
--		end
--	end
--	str = str .. "Çëµ½´óÀíµÄÖ÷¿¼¹Ù´¦½øÐÐµÚ¶þÂÖÃæÊÔ."
	...
	for i,EachStudent in CANDIDATE_EXAM do
		if EachStudent then 
			if EachStudent.guid ~= 0 then
				...
				str = "  #{_INFOUSR"..tName.."}."
				if str ~= "  "  then
					--ÐèÒªÐÞ¸Ä£¬Ôö¼Ó·¢ËÍÓÊ¼þ¹¦ÄÜ½øÈëµÚ¶þÂÖµÄÍæ¼Òmodi:lby20071209
					--local tGuid = LuaFnGuid2ObjId(sceneId,EachStudent.guid)
					--local strName=GetName(sceneId,tGuid);
					
					--changed by xindefeng
					--LuaFnSendSystemMail(sceneId, strName, "¹§Ï²Ngß½i ¿Æ¾ÙÃûÁÐÇ°Ã©£¬³É¹¦½øÈëÊ¡ÊÔ.ÇëËÙµ½ËÙµ½´óÀí²Î¼Ó¿Æ¾ÙÊ¡ÊÔ.")
					local strMai = format("#YXin chúc m×ng, các hÕ ðã l÷t vào vòng trong, hãy ðªn #G Giám quan 1 (46,152) #Y Ð¬ tiªp tøc dñ thi vòng kª vào lúc #G21h30.#r                                  %s-%s-%s", Year, Month, Day)					
...
--					local tName=GetName(sceneId,tGuid);
					...
					--ÐèÒªÐÞ¸Ä£¬Ôö¼Ó·¢ËÍÓÊ¼þ¹¦ÄÜÇ°Èý¼×µÄÈËÔ±modi:lby20071209
					--changed by xindefeng
					--strMai	= format( "¹§Ï²Ngß½i ¸ßÖÐ±¾´Î¿Æ¾ÙµÄ%s£¬Ngß½i »ñµÃÁË³¯Í¢°ä·¢µÄ¼Î½±£¬Çëµ½´óÀíÖ÷¿¼¹Ù´¦ÁìÈ¡.", strMingCi )
					local strMai = format("#YChúc m×ng các hÕ ðã ðÕt giäi #G%s#Y vui lòng ðªn ÐÕi Lý ð¬ nh§n thß·ng#G. Ph¥n thß·ng chï có th¬ nh§n trong ngày.#r                                  %s-%s-%s", strMingCi, Year, Month, Day)
					--local tGuid = LuaFnGuid2ObjId(sceneId,EachStudent.guid)
					--local strName=GetName(sceneId,tGuid);
					LuaFnSendSystemMail(sceneId, tName, strMai)
				end
...
--		Msg2Player( sceneId,selfId,"Ngß½i »ñµÃÁËÒ»¸ö#B#{_ITEM"..x801016_g_Reward_Reborn_Medicine.."}#W.",MSG2PLAYER_PARA)
		--»î¶¯½±ÀøÍ³¼Æ
		local itemName;
		_,itemName,_ = GetItemInfoByItemId(x801016_g_Reward_Reborn_Medicine)
		LuaFnAuditItemCreate(sceneId,selfId,1,x801016_g_Reward_Reborn_Medicine,itemName,"#PKhoa cØ - Thám Hoa")
...
--			Msg2Player( sceneId,selfId,"Ngß½i »ñµÃÁËÒ»¸ö#B#{_ITEM"..x801016_g_Reward_Gem[rrr].."}#W.",MSG2PLAYER_PARA)
			str_title = "Bäng Nhãn"
			--»î¶¯½±ÀøÍ³¼Æ
...
--			Msg2Player( sceneId,selfId,"Ngß½i »ñµÃÁËÁ½¸ö#B#{_ITEM"..x801016_g_Reward_Prolong_Medicine.."}#W.",MSG2PLAYER_PARA)
	...
	if ret > 0 then
		...
		if huodongItemIndex and huodongItemIndex > -1 then
			...
			local strMsg = "#PTrong lúc l­ cß¾i c¤p hoàng gia t± chÑc mi­n phí, do#W#{_INFOUSR"..GetName(sceneId, selfId).."}#P trong hoÕt ðµng khoa cØ ðÕt ðßþc #Ykhoa cØ"..str_title.."#P danh xßng. Tri«u ðình tán thß·ng ph¥n thß·ng ð£c bi®#{_INFOMSG"..strHodongItem.."}"
...
--		PrintNum(Question_Sequence_Before)
	...
	if wenti == 0 then
		--Ëæ»ú³öÒ»¸öºÅÂë
		-- Èç¹ûÊÇÇ°5ÂÖ£¬¾ÍÔÚ4ÀïÍ·Ëæ»ú£¬ºóÃæµÄÔÚ5ÖÐËæ»ú
		-- »ñµÃµ±Ç°ÊÇµÚ¼¸ÂÖ
	...
	if Examinator_Sequence > x801016_g_Count_Examinant then
		...
		if Examinator_Sequence+1 >= x801016_g_Count_Examinant and Question_Sequence+1 == x801016_g_Count_Question and 
		   ExamNPCName == x801016_g_Examinant_Name[x801016_g_Count_Examinant] and x801016_EstimationPhase(sceneId,selfId) == 1 then
			...
			BroadMsgByChatPipe( sceneId,selfId,"#W#{_INFOUSR"..myname.."}#P  trong vòng khoa cØ ð¥u ðã thu§n lþi ðáp hªt m÷i ð« møc.",4) 
--			x801016_Add_To_Candidate( sceneId, selfId, current_time, selfGuid)
			...
			AuditCandidate(sceneId,selfId)                               --Í³¼ÆÍ¨¹ýµÚÒ»ÂÖµÄ´ÎÊý add by Dengxianxi
		elseif x801016_EstimationPhase(sceneId,selfId) == 2 then
...
--				x801016_Add_To_Top( sceneId, selfId, current_time, selfGuid)
		...
		elseif Question_Sequence >= x801016_g_Count_Question - 1 then
			...
			SetMissionData(sceneId,selfId,MD_EXAM_FIGHT_FLAG,0)
--dong yi ÈÃÎÒ¸ÉµÄ
			--local msg = "ÄúµÄ»Ø´ðÍêÈ«ÕýÈ·£¬ÇëÕÒÏÂÒ»Î»¿¼¹ÙÓ¦ÊÔ.#rÄúÔö¼ÓÁËÒ»´ÎµÄ¿ÉÒÔ´ð´íµÄ»ú»á.#rÏÂÒ»¸ö¿¼¹ÙÎ»ÖÃÔÚ("..x801016_g_Examinant_Position[Examinator_Sequence+2].x..", "..x801016_g_Examinant_Position[Examinator_Sequence+2].z..")."
...
function x801016_GenPreminuTxt1(rr,szItem)
		local str = ""
		if rr == 1 then
			str = "#PTrong lúc thi khoa cØ, tìm mãi không th¤y bút ðâu, chï còn cách tìm #W#{_INFOMSG"..szItem.."}#P thay thª bút."
		elseif rr == 2 then
			str = "#PTrong lúc thi khoa cØ, phát hi®n trên bàn có 1#W#{_INFOMSG"..szItem.."}"
		elseif rr == 3 then
			str = "#PTrong lúc thi khoa cØ, phát hi®n trên bàn có 1#W#{_INFOMSG"..szItem.."}"
		end
		return str
...
function x801016_GenPreminuTxt2(rr,szItem)
		local str = ""
		if rr == 1 then
			str = "#PTrong lúc thi khoa cØ, do ðßþc quan chü khäo chú ý tr÷ng døng, ðßþc t£ng 1 #W#{_INFOMSG"..szItem.."}"
		elseif rr == 2 then
			str = "#PTrong lúc thi khoa cØ, phát hi®n trên bàn có 1#W#{_INFOMSG"..szItem.."}#P."
		elseif rr == 3 then
			str = "#PTrong lúc thi khoa cØ, phát hi®n trên bàn có 1#W#{_INFOMSG"..szItem.."}#P."
		end
		return str
...
function x801016_GenPreminuTxt3(type,rr,idItem)
		...
		if rr == 1 and type == 1 then
			str = "#PTrong lúc thi khoa cØ, tìm mãi không th¤y bút ðâu, chï còn cách tìm #W#{_ITEM"..idItem.."}#P thay thª bút. Tiªc là lúc ra khöi khäo trß¶ng, do tay näi ðã ð¥y, #{_ITEM"..idItem.."}b¸ Khäo Quan xung công r°i."
		elseif rr == 1 and type == 2 then
			str = "#PTrong lúc thi khoa cØ, do ðßþc quan chü khäo chú ý tr÷ng døng. Quan chü khäo dñ tính t£ng #W#{_ITEM"..idItem.."}#P 1 món, vì tay näi ðã ð¥y nên ðã thôi."
		else
			str = "#PTrong lúc thi khoa cØ, phát hi®n trên bàn có 1 #W#{_ITEM"..idItem.."}#P. Ðáng tiªc tay näi ðã ð¥y, mu¯n mang ði cûng mang không ðßþc."
		end
		return str
end
...
function x801016_MyAwardTitle( sceneId, selfId, place )
	...
	if curTitle > 0 then
		...
		if newTitle <= curTitle then
			...
			AwardTitle( sceneId, selfId, 4, newTitle, 24 * 7 )	--ÓÐÐ§ÆÚ£ºÒ»ÖÜ
...
function x801016_JinBangTiMing( sceneId, selfId, targetId )
	...
	x801016_MessageBox( sceneId, selfId, "Chúc các hÕ mµt ngày vui vë, hy v÷ng các hÕ c¯ g¡ng h½n vào l¥n sau !" )
