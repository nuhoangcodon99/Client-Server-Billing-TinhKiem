...
-- ÏûºÄÎïÆ·¶ÔÓ¦±í
x805027_g_NeedItemBonus={{sqid01=10300000, sqid02=10302000, sqid03=10304000, sqid04=10305000, sfid={ 30505800, 30505801, 30505802, 30505803, 30505804, 30505805, 30505806 }},		-- ñÒÉÙÎ¢£¬2008.5.30¡£Ìí¼Ó30505806ÐÂÃ§Éñ·û7¼¶¿ÉÒÔÐÞÀíµÍ¼¶±ðÉñÆ÷
												...
												{sqid01=10308090, sqid02=10308091, sqid03=10308092, sqid04=10308099, sfid={30505806}},
												{sqid01=10308100, sqid02=10308101, sqid03=10308108, sqid04=10308109, sfid={30505806}},
												{sqid01=10308110, sqid02=10308117, sqid03=10308118, sqid04=10308119, sfid={30505806}},
												{sqid01=10308126, sqid02=10308127, sqid03=10308128, sqid04=10308135, sfid={30505806}},
												{sqid01=10308136, sqid02=10308137, sqid03=10308144, sqid04=10308145, sfid={30505806}},
												{sqid01=10308146, sqid02=10308153, sqid03=10308154, sqid04=10308155, sfid={30505806}},
												{sqid01=10308162, sqid02=10308163, sqid03=10308164, sqid04=10308171, sfid={30505806}},
												{sqid01=10308172, sqid02=10308173, sqid03=10308171, sqid04=10308171, sfid={30505806}},
												}
...
--Íê³É	
	...
	if ItemIndex >= x805027_g_SHENQI_BEGIN and ItemIndex <= x805027_g_SHENQI_END then	
		...
		if ItemCount < 1 then
			BeginEvent(sceneId)
			local strNotice = "C¥n có ".."#{_ITEM"..(cailiaoList[1]).."}"
	...
	if price < 0 then
		BeginEvent(sceneId)
		AddText(sceneId,"Thiªt b¸ cüa các hÕ b¸ l²i dæ li®u, không th¬ sØa chæa")
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	...
	elseif ret == -2 then
		...
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		Msg2Player(sceneId,selfId,"#Rngân lßþng#W cüa ngß½i không th¬ chi trä cho chi phí sØa chæa", 8)
	elseif ret == -4 then
		BeginEvent(sceneId)
		AddText(sceneId,"SØa chæa th¤t bÕi, s¯ l¥n trang b¸ cüa các hÕ có th¬ sØa chæa -1")
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		Msg2Player(sceneId,selfId,"SØa chæa th¤t bÕi, s¯ l¥n trang b¸ cüa các hÕ có th¬ sØa chæa #R-1#W", 8)
	elseif ret == -5 then
		BeginEvent(sceneId)
	...
	elseif ret == 0 then
		...
		if x805027_g_ZHUCAI_INDEX ~= 0 then
			...
			local ItemEX
			-- »ñµÃµ±Ç°¿ÉÒÔÏûºÄµÄµÚÒ»¸öÉñ²ÄÔÚ±³°üÖÐµÄÎ»ÖÃ	
			local scbagpos = -1
			...
			if scbagpos == -1 then
				BeginEvent(sceneId)
				local strNotice = "C¥n có ".."#{_ITEM"..(x805027_g_ZHUCAI_INDEX).."}"
				AddText(sceneId, strNotice)
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
				Msg2Player(sceneId,selfId, strNotice, 8)				 			
			...
			if res == 0 then
				...
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,selfId)
				Msg2Player(sceneId,selfId,"#GsØa chæa thành công#W", 8)
			end
