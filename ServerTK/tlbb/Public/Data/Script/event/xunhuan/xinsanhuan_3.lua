...
x050222_g_SubmitInfo = "#{LLFB_80816_53}"										-- Ìá½»Ê±µÄ´ð¸´
...
x050222_g_IsKillBossFire = 1														-- 1 ºÅÎ»ÖÃ£ºÉ±ËÀ»ðÑæÑýÄ§ÊýÁ¿(0ºÍ1)
x050222_g_Param_sceneid = 3															-- 3 ºÅÎ»ÖÃ£ºµ±Ç°ÈÎÎñÊý¾ÝµÄ3Î»ÖÃÉèÖÃÎª³¡¾°ID
...
{58,	118},{69,	115},{89,	113},{98,	110},{91,	98},{54,	114},{43,	105},{52,	99},{69,	105}, --¼õÉÙ1¸ö
{80,	84},{85,	81},{97,	89},{105,	76},{51,	80},{45,	73},{80,	74},{97,	62},{50,	60},{69,	55},
{73,	51},{44,	49},{56,	42},{48,	32},{43,	34},{67,	28},{77,	35},{85,	38},{102,	31},--¼õÉÙ1¸ö
{106,	40},{108,	49},{72,	39},{186,	156},{147,	199},{137,	211},{130,	190},{139,	163},{211,	175},{216,	177},
{220,	167},{67,	88},{64,	91},{91,	130},{85,	133},{179,	101},--¼õÉÙ4¸ö
{186,	104},{139,	91},{215,	88},{176,	53},{135,	96},{139,	52},{135,	56},{133,	206},--¼õÉÙ2¸ö
...
{214,	189},{221,	187},{207,	197},{217,	197},{201,	202},{221,	204},{208,	209},{215,	210}--¼Ó8¸ö
}

x050222_g_NumText_Main = 1					-- ½ÓÈÎÎñµÄÑ¡Ïî
x050222_g_NumText_EnterCopyScene = 2		-- ÒªÇó½øÈë¸±±¾µÄÑ¡Ïî
...
x050222_g_CopySceneType = FUBEN_RONGYANZHIDI		-- ¸±±¾ÀàÐÍ£¬¶¨ÒåÔÚScriptGlobal.luaÀïÃæ
x050222_g_LimitMembers = 1					-- ¿ÉÒÔ½ø¸±±¾µÄ×îÐ¡¶ÓÎéÈËÊý
x050222_g_TickTime = 5						-- »Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x050222_g_LimitTotalHoldTime = 360			-- ¸±±¾¿ÉÒÔ´æ»îµÄÊ±¼ä£¨µ¥Î»£º´ÎÊý£©,Èç¹û´ËÊ±¼äµ½ÁË£¬ÔòÈÎÎñ½«»áÊ§°Ü
x050222_g_CloseTick = 6						-- ¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊý£©
x050222_g_NoUserTime = 30					-- ¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
...
x050222_g_TakeTimes = 5											-- Ã¿Ìì×î¶àÁìÈ¡´ÎÊý
--**********************************
-- ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x050222_OnDefaultEvent( sceneId, selfId, targetId )	-- µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	if GetName( sceneId, targetId ) ~= x050222_g_Name then		-- ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		return
	end
			...
			BeginEvent( sceneId )
				...
				if bDone == 1 then
					AddText( sceneId, x050222_g_SubmitInfo )
						...
						AddNumText( sceneId, x050222_g_ScriptId, "Tiªn vào Dung Nham Chi Ð¸a", 10, x050222_g_NumText_EnterCopyScene )
						EndEvent( sceneId )
					DispatchEventList( sceneId, selfId, targetId )
					return
				end
...
function x050222_CheckAccept( sceneId, selfId, targetId )
	...
	-- Ò»ÌìÖ»ÄÜÁì x050222_g_TakeTimes ´Î
	...
	local iTime = GetMissionData( sceneId, selfId, MD_XINSANHUAN_3_LAST )			-- ÉÏÒ»´Î·ÅÆúÈÎÎñµÄÊ±¼ä(µ¥Î»£ºÒ»¿ÌÖÓ)
	...
	if IsHaveMission( sceneId, selfId, x050222_g_MissionId ) > 0 then					-- ÓÐÈÎÎñ²Å¿ÉÒÔ×ßÕâÀï
		...
		-- ½«×Ô¼º´«ËÍµ½¸±±¾³¡¾°
			if GetMissionParam( sceneId, selfId, misIndex, x050222_g_IsMissionOkFail ) == 2 then
				x050222_NotifyFailBox( sceneId, selfId, targetId, "#{LLFB_80816_46}" )--ÌáÊ¾ÈÎÎñÊ§°Ü
		...
		LuaFnSetSceneLoad_Map( sceneId, x050222_g_CopySceneMap )						-- µØÍ¼ÊÇ±ØÐëÑ¡È¡µÄ£¬¶øÇÒ±ØÐëÔÚConfig/SceneInfo.iniÀïÅäÖÃºÃ
		...
		LuaFnSetCopySceneData_Param( sceneId, 0, x050222_g_CopySceneType )				-- ÉèÖÃ¸±±¾Êý¾Ý£¬ÕâÀï½«0ºÅË÷ÒýµÄÊý¾ÝÉèÖÃÎª999£¬ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
		...
		if bRetSceneID > 0 then
			x050222_NotifyFailTips( sceneId, selfId, "D¸ch chuy¬n thành công! Vui lòng ðþi giây lát s¨ d¸ch chuy¬n!" )
			AuditXinSanHuanCreateFuben( sceneId, selfId, 3 )
		else
			x050222_NotifyFailTips( sceneId, selfId, "S¯ lßþng bän sao vßþt quá gi¾i hÕn, ð« ngh¸ lát næa thØ lÕi!" )
		end
	end
end
...
--objËÀÍö
--**********************************
function x050222_OnDie( sceneId, objId, selfId )--Õâ¸öselfId¿ÉÄÜÊÇ³èÎï
	...
	if szName == x050222_g_BossHuoYanYao then
		...
		for i = 0, num - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then						-- ²»ÔÚ³¡¾°µÄ²»×ö´Ë²Ù×÷
				x050222_NotifyFailTips( sceneId, mems[i], "Hoàn thành nhi®m vø." )
				x050222_NotifyFailTips( sceneId, mems[i], "Ðã giªt  "..x050222_g_BossHuoYanYao.." ðßþc 1/1" )
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x050222_OnKillObject( sceneId, selfId, objdataId, objId )						-- ²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
...
function x050222_OnAbandon( sceneId, selfId )
	...
	if LuaFnDelAvailableItem( sceneId, selfId, x050222_g_xuanfouzhu, 1 ) > 0 then
		...
		if sceneId == copyscene and fubentype == x050222_g_CopySceneType then											-- Èç¹ûÔÚ¸±±¾ÀïÉ¾³ýÈÎÎñ£¬ÔòÖ±½Ó´«ËÍ»Ø
...
--	local LevelGap = LuaFnGetCopySceneData_Param( sceneId, CopyScene_LevelGap )
	...
	if leaveFlag == 1 then															-- ÐèÒªÀë¿ª
		...
		if leaveTickCount >= x050222_g_CloseTick then										-- µ¹¼ÆÊ±¼äµ½£¬´ó¼Ò¶¼³öÈ¥°É
			...
			local strText = format( "Các hÕ s¨ r¶i khöi ðây sau %d giây!", ( x050222_g_CloseTick - leaveTickCount ) * x050222_g_TickTime )
	...
	elseif TickCount == x050222_g_LimitTotalHoldTime then									-- ¸±±¾×ÜÊ±¼äÏÞÖÆµ½ÁË
		...
		for	i = 0, membercount - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then
				x050222_NotifyFailTips( sceneId, mems[i], "Nhi®m vø th¤t bÕi, quá gi¶ !" )
				if IsHaveMission( sceneId, mems[i], x050222_g_MissionId ) > 0 then
					misIndex = GetMissionIndexByID( sceneId, mems[i], x050222_g_MissionId )
					if GetMissionParam( sceneId, mems[i], misIndex, x050222_g_IsMissionOkFail ) ~= 1 then
						SetMissionByIndex( sceneId, mems[i], misIndex, x050222_g_IsMissionOkFail, 2 )	-- Èç¹ûÃ»ÓÐÍê³É¾ÍÉèÖÃÎªÊ§°Ü
					end
				end
				x050222_Exit( sceneId, mems[i] )
			end
	...
	elseif TickCount == 1 then									-- Ë¢¹Ö
		...
		MonsterTalk( sceneId, monsterID, "BOSS", "Ta ðây chï c¥n thét 1 tiªng là khói lØa ng§p tr¶i." )
		...
		for	i = 0, membercount - 1 do
			if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 and oldteamid ~= GetTeamId( sceneId, mems[i] ) then
				x050222_NotifyFailTips( sceneId, mems[i], "Ngß½i không · trong ðµi !" )
				x050222_Exit( sceneId, mems[i] )
			end
		end
		...
		if mod( TickCount*x050222_g_TickTime, 60 ) == 0 and mod( TickCount*x050222_g_TickTime+60, 120 ) == 0 then
			for	i = 0, membercount - 1 do
				if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then
					...
					x050222_NotifyFailTips( sceneId, mems[i], "Th¶i gian hoàn thành nhi®m vø còn "..Minute.." phút." )
				end
			end	
		end
...
function x050222_CheckSubmit( sceneId, selfId )
	...
	local misIndex = GetMissionIndexByID( sceneId, selfId, x050222_g_MissionId )				-- µÃµ½ÈÎÎñµÄÐòÁÐºÅ
...
function x050222_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x050222_CheckSubmit( sceneId, selfId ) == 1 then
		...
		if LuaFnDelAvailableItem( sceneId, selfId, x050222_g_xuanfouzhu, 1 ) > 0 then--ÐèÒª¿Û³ýÐþ·ðÖé£¬Ö»¿ÛÒ»¸ö
			DelMission( sceneId, selfId, x050222_g_MissionId )
		
			local exp = GetLevel( sceneId, selfId ) * 3323 - 45613 --²Î¿¼ÁËÀÏÈý»·¸±±¾µÄ¾­Ñé¼ÆËã
			--»ñµÃ¾­Ñé
			if exp < 1 then
				exp = 1
			end
			...
			-- ÈÎÎñ»ò»î¶¯Í³¼Æ
			LuaFnAuditQuest(sceneId, selfId, "Lâu Lân Liên Hoàn Nhi®m Vø Dung Nham Chi Ð¸a")
		end
	end
end
