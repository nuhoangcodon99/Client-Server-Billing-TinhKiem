...
function x801010_FinishAdjust( sceneId, selfId, styleId)
	...
	if styleId < 0 then														
		BeginEvent(sceneId)
			AddText(sceneId, "Không có Phát Hình Ğ°");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	...
	if ItemCount > nItemNum then
		BeginEvent(sceneId)
			AddText(sceneId, "Phát Hình Ğ° b¸ khoá ho£c không có");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	...
	if (moneyJZ + money >= 50000)	then
		...
		elseif ret == 1  then														--ËùÑ¡µÄ·¢ĞÍÓëÍæ¼Òµ±Ç°µÄ·¢ĞÍÒ»ÖÂ
			BeginEvent(sceneId)
				AddText(sceneId, "Hãy ch÷n ki¬u tóc khác");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		elseif ret == 3 then														--Ã»ÓĞĞèÒªÏûºÄµÄÎïÆ·»ò¸ÃÎïÆ·±»Ëø¶¨
			BeginEvent(sceneId)
				AddText(sceneId, "Phát Hình Ğ° không có ho£c b¸ khoá");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		elseif ret == 4 then														
			BeginEvent(sceneId)
				AddText(sceneId, "Ngân lßşng không ğü");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
			return
		else
