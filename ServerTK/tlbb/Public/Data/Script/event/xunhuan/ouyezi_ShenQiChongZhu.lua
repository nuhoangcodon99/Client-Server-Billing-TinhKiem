...
x500504_g_ChongXinYeLianInfo			= "SØa th¥n khí"

x500504_g_Impact_ShenQi = 47						-- ÖØÐÂÒ±Á¶ÉñÆ÷³É¹¦Ê±µÄÌØÐ§ID

x500504_g_ItemBonus={{id=30505700,num=1,sqlvl=42},{id=30505701,num=1,sqlvl=52},{id=30505702,num=1,sqlvl=62},{id=30505703,num=1,sqlvl=72},{id=30505704,num=1,sqlvl=82},{id=30505705,num=1,sqlvl=92}}   -- ²ÐÈ±µÄÉñ½Ú
...
function x500504_OnDefaultEvent( sceneId, selfId, targetId )
	...
	--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦µÄnpc
	if LuaFnGetName( sceneId, targetId ) ~= x500504_g_Name then
		x500504_NotifyTip( sceneId, selfId, "Kªt n¯i v¾i máy chü l²i..." )			
...
function x500504_ChongXinYeLian( sceneId, selfId, targetId, ItemID, bagpos )	
  ...
  if ItemID == x500504_g_Item_ShenQi[1].id06 or ItemID == x500504_g_Item_ShenQi[2].id06
		...
		if ret > 0 then								
			...
			local strMsg = "Nh§n ðßþc ".."#{_ITEM"..(ItemBonusID).."}" -- ²ÐÈ±µÄÉñ½Ú			
			...
			if EraseRet < 0 then      --Èç¹ûÉ¾³ýÊ§°Ü,½«²»»á²úÉúÈÎºÎÐ§¹û
			  local strMsg = "SØa chæa th¤t bÕi"
				x500504_NotifyTip( sceneId, selfId, strMsg)
				return 0
			end			
...
function x500504_OnMissionCheck( sceneId, selfId, npcid, scriptId, index1, index2, index3, petindex )
	...
	--***********************************************************
	-- zchw ÖØÖýµÄÊ±ºò£¬Ö»ÓÐÉñÆ÷ÊÇ±ØÐëµÄ£¬ËùÒÔÖ»ÐèÒª¶Ôindex1×ö¼ì²é
	if index1 >= 100 then
	...
	-- ñÒÉÙÎ¢£¬2008.5.30¡£102¼¶ÉñÆ÷²»ÄÜ¹»ÖØÖýÁË¡£ÐÑÄ¿ÌáÊ¾¡£
	...
	if ItemID == 10300100 or ItemID == 10300101 or ItemID == 10300102 
		...
		x500504_NotifyTip( sceneId, selfId, "ÐÆng c¤p trang b¸ 102 không th¬ tÕo lÕi!" )
		return 0
	end
