...
x500501_g_strHelpFinishedText = "  Ta g£p r¡c r¯i v¾i cánh cØa nên nh¶ sñ giúp ðÞ cüa các hÕ ð¬ sØa lÕi #r Nhi®m vø hoàn thành#W"
...
function x500501_ACT_Duanwu( sceneId, selfId, iDayHuan ) --¶ËÎç»î¶¯
	...
	if AddRet > 0 then
		...
		if GemRet > 0 then
			...
			local strformat	= "#{_INFOUSR%s}#Trong hoÕt ðµng Tiªt Ðoan Ng÷, do hoàn thành 20 nhi®m vø sß môn, không nhæng nh§n ðßþc 1 cái bánh ú, mà còn nh§n ðßþc thêm 1#Y#{_INFOMSG%s}. "
...
--		if GetLevel(sceneId, selfId) >= 50 and iDayHuan == 20 then --50¼¶ÒÔÉÏ²Å»ñµÃ£¬Ö»ÓÐµÚÒ»¸ö20»·²Å¸ø£¬¶ø²»ÊÇÃ¿20»·¶¼¸ø
		if 0 > 1 then
			...
			if canAdd == 0  then
				...
				local strMsg = format("Các hÕ nh§n ðßþc #H#{_ITEM%d}#W.", x500501_g_LingShouDanId[randomIndex])
			...
			--[/tx42684]			
				...
				if random(100) <= 5  then
					...
					if canAdd == 0  then
						...
						--¸ø×Ô¼º·¢ËÍµÃµ½ÎïÆ·µÄÏûÏ¢....
						strMsg = format("Các hÕ nh§n ðßþc #H#{_ITEM%d}#W.", x500501_g_LingShouDanId[randomIndex])
						Msg2Player(sceneId, selfId, strMsg, MSG2PLAYER_PARA)
...
--				end
				...
				--¸ø×Ô¼º·¢ËÍµÃµ½ÎïÆ·µÄÏûÏ¢....
				local strMsg = format("Các hÕ nh§n ðßþc #H#{_ITEM%d}#W.", x500501_g_ItemId[randomIndex])
				Msg2Player(sceneId, selfId, strMsg, MSG2PLAYER_PARA)
...
--	Msg2Player(  sceneId, selfId,"Các hÕ nh§n ðßþc #Y1#WµãÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
...
--	Msg2Player(  sceneId, selfId,"Các hÕ nh§n ðßþc #Y2#WµãÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
--else --mod(iDayHuan, 10) must return 0
--	SetHumanMenpaiPoint(sceneId, selfId, GetHumanMenpaiPoint(sceneId, selfId)+3	)
--	Msg2Player(  sceneId, selfId,"Các hÕ nh§n ðßþc #Y3#WµãÃÅÅÉ¹±Ï×¶È." ,MSG2PLAYER_PARA )
--end
	...
	Msg2Player(  sceneId, selfId,"Các hÕ nh§n ðßþc #Y" .. menpaiPoint .. "#W ði¬m ðµ c¯ng hiªn môn phái." ,MSG2PLAYER_PARA )
	...
	--Msg2Player(  sceneId, selfId,"Các hÕ nh§n ðßþc #{_MONEY" .. tostring(x500501_g_Money) .. "}" ,MSG2PLAYER_PARA )
	--Msg2Player(  sceneId, selfId,"Các hÕ nh§n ðßþc " .. tostring(x500501_g_Exp) .. "µã¾­ÑéÖµ" ,MSG2PLAYER_PARA )
