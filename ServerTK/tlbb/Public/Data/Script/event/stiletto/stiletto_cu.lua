...
function x311200_OnStiletto( sceneId, selfId, idBagPos, idBagPosStuff )
	...
	if find == 0 then
		x311200_NotifyTip( sceneId, selfId, "Trang b¸ này không th¬ thêm l² khäm !!" )
		return
	end
	...
	for i=1,getn( x311200_g_Pos_normal ) do
		if EquipType ==16 then
			...
			if x311200_g_Pos_normal[i].left[1] <= posX and posX <= x311200_g_Pos_normal[i].right[1] and x311200_g_Pos_normal[i].left[2] <= posZ and posZ <= x311200_g_Pos_normal[i].right[2] then
				local text = "Th§t thÑ l²i. N½i này không h² trþ tÕo l² khäm cho th¶i trang !! Hãy ðªn LÕc Dß½ng (342,270) ð¬ thñc hi®n tÕo l² cho th¶i trang ! " 
				...
				local itemID = GetItemTableIndexByIndex(sceneId, selfId, idBagPosStuff )
					if itemID ~= 30503134 then
						x311200_NotifyTip( sceneId, selfId, "BÕn phäi cho vào Kim Thoa m¾i có th¬ ðøc l² th¶i trang ðßþc")
						return
					end
			end
		end
	...
	for j=1,getn( x311200_g_Pos_fashion ) do
		if EquipType ~=16 then
			if x311200_g_Pos_fashion[j].left[1] <= posX and posX <= x311200_g_Pos_fashion[j].right[1] and x311200_g_Pos_fashion[j].left[2] <= posZ and posZ <= x311200_g_Pos_fashion[j].right[2] then  
				local text = "Th§t thÑ l²i, khu này chï h² trþ khäm ng÷c cho th¶i trang !! Xin hãy t¾i LÕc Dß½ng (274,321) ð¬ tÕo l² khäm cho trang b¸ ! " 
				x311200_NotifyTip( sceneId, selfId, text )
				return
			end
		end
	...
	if ret == -1 then
		x311200_NotifyTip( sceneId, selfId, "Ki¬m tra tay näi có sai sót. Liên h® Game Master" )
		return
	elseif ret == -2 then
		x311200_NotifyTip( sceneId, selfId, "BÕn không có ðúng loÕi nguyên li®u c¥n thiªt, không th¬ thñc hi®n hành ðµng này !!" )
		return
	elseif ret == -3 then
		x311200_NotifyTip( sceneId, selfId, "BÕn không có ðü ti«n, không th¬ tiªn hành tÕo l² khäm !" )
		return
	elseif ret == -4 then
		x311200_NotifyTip( sceneId, selfId, "BÕn không ðü nguyên li®u, không th¬ thñc hi®n tÕo l² khäm !!" )
		return
	elseif ret == 0 then
		x311200_NotifyTip( sceneId, selfId, "Kh¤u tr× tiªn th¤t bÕi !" );
		return
	end
	...
	if ret == -1 then
		x311200_NotifyTip( sceneId, selfId, "TÕo l² khäm th¤t bÕi !!" )
	elseif ret == -2 then
		x311200_NotifyTip( sceneId, selfId, "L²i nghiêm tr÷ng" )
	elseif ret == -3 then
		x311200_NotifyTip( sceneId, selfId, "C¥n cho trang b¸ vào" )
	elseif ret == -4 then
		x311200_NotifyTip( sceneId, selfId, "N½i này chï h² trþ khäm 3 l²" ) --modi:lby2008-5-21µ±Íæ¼ÒÆóÍ¼´òµÚËÄ¸ö¿ÕµÄÊ±ºò±¨´í
		--x311200_NotifyTip( sceneId, selfId, "²»ÄÜÔÙ´ò¿×ÁË" ) 	
	elseif ret == 1 then
		x311200_NotifyTip( sceneId, selfId, "TÕo l² khäm thành công !!" )
		--Ôö¼ÓÌØÐ§
...
function x311200_OnStiletto_Four( sceneId, selfId, idBagPos, idBagPosStuff, type )
	...
	if (IsItemDark(sceneId, selfId, taozhuangid) == 1) then
	 ...
	 if type ~= 1 and type ~= 2 then
	    x311200_NotifyTip( sceneId, selfId, "Trang b¸ có sai sót !!" )
			return
	 end
	...
	if ret == -1 then
		x311200_NotifyTip( sceneId, selfId, "Ki¬m tra l²i th¤t bÕi. Liên h® v¾i Game Master !" )
		return
	elseif ret == -2 then
		if type == 1 then
	...
	elseif ret == 0 then --zchw
		x311200_NotifyTip( sceneId, selfId, "Kh¤u tr× ti«n th¤t bÕi ! !!" );
		return
	end
	...
	if ret == -1 then
		x311200_NotifyTip( sceneId, selfId, "TÕo l² khäm th¤t bÕi !!" )
	elseif ret == -2 then
		x311200_NotifyTip( sceneId, selfId, "Ðã có l²i nghiêm tr÷ng !!" )
	elseif ret == -3 then
