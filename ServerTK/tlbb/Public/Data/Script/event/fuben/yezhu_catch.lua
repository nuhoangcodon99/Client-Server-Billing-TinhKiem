...
--¸±±¾Ãû³Æ
x402105_g_CopySceneName = "Tr¤n áp Dã Trß bÕo loÕn"
...
function x402105_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnHasTeam(sceneId,selfId) < 1  then
		BeginEvent(sceneId)
			AddText(sceneId,"#BTr¤n áp Dã Trß Vß½ng");
			AddText(sceneId,"  C¥n t± ğµi m¾i có th¬ tham gia!");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	if GetTeamSize(sceneId,selfId) < x402105_g_LimitMembers  then
		BeginEvent(sceneId)
			AddText(sceneId,"#BTr¤n áp Dã Trß Vß½ng");
			AddText(sceneId,"  C¥n t± ğµi 3 ngß¶i tr· lên m¾i có th¬ tham gia");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	if LuaFnGetAvailableItemCount(sceneId,selfId,x402105_g_Item) < 1  then
		BeginEvent(sceneId)
			AddText(sceneId,"#BTr¤n áp Dã Trß Vß½ng");
			AddText(sceneId,"  Ğ¬ tham gia phø bän c¥n có ğÕo cø Tung Tích cüa Dã Trß Vß½ng");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
	...
	if ret == 0  then
		...
		BeginEvent(sceneId)
			AddText(sceneId,"#BTr¤n áp Dã Trß Vß½ng");
			AddText(sceneId,"  Trong nhóm có thành viên #r#R " .. szAllName .. " chßa ğü c¤p ğµ 40, không th¬ tham gia");
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
		return
	end
...
function x402105_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x402105_g_ScriptId, "Tr¤n áp Dã Trß Vß½ng",10 ,-1  )
...
function x402105_MakeCopyScene( sceneId, selfId, targetId )
	...
	if bRetSceneID>0 then			
		...
		BeginEvent(sceneId)
			AddText(sceneId,"D¸ch chuy¬n thành công ! Vui lòng ch¶ !")
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	else
		BeginEvent(sceneId)
			AddText(sceneId,"Bän sao ğã ğªn gi¾i hÕn, vui lòng thØ lÕi sau !")
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
	end
...
function x402105_OnDie( sceneId, objId, killerId )
	...
	if GetName(sceneId, objId) == "Tiêm Nha Dç Trß"  then
		LuaFnSetCopySceneData_Param(sceneId,20,nKillCount+1)
		x402105_TipAllHuman(sceneId, "Ğã giªt ğßşc Tiêm Nha Dã Trß : " .. tostring(nKillCount+1) .. "/30")
	end
end
--**********************************
--½øÈëÇøÓòÊÂ¼ş
...
function x402105_OnCopySceneTimer( sceneId, nowTime )
	...
	if nStep == 2  then
		...
		if nHaveMonster == 0  then
			...
			-- ·¢ÊÀ½ç¹«¸æ
			if nType == 0  then
				str = format("#{_INFOUSR%s}#P th¯ng lînh ğµi ngû tÕi #GNông trß¶ng Dã Trß#P ğã tiêu di®t #{_BOSS20}#P, th§t là ghê g¾m !",szLeaderName)
			else
				str = format("#{_INFOUSR%s}#P th¯ng lînh ğµi ngû tÕi #GNông trß¶ng Dã Trß#P giªt chªt #{_BOSS21}#P c½n bão cu¯i cùng cûng b¸ d§p t¡t !",szLeaderName)
			end
			BroadMsgByChatPipe(sceneId, nLeaderId, str, 4)
			x402105_TipAllHuman(sceneId,"Phø bän s¨ ğóng sau 3 phút næa")
		end
	end
	...
	if nStep == 3  then
		...
		if LuaFnGetCurrentTime()-nPreTime >= 165  then
			x402105_TipAllHuman(sceneId,"Phø bän s¨ ğóng sau 15 giây næa")
	...
	if nStep == 4  then
		...
		if LuaFnGetCurrentTime()-nPreTime >= 175  then
			x402105_TipAllHuman(sceneId,"Phø bän s¨ ğóng sau 5 giây næa")
...
function x402105_CreateMonster_2(sceneId)
	...
	SetCharacterTitle(sceneId, nNpcId, "Liêu Nha Vß½ng")
	x402105_TipAllHuman(sceneId, "Phát hi®n Tiêm Nhã Trß Vß½ng !")
	...
	if nRand <= x402105_g_BossRand_2   then
	  ...
	  if nInilevel >= 11 then
		...
		str = format("#PDß¾i sñ lãnh ğÕo cüa #{_INFOUSR%s}#P ğµi ngû ğã tiêu di®t hªt dã trß, ğ°ng th¶i hÕ gøc cä #{_BOSS21} #Pchï trong 1 hit. Quä th§t là võ công thâm h§u !",szLeaderName)
		BroadMsgByChatPipe(sceneId, nLeaderId, str, 4)
