...
-- ÈÎÎñÎÄ±¾ÃèÊö
x402047_g_MissionName = "Ti«n vãng Trác Lµc C± chiªn trß¶ng"
...
x402047_g_MaxMembers = 60					-- ±¾°ï¿ÉÒÔ½ø¸±±¾µÄ×î´óÈËÊý  --tt50777 ÈËÊýÉÏÏÞÓÉ80ÈËÐÞ¸ÄÎª25ÈË
...
x402047_g_Win_PerHonour = 100 --»ñÊ¤·½Ã¿¸öÈË»ñµÃµÄÈÙÓþÖµ
x402047_g_Fail_TotalHonour = 2000 --Ê§°Ü·½×Ü¹²»ñµÃµÄÈÙÓþÖµÉÏÏÞ£.¡×¢ÒâÊÇ×Ü¹²µÄ£.¡
x402047_g_Fail_PerMaxHonour = 100 --Ê§°Ü·½Ã¿¸öÈË»ñµÃµÄÈÙÓþÖµÉÏÏÞ
...
--ÕóÓªNPC
x402047_g_A_Platform = "Viêm Hoàng Ðài"
x402047_g_A_FenXiang_Tower = "Viêm Hoàng Ph¥n Hß½ng Tháp"
x402047_g_A_LuoXing_Tower = "Viêm Hoàng LÕc Tinh Tháp"
x402047_g_A_Flag = "Viêm Hoàng Chiªn KÏ"

x402047_g_B_Platform = "Xuy Vßu Ðài"
x402047_g_B_FenXiang_Tower = "Xuy Vßu Ph¥n Hß½ng Tháp"
x402047_g_B_LuoXing_Tower = "Xuy Vßu LÕc Tinh Tháp"
x402047_g_B_Flag = "Xuy Vßu Chiªn KÏ"
...
function x402047_OnDefaultEvent( sceneId, selfId, targetId )	-- µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	...
	elseif numText == x402047_g_GetPrizeTitle then
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x402047_g_ScriptId, "Siêu C¤p KÏ Thü", 4, x402047_g_GetPrizeTitle_Flag)
			AddNumText(sceneId, x402047_g_ScriptId, "Sßu Bäo Chuyên Gia", 4, x402047_g_GetPrizeTitle_Source)
		EndEvent(sceneId)
	...
	elseif numText == x402047_g_GetPrizeTitle_Kill then
		...
		if bHave == 1 then
			...
			LuaFnAwardTitle( sceneId, selfId, x402047_g_PrizeTitle[1].AwardPos, x402047_g_PrizeTitle[1].Index, 7*24) --ÓÐÐ§ÆÚÒ»ÖÜ
	...
	elseif numText == x402047_g_GetPrizeTitle_Flag then
		...
		if bHave == 1 then
			...
			LuaFnAwardTitle( sceneId, selfId, x402047_g_PrizeTitle[2].AwardPos, x402047_g_PrizeTitle[2].Index, 7*24) --ÓÐÐ§ÆÚÒ»ÖÜ
			SetCurTitle(sceneId, selfId, x402047_g_PrizeTitle[2].SetPos, x402047_g_PrizeTitle[2].Index)
			LuaFnDispatchAllTitle(sceneId, selfId)
			
			x402047_NotifyFailBox( sceneId, selfId, targetId, "#{BHXZ_081103_145}".." Siêu C¤p KÏ Thü ".."#{BHXZ_081103_146}" )
			if GetHumanGuildID(sceneId, selfId) ~= -1 then --Ã»ÓÐ°ïÅÉµÄÊ±ºò²»·¢°ïÅÉ¹«¸æ
	...
	elseif numText == x402047_g_GetPrizeTitle_Source then
		...
		if bHave == 1 then
			...
			LuaFnAwardTitle( sceneId, selfId, x402047_g_PrizeTitle[3].AwardPos, x402047_g_PrizeTitle[3].Index, 7*24) --ÓÐÐ§ÆÚÒ»ÖÜ
			SetCurTitle(sceneId, selfId, x402047_g_PrizeTitle[3].SetPos, x402047_g_PrizeTitle[3].Index)
			LuaFnDispatchAllTitle(sceneId, selfId)
			
			x402047_NotifyFailBox( sceneId, selfId, targetId, "#{BHXZ_081103_145}".." Sßu Bäo Chuyên Gia ".."#{BHXZ_081103_146}" )
			if GetHumanGuildID(sceneId, selfId) ~= -1 then --Ã»ÓÐ°ïÅÉµÄÊ±ºò²»·¢°ïÅÉ¹«¸æ
...
function x402047_OnEnumerate( sceneId, selfId, targetId )
	if GetName( sceneId, targetId ) ~= x402047_g_Name then		--ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
		--x402047_NotifyFailTips( sceneId, selfId, "Tên:"..GetName( sceneId, targetId ).."." )
		--return --hd edit
	end
...
function x402047_AcceptEnterCopyScene( sceneId, selfId, targetId )
	...
	--hd add fix bug 19/8 khoang thach
	local g_LingShiID = {30900051,30900052,30900053,30900054,30900055}
	for i = 1, getn( g_LingShiID ) do
		local num = LuaFnGetAvailableItemCount( sceneId, selfId, g_LingShiID[i] )
		if num >0 then
			x402047_NotifyFailTips( sceneId, selfId, "Nhân v§t có mang theo khoáng thÕch. Không th¬ tiªn vào chiªn trß¶ng." )
			return
		end
	end
	if LuaFnHasTeam(sceneId, selfId) == 1 then
		...
		for i = 0, nearMemberCount - 1 do
			...
			if sceneMemId and sceneMemId >= 0 then
				for i = 1, getn( g_LingShiID ) do
					local num = LuaFnGetAvailableItemCount( sceneId, sceneMemId, g_LingShiID[i] )
					if num >0 then
						x402047_NotifyFailTips( sceneId, selfId, "Nhân v§t trong t± ðµi có mang theo khoáng thÕch. Không th¬ tiªn vào chiªn trß¶ng." )
						return
					end
				end
			end
...
function x402047_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if bRetSceneID > 0 then
		x402047_NotifyFailTips( sceneId, selfId, "Phø bän ðßþc tÕo thành công !" )
...
function x402047_OnPlayerEnter( sceneId, selfId )
	...
	elseif guildid == Bguildid then
		...
		x402047_NotifyFailTips( sceneId, selfId, "Các hÕ không thuµc Bang Phái, không th¬ tiªn nh§p" )
		SetCanChangeScene(sceneId, selfId, 1)--ÊÖ¶¯ÉèÖÃ×¢ÒâÕâ¸ö½Ó¿ÚµÄÓÃ·¨£¬ÆäËûµØ·½²»ÒªÂÒÊ¹ÓÃ
		x402047_Exit(sceneId, selfId)
		return
	end
	...
	if humanguildid == Aguildid then --ËÀÍöÍæ¼ÒÊÇA°ïµÄ
		...
		if Tankbuf and Tankbuf ~= 0 then --Õ½³µÍæ¼ÒËÀÍö
			...
			if otherselfId > 0 then
				...
				msg = LuaFnGetGuildName(sceneId, selfId).."#{BHXZ_081103_125}"..x402047_g_TankName[Tankbuf-x402047_g_AttrBuff[1]+1].." dî kinh b¸ ð¯i phß½ng phá huÖ !"
			end
			for i = 0, num - 1 do
				if LuaFnIsObjValid( sceneId, mems[i] ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, mems[i] ) == 1 then
					x402047_NotifyFailTips(sceneId, mems[i], msg)
...
function x402047_OnCopySceneTimer( sceneId, nowTime )
	...
	if leaveFlag == 1 then															-- ÐèÒªÀë¿ª
		...
		if leaveTickCount >= x402047_g_CloseTick then										-- µ¹¼ÆÊ±¼äµ½£¬´ó¼Ò¶¼³öÈ¥°É
			...
			local strText = format( "Các hÕ s¨ r¶i khöi ðây sau %d giây", ( x402047_g_CloseTick - leaveTickCount ) * x402047_g_TickTime )
	...
	elseif TickCount == x402047_g_StartPoint then --¿ªÊ¼¼Ç·Ö
		...
		if TickCount == (x402047_g_StartPoint+10*60) or TickCount == (x402047_g_StartPoint+20*60) or TickCount == (x402047_g_StartPoint+30*60) then
			...
			local msg = "#{BHXZ_081103_133} "..floor((TickCount-x402047_g_StartPoint)/60).." #{BHXZ_081103_134}#r"..AGuildName.." #{BHXZ_081103_135}"..Acount.."#{BHXZ_081103_136} "..Apoint..", "..BGuildName.." #{BHXZ_081103_135} "..Bcount.." #{BHXZ_081103_136} "..Bpoint..","..leadmsg
			for i=0, membercount-1 do
