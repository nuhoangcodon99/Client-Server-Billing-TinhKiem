...
x402030_g_LimitMembers = 3				--¿ÉÒÔ½ø¸±±¾µÄ×îÐ¡¶ÓÎéÈËÊý
x402030_g_TickTime = 1						--»Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x402030_g_LimitTotalHoldTime = 360--¸±±¾¿ÉÒÔ´æ»îµÄÊ±¼ä£¨µ¥Î»£º´ÎÊý£©,Èç¹û´ËÊ±¼äµ½ÁË£¬ÔòÈÎÎñ½«»áÊ§°Ü
x402030_g_LimitTimeSuccess = 500	--¸±±¾Ê±¼äÏÞÖÆ£¨µ¥Î»£º´ÎÊý£©£¬Èç¹û´ËÊ±¼äµ½ÁË£¬ÈÎÎñÍê³É
x402030_g_CloseTick = 3						--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊý£©
x402030_g_NoUserTime = 300				--¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
x402030_g_DeadTrans = 0						--ËÀÍö×ªÒÆÄ£Ê½£¬0£ºËÀÍöºó»¹¿ÉÒÔ¼ÌÐøÔÚ¸±±¾£¬1£ºËÀÍöºó±»Ç¿ÖÆÒÆ³ö¸±±¾
...
function x402030_OnDefaultEvent( sceneId, selfId, targetId )
	...
	-- 1£¬¼ì²é»î¶¯Ê±¼ä £¨Ã¿ÖÜÁùµÄÏÂÎç1£º00µ½10£º00Ö®¼ä£©
	...
	-- ÏÈ¼ì²â¶Ó³¤×Ô¼ºÊÇ²»ÊÇ×ã¹»Ìõ¼þ½ø¸±±¾
	--  ÏÈ×öÍê¶Ó³¤µÄ¸öÈË¼ì²âÖ®ºó£¬ÔÙ¼ì²â¶ÓÔ±ÊÇ²»ÊÇ¹»Ìõ¼þ
	...
	--5£¬¼ì²â¶ÓÎéÀïÍ·µÄÃ¿¸öÍæ¼ÒÊÇ²»ÊÇÉíÉÏ¶¼ÓÐÕâ¸öÎïÆ·ÁË
	local ret = 1
	...
	--6,¼ì²âÍæ¼Ò½ñÌìÄÚÊÇ²»ÊÇ½øÐÐ¹ý3´Î½Ë·Ë
	for i=0, nPlayerNum-1  do
		local nPlayerId = GetNearTeamMember(sceneId,selfId, i)
...
function x402030_OnDie( sceneId, objId, killerId )
	...
	elseif GetName(sceneId, objId) == x402030_g_BossMonsterName  then
		if random(100) < 60 then
			AddMonsterDropItem( sceneId, objId, killerId, 30008053 )
		end
		...
		local str = ""
		str = format("#{_INFOUSR%s}#P dçn d¡t ðµi vào #GKính H° Phï TrÕi#P khiêu chiªn#Y%s#P. ÐÕo cao 1 thß¾c, Ma cao 1 trßþng, trong lúc 2 bên ðang h²n chiªn, #W#{_INFOUSR%s}#P ðã chém chªt #Y%s#P trong phï trÕi! ", GetName(sceneId,nLeaderId), "#{_BOSS43}", GetName(sceneId,nLeaderId), "#{_BOSS43}")
...
function x402030_OnCopySceneTimer( sceneId, nowTime )
	...
	elseif nStep==101 then
		...
		SetLevel(sceneId, nNpc1, nMonsterLevel)
		SetNPCAIType(sceneId, nNpc1, 0)
		SetCharacterTitle(sceneId, nNpc1, "Thiªt Truy ThuÖ Thßþng Phiêu")
