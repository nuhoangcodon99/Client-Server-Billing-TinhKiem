...
x401001_g_limitMembers = 3;					--¿ÉÒÔ½ø¸±±¾µÄ×îÐ¡¶ÓÎéÈËÊý
...
x401001_g_Back_X_Ly = 364;--268;						--ÂåÑôÔ´³¡¾°Î»ÖÃX
x401001_g_Back_Z_Ly = 226;--87;						--ÂåÑôÔ´³¡¾°Î»ÖÃZ
x401001_g_Back_X_Sz = 265;--175;						--ËÕÖÝÔ´³¡¾°Î»ÖÃX
x401001_g_Back_Z_Sz = 243;--145;						--ËÕÖÝÔ´³¡¾°Î»ÖÃZ
x401001_g_Back_X_Dl = 282;--275;						--´óÀíÔ´³¡¾°Î»ÖÃX
x401001_g_Back_Z_Dl = 145;--95;						--´óÀíÔ´³¡¾°Î»ÖÃZ
...
function x401001_CheckAndEnterScene(sceneId, selfId, targetId, eventId)
	...
	elseif eventId == x401001_defaultEvent then
		BeginEvent(sceneId)	
			AddText(sceneId, "#{function_help_063}#r  nªu bÕn nhìn th¤y dòng này, thì do bÕn có card mÕng cao c¤p.");
		EndEvent(sceneId)
...
function x401001_OnDie(sceneId, selfId, killerId)						-- ³¡¾°ID, ±»É±µÄObjId, É±ÊÖObjId
	...
	if objType and objType == LastBoss[mgroup] then
		...
		for	i = 0, membercount - 1 do
			...
			--hd add tam dac 02/08/2014 
			local lastTimes = GetMissionData(sceneId,memId,MD_HUKHONG_LLTB)	-- C¤u trúc: Ngày x 100+S¯ l¥n*10+Cho phép
			local Active = mod(lastTimes,10)
			--if Active == 2 then
				
				SetMissionData(sceneId,memId,MD_MJ_TAMDAC,GetMissionData(sceneId,memId,MD_MJ_TAMDAC)+10)
				BeginEvent(sceneId)
					strText = format("Ðã nh§n ðßþc 10 Tâm Ð¡c Võ H÷c")
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,memId)
				SetMissionData(sceneId,memId,MD_HUKHONG_LLTB,floor(lastTimes/10)*10)
				BeginEvent(sceneId)
					strText = format("Nhi®m vø Hß Không Trân Long KÏ Cuµc hoàn thành.")
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,memId)
			--end
			--end hd add tam dac 02/08/2014 
		...
		if teamLeaderName or firstMemName then
			...
			if teamLeaderName then
				...
				winMessage[3] = format("#W#{_INFOUSR%s}#P ðang cùng#G%s#P t§p trung chú ý ð¯i c¶, các chiªn hæu t× phía sau chia làm 3 ðß¶ng t§p kích #G%s#P, dçn ðªn ðÕi th¡ng thª c¶ Trân Long, thª là#G%s#P nu¯t l® nói: Các ngß½i...Các ngß½i..", teamLeaderName, bossName, bossName, bossName);
			...
			elseif firstMemName then
				winMessage[1] = format("#W#{_INFOUSR%s}#P ðã ðánh bÕi #G%s#P trong Trân Long KÏ Cuµc, #G%s#P trß¾c khi lâm chung than th·: Sñ ð¶i nhß ván c¶ m¾i, L¥n sau Ta nh¤t ð¸nh n² lñc..", firstMemName, bossName, bossName);
		...
		for	i = 0, membercount - 1 do
			...
			if LuaFnIsObjValid( sceneId, memId ) == 1 and LuaFnIsCanDoScriptLogic( sceneId, memId ) == 1 then
				...
				local lastTimes = GetMissionData(sceneId,memId,MD_HUKHONG_LLTB)	-- C¤u trúc: Ngày x 100+S¯ l¥n*10+Cho phép
				local Active = mod(lastTimes,10)
				--x401001_NotifyFailTips(sceneId, memId, "lastTimes: "..lastTimes)
				--x401001_NotifyFailTips(sceneId, memId, "Active: "..Active)
				--if Active == 2 then
					SetMissionData(sceneId,memId,MD_MJ_TAMDAC,GetMissionData(sceneId,memId,MD_MJ_TAMDAC)+1)
					BeginEvent(sceneId)
						strText = format("Ðã nh§n ðßþc 1 Tâm Ð¡c Võ H÷c")
						AddText(sceneId,strText);
					EndEvent(sceneId)
					DispatchMissionTips(sceneId,memId)
				--end
				--end add tam dac
				--- Drop su kien
				if TRANLONG_ACTIVE == 1 then
					if random(100) <= DROP_RATE then
						AddMonsterDropItem( sceneId, selfId, memId, DROP_ITEM[random(getn(DROP_ITEM))] )
					end
				end
			end
