...
function x210217_OnAccept( sceneId, selfId )
	...
	Msg2Player(  sceneId, selfId,"#YNh§n nhi®m vø#W: giªt thêm nhi«u quái",MSG2PLAYER_PARA )
	...
	if ret > 0 then
		...
		if ret > 0 then
			MissionCom( sceneId, selfId, x210217_g_MissionId )
				...
				Msg2Player(  sceneId, selfId,"#YHoàn t¤t nhi®m vø#W: giªt thêm nhi«u quái",MSG2PLAYER_PARA )
			end
		else
		--ÈÎÎñ½±ÀøÃ»ÓÐ¼Ó³É¹¦
			BeginEvent(sceneId)
		...
		for j=0,num-1  do
			...
			if IsHaveMission(sceneId, humanObjId, x210217_g_MissionId) > 0 then
			    ...
			    SetMissionByIndex(sceneId,humanObjId,misIndex,1,nNum+1)
			  	BeginEvent(sceneId)
					strText = format("Ðã giªt chªt %d/8 S½n Thù", GetMissionParam(sceneId,humanObjId,misIndex,1) )
					AddText(sceneId,strText);
			  	EndEvent(sceneId)
