...
function x210213_OnAccept( sceneId, selfId )
	...
	Msg2Player(  sceneId, selfId,"#YNh§n nhi®m vø#W: L¥n ð¥u giªt quái",MSG2PLAYER_PARA )
	...
	if ret > 0 then
		...
		if ret > 0 then
			MissionCom( sceneId, selfId, x210213_g_MissionId )
				...
				Msg2Player(  sceneId, selfId,"#YHoàn t¤t nhi®m vø#W: L¥n ð¥u giªt quái",MSG2PLAYER_PARA )
			end
		else
		--ÈÎÎñ½±ÀøÃ»ÓÐ¼Ó³É¹¦
			BeginEvent(sceneId)
