...
x210231_g_MissionTarget="ó #G Ti¨u Mµc Nh‚n H’ng#W gi™t ch™t 5 con#R Mµc –•u Nh‚n#W, sau Û quay v´ g£p #RHo‡ng Mi TÂng #W#{_INFOAIM275,49,2, Ho‡ng Mi TÂng}."	--»ŒŒÒƒø±Í
x210231_g_ContinueInfo="C·c h’ „ gi™t ch™t 5 con Mµc –•u Nh‚n chﬂa?"	--Œ¥ÕÍ≥…»ŒŒÒµƒnpc∂‘ª∞
	...
	if x210231_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		if ret > 0 then
			...
			if ret > 0 then
				...
				Msg2Player(  sceneId, selfId,"#YHo‡n t§t nhiÆm v¯#W: Ti¨u Mµc Nh‚n H’ng",MSG2PLAYER_PARA )
				CallScriptFunction( 210232, "OnDefaultEvent",sceneId, selfId, targetId)
			end
		else
		--»ŒŒÒΩ±¿¯√ª”–º”≥…π¶
			BeginEvent(sceneId)
				strText = "KhÙng th¨ Ho‡n t§t nhiÆm v¯"
		...
		for j=0,num-1  do
			...
			if IsHaveMission(sceneId, humanObjId, x210231_g_MissionId) > 0 then
			    ...
			    SetMissionByIndex(sceneId,humanObjId,misIndex,1,nNum+1)
			  	BeginEvent(sceneId)
					strText = format("–„ gi™t ﬂ˛c %d/5 Mµc –•u Nh‚n", GetMissionParam(sceneId,humanObjId,misIndex,1) )
					AddText(sceneId,strText);
			  	EndEvent(sceneId)
