...
x210202_g_MoneyBonus=10
x210202_g_SignPost = {x = 110, z = 159, tip = "–≤ Tÿ –¢ng "}
...
function x210202_OnAccept( sceneId, selfId )
	...
	Msg2Player(  sceneId, selfId,"#YNhßn nhiÆm v¯#W: b·nh bao th— nh§t",MSG2PLAYER_PARA )
...
function x210202_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x210202_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		if DelRet == 0  then 
			Msg2Player(  sceneId, selfId,"#Ykh§u tr◊ b·nh bao th§t b’i#W",MSG2PLAYER_PARA )
			BeginEvent(sceneId)
				strText = "Kh§u tr◊ b·nh bao th§t b’i, ph‰i chÂng „ b∏ khÛa?"
...
--					end
--				end
				Msg2Player(  sceneId, selfId,"#YHo‡n t§t nhiÆm v¯#W: b·nh bao th— nh§t",MSG2PLAYER_PARA )
