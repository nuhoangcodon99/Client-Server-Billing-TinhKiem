...
function x210211_OnAccept( sceneId, selfId )
	...
	if ret <= 0 then
		...
		if ret1 > 0  then
			...
			Msg2Player( sceneId, selfId,"#YNh§n nhi®m vø#W: ğ¯t Hoä pháo",MSG2PLAYER_PARA )
		...
		--Ìí¼ÓÈÎÎñ½±Àø
			if ret > 0 then
				...
				if ret > 0 then
					...
					AddItemListToHuman(sceneId,selfId)
					Msg2Player(  sceneId, selfId,"#YHoàn t¤t nhi®m vø#W: Ğ¯t Hoä pháo",MSG2PLAYER_PARA )
...
function x210211_OnUseItem( sceneId, selfId, targetId, eventId )
	...
	if Distance > 5 then
		BeginEvent(sceneId)
			AddText(sceneId,"Ği¬m ğ¯t Hoä pháo truy«n tin còn cách khoäng "..Distance.." GÕo")
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
