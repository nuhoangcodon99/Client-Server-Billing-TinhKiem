...
x808121_monster_chufeng = {ID = 13281, Aitype = 0, AiscriptId = 0, scriptId = 808121, name = "S° Phßþng Trß·ng Thành"}     --³û·ï
...
x808121_monster_feitianmao = {ID = 13287, Aitype = 0, AiscriptId = 270, scriptId = 808121, name = "Vô Ð¸ch Phi Thiên Miêu"}   --ÎÞµÐ·ÉÌìÃ¨
...
--**********************************
	BeginEvent( sceneId )
		AddText(sceneId, "Thiên Giáng KÏ Thú ðã ðªn Huy«n Vû Ðäo r°i, hãy nhanh chân ði xem phong thái cüa KÏ Thú !")
		AddNumText( sceneId, x808121_g_scriptId, "Thiên Giáng KÏ Thú", 6, 1 )
	EndEvent( sceneId )
...
function x808121_OnEventRequest( sceneId, selfId, targetId, eventId )
	...
	if GetNumText() == 1 then
		...
		local strOutmsg = "";
		if (nCheckRet == 1) then
			strOutmsg = "Khiêu chiªn kÏ thú, c¥n có t± ðµi t× 3 ngß¶i tr· lên.";
		elseif (nCheckRet == 2) then
			strOutmsg = "Các hÕ không phäi ðµi trß·ng, g÷i ðµi trß·ng ðªn ðây.";
		elseif (nCheckRet == 3) then
			strOutmsg = "Khiêu chiªn kÏ thú, c¥n có t± ðµi t× 3 ngß¶i tr· lên."           --²»¹»ÈýÈË
		elseif (nCheckRet == 4) then
			strOutmsg = "Ðµi hæu không · g¥n ðây.";
		elseif (nCheckRet == 5 or nCheckRet == 6) then
			strOutmsg = "Xin l²i, thành viên trong ðµi không ðáp Ñng ðü ði«u ki®n tiªn nh§p phø bän \n"           --5,6Ò»¶¨ÊÇ¿òÏÔÊ¾
			...
			for i = 0, nearMemberCount - 1 do
				...
				if sceneMemId and sceneMemId >= 0 then			
					if (GetLevel(sceneId, sceneMemId) < x808121_g_Low_Level) then
						strOutMsgTemp[i] = strOutMsgTemp[i]..":\n  C¤p yêu c¥u :"..tostring(x808121_g_Low_Level).."    #cff0000 không ðÕt#W\n"
					else
						strOutMsgTemp[i] = strOutMsgTemp[i]..":\n  C¤p yêu c¥u :"..tostring(x808121_g_Low_Level).."    #c00ff00 ðÕt#W\n"
					end
					if (0 == x808121_CheckAllXinfaLevel(sceneId, sceneMemId)) then
						strOutMsgTemp[i] = strOutMsgTemp[i].."  Tâm pháp yêu c¥u : "..tostring(x808121_g_Low_Level_xinfa).."    #cff0000 không ðÕt#W\n"
					else
						strOutMsgTemp[i] = strOutMsgTemp[i].."  Tâm pháp yêu c¥u : "..tostring(x808121_g_Low_Level_xinfa).."    #c00ff00 ðÕt#W\n"
					end
				else
					strOutMsgTemp[i] = strOutMsgTemp[i]..memName..": tâm pháp không ðü ðÆng c¤p"
				end
			end
...
function x808121_MakeCopyScene(sceneId, selfId, targetId, nearmembercount)
	...
	BeginEvent(sceneId)
		if bRetSceneID>0 then
			...
			AddText(sceneId,"S¯ lßþng bän sao ðã quá hÕn, vui lòng thØ lÕi sau !");
		end
	EndEvent(sceneId)
...
function x808121_OnCopySceneTimer(sceneId, nowTime)
	...
	if (nStep == 0) then        --¸±±¾¸Õ¸Õ¿ªÊ¼
		...
		for	i = 0, membercount - 1 do
			...
			if LuaFnIsObjValid(sceneId, memId) == 1 then
 				x808121_NotifyFailTips(sceneId, memId, "Thiên Giáng KÏ Thú : Lôi Lân ðã ðªn Huy«n Vû Ðäo.");
 			end
		end		
		...
		SetCharacterTitle(sceneId, objId, "Huy«n Vû Ðäo Trinh Sát Ðµi Trß·ng")
	...
	if (curTickCount >=240 and curTickCount <= 300) then
		...
		if(curTickCount == 240) then
			strOutMsg = "Thiên Giáng KÏ Thú : S° Phßþng s¨ xu¤t hi®n sau 60 giây."
		elseif(curTickCount == 270) then
			strOutMsg = "Thiên Giáng KÏ Thú : S° Phßþng s¨ xu¤t hi®n sau 30 giây."
		elseif(curTickCount == 290) then
			strOutMsg = "Thiên Giáng KÏ Thú : S° Phßþng s¨ xu¤t hi®n sau 10 giây."
		elseif(curTickCount == 295) then
			strOutMsg = "Thiên Giáng KÏ Thú : S° Phßþng s¨ xu¤t hi®n sau 5 giây."
		elseif(curTickCount == 300) then
			strOutMsg = "Thiên Giáng KÏ Thú : S° Phßþng ðã xu¤t hi®n tÕi Huy«n Vû Ðäo."
			--³öÏÖµÚ¶þÅú¹Ö
	...
	elseif (curTickCount >=540 and curTickCount <= 600) then
		...
		if(curTickCount == 540) then
			strOutMsg = "Thiên Giáng KÏ Thú : Lôi Lân s¨ xu¤t hi®n sau 60 giây."
		elseif(curTickCount == 570) then
			strOutMsg = "Thiên Giáng KÏ Thú : Lôi Lân s¨ xu¤t hi®n sau 30 giây."
		elseif(curTickCount == 590) then
			strOutMsg = "Thiên Giáng KÏ Thú : Lôi Lân s¨ xu¤t hi®n sau 10 giây."
		elseif(curTickCount == 595) then
			strOutMsg = "Thiên Giáng KÏ Thú : Lôi Lân s¨ xu¤t hi®n sau 5 giây."
		elseif(curTickCount == 600) then
			strOutMsg = "Thiên Giáng KÏ Thú : Lôi Lân ðã xu¤t hi®n tÕi Huy«n Vû Ðäo."
			--³öÏÖµÚÈýÅú¹Ö
	...
	elseif (curTickCount >=2340) then
		...
		if (curTickCount == 2340) then
			strOutMsg = "Phø bän s¨ ðóng sau 60 giây."
		elseif(curTickCount == 2370) then
			strOutMsg = "Phø bän s¨ ðóng sau 30 giây."
		elseif(curTickCount == 2380) then
			strOutMsg = "Phø bän s¨ ðóng sau 20 giây."
		elseif(curTickCount == 2390) then
			strOutMsg = "Phø bän s¨ ðóng sau 10 giây."
		elseif(curTickCount == 2395) then
			strOutMsg = "Phø bän s¨ ðóng sau 5 giây."
...
--	AllMonsterNum = x808121_gFirst_MonstersTbl.size + x808121_gSecond_MonstersTbl.size + x808121_gThird_MonstersTbl.size + x808121_gFourth_MonstersTbl.size  
	...
	if (szName == x808121_monster_chufeng.name) then
		...
		strOutMsg = strOutMsg.."Khiêu chiªn Thiên Niên KÏ Thú S° Phßþng : "..tostring(nKilledMonsterNum_feng).."/"..tostring(x808121_monster_Second_num + x808121_monster_Fourth_num)
		AuditType = 1
	elseif (szName == x808121_monster_leilin.name) then
		...
		strOutMsg = strOutMsg.."Khiêu chiªn Thiên Niên KÏ Thú Lôi Lân :"..tostring(nKilledMonsterNum_long).."/"..tostring(x808121_monster_Second_num + x808121_monster_Fourth_num)
		AuditType = 2
	...
	elseif (szName == x808121_monster_feitianmao.name) then
		nKilledMonsterNum_boss = nKilledMonsterNum_boss + 1
		LuaFnSetCopySceneData_Param(sceneId, 13, nKilledMonsterNum_boss);
		strOutMsg = strOutMsg.."Ðã giªt chªt Vô Ð¸ch Phi Thiên Miêu : 1/1."
		AuditType = 4
