--´óÌÓÉ±
--»î¶¯½øÐÐÖÐ
--¸±±¾µØÍ¼´óÐ¡Îª256*256£¬¹²°üº¬64*64µÄ16¸öÐ¡·¿¼ä
--line 328
x808002_g_rate=100
--½Å±¾ºÅ
...
x808002_g_LevelMin			= 40				--×îµÍµÈ¼¶ÏÞÖÆ
x808002_g_GodBuff				= 112				--ÎÞµÐBUFF
x808002_g_CampA					= 10				--ÌôÕ½A·½ÕóÓªºÅ
x808002_g_CampB					= 11				--ÌôÕ½B·½ÕóÓªºÅ
...
x808002_g_CopySceneType	= FUBEN_DATAOSHA--¸±±¾ÀàÐÍ£¬¶¨ÒåÔÚScriptGlobal.luaÀïÃæ
x808002_g_CopySceneMap	= "dataosha.nav"
x808002_g_TickTime			= 5					--»Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x808002_g_CloseTick			= 6					--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊý£©
x808002_g_NoUserTime		= 600				--¸±±¾ÖÐÃ»ÓÐÈËºó¿ÉÒÔ¼ÌÐø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£©
...
x808002_g_ScnParam			=
{
	["typ"]	= 0,	--ÉèÖÃ¸±±¾Êý¾Ý£¬ÕâÀï½«0ºÅË÷ÒýµÄÊý¾ÝÉèÖÃÎª999£¬ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
...
x808002_g_Msg						=
{
	...
	["pop1"]	= "  ÐÕi hµi thí võ hi®n chính thÑc b¡t ð¥u. Nhæng thí sinh l¥n ð¥u tham dñ m²i ngß¶i phäi nµp #{_EXCHG"..x808002_g_GoldMin.."} phí báo danh, các hÕ có mu¯n tham gia không?",
	["gld"]		= "Ngân lßþng trên ngß¶i các hÕ không ðü #{_EXCHG"..x808002_g_GoldMin.."}, xin gom ðü s¯ ngân lßþng r°i hÇng t¾i tìm ta báo danh",
...
function x808002_OnPlyBounty( sceneId, selfId, targetId, rnd )
	...
	--±ÈÈü½á¹û£¬ÒÔÆä"·ûºÅÎ»"±êÊ¶Ê¤¸º£¬ÒÔÆä"¾ø¶ÔÖµÊÇ·ñ´óÓÚ1"±êÊ¶ÊÇ·ñÁìÈ¡¹ý½±Àø£¬0µÈÍ¬ÓÚÃ»ÓÐ¹ý±ÈÈü
	ret_self	= GetMissionData( sceneId, selfId, MD_DATAOSHA_RESULT )
	...
	--Ôö¼Ó¾­ÑéÖµ
		...
		AddExp( sceneId, selfId, exp )
		BeginEvent( sceneId )
			AddText( sceneId, "  Ta chï có chút quà m÷n này, mong ðÕi hi®p ch¾ cß¶i chê" )
		EndEvent( sceneId )
		DispatchEventList( sceneId, selfId, targetId )
	--ÁìÈ¡½±ÀøÔÙ½øÐÐÅÅÃû
	--ÀíÓÉ£º°´ÕÕµÈ¼¶»®·ÖÎª¶à×é£¬Ã¿×é¶ÔÓ¦¶à¸±±¾£¬¶àÏß³Ì
	--»ñÈ¡±íµ¥ID
	end
	tabId			= CallScriptFunction( x808002_g_ScriptId_Ctrl, "GetTableID", sceneId, selfId )
	--Add Log
	...
	--ÅÅÐÐ¼üÖµ£¨½µÐò£¬¼üÖµÔ½´ó£¬ÅÅÃûÔ½¿¿Ç°£©£º»·Êý * 3 £¬Ê¤Õß¼Ó2£¬¸ºÕß¼Ó1
	key				= rnd_self * 3
	if ret_self > 0 then
		key			= key + 2				--Ê¤Õß
	...
	if isWin == 0 and
		...
		if idBag >= 0 then
			...
			szMsg	= format( "Ðinh Ðinh Ðang Ðang, TÖ Võ Hµi Trß¶ng, #{_INFOUSR%s} ðã chiªn th¡ng các ð¯i thü trong vòng. Mµt ði«u b¤t ng¶ ðã thñc sñ xäy ra #{_INFOMSG%s} xu¤t hi®n.",
...
function x808002_MakeCopyScene( sceneId, selfId, tabId, rnd, tabId )
	...
	LuaFnSetSceneLoad_Map( sceneId, x808002_g_CopySceneMap )							--µØÍ¼ÊÇ±ØÐëÑ¡È¡µÄ£¬¶øÇÒ±ØÐëÔÚConfig/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param( sceneId, x808002_g_ScnParam["typ"], x808002_g_CopySceneType )			--ÉèÖÃ¸±±¾Êý¾Ý£¬ÕâÀï½«0ºÅË÷ÒýµÄÊý¾ÝÉèÖÃÎª999£¬ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êý×Ö×Ô¶¨Òå)
...
--LuaFnSetCopySceneData_Param( sceneId, x808002_g_ScnParam["cls"], 0 )
	...
	--»î¶¯Í³¼Æ
	LuaFnAuditQuest( sceneId, selfId, "TÖ Võ ÐÕi Hµi" )
	
	--¿Û³ý½ðÇ®£¨Ö»ÔÚµÚÒ»ÂÖ¿ÛÇ®£©
	if rnd <= 1 then

		-- Ê¹ÓÃ´øÓÅÏÈ¼¶µÄ½ðÇ®ÏûºÄº¯Êý
		-- Ô­À´¿Û½ð±ÒÃ»ÓÐÅÐ¶ÏÊÇ·ñ¿ÛÇ®Ê§°Ü£¬ËùÒÔ¿Û½»×ÓÒ²Î´¼ÓÊÇ·ñ¿ÛÇ®Ê§°ÜµÄ´¦Àí£¡				
	...
	--¼ÇÂ¼±ÈÎä»ØºÏÊý£¨Äê+µ±ÄêµÚ¼¸Ìì+»ØºÏÊý£ºYY+DDD+RR£©
...
function x808002_OnPlayerEnter( sceneId, selfId )
	...
	if LuaFnGetCopySceneData_Param( sceneId, x808002_g_ScnParam["typ"] ) == x808002_g_CopySceneType then
		...
		if LeaveFlag > 0 then
			x808002_MsgBox( sceneId, selfId, "ÐÕi hµi thí võ l¥n này ðã kªt thúc" )
			x808002_OnGoBack( sceneId, selfId )
			return
		end
...
function x808002_OnHumanDie( sceneId, selfId, killerId )
	...
	--ÉèÖÃÍæ¼ÒµÄÖ¸¶¨PvpÊÚÈ¨Î»
	--SetPvpAuthorizationFlagByID( sceneId, killerId, 2, 0 )		--2ÊÇ¾º¼¼ÊÚÈ¨±ê¼Ç
...
function x808002_OnCopySceneTimer( sceneId, nowTime )
		...
		--ÊÇ·ñ¿ªÊ¼µ¹¼ÆÊ±
			...
			elseif TickCount > JoinTime then
				...
				for i = 0, mem_num - 1 do
					...
					--ÉèÖÃÍæ¼ÒµÄÖ¸¶¨PvpÊÚÈ¨Î»
					SetPvpAuthorizationFlagByID( sceneId, mem_obj, 2, 1 )			--2ÊÇ¾º¼¼ÊÚÈ¨±ê¼Ç
				end
			end
	...
	--»Ø³Ç
		...
		for i = 0, mem_num - 1 do
			...
			--ÉèÖÃÍæ¼ÒµÄÖ¸¶¨PvpÊÚÈ¨Î»
			--SetPvpAuthorizationFlagByID( sceneId, mem_obj, 2, 0 )			--2ÊÇ¾º¼¼ÊÚÈ¨±ê¼Ç
...
--**********************************
--±ÈÎä½áÊøÉ¨Ãè»á³¡
--¹æÔò£º»î¶¯½áÊøµÄÊ±ºò¶ÔµØÍ¼ËùÓÐ·¿¼ä
--É¨ÃèÒ»´Î£¬·¿¼äÈËÊý>1µÄÈÎÎñÊ§°Ü£¬´«
--ËÍÖÁËÕÖÝ±¨ÃûµØµãÅÔ±ßµÄÎ»ÖÃ¡£
--**********************************
function x808002_OnScanRoom( sceneId, mem_num )
	...
	--É¨Ãè32¸ö·¿¼ä£¬¾ö¶¨Ê¤¸º
	local	rom_px, rom_pz		--Ã¿¸ö·¿¼äµÄ×óÉÏ½Ç×ø±ê
	local	num, id
	for i = 0, 31 do
