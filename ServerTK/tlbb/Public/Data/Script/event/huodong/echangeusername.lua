...
function x808008_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x808008_g_ScriptId, "Ta mu¯n ð±i tên", 6, x808008_g_Key["ChgU"] )
...
function x808008_IsCanChangeUserName( sceneId, selfId )
	...
	x808008_g_name_before = UserName; --gán vào biªn global
...
function x808008_CallBackChangeUserNameAfter( sceneId, selfId, nRetType )
	...
	elseif nRetType == 4 then
		...
		str = format("#cff99cc#{_INFOUSR%s} sØ døng #Y[Thë Ð±i Tên] #P tÕi #YPhÕm Di Thành #P(#GLÕc Dß½ng#P) ð±i sang tên m¾i là #{_INFOUSR%s} thành công!", x808008_g_name_before,GetName(sceneId,selfId))
		BroadMsgByChatPipe(sceneId, selfId, str, 4)
		local reply =LuaFnDelAvailableItem(sceneId,selfId,30008105,1) --add del item the doi ten
		if reply < 1 then
			x808008_MsgBox( sceneId, selfId, "Xoá thë ð±i tên th¤t bÕi!" )
		end
	end
