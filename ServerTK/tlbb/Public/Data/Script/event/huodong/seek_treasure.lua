...
x808039_g_tandiep_drop = 38000529; --bi t¸ch tàn di®p --30505078 : thiên thß tàn di®p //hd  edit 18/1/2014;


--¸±±¾Ãû³Æ
x808039_g_CopySceneName = "Lâu Lan Bäo Tàng Ğµng"
...
x808039_g_MonsterChangeTime = 90 * 1000; -- ¹ÖÎï±äÉíÊ±¼ä 90Ãë (ºÁÃë)

x808039_g_CopySceneType = FUBEN_SEEK_TREASURE	--¸±±¾ÀàĞÍ£¬¶¨ÒåÔÚScriptGlobal.luaÀïÃæ

x808039_g_LimitMembers = 3							--¿ÉÒÔ½ø¸±±¾µÄ×îĞ¡¶ÓÎéÈËÊı
x808039_g_TickTime = 5									--»Øµ÷½Å±¾µÄÊ±ÖÓÊ±¼ä£¨µ¥Î»£ºÃë/´Î£©
x808039_g_LimitTotalHoldTime = 720 			--¸±±¾¿ÉÒÔ´æ»îµÄÊ±¼ä£¨µ¥Î»£º´ÎÊı£©,Èç¹û´ËÊ±¼äµ½ÁË£¬ÔòÈÎÎñ½«»áÊ§°Ü 72*5 =60·ÖÖÓ
x808039_g_StartTickCount = 7  					--Õ½¶·¿ªÊ¼ÌáĞÑ£¨µ¥Î»£º´ÎÊı£©
x808039_g_CloseTick = 6									--¸±±¾¹Ø±ÕÇ°µ¹¼ÆÊ±£¨µ¥Î»£º´ÎÊı£©
x808039_g_NoUserTime = 36								--¸±±¾ÖĞÃ»ÓĞÈËºó¿ÉÒÔ¼ÌĞø±£´æµÄÊ±¼ä£¨µ¥Î»£ºÃë£© 3·ÖÖÓ
x808039_g_Fuben_X = 65									--½øÈë¸±±¾µÄÎ»ÖÃX need modify
x808039_g_Fuben_Z = 94									--½øÈë¸±±¾µÄÎ»ÖÃZ
x808039_g_Back_X = 163									--Ô´³¡¾°Î»ÖÃX
x808039_g_Back_Z = 77										--Ô´³¡¾°Î»ÖÃZ
x808039_g_Back_SceneId = 186						--Ô´³¡¾°Id Â¥À¼
...
x808039_g_NianShouCreatePos = --ÄêÊŞË¢³ö×ø±ê
{
	{34,65},{38,49},{43,42},{73,35},{91,48},
	{95,60},{92,78},{80,91},{48,91},{97,66},
}
x808039_g_NianShouId = --ÄêÊŞID
{
	12206, --70
	12207,
	12208,
...
x808039_g_SmallMonsterId[2] = {12111,12112,12113,12114,12115,12116,12117,12118,12119} --70,80,90...150 ½ğÉ«
-- ±¦ÏäÍ¯×Ó
...
function x808039_OnEnumerate( sceneId, selfId, targetId )	
	...
	if Active == 1 then
		AddNumText(sceneId, x808039_g_ScriptId, "Khiêu chiªn Hß Không Lâu Lan T¥m Bäo", 6, 1);
	else
		if Active >= 2 then
			SetMissionData(sceneId,selfId,MD_HUKHONG_LLTB,floor(lastTimes/10)*10)
		end
		AddNumText(sceneId, x808039_g_ScriptId, "Lâu Lan T¥m Bäo", 6, 1);
	end
	AddNumText(sceneId, x808039_g_ScriptId, "Trş giúp hoÕt ğµng Lâu Lan T¥m Bäo", 11, 2);
end
...
function x808039_OnDie( sceneId, objId, killerId )
	...
	if killednumber <= x808039_g_TotalNeedKill then
		for i=0,num-1 do
			...
			if LuaFnIsObjValid(sceneId, humanObjId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, humanObjId) == 1 then
				BeginEvent(sceneId)
					strText = format("Ğã tiêu di®t %d/%d", killednumber, x808039_g_TotalNeedKill )
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,humanObjId)
					-- Add Drop Huw Khoong
				local lastTimes = GetMissionData(sceneId,humanObjId,MD_HUKHONG_LLTB)	-- C¤u trúc: Ngày x 100+S¯ l¥n*10+Cho phép
				local Active = mod(lastTimes,10)
				--if Active == 2 then
					SetMissionData(sceneId,humanObjId,MD_MJ_TAMDAC,GetMissionData(sceneId,humanObjId,MD_MJ_TAMDAC)+1)
					BeginEvent(sceneId)
						strText = format("Ğã nh§n ğßşc 1 Tâm Ğ¡c Võ H÷c")
						AddText(sceneId,strText);
					EndEvent(sceneId)
					DispatchMissionTips(sceneId,humanObjId)
				--end
				--- Drop su kien
				if LLTB_ACTIVE == 1 then
					if random(100) <= DROP_RATE then
						AddMonsterDropItem( sceneId, selfId, humanObjId, DROP_ITEM[random(getn(DROP_ITEM))] )
					end
				end
		...
		----
		if killednumber == x808039_g_TotalNeedKill then --15Ãëºó³öÁúÍõ
	...
	elseif killednumber > x808039_g_TotalNeedKill then --ÁúÍõ¶¼±»¸ÉµôÁË
		...
		for i=0,num-1 do
			...
			if LuaFnIsObjValid(sceneId, humanObjId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, humanObjId) == 1 then
				BeginEvent(sceneId)
					strText = format("Nhi®m vø hoàn thành, r¶i khöi ğây sau %d giây", x808039_g_CloseTick*x808039_g_TickTime )
					AddText(sceneId,strText);
				EndEvent(sceneId)
				DispatchMissionTips(sceneId,humanObjId)
					-- Add Drop Huw Khoong
				...
				--if Active == 2 then
					if random(100) <= 5 then
						AddMonsterDropItem( sceneId, objId, humanObjId, x808039_g_tandiep_drop )
					end
					SetMissionData(sceneId,humanObjId,MD_MJ_TAMDAC,GetMissionData(sceneId,humanObjId,MD_MJ_TAMDAC)+10)
					BeginEvent(sceneId)
						strText = format("Ğã nh§n ğßşc 10 Tâm Ğ¡c Võ H÷c")
						AddText(sceneId,strText);
					EndEvent(sceneId)
					DispatchMissionTips(sceneId,humanObjId)
					SetMissionData(sceneId,humanObjId,MD_HUKHONG_LLTB,floor(lastTimes/10)*10)
					BeginEvent(sceneId)
						strText = format("Nhi®m vø Hß Không Lâu Lan T¥m Bäo hoàn thành.")
						AddText(sceneId,strText);
					EndEvent(sceneId)
					DispatchMissionTips(sceneId,humanObjId)
				--end
...
function x808039_OnCopySceneTimer( sceneId, nowTime )
	...
	local tickCount = LuaFnGetCopySceneData_Param(sceneId, 2); --¸±±¾¾­¹ıµÄtickÊı
	...
	if leaveFlag == 1 then -- ĞèÒªÀë¿ª		
		...
		elseif leaveTickCount < x808039_g_CloseTick then
			...
			for	i=0,membercount-1 do
				...
				if LuaFnIsObjValid(sceneId, mems[i]) == 1 and LuaFnIsCanDoScriptLogic(sceneId, mems[i]) == 1 then
	  			BeginEvent(sceneId)
	  				strText = format("Các hÕ s¨ r¶i khöi ğây sau %d giây !", (x808039_g_CloseTick-leaveTickCount)*x808039_g_TickTime )
	  				AddText(sceneId,strText);
	  			EndEvent(sceneId)
	...
	elseif tickCount < x808039_g_StartTickCount then 
		...
		for i=0, membercount-1 do
			...
			if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
				...
				BeginEvent(sceneId)
					local	msg = format("%d giây næa quái v§t s¨ xu¤t hi®n.", tm);
					AddText(sceneId, msg)
				EndEvent()
	...
	elseif tickCount == x808039_g_StartTickCount then
		...
		for i=0, membercount-1 do
			...
			if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
				BeginEvent(sceneId)
					AddText(sceneId, "Chiªn ğ¤u b¡t ğ¥u");
				EndEvent()
				DispatchMissionTips(sceneId, objId);
			end
		end	
		--Ë¢µÚÒ»Åú¹Ö
		local grade = LuaFnGetCopySceneData_Param(sceneId, 10); 		-- ¹ÖÀàĞÍ 
		local mstColor = LuaFnGetCopySceneData_Param(sceneId, 11); 	-- ¹ÖÑÕÉ«
		local mstLvl = LuaFnGetCopySceneData_Param(sceneId, 13); 		-- ¹ÖµÈ¼¶ 
		...
		for i = 0, monsterCount - 1 do
			...
			if LuaFnIsCharacterLiving(sceneId, monsterObjId) > 0 then				--»îµÄ¹ÖÎï
				local monstertype = GetMonsterDataID(sceneId, monsterObjId);	--¹ÖÎïÀàĞÍ
		...
		if ret ~= nil then
			if oldFlushMonsterTime + x808039_g_MonsterFlushSpeed[ret]["speed"] <= tickCount then 
				LuaFnSetCopySceneData_Param( sceneId, 14, 0 ); --ÇåÖĞ¶Ï±êÖ¾
				...
				if monsterBatch <= 50 and monsterBatch >= 1 then
					...
					LuaFnSetCopySceneData_Param(sceneId, 8, monsterBatch); -- Ë¢¹ÖÅúÊı+1	
				end
				LuaFnSetCopySceneData_Param(sceneId, 9, tickCount); --¼ÇË¢¹ÖÊ±¼ä	
				-- Ë¢30ÅúºóĞªĞª
				if monsterBatch == 31 then
					LuaFnSetCopySceneData_Param( sceneId, 9, tickCount+18-5 ); --¼ÇË¢¹ÖÊ±¼ä	Ğª90Ãë
					LuaFnSetCopySceneData_Param( sceneId, 14, 1 ); --ÖÃÖĞ¶Ï±êÖ¾
					--Õâ¶´ÖĞµÄ±¦ÏäÒÑ¾­±»ÄãÔÒËéÁË´ó°ë£¬Äã¿ÉÒÔÏÈĞ¡ĞªÒ»»á¶ù£¬ÎÈÎÈÕó½Å¡£
					local membercount = LuaFnGetCopyScene_HumanCount(sceneId);
					for i=0, membercount-1 do
						local objId = LuaFnGetCopyScene_HumanObjId(sceneId, i);
						if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
			...
			if diffCount <= 6 and diffCount >= 1 then
				...
				for i=0, membercount-1 do
					...
					if LuaFnIsObjValid(sceneId, objId) == 1 and LuaFnIsCanDoScriptLogic(sceneId, objId) == 1 then
						...
						msg = format("%d giây næa chiªn ğ¤u b¤t ğ¥u lÕi", tm);
		...
		if num > 0 then
			if num == 4 then -- ³öÁúÍõ
				...
				SetLevel( sceneId, objId, mstLvl )	
				--ÅİÅİ
				CallScriptFunction((200060), "Paopao", sceneId, "Tr¤n Bäo Long Vß½ng", "Lâu Lan Bäo Tàng Ğµng", "Ha ha, các ngß½i không có vi®c gì làm mà dám ğªn cß¾p bäo tàng cüa ta à ?!")
...
function x808039_OnPlayerEnter( sceneId, selfId )
	...
	if Active == 1 then
		SetMissionData(sceneId,selfId,MD_HUKHONG_LLTB,lastTimes+1)
		x808039_ShowMsg(sceneId, selfId, "Các hÕ tham gia phø bän v¾i trÕng thái Khiêu chiªn Hß Không Huy­n Cänh");
	end
	--ÉèÖÃËÀÍöºó¸´»îµãÎ»ÖÃ
...
function x808039_MakeCopyScene(sceneId, selfId)
	...
	LuaFnSetSceneLoad_Map(sceneId, x808039_g_FuBen_Data.MapFile); --µØÍ¼ÊÇ±ØĞëÑ¡È¡µÄ£¬¶øÇÒ±ØĞëÔÚConfig/SceneInfo.iniÀïÅäÖÃºÃ
	...
	LuaFnSetCopySceneData_Param(sceneId, 0, x808039_g_CopySceneType);--ÉèÖÃ¸±±¾Êı¾İ£¬ÕâÀï½«0ºÅË÷ÒıµÄÊı¾İÉèÖÃÎª999£¬ÓÃÓÚ±íÊ¾¸±±¾ºÅ999(Êı×Ö×Ô¶¨Òå)
	LuaFnSetCopySceneData_Param(sceneId, 1, x808039_g_ScriptId);--½«1ºÅÊı¾İÉèÖÃÎª¸±±¾³¡¾°ÊÂ¼ş½Å±¾ºÅ
	...
	LuaFnSetCopySceneData_Param(sceneId, 9, 0);		--ÉÏ´ÎË¢³ö¹ÖµÄÊ±¼ä tickCount±íÊ¾
	...
	LuaFnSetCopySceneData_Param(sceneId, 10, iniLevel) --Ğ¡¹Ö¼¶±ğ
	LuaFnSetCopySceneData_Param(sceneId, 11, 1)--ÒøÉ«£¿½ğÉ«£¿
	-- Ê¹ÓÃµÚ12Î»£¬¼ÇÂ¼³¡¾°ÊÇ·ñÒª¹Ø±Õ
	LuaFnSetCopySceneData_Param(sceneId, 12, 0)
	-- Ê¹ÓÃµÚ13Î»£¬¼ÇÂ¼¹ÖÎïÊµ¼ÊµÄµÈ¼¶
	LuaFnSetCopySceneData_Param(sceneId, 13, mylevel)
	-- Ê¹ÓÃµÚ14Î»£¬¼ÇÂ¼ÊÇ·ñĞèÒªË¢¹Ö30ÅúºóÖĞ¶Ï
...
function x808039_CheckEnterCondition(sceneId, selfId)
  ...
  for i=0, teamSize-1 do
  	...
  	if LuaFnGetHumanPKValue(sceneId, objId) > 0 then
  		...
  		local msg = format("Thành viên %s có sát khí quá cao, không th¬ tham gia phø bän", name);
  		return 0, msg;
  	end  	
  end
  --ok
...
function x808039_CheckMemberInfo(sceneId, selfId, targetId)
  ...
  local x808039_member_info = {
		{ name = "", levelReq = "#GğÕt ", xinfaReq = "#GğÕt ", taskCount = "#GğÕt " },
		{ name = "", levelReq = "#GğÕt ", xinfaReq = "#GğÕt ", taskCount = "#GğÕt " },
		{ name = "", levelReq = "#GğÕt ", xinfaReq = "#GğÕt ", taskCount = "#GğÕt " },
		{ name = "", levelReq = "#GğÕt ", xinfaReq = "#GğÕt ", taskCount = "#GğÕt " },
		{ name = "", levelReq = "#GğÕt ", xinfaReq = "#GğÕt ", taskCount = "#GğÕt " },
  ...
  for i=0, teamSize-1 do
  	...
  	local bXinfaOK = x808039_CheckXinfaLevel(sceneId, objId, 40) --ĞÄ·¨´ïµ½40¼¶£¿
  	...
  	if GetTime2Day() == joinDate then
  		x808039_member_info[i+1]["taskCount"] = "#cff0000 không ğÕt ";
  		bSucc = 0;  		
  	end
  	if level < 75 then
  		x808039_member_info[i+1]["levelReq"] = "#cff0000 không ğÕt ";
  		bSucc = 0;
  	end
  	if bXinfaOK == 0 then
  		x808039_member_info[i+1]["xinfaReq"] = "#cff0000 không ğÕt ";
  		bSucc = 0;
  	end
  end
  ---------------------
	if bSucc == 0 then
		...
		AddText(sceneId, "  Thành viên ğµi ngû : ");
		for i, mem in x808039_member_info do
			if i > teamSize then
				break
			end	
			msg = format("  #B ğµi viên %s : ", mem["name"]);
			if x808039_member_info[i]["levelReq"] == "#cff0000 không ğÕt " then
				msg = msg.."#r  #cff0000 ğÆng c¤p 75             không ğÕt ";
			else
				msg = msg.."#r  #G ğÆng c¤p 75             ğÕt ";
			end
			---------------
			if x808039_member_info[i]["xinfaReq"] == "#cff0000 không ğÕt " then
				msg = msg.."#r  #cff0000 tâm pháp 40             không ğÕt " ;
			else
				msg = msg.."#r  #G tâm pháp 40             ğÕt ";
			end
			---------------
			if x808039_member_info[i]["taskCount"] == "#cff0000 không ğÕt " then
				msg = msg.."#r  #cff0000 ğã nh§n nhi®m vø               không ğÕt ";
			else
				msg = msg.."#r  #G ğã nh§n nhi®m vø               ğÕt ";
			end
			AddText(sceneId, msg); 
		end	 -- end of for
		EndEvent()
