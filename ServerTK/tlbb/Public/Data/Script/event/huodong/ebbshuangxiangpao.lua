...
function x808075_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if NumText == 0	then
		...
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x808075_g_ScriptId, "Ta mu¯n tham gia rút thåm", 6, 2 )
			AddNumText( sceneId, x808075_g_ScriptId, "Ta mu¯n nh§n BUFF Thiên Long", 6, 3 )
		EndEvent(sceneId)
	...
	elseif NumText == 3	then
		...
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x808075_g_ScriptId, "Ð¬ ta nghî lÕi", 6, 5 )
		EndEvent(sceneId)
		DispatchEventList(sceneId,selfId,targetId)
...
function x808075_OnEnumerate( sceneId, selfId, targetId )
	...
	if 1 == x808075_CheckRightTime() then
		AddNumText(sceneId, x808075_g_ScriptId, "Bäo Bäo Song Hß·ng Pháo hoÕt ðµng", 1, 0 )
		AddNumText(sceneId, x808075_g_ScriptId, "Gi¾i thi®u Bäo Bäo Song Hß·ng Pháo hoÕt ðµng", 13, 1 )
	end
...
function x808075_SXPGiveGift( sceneId, selfId, targetId )
	...
	local msg2 = "@*;SrvMsg;SCA:".."#PTrong #G Sñ ki®n Bäo Bäo Song Hß·ng Pháo #P, #{_INFOUSR%s}#P thu th§p ðü các ký tñ ðªn#G ÐÕi Lý#Y Cung Thái Vân#G(170,122)#P ðã may m¡n trúng thß·ng #Y%d#P."
