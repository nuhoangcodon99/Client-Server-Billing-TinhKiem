...
x809320_g_petList[30309764] = {type=1, dataId=22869, level=1} --Chí Tôn Th¥n Thú Tù Lan
...
function x809320_OnConditionCheck( sceneId, selfId )
	...
	if not checkCreatePet or checkCreatePet ~= 1 then
		x809320_NotifyFailTips(sceneId, selfId, "Các hÕ không th¬ mang thêm trân thú. ");
		return 0;
	end

	local itemTblIndex = LuaFnGetItemIndexOfUsedItem( sceneId, selfId );
	local petItem = x809320_g_petList[itemTblIndex];
	if not petItem then
		x809320_NotifyFailTips(sceneId, selfId, "ĞÕo cø chßa m·, không th¬ sØ døng. ");
		return 0;
	end
	...
	if not takeLevel or not humanLevel or takeLevel > humanLevel then
		x809320_NotifyFailTips(sceneId, selfId, "Chßa ğü c¤p ğµ ğ¬ sØ døng. ");
		return 0;
	end
	end
	return 1
...
function x809320_OnActivateOnce( sceneId, selfId )
	...
	local petItem = x809320_g_petList[itemTblIndex];
	if not petItem then
		x809320_NotifyFailTips(sceneId, selfId, "Chßa m· ğÕo cø, không th¬ sØ døng. ");
		return 0;
	end
	...
	if petItem.type == 1 then
		...
		if ret and ret == 1 then
			x809320_OnGivePlayerPet( sceneId, selfId, petItem.dataId, petGUID_H, petGUID_L, ItemInfo )
		end
	  ...
	  for i,pet in petItem.dataIds do
	  	if level >=pet.minHumanLevel and level <= pet.maxHumanLevel then
			  	ret = CallScriptFunction( 800105, "CreateRMBPetToHuman", sceneId, selfId, pet.dataId, petItem.level);
					if ret and ret == 1 then
						x809320_OnGivePlayerPet( sceneId, selfId, pet.dataId, petGUID_H, petGUID_L, ItemInfo )
					end
	  	end
	  end
	else
...
-- ĞÑÄ¿Ê§°ÜÌáÊ¾
--**********************************
function x809320_NotifyFailTips( sceneId, selfId, Tip )
	BeginEvent( sceneId )
		AddText( sceneId, Tip )
	EndEvent( sceneId )
...
function x809320_OnGivePlayerPet( sceneId, selfId, petId, petGUID_H, petGUID_L, ItemInfo )

	local petName = GetPetName( petId )
	if petName then
		x809320_NotifyFailTips(sceneId, selfId, "Chúc m×ng, các hÕ nh§n ğßşc "..petName..".")
	end
