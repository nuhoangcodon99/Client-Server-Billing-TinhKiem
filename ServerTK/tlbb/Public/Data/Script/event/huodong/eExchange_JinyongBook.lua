...
function x808035_CheckEnoughItem( sceneId, selfId )
    ...
    if ItemCount_de < 2 then    --"µÄ"±ØÐëÓÐ2±¾ÒÔÉÏ
        return 0
    end
...
function x808035_ExchangeTLBB( sceneId, selfId, targetId )
	...
	--¿Û³ýÎïÆ·,¼ÙÈç¿Û³ýÊ§°Ü,½«²»»á¸øÓèÎïÆ·<ÌìÁú°Ë²¿>
    for i=1, 12 do
		ret = LuaFnDelAvailableItem(sceneId, selfId, ItemList[ i ], 1)
		if ret ~= 1 then
			return   --¼ÙÈçÉ¾³ý²Ù×÷ÓÐÈÎºÎÒ»¸öÎïÆ·Ê§°Ü,ÔòÖÐ¶Ï²Ù×÷,²»»á¸øÓèÍæ¼ÒÈÎºÎÎïÆ·
		end
	end
...
function x808035_OnDefaultEvent( sceneId, selfId, targetId )
	...
	--			--AddText( sceneId,"  ¶Ô²»Æð£¬ÏÖÔÚÃ»ÓÐÕýÔÚ¾ÙÐÐµÄ»î¶¯¡£")
	--		EndEvent()
	--		DispatchEventList( sceneId, selfId, targetId )
    --  else
    --       BeginEvent(sceneId)
