...
x808038_g_AccomplishNPC_Name="Cung Th·i V‚n "

--»ŒŒÒ∫≈
x808038_g_MissionId = 1130;

--»ŒŒÒƒø±Ínpc
x808038_g_Name 					= "Cung Th·i V‚n "
--»ŒŒÒπÈ¿‡
...
--»ŒŒÒŒƒ±æ√Ë ˆ
x808038_g_MissionName			= "Thu thßp chÊ k˝"
--»ŒŒÒ√Ë ˆ
x808038_g_MissionInfo			= "Thu thßp 15 chÊ k˝ c¸a h‰o hÊu ™n Cung Th·i V‚n ¨ nhßn qu‡"
--»ŒŒÒƒø±Í
x808038_g_MissionTarget		= "#{SJQM_8825_20}"
--Œ¥ÕÍ≥…»ŒŒÒµƒnpc∂‘ª∞
x808038_g_ContinueInfo		= "CÛ vÎ nhﬂ c·c h’ khÙng l‡m ‡?"
--ÕÍ≥…»ŒŒÒnpcÀµµƒª∞
...
x808038_g_Custom	= { {id="Thu thßp chÊ k˝ c¸a h‰o hÊu",num=15} }
--MisDescEnd
----------------
...
function x808038_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if id == 1 then 
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x808038_g_ScriptId, "Tham gia ho’t µng thu thßp chÊ k˝", 6, 3);
			AddNumText(sceneId, x808038_g_ScriptId, "Nhßn gi‰i thﬂ∑ng", 6, 4);
			EndEvent()
	...
	elseif id == 4 then
		...
		if LuaFnDelAvailableItem(sceneId, selfId, x808038_g_Signature_id, 15) == 0 then
			x808038_ShowMsg(sceneId, selfId, "Kh§u tr◊ danh thi™p th§t b’i!");
		  return
		end
		--ø€≥˝«©√˚≤æ
		if LuaFnDelAvailableItem(sceneId, selfId, x808038_g_SignatureList_id, 1) == 0 then
			x808038_ShowMsg(sceneId, selfId, "Kh§u tr◊ danh s·ch chÊ k˝ th§t b’i");
		  return
		end		
		-- Ω±¿¯æ≠—È
		local lvl = GetLevel(sceneId, selfId);
...
function x808038_OnEnumerate( sceneId, selfId, targetId )	
	AddNumText(sceneId, x808038_g_ScriptId, "Ho’t µng thu thßp chÊ k˝", 6, 1);
	AddNumText(sceneId, x808038_g_ScriptId, "C·c ho’t µng gi˙p thu thßp chÊ k˝", 0, 2);
end
