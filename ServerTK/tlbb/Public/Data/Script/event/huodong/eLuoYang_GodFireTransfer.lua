...
--ÈÎÎñÊÇ·ñÒÑ¾­Íê³É
x808080_g_IsMissionOkFail	= 0		--ÈÎÎñ²ÎÊıµÄµÚ0Î»

--ÈÎÎñÎÄ±¾ÃèÊö
x808080_g_MissionName			= "Nhi®m vø Thö và Rùa"
--ÈÎÎñÃèÊö
...
x808080_g_Custom	= { {id="Ğã cùng LÕc Dß½ng Tri®u Minh Thành ğ¯i thoÕi ",num=1}, {id="Ğã cùng Tô Châu Løc Sî Tranh ğ¯i thoÕi ",num=1}, {id="Ğã cùng ĞÕi Lı Vß½ng Nhßşc Vû ğ¯i thoÕi ",num=1}, {id=" Ğã châm lØa vào nhæng n½i có ng÷n ğèn cüa Kh±ng Minh",num=24} }
--MisDescEnd

--ÈÎÎñÊÇ·ñÍê³É
x808080_g_Mission_IsComplete = 0		--ÈÎÎñ²ÎÊıµÄµÚ0Î»
--»ğ°ÑÂåÑô´ò¿¨±ê¼Ç
x808080_g_RecordIdx 				 = 1		--ÈÎÎñ²ÎÊıµÄµÚ1Î» 1.ÂåÑô 2.ËÕÖİ 3.´óÀí
--¿×Ã÷µÆÍê³ÉÊıÁ¿±ê¼Ç
x808080_g_KongMing_Lighten	 = 4		--ÈÎÎñ²ÎÊıµÄµÚ4Î»
--ÈÎÎñ½Å±¾ºÅ¼ÇÂ¼
x808080_g_MissScriptID_Idx	 = 5		--ÈÎÎñ²ÎÊıµÄµÚ5Î»
--ÈÎÎñ·¢²¼NPC±ê¼Ç
x808080_g_AcceptNPC_Idx			 = 6		--ÈÎÎñ²ÎÊıµÄµÚ6Î» 1.ÂåÑôNPC 2.ËÕÖİNPC 3.´óÀíNPC
--¹êÍÃÈüÅÜÈÎÎñÃ¿ÌìÂÖÊı±ê¼Ç
x808080_g_RoundIndex 				 = 7		--ÈÎÎñ²ÎÊıµÄµÚ7Î»


x808080_g_AcceptMission_IDX		= 520	--¹êÍÃÈüÅÜ½ÓÊÕÈÎÎñË÷Òı
x808080_g_CompleteMission_IDX	= 521	--»ğ°ÑÌá½»ÈÎÎñË÷Òı
x808080_g_MissionInfo_IDX			= 522	--ÈÎÎñËµÃ÷Ë÷Òı

x808080_g_Jion_Count = 0						--±¨ÃûÈËÊı±ê¼Ç
x808080_g_Join_CountMax = 50				--±¨ÃûÈËÊıÉÏÏŞ

x808080_g_KongMingLighten_MaxCnt	=	24	-- ¹êÍÃÈüÅÜ»î¶¯ĞèÒªµãÈ¼µÄ¿×Ã÷µÆ×ÜÊı

--ËùÓµÓĞµÄÊÂ¼şIDÁĞ±í
x808080_g_EventList	= {}

x808080_g_Impact_Accept_Mission 	= 47	-- ½ÓÊÜÈÎÎñÊ±µÄÌØĞ§ID
x808080_g_Impact_Light_Complete 	= 38	-- µãÁÁ¿×Ã÷µÆÊ±µÄÌØĞ§
x808080_g_Impact_Transport_Mission = 113 -- ÔËÊä×´Ì¬ÌØĞ§
x808080_g_Impact_GodOfFire_Mission = 5929 -- ¹êÍÃÈüÅÜ×´Ì¬ÌØĞ§
x808080_g_PlayerSlow_LVL					= 20	-- ½ÓÊÜÈÎÎñµÄ×îµÍµÈ¼¶
...
x808080_g_ItemId = { id=40004444,num=1 }				-- ÈÎÎñÎïÆ·
...
function x808080_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if LuaFnGetName( sceneId, targetId ) ~= x808080_g_Name then
		x808080_NotifyTip( sceneId, selfId, "Không ch¤p nh§n nhi®m vø" )					
		return 0
	end
...
function x808080_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText( sceneId, x808080_g_ScriptId, "Ta mu¯n tham gia thi Rùa và Thö", 7, x808080_g_AcceptMission_IDX )
	AddNumText( sceneId, x808080_g_ScriptId, "Ta ğã hoàn thành nhi®m vø Rùa và Thö", 7, x808080_g_CompleteMission_IDX )
	AddNumText( sceneId, x808080_g_ScriptId, "Gi¾i thi®u hoÕt ğµng Rùa và Thö", 11, x808080_g_MissionInfo_IDX )
...
function x808080_OnAbandon( sceneId, selfId )
  ...
  if HaveItem(sceneId, selfId, x808080_g_ItemId.id) > 0 then
  	if LuaFnGetAvailableItemCount(sceneId, selfId, x808080_g_ItemId.id) >= x808080_g_ItemId.num then
	  	...
	  	x808080_NotifyTip( sceneId, selfId, "V§t ph¦m không có ho£c ğã b¸ khoá" )			
			return 0
  	end
  end
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x808080_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊıÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎï
end
