...
function x808066_CheckOpenBigBox( sceneId, selfId )
	...
	if x808066_g_IsBigBoxOpening == 1 then
		...
		--Èç¹ûÊÇ±ðÈËÔÚ¿ª²¢ÇÒËûÒÑ¾­³¬Ê±ÁË....ÔòÈÃÎ»¸øÎÒÀ´¿ª....
...
function x808066_GiveSkillBook( sceneId, selfId, activatorId )
	...
	if (rand < 80) then			--80%¼¸ÂÊÔÚËùÓÐ¿ÉÄÜµôÂäµÄÊéÀïËæ»ú
	...
	if BagIndex ~= -1 then
		...
		BeginEvent( sceneId )
			AddText( sceneId, "Các hÕ nh§n ðßþc #{_ITEM"..BookID.."}" )
		EndEvent( sceneId )
		...
		local str = format( "#W#{_INFOUSR%s}#P tÕi #GThánh Thú S½n #Pl¤y ðu¯c ð¯t soi tìm #YÐÕi Bäo Sß½ng#P, trong lúc vô tình v¤p cøc ðá ð§p m£t vào #{_INFOMSG%s}.", PlayerName, ItemTransfer )
...
function x808066_GiveBOSS( sceneId, selfId, activatorId )
	...
	--¹«¸æ....
	local PlayerName = GetName(sceneId, activatorId)
	local str = format( "#GThánh Thú S½n #P, anh hùng cái thª ðoÕt bäo, #W#{_INFOUSR%s}#P c§n chiªn ðánh bÕi #YBÕch Mãng Bäo Sß½ng#P chï ð¬ cß¾p ði báu v§t t× #WBäo Sß½ng Ð°ng TØ#P mà thôi.", PlayerName )
	BroadMsgByChatPipe( sceneId, activatorId, str, 4 )

end

--**********************************
--´ó±¦Ïä±»´ò¿ªÊÂ¼þ_µôÂäÒ»¶ÑµôÂä°ü....
--**********************************
function x808066_GiveDropBox( sceneId, selfId, activatorId )
	...
	--¹«¸æ....
	local PlayerName = GetName(sceneId, activatorId)
	local str = format( "#P TÕi #GThánh Thú S½n #W#{_INFOUSR%s}#P ðã ðánh bÕi #YBÕch Mãng Bäo Sß½ng#P ánh ðèn cüa khoa báu · kh¡p m÷i n½i ! C¯ lên !", PlayerName )
	BroadMsgByChatPipe( sceneId, activatorId, str, 4 )
