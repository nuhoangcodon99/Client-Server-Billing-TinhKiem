...
function x808092_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if(ItemID == 808092000) then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x808092_g_ScriptId, "我要領取獸欄", 6, 808092002 )
			AddNumText( sceneId, x808092_g_ScriptId, "還是算了吧", 11, 808092003 )
		EndEvent( sceneId )
...
function x808092_OnEnumerate( sceneId, selfId, targetId )
	if (x808092_IsActivityTimeOut() == 0) then
		AddNumText(sceneId, x808092_g_ScriptId, "春暖花開師門贈禮活動", 1, 808092000 )
		AddNumText(sceneId, x808092_g_ScriptId, "春暖花開師門贈禮活動幫助", 11, 808092001 )	
	end
end
...
function x808092_PresentPetCage(sceneId, selfId, targetId)	
	...
	if (condition == 0) then
		...
		--公告玩家自己
		BroadMsgByChatPipe( sceneId, selfId, "你成功兌換一個一級獸欄", MSG2PLAYER_PARA )	
		
		--醒目提示
		BeginEvent( sceneId )
			AddText( sceneId, "你成功兌換一個一級獸欄" )
		EndEvent( sceneId )
	...
	elseif (condition == 1) then
		BeginEvent(sceneId)
			AddText(sceneId, "你的道具背包已經滿了，整理一下再來吧")
		EndEvent(sceneId)
		DispatchEventList(sceneId, selfId, targetId)
		return 0
	end
