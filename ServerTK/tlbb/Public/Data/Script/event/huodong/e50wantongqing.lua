	...
	if (nowYear == 2008) then
		if(nowMonth==0) then
			if(nowDate>=16 and nowDate<=27) then
				AddNumText(sceneId, x808074_g_scriptId, "#gFF0FA0Tham gia hoÕt ðµng m×ng phiên bän m¾i", 1, 1 )
				AddNumText(sceneId, x808074_g_scriptId, "Gi¾i thi®u hoÕt ðµng m×ng phiên bän m¾i", 11, 3 )
			end
		end
	end
...
function x808074_OnDefaultEvent( sceneId, selfId, targetId )
	...
	elseif TextNum == 1 then	
		...
		AddNumText(sceneId, x808074_g_scriptId, "Tôi mu¯n tham gia rút thåm", 8, 5 )
		AddNumText(sceneId, x808074_g_scriptId, "Ð¬ tôi suy nghî lÕi...", 8, 6 )
		EndEvent( sceneId )
