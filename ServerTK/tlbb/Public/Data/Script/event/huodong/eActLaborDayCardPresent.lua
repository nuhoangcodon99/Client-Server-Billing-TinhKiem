...
function x808091_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if(ItemID == 808091001) then
		BeginEvent(sceneId)
			...
			AddNumText( sceneId, x808091_g_ScriptId, "我要參加", 6, 808091003 )
			AddNumText( sceneId, x808091_g_ScriptId, "還是算了吧", 13, 808091004 )
		EndEvent( sceneId )
	...
	elseif(ItemID == 808091003) then
		...
		elseif(condition == 1) then
			BeginEvent(sceneId)
				AddText(sceneId, "對不起，師門贈禮活動已經結束兌換簽名牌了啊")
			EndEvent(sceneId)
			DispatchEventList(sceneId, selfId, targetId)
...
function x808091_OnEnumerate( sceneId, selfId, targetId )
	if (x808091_IsActivityTimeOut() == 0) then
		AddNumText( sceneId, x808091_g_ScriptId, "春暖花開師門贈禮活動", 6, 808091001 )
		AddNumText( sceneId, x808091_g_ScriptId, "春暖花開師門贈禮活動幫助", 11, 808091002 )
	end
end
...
function x808091_PresentPetCageCard(sceneId, selfId, targetId)
	...
	if (present_card_condition == 0) then
		...
		--公告玩家自己
		BroadMsgByChatPipe( sceneId, selfId, "你獲得一個獸欄贈禮簽名牌", MSG2PLAYER_PARA )	
		
		--醒目提示
		BeginEvent( sceneId )
			AddText( sceneId, "你獲得一個獸欄贈禮簽名牌。" )
		EndEvent( sceneId )
	...
	elseif(present_card_condition == 1) then		
		BeginEvent(sceneId)
			AddText(sceneId, "你的任務背包已經滿啦，先整理一下吧")
		EndEvent(sceneId)
		DispatchMissionTips( sceneId, selfId )
		return 0
	end
