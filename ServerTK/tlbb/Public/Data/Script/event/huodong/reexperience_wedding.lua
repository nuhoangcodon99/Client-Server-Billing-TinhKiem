...
--ÖØÎÂ»éÀñ½ðÇ®ºÄ·ÑÏµÊý£¬Òª³ËÒÔÐèÇóÎïÆ·¸öÊýµÃµ½×îºó½ðÇ®£¬µ¥Î»Í­
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x808122_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	...
	if opt == 1 then
		BeginEvent(sceneId)
			...
			--»ñµÃ»¨³µID£¬²¢Í¨¹ý»¨³µIDÀ´»ñµÃÖØÎÂ»éÀñ¼¶±ðµÄ×Ö·û´®
...
function x808122_ReExperienceWedding(sceneId, selfId, targetId)
	...
	--ÅÐ¶Ï»¨³µÀàÐÍ012´ÎÎª3¼¶ 45´ÎÎª4¼¶ 5678´ÎÎª5¼¶
	local busId = x808122_g_busDefaultDataId;
	local todayCount = x808122_GetTodayCount( sceneId, selfId );
	if todayCount < getn(x808122_g_busDataIds) and todayCount >= 0 then
		busId = x808122_g_busDataIds[todayCount + 1];
	...
	if busObjID and busObjID ~= -1 then
		...
		if addPassergerRet and addPassergerRet == OR_OK then
			...
			if busStartRet and busStartRet == 1 then
				...
				strText = "L²i nµi bµ(start failed) kh·i ðµng tu¥n du xa hoa th¤t bÕi"
			end
		end
		------------
		if succeeded ~= 1 then
...
function x808122_OnEnumerate( sceneId, selfId, targetId )
	if GetTime2Day() >= x808122_g_StartTime then
		AddNumText(sceneId, x808122_g_ScriptId, "Trùng Ôn Hôn L­", 6, 1);
		AddNumText(sceneId, x808122_g_ScriptId, "Liên quan Trùng Ôn Hôn L­", 11, 2);
	end
end
...
function x808122_CheckAccept( sceneId, selfId )
	...
	--ÅÐ¶Ï¾àÀë·þÎñÆ÷ÉÏÒ»´Î»¨³µÑ²ÓÎµÄÊ±¼äÊÇ·ñ³¬¹ý3·ÖÖÓ
...
--»ñµÃ±¾ÈÕÒÑ¾­ÖØÎÂ´ÎÊý£¬²»°üÀ¨Õâ´Î£¬Ã»ÓÐÖØÎÂ¾ÍÊÇ0
