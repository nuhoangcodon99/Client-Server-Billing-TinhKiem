...
function x006672_CheckAccept( sceneId, selfId, missionIndex )
	...
	if nLevel < limitLevel then
		x006672_NotifyFailBox( sceneId, selfId, -1, "    Kinh nghi®m giang h° cüa các hÕ, sþ không chiªn th¡ng ðßþc, ðþi" .. limitLevel .. "Sau khi thång c¤p t¾i tìm ta" )
		return 0
	end
...
function x006672_RewardRelationShip( sceneId, selfId, missionIndex, targetId )
	...
	if value > x006672_g_MaxRelationship then
		...
		elseif value == 6500  then
			BeginEvent(sceneId)
				local strText = "Các hÕ và#G" .. szNpcName .. "#WQuan h® ðã phát tri¬n ðªn mÑc#Yb¢ng hæu vào sinh ra tØ#W."
				AddText(sceneId,strText);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
