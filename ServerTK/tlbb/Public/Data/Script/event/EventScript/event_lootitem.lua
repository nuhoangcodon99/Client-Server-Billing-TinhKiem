...
function x006667_OnSubmit( sceneId, selfId, targetId, selectRadioId, missionIndex )
	...
	if x006667_CheckSubmit( sceneId, selfId, missionIndex ) > 0 then
		...
		if ret <= 0 then
			x006667_NotifyFailTips( sceneId, selfId, "Tai näi ðã ð¥y, không th¬ hoàn thành nhi®m vø" )
			return
		end
		...
		if dynamicFlag ~= 2 then		
			...
			if (ItemPos == -1) then
				x006667_NotifyFailTips( sceneId, selfId, "Nhi®m vø th¤t bÕi, hãy thØ lÕi." )					
				return 0
			end
...
function x006667_OnItemChanged( sceneId, selfId, itemdataId, missionIndex )
	...
	for i = 1, killDataCount do
		...
		if itemdataId == a[i].item then
			...
			if a[i].ct >= nItemNum then
				strText = format( "Ðã ðÕt ðßþc #{_ITEM%s} %d/%d", a[i].item, nItemNum, a[i].ct )
				x006667_NotifyFailTips( sceneId, selfId, strText )
			end
		end
	end
...
function x006667_OnMissionCheck( sceneId, selfId, targetId, scriptId, index1, index2, index3, petindex, missionIndex )
	...
	if scriptId ~= x006667_g_scriptId then
		x006667_NotifyFailTips( sceneId, selfId, "V§t ph¦m ðã giao th¤t bÕi" )					
		return 0
	end
			...
			if submitcnt > 0 then
				...
				if ItemType == 1 then													-- ÊÇ·ñÊôÓÚ¿ÉÒÔ¼ø¶¨µÄÎïÆ·
					...
					if LuaFnIsJudgeApt(sceneId, selfId, index) ~= 1 then					
						BeginEvent(sceneId)
							AddText(sceneId, "Thiªt b¸ chßa ðßþc giám ð¸nh, có l¨ nó là mµt #Gcñc ph¦m#W à ? Ta không th¬ l¤y thiªt b¸ có giá tr¸ h½n à ?")
						EndEvent( )
						DispatchEventList( sceneId, selfId, targetId )
						return 0
					end
				...
				if res == 0 then
					x006667_NotifyFailTips( sceneId, selfId, "Không trä ðßþc nhi®m vø" )					
					return 0
				end
				...
				x006667_NotifyFailTips( sceneId, selfId, "Không trä ðßþc nhi®m vø" )					
				BeginEvent(sceneId)
					AddText(sceneId, "Không ðúng các v§t ph¦m ta c¥n")
					EndEvent( )
				DispatchEventList( sceneId, selfId, targetId )
	...
	x006667_NotifyFailTips( sceneId, selfId, "V§t ph¦m giao th¤t bÕi" )					
	return 0
