...
function x006669_CheckAccept( sceneId, selfId, missionIndex )
	...
	if nLevel < limitLevel then
			TAddText(sceneId, "Kinh nghiÆm giang h∞ c¸a c·c h’, s˛ khÙng chi™n th°ng ﬂ˛c, ˛i".. tostring(limitLevel) .."Sau khi thÂng c§p tæi tÏm ta")
		TEndEvent()
...
function x006669_OnSubmit( sceneId, selfId, targetId, selectRadioId, missionIndex )
	if x006669_CheckSubmit(sceneId, selfId, missionIndex) > 0 then
		...
		if ret > 0 then
			...
			--»ŒŒÒΩ±¿¯√ª”–º”≥…π¶
			BeginEvent(sceneId)
				AddText(sceneId, "KhÙng th¨ Ho‡n t§t nhiÆm v¯");
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
		end		
	end
