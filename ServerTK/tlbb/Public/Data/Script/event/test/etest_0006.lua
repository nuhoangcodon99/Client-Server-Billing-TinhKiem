...
local  PlayerName=""
--ÈÎÎñÃû
x804006_g_missionName="Ğào ğßşc Di®p nhân sâm tÑ ph¦m"
x804006_g_missionText_0="Có biªt công hi®u kÏ di®u cüa nhân sâm không?"
x804006_g_missionText_1="Nghe nói · Chung Nam S½n cách không ğây không xa có th¬ ğào ğßşc nhân sâm nghìn nåm"

x804006_g_missionText_2="Ngß½i là ai? T¾i c¯c ta làm gì?"
  ...
  --if (IsMissionHaveDone(sceneId,selfId,x804006_g_MissionId) > 0 ) then
	...
	if( IsHaveMission(sceneId,selfId,x804006_g_MissionId) > 0)  then
		if x804006_CheckSubmit(sceneId,selfId) == 1 then
			...
			BeginEvent(sceneId)
			AddText(sceneId,x804006_g_missionName)
			AddText(sceneId,"Ngß½i không có ğÕo cø nhi®m vø 20001001")
			AddMoneyBonus( sceneId, x804006_g_MoneyBonus )
...
function x804006_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x804006_CheckSubmit( sceneId, selfId ) == 1 then
		...
		--ÈÎÎñ½±ÀøÃ»ÓĞ¼Ó³É¹¦
			PrintStr("Không gian tay näi không ğü")
		end
