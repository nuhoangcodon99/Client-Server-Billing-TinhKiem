...
local  PlayerName=""
--ÈÎÎñÃû
x804009_g_missionName="TØ ğàn mµc"
x804009_g_missionText_0="TØ Hình Møc, TØ Hình Møc có th¬ dùng làm thu¯c"
x804009_g_missionText_1="Ğào 3 t¤c ğ¤t"

x804009_g_missionText_2="Ngß½i là ai? T¾i c¯c ta làm gì?"
...
function x804009_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if( IsHaveMission(sceneId,selfId,x804009_g_MissionId) > 0)  then
		if x804009_CheckSubmit(sceneId,selfId) == 1 then
			...
			BeginEvent(sceneId)
			AddText(sceneId,x804009_g_missionName)
			AddText(sceneId,"Không có ğÕo cø nhi®m vø 20001003")
			EndEvent( )
  ...
  elseif x804009_CheckAccept(sceneId,selfId) > 0 then
		...
		playname = format("Danh xßng cüa ngß¶i ch½i là : %s\n",name)
...
function x804009_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x804009_CheckSubmit( sceneId, selfId ) then
