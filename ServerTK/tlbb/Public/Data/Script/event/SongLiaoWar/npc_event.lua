...
--************************--
x910072_g_On_Event_Time=19 							--Th¶i gian chiªn trß¶ng
...
--************************--
x910072_g_LimitedMember=0							--S¯ ngß¶i t¯i thi¬u m²i bên
...
function x910072_OnWarStartTime(sceneId)														
	...
	if nHuman_Song<x910072_g_LimitedMember or nHuman_Liao<x910072_g_LimitedMember or abs(nHuman_Song-nHuman_Liao)>x910072_g_MemberCompare then		--Không ðü ði«u ki®n tham chiªn
		AddGlobalCountNews(sceneId,"#RCuµc chiªn #GT¯ng Liêu#R l¥n này quân s¯ hai bên không ðÕt yêu c¥u ð« ra, nên tr§n ð¤u ðã b¸ hüy bö!")
	else
		AddGlobalCountNews(sceneId,"#RCuµc chiªn #GT¯ng Liêu#R ðã b¡t ð¥u. Quân s¯ bên T¯ng là #G"..nHuman_Song.."#R, quân s¯ bên Liêu là #G"..nHuman_Liao.."#R. Không biªt l¥n này ph¥n th¡ng s¨ nghiêng v« bên nào ðây? Chúng ta cùng dñ ðoán nhé!")
...
function x910072_OnPlayerEnter(sceneId,playerId)														
	...
	if nDay~=GetMissionData(sceneId,playerId,MD_SONGLIAO_LAST_DAY_TIME) then																--Không phäi th¶i gian hoÕt ðµng l¥n này
		...
		NewWorld(sceneId,playerId,x910072_g_Map_Out,x910072_g_Out_Pos_X,x910072_g_Out_Pos_Y)												--Ðßa ngß¶i ch½i v« ð¸a ði¬m báo danh ban ð¥u
		return
	end
	--*****************--
	local nHours=GetHour()
...
function x910072_KillPlayer(sceneId,killerId,diedId)
	...
	local Bonus=Point_Died+1																												--S¯ ði¬m thß·ng cho Killer
...
function x910072_OnExitTime(sceneId)														
	...
	elseif Point_Song<Point_Liao then
		...
		Winner="No One"																											--Hai bên hòa
		Guild_Winner=-1																											--Không ðµi nào chiªn th¡ng
	end
	--*****************--
	if Winner~="No One" then
		AddGlobalCountNews(sceneId,"#RTrong tr§n ð¤u #GT¯ng Liêu#R rñc lØa v×a di­n ra, phe #G"..Winner.."#R ðã giành th¡ng lþi tuy®t ð¯i. Nhân sÛ phe #G"..Lost.."#R l¥n sau hãy c¯ g¡ng h½n!")
	else
		AddGlobalCountNews(sceneId,"#RTrong tr§n ð¤u #GT¯ng Liêu#R rñc lØa v×a di­n ra, kªt quä cä hai phe #GT¯ng#R và #GLiêu#R hòa nhau v¾i cùng ði¬m s¯. Hy v÷ng l¥n sau s¨ có bên vßþt trµi h½n!")
	end
	--*****************--
	local nHuman=LuaFnGetCopyScene_HumanCount(sceneId)																			--T±ng s¯ ngß¶i ch½i trong chiªn trß¶ng
	for i=0,nHuman-1 do
		...
		local str=format("#Y[Chiªn trß¶ng T¯ng Liêu]#rTrong tr§n ð¤u #GT¯ng Liêu#W v×a r°i, các hÕ dành ðß½c #G%d#W ði¬m tích lûy, ðÑng · v¸ trí #G%d#W, hãy ðªn g£p ngß¶i phø trách chiªn trß¶ng ð¬ nh§n thß·ng!",nPoint,nRank)								
		...
		if Flag==Guild_Winner then
			local str=format("#Y[Chiªn trß¶ng T¯ng Liêu]#rTrong tr§n ð¤u #GT¯ng Liêu#W v×a r°i, phe cüa các hÕ là phe #G%s#W ðã dành th¡ng lþi chung cuµc, dçn ð¥u ði¬m s¯. Chúc m×ng các hÕ cùng toàn th¬ thành viên phe #G%s#W!",Winner,Winner)								
			LuaFnSendSystemMail(sceneId,GetName(sceneId,HumanId),str)															--Send mail
