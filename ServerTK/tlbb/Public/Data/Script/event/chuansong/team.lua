				...
				for	i = 0, followMemberCount - 1 do
					...
					if followMembers[i] and followMembers[i] ~= -1 then
						...
						if followMembers[i] ~= selfId and IsHaveMission(sceneId, followMembers[i], 4021) > 0 then
							x400900_NotifyFailTips(sceneId, selfId, "Các hÕ và bÕn bè vçn · trong trÕng thái Tào v§n, không th¬ chuy¬n ð±i.")
							return
						end
						
						if followMembers[i] ~= selfId and GetItemCount(sceneId, followMembers[i], x400900_g_Yinpiao) >=1 then
							x400900_NotifyFailTips(sceneId, selfId, "Các hÕ và bÕn bè vçn · trong trÕng thái bào thß½ng, không th¬ chuy¬n ð±i.")
							return
						end
						...
						if memberHasDRideFlag and memberHasDRideFlag == 1 then
							...
							if memberIsDRideMountOwner and memberIsDRideMountOwner == 1 then
								...
								xx400900_NotifyFailTips(sceneId, selfId, "2 ngß¶i cßÞi không th¬ cùng t°n tÕi v¾i t± ðµi.")
								return
							end
						end
			...
			if isDRideMountOwner and isDRideMountOwner == 1 then
				...
				if drideTargetID and drideTargetID ~= -1 then
					...
					if drideTargetID ~= selfId and IsHaveMission(sceneId, drideTargetID, 4021) > 0 then
						x400900_NotifyFailTips(sceneId, selfId, "Các hÕ ho£c trong nhóm cüa các hÕ có ð¯i phß½ng 2 ngß¶i cßÞi · trÕng thái Tào v§n, không th¬ chuy¬n ð±i.")
						return
					end
					
					if drideTargetID ~= selfId and GetItemCount(sceneId, drideTargetID, x400900_g_Yinpiao) >=1 then
						x400900_NotifyFailTips(sceneId, selfId, "Các hÕ ho£c trong tuÏ tùng cüa các hÕ có ð¯i phß½ng 2 ngß¶i cßÞi · trÕng thái bào thß½ng, không th¬ chuy¬n ð±i.")
						return
					end
