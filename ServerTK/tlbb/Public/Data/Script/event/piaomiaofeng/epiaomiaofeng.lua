...
x402263_g_FuBenTime		= 1*60/2*60	--hd edit thoi gian phu ban 30'
...
function x402263_CheckCanEnter( sceneId, selfId, targetId )
	...
	if nHumanNum > 0 then
		...
		local msg = "    Trong nhóm cüa các hÕ có "
		for i=0, nHumanNum-2 do
			msg = msg .. Humanlist[i] .. ", "
		end
		msg = msg .. Humanlist[nHumanNum-1] .. " vçn chßa ðü ðÆng c¤p 90, ta không th¬ cho các ngß½i li«u lînh ðßþc !"
		return 0, msg
	...
	if nHumanNum > 0 then
		...
		msg = msg .. Humanlist[nHumanNum-1] .. " ðã träi qua ðü l¥n thØ thách trong ngày."
		return 0, msg
...
function x402263_MakeCopyScene( sceneId, selfId )
	...
	BeginEvent(sceneId)
		if bRetSceneID>0 then
			...
			AddText(sceneId,"S¯ lßþng phø bän ðã quá hÕn, vui lòng thØ lÕi sau !");
		end
	EndEvent(sceneId)
...
function x402263_CheckHaveBOSS( sceneId )
	...
	if nBossNum > 0 then
		local msg = "Chiªn ð¤u v¾i "
		for i=0, nBossNum-2 do
			msg = msg .. BossList[i] .. ", "
		end
		msg = msg .. BossList[nBossNum-1] .. " BOSS"
		return 1, msg
	end
