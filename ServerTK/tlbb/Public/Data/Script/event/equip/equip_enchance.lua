...
function x809262_FinishEnhance( sceneId, selfId, itemidx1, itemidx2 )
	...
	if ret ~= 1 then
		...
		AddText(sceneId,"Cß¶ng hóa tinh hoa không th¬ dùng.");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	...
	if equip_level < 40 then
		...
		if gem_index ~= 30900006 and gem_index ~= x809262_g_QianghualuId then
			--¸ß¼¶Ç¿»¯¾«»ª
			str	= format( "Mu¯n cß¶ng hóa trang b¸ c¥n %s", GetItemName( sceneId, 30900006 ), GetItemName( sceneId, x809262_g_QianghualuId ) )
			BeginEvent(sceneId)
			AddText(sceneId,str);
			EndEvent(sceneId)
			DispatchMissionTips(sceneId,selfId)
	...
	if (equip_index ==10423024) or (equip_index == 10422016) then --ÖØÂ¥Óñ£¬ÖØÂ¥½ä
		if tonumber(gem_index) == 30900006 then --Ç¿»¯¾«»ª
			...
			if bindStatus == 1 then --°ó¶¨
				text = "Trang b¸ này c¯ ð¸nh không th¬ tiªn hành cß¶ng hoá";
				BeginEvent(sceneId);
	...
	if ret == -7 then
		text="Trang b¸ này không ðßþc Cß¶ng hóa"
	end
...
function x809262_DoFinishEnhance( sceneId, selfId, index )
	...
	if ret == -5 then
		text="Cß¶ng hóa trang b¸ c¥n #{_MONEY%d}, ngân lßþng trên ngß¶i các hÕ không ðü" --zchw
		text=format( text, arg0 )
	end
