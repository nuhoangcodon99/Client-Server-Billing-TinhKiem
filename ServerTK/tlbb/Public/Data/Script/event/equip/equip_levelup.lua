...
function x809263_EquipLevelUp( sceneId, selfId, nItemIndex1, nItemIndex2)
	...
	if ret ~= 1 then
		BeginEvent(sceneId)
		AddText(sceneId,"Thång c¤p Bäo ThÕch không có tác døng");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	...
	if gem_index ~= 30900008 then
		--Éý¼¶±¦Ê¯
		BeginEvent(sceneId)
		AddText(sceneId,"Trang b¸ thång c¤p c¥n thång c¤p Bäo ThÕch");
		EndEvent(sceneId)
		DispatchMissionTips(sceneId,selfId)
		return
	end
	...
	if ret == 0 then
		...
		if arg0 == 0 then
			text="Chúc m×ng!! Trang b¸ nâng c¤p thành công, mau ði xem trang b¸ m¾i sau khi thång c¤p"
		else
			text="Chúc m×ng!! Trang b¸ nâng c¤p thành công, nhßng ðÆng c¤p làm mÕnh b¸ giäm 1, gi¶ ðây ðÆng c¤p làm mÕnh trang b¸ là"..tostring(arg0)
		end
	end
	...
	if ret == -3 then
		text="Thång c¤p Bäo ThÕch không có tác døng"
	end
	...
	if ret == -8 then
		text="Trang b¸ không th¬ thång c¤p"
	end
