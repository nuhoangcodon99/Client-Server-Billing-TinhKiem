...
-- ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x600051_OnDefaultEvent( sceneId, selfId, targetId )	-- µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
----	if GetName( sceneId, targetId ) ~= x600051_g_Name then		-- ÅÐ¶Ï¸ÃnpcÊÇ·ñÊÇ¶ÔÓ¦ÈÎÎñµÄnpc
----		return
----	end
...
--		end
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x600051_g_ScriptId, "Xác ð¸nh", 8, numText+6 )
			AddNumText(sceneId, x600051_g_ScriptId, "Bö qua", 8, 6 )
		EndEvent(sceneId)
...
function x600051_PowerUptank( sceneId, selfId, targetId, SelectTankType, isA, isAdd )
	...
	--´ÓÕ½³µ»¤ÎÀÄÇÀï¿ªµÄÕ½³µÒªÅÐ¶ÏÀäÈ´Ê±¼äÊÇ·ñµ½ÁË
	...
	if 1 <= SelectTankType and SelectTankType <= 5 then
		Msg2Player(sceneId, selfId, "Các hÕ b¡t ð¥u "..x600051_g_TankName[SelectTankType]..". " , MSG2PLAYER_PARA)
	elseif 6 <= SelectTankType and SelectTankType <= 10 then
		Msg2Player(sceneId, selfId, "Các hÕ b¡t ð¥u "..x600051_g_TankName[SelectTankType-5]..". " , MSG2PLAYER_PARA)
	end
...
--**********************************
-- ÕÒµ½Ò»¸ö¿ÉÒÔ·ÅÕ½³µÍæ¼ÒselfIdµÄ¸±±¾Ë÷Òý£¬ÓÐÊýÁ¿ÅÐ¶Ï
--**********************************
function x600051_FindTankManIndex( sceneId, IsAGuild, bufftype )
	
	local TankManFirst = 0
	local TankManSecond = 0
	local ExchangeManIndex = 0 --Õâ´ÎÒª±ä»¯³ÉÕ½³µµÄmanÔÚ¸±±¾Êý¾ÝÖÐµÄË÷Òý
	...
	local AalreadyTankCount = 0 --A°ï¶ªÆúÔÚ³¡¾°ÀïµÄÕ½³µÊýÁ¿
	local BalreadyTankCount = 0 --B°ï¶ªÆúÔÚ³¡¾°ÀïµÄÕ½³µÊýÁ¿
	local Abufftype = -1
	local Bbufftype = -1
	...
	if TankManFirst == 0 or (TankManFirst ~= 0 and CallScriptFunction(402047, "HaveTankBuff", sceneId, TankManFirst) == 0) then
		if IsAGuild == 1 then
			...
			if TankManSecond ~= 0 and CallScriptFunction(402047, "HaveTankBuff", sceneId, TankManSecond) == x600051_g_AttrBuff[bufftype] then --ÒªÑ¡ÔñµÄÕ½³µÀàÐÍºÍÈËÒ»ÖÂ
				ExchangeManIndex = -1
			end
			if Abufftype+5 == bufftype then --ÒªÑ¡ÔñµÄÕ½³µÀàÐÍºÍÒ°Íâ¶ªÆúµÄÕ½³µÀàÐÍÒ»ÖÂ
				ExchangeManIndex = -1
			end
			...
			if Bbufftype == bufftype then --ÒªÑ¡ÔñµÄÕ½³µÀàÐÍºÍÒ°Íâ¶ªÆúµÄÕ½³µÀàÐÍÒ»ÖÂ
				ExchangeManIndex = -1
			end
	...
	elseif TankManSecond == 0 or (TankManSecond ~= 0 and CallScriptFunction(402047, "HaveTankBuff", sceneId, TankManSecond) == 0) then
		if IsAGuild == 1 then
			...
			if TankManFirst ~= 0 and CallScriptFunction(402047, "HaveTankBuff", sceneId, TankManFirst) == x600051_g_AttrBuff[bufftype] then --ÒªÑ¡ÔñµÄÕ½³µÀàÐÍºÍÈËÒ»ÖÂ
				ExchangeManIndex = -1
			end
			if Abufftype+5 == bufftype then --ÒªÑ¡ÔñµÄÕ½³µÀàÐÍºÍÒ°Íâ¶ªÆúµÄÕ½³µÀàÐÍÒ»ÖÂ
				ExchangeManIndex = -1
...
--**********************************
-- ÕÒµ½Ò»¸ö¿ÉÒÔ·ÅÕ½³µÍæ¼ÒselfIdµÄ¸±±¾Ë÷Òý£¬ÎÞÊýÁ¿ÅÐ¶Ï£¬Ö»ÕÒ¿ÕÍæ¼ÒË÷Òý
--**********************************
