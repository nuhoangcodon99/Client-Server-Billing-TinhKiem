...
x808004_g_Pet			=
{
	...
	[2]	= { id = 3040, name = "Nhím", itemList = { { id = 30601001, num = 20 }, { id = 30604001, num = 20 } } },
	[3]	= { id = 3050, name = "H¥u TØ", itemList = { { id = 30601001, num = 20 }, { id = 30605001, num = 20 } } },
}
...
function x808004_OnDefaultEvent( sceneId, selfId, targetId, menpai )
	...
	if key == x808004_g_Key["eqp"] then
		...
		if TryRecieveItem( sceneId, selfId, x808004_g_Eqp[mp].weapon, 1 ) >= 0 then
			str		= "#YCác hÕ ğã nh§n ğßşc #W"..GetItemName( sceneId, x808004_g_Eqp[mp].weapon ).."."
			Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
			x808004_NotifyTip( sceneId, selfId, str )
		end
		if TryRecieveItem( sceneId, selfId, x808004_g_Eqp[mp].armor, 1 ) >= 0 then
			str		= "#YCác hÕ ğã nh§n ğßşc #W"..GetItemName( sceneId, x808004_g_Eqp[mp].armor ).."."
			Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
			x808004_NotifyTip( sceneId, selfId, str )
		end
	...
	elseif key == x808004_g_Key["pet"] then
		...
		if createPetRet and createPetRet == 1 then
			...
			str		= "#YCác hÕ ğã ğoÕt ğßşc 1 con #W"..x808004_g_Pet[rnd].name.."."
			Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
			x808004_NotifyTip( sceneId, selfId, str )
	...
	elseif key == x808004_g_Key["menpai"] then
		...
		if TryRecieveItem( sceneId, selfId, x808004_g_Eqp[mp].menpai, 1 ) >= 0 then
			str		= "#YCác hÕ ğã nh§n ğßşc #W"..GetItemName( sceneId, x808004_g_Eqp[mp].menpai ).."."
			Msg2Player( sceneId, selfId, str, MSG2PLAYER_PARA )
			x808004_NotifyTip( sceneId, selfId, str )
		end
...
function x808004_OnEnumerate( sceneId, selfId, targetId )
	...
	local	fPet	= flag - fEqp * 10
	if fEqp == 0 then
		AddNumText( sceneId, x808004_g_ScriptId, "Lînh trang b¸", 4, x808004_g_Key["eqp"] )
	end
	if fPet == 0 then
		AddNumText( sceneId, x808004_g_ScriptId, "Lînh trân thú", 4, x808004_g_Key["pet"] )
	end
	if bShiZhuang <= 0 then
		AddNumText( sceneId, x808004_g_ScriptId, "Lînh th¶i trang môn phái", 4, x808004_g_Key["menpai"] )
	end
