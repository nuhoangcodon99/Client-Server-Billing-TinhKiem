...
function x311112_PetSkillLevelup( sceneId, selfId, petHid, petLid, skillindex, ItemIndexInBag )
	...
	if checkAvailable and checkAvailable == 1 then
		...
		if ConsumeMoney > 0 then
			...
			if jz == -1 then
				x311112_NotifyFailTips(sceneId, selfId, "Kh¤u tr× kim t® th¤t bÕi !")
				return
			end
		end
		...
		if EraseItem(sceneId, selfId, ItemIndexInBag) ~= 1 then
			x311112_NotifyFailTips(sceneId, selfId, "Kh¤u tr× v§t ph¦m th¤t bÕi !")
			return
		end
		...
		if IsBroadCast == 1 then
			...
			local message = format("#H#{_INFOUSR%s}#{J1}#{_INFOMSG%s}#{J2}#{_INFOMSG%s}#{J3}#G%s#P.",LuaFnGetName(sceneId, selfId),szPetTrans,ItemInfo,GetSkillName(SkillLevelUpID))--[tx43999]
...
function x311112_PetMedicineHC( sceneId, selfId, ItemIndex1, ItemIndex2, ItemIndex3, ItemIndex4, ItemIndex5)
	...
	if BagIndex ~= -1 then
		...
		x311112_NotifyFailTips(sceneId, selfId, "Kªt hþp thành công !")
