...
function x311111_OnEnumerate( sceneId, selfId, targetId, sel )
	if(sel == 6) then
		...
		if(0 == ret) then
			Msg2Player( sceneId,selfId,"Hi®n không có trân thú ðång ký",MSG2PLAYER_PARA)
		end
	elseif (sel == 1) then
		BeginUICommand(sceneId)
...
function x311111_PetSkillStudy_MenPai_Learn(sceneId, selfId, petHid, petLid, skillId)
	...
	if( 1 == ret ) then
		Msg2Player( sceneId,selfId,"H÷c t§p kÛ nång môn phái trân thú thành công",MSG2PLAYER_PARA)
	else
		Msg2Player( sceneId,selfId,"H÷c t§p kÛ nång môn phái trân thú th¤t bÕi",MSG2PLAYER_PARA)
	end
end
...
--local ret = PetDomestication(sceneId, selfId, petHid, petLid);
	...
	if checkAvailable and checkAvailable == 1 then
		...
		Msg2Player( sceneId, selfId, "Nuôi dßÞng trân thú thành công", MSG2PLAYER_PARA )
	else
		Msg2Player( sceneId, selfId, "Không th¬ thñc hi®n thao tác này trong trÕng thái giao d¸ch", MSG2PLAYER_PARA )
	end
end
...
function x311111_PetInviteFriend_Ask_NewPage(sceneId, selfId, npcId, guid1, guid2, dir)
	...
	if(0 == ret) then
		Msg2Player( sceneId,selfId,"Không có nhi«u trân thú",MSG2PLAYER_PARA)
	end
end
