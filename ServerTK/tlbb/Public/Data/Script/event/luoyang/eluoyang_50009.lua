...
--ÈÎÎñÊÇ·ñÒÑ¾­Íê³É
x250009_g_IsMissionOkFail = 0		--±äÁ¿µÄµÚ0Î»

x250009_g_PetDataID=3080	--ÈÎÎñÕäÊÞµÄ±àºÅ
--ÒÔÉÏÊÇ¶¯Ì¬**************************************************************

--ÈÎÎñ±äÁ¿µÚÒ»Î»ÓÃÀ´´æ´¢Ëæ»úµÃµ½µÄ½Å±¾ºÅ

--ÈÎÎñÎÄ±¾ÃèÊö
x250009_g_MissionName="B¡t trân thú"
x250009_g_MissionInfo="Ti¬u thß nhà tôi mu¯n 1 con Mao Lß. "  --ÈÎÎñÃèÊö
x250009_g_MissionTarget="Bang Vân Hán Nhi tìm ðßþc 1 con Mao Lß. "		--ÈÎÎñÄ¿±ê
x250009_g_ContinueInfo="Các hÕ b¡t ðßþc Mao Lß chßa?"		--Î´Íê³ÉÈÎÎñµÄnpc¶Ô»°
x250009_g_MissionComplete="Làm t¯t l¡m. "					--Íê³ÉÈÎÎñnpcËµ»°µÄ»°
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x250009_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	--Èç¹ûÒÑ½Ó´ËÈÎÎñ
...
function x250009_OnAccept( sceneId, selfId )
  ...
  if( 0 == x250009_GetTimePass( sceneId, selfId ) ) then      
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x250009_g_MissionId)			--µÃµ½ÈÎÎñµÄÐòÁÐºÅ
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ0Î»ÖÃ0 (ÈÎÎñÍê³ÉÇé¿ö)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x250009_g_ScriptId)						--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ1Î»ÖÃÎªÈÎÎñ½Å±¾ºÅ
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x250009_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
	...
	if indexpet == 255 then --Ë÷ÒýÖµ·µ»Ø255±íÊ¾¿Õ£¬Ã»Ìá½»ÕäÊÞ
		...
		if petdataid == x250009_g_PetDataID then
			--½±Àø½ðÇ®²¢ÌáÊ¾
				...
				BeginEvent(sceneId)
					AddText(sceneId,"Chúc m×ng ðã Hoàn t¤t nhi®m vø, t£ng cho các hÕ #{_MONEY"..x250009_g_MoneyBonus.."}")
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,npcid)
			ret0 = DelMission( sceneId, selfId, x250009_g_MissionId ) --É¾³ýÈÎÎñ
