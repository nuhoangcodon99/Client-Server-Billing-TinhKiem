...
--ÈÎÎñÊÇ·ñÒÑ¾­Íê³É
x250011_g_IsMissionOkFail = 0		--±äÁ¿µÄµÚ0Î»

x250011_g_PetDataID=3100	--ÈÎÎñÕäÊÞµÄ±àºÅ
--ÒÔÉÏÊÇ¶¯Ì¬**************************************************************

--ÈÎÎñ±äÁ¿µÚÒ»Î»ÓÃÀ´´æ´¢Ëæ»úµÃµ½µÄ½Å±¾ºÅ

--ÈÎÎñÎÄ±¾ÃèÊö
x250011_g_MissionName="B¡t trân thú"
x250011_g_MissionInfo="Ti¬u thß nhà tôi mu¯n 1 con H¥u TØ. "  --ÈÎÎñÃèÊö
x250011_g_MissionTarget="Bang Vân Hán Nhi tìm ðßþc 1 con H¥u TØ. "		--ÈÎÎñÄ¿±ê
x250011_g_ContinueInfo="Các hÕ b¡t ðßþc H¥u TØ chßa?"		--Î´Íê³ÉÈÎÎñµÄnpc¶Ô»°
x250011_g_MissionComplete="Làm t¯t l¡m. "					--Íê³ÉÈÎÎñnpcËµ»°µÄ»°
...
--ÈÎÎñÈë¿Úº¯Êý
--**********************************
function x250011_OnDefaultEvent( sceneId, selfId, targetId )	--µã»÷¸ÃÈÎÎñºóÖ´ÐÐ´Ë½Å±¾
	--Èç¹ûÒÑ½Ó´ËÈÎÎñ
...
function x250011_OnAccept( sceneId, selfId )
  ...
  if( 0 == x250011_GetTimePass( sceneId, selfId ) ) then      
	...
	misIndex = GetMissionIndexByID(sceneId,selfId,x250011_g_MissionId)			--µÃµ½ÈÎÎñµÄÐòÁÐºÅ
	SetMissionByIndex(sceneId,selfId,misIndex,0,0)						--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ0Î»ÖÃ0 (ÈÎÎñÍê³ÉÇé¿ö)
	SetMissionByIndex(sceneId,selfId,misIndex,1,x250011_g_ScriptId)						--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ1Î»ÖÃÎªÈÎÎñ½Å±¾ºÅ
...
--É±ËÀ¹ÖÎï»òÍæ¼Ò
--**********************************
function x250011_OnKillObject( sceneId, selfId, objdataId ,objId)--²ÎÊýÒâË¼£º³¡¾°ºÅ¡¢Íæ¼ÒobjId¡¢¹ÖÎï±íÎ»ÖÃºÅ¡¢¹ÖÎïobjId
end
	...
	if indexpet == 255 then --Ë÷ÒýÖµ·µ»Ø255±íÊ¾¿Õ£¬Ã»Ìá½»ÕäÊÞ
		...
		if petdataid == x250011_g_PetDataID then
			--½±Àø½ðÇ®²¢ÌáÊ¾
				...
				BeginEvent(sceneId)
					AddText(sceneId,"Chúc m×ng ðã Hoàn t¤t nhi®m vø, t£ng cho các hÕ #{_MONEY"..x250011_g_MoneyBonus.."}")
				EndEvent(sceneId)
				DispatchEventList(sceneId,selfId,npcid)
			ret0 = DelMission( sceneId, selfId, x250011_g_MissionId ) --É¾³ýÈÎÎñ
