...
x250036_g_eventId_begin = 0;	--µã»÷»¨³µÑ²ÓÎÈÎÎñµÄÊÂ¼þ
x250036_g_eventId_start = 1;	--µã»÷¿ªÊ¼Ñ²ÓÎµÄÊÂ¼þ
x250036_g_eventId_close = 2;	--µã»÷µÈ»áÔÙ¿ªÊ¼µÄÊÂ¼þ

x250036_g_busDataIds = {3, 4, 5};	--»¨³µµÄDataID
...
function x250036_OnAccept(sceneId, selfId, marryLevel)
	...
	misIndex = GetMissionIndexByID(sceneId, selfId, x250036_g_MissionId);			--µÃµ½ÈÎÎñµÄÐòÁÐºÅ
	if misIndex and misIndex >= 0 then
		StartMissionTimer(sceneId,selfId, x250036_g_MissionId);
		SetMissionByIndex(sceneId,selfId,misIndex, 0, 1);						--¸ù¾ÝÐòÁÐºÅ°ÑÈÎÎñ±äÁ¿µÄµÚ0Î»ÖÃ0 (ÈÎÎñÍê³ÉÇé¿ö)
		...
		Msg2Player(sceneId, selfId,"#YNh§n nhi®m vø:"..x250036_g_MissionName.."",MSG2PLAYER_PARA);	--ÁÄÌì´°¿ÚÌáÊ¾
	end
end
	...
	if misIndex and misIndex >= 0 then
		...
		if stateCode and stateCode == 2 then
			...
			BeginEvent(sceneId);
				...
				AddNumText(sceneId, x250036_g_ScriptId, "Gi¶ hãy b¡t ð¥u rß¾c", 8, x250036_g_eventId_start);
...
function x250036_OnStart(sceneId, selfId, targetId)
	...
	if misIndex and misIndex >= 0 then
		...
		x250036_MessageBox(sceneId, selfId, targetId, "Chßa tìm ðßþc nhi®m vø rß¾c ki®u hoa");
		return 0;
	end
	...
	if busObjID and busObjID ~= -1 then
		...
		if addPassergerRet and addPassergerRet == OR_OK then
			...
			if busStartRet and busStartRet == 1 then
				...
				strText = "Kh·i ðµng có l²i, rß¾c ki®u hoa th¤t bÕi, xin liên h® v¾i GM"
			end
		end
