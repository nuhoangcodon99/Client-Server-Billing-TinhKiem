...
function x212117_OnAccept( sceneId, selfId, targetId )
	...
	Msg2Player(  sceneId, selfId,"#Ynh§n nhi®m vø: t§n ði¬m nhân sñ#W",MSG2PLAYER_PARA )
...
function x212117_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	...
	if x212117_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		if CostMoney(sceneId, selfId, 50000)  == 1  then
		    ...
		    BeginEvent(sceneId)
					AddText(sceneId,"Các hÕ ðã h÷c ðßþc kÛ nång Kim chân ðµ kiªp.")
		    EndEvent( )
		    DispatchMissionTips(sceneId,selfId)
			end
			...
			Msg2Player(  sceneId, selfId,"#Yhoàn thành nhi®m vø: t§n ði¬m nhân sñ",MSG2PLAYER_PARA )
