...
x050018_g_Custom	= { {id="已燃放中秋快樂",num=1} }
...
--活动时间
x050018_g_StartDayTime = 8257   --活动开始时间 2008-9-14,包含当日
x050018_g_EndDayTime = 8282   --活动结束时间 2008-10-09,包含当日
...
function x050018_OnAccept( sceneId, selfId, targetId )
	...
	if ret <= 0 then
		Msg2Player(sceneId, selfId, "#Y你的任務日誌已經滿了", MSG2PLAYER_PARA);
		return
	end

	Msg2Player(sceneId, selfId, "#Y接受任務："..x050018_g_MissionName.."。", MSG2PLAYER_PARA);
...
function x050018_OnSubmit( sceneId, selfId, targetId, selectRadioId )
	if x050018_CheckSubmit( sceneId, selfId, selectRadioId ) == 1 then
		...
		Msg2Player( sceneId, selfId,"#Y完成任務："..x050018_g_MissionName.."。",MSG2PLAYER_PARA )
...
function x050018_OnItemUsed( sceneId, selfId, itemdataId )
	...
	if xPos and zPos and sceneId
		...
		if misParam0 and misParam0 ~= 1 then
			...
			x050018_NotifyTips(sceneId, selfId, "任務已完成");
		end
	end
end
