...
x050020_g_strGongGaoInfo = {
	...
	"#Y岳常圓#P大笑道：#W#{_INFOUSR%s}#P英雄，這三個#{_INFOMSG%s}#P多半是從#Y餘毒#P手中奪來的吧？我不會食言，這顆#{_INFOMSG%s}#P就是你的了！",
	"#Y岳常圓#P大驚：#W#{_INFOUSR%s}#P，你竟然真的從#Y紅熊王#P手中奪來這三塊#{_INFOMSG%s}#P！這顆#{_INFOMSG%s}#P說什麼也得收下了！",
	"#Y岳常圓#P看著#W#{_INFOUSR%s}#P從#Y葛榮#P手中奪來的三塊#{_INFOMSG%s}#P欣喜若狂，拿出一顆#{_INFOMSG%s}#P作為酬謝。",
...
function x050020_OnDefaultEvent( sceneId, selfId, targetId )
	...
	if NumText == 101 then
		...
		BeginEvent(sceneId)
			...
			AddNumText(sceneId, x050020_g_ScriptId, "換取", 6, 201 )
		EndEvent(sceneId)
...
function x050020_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText(sceneId, x050020_g_ScriptId, "我要變身成月餅", 6, 101 )													
end
