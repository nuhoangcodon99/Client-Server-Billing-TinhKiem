...
x050017_g_redeemList[x050017_g_eventId_redeem_55] = {count=55, level=55, item=30505128, petName="蟾宮玉兔"}
...
function x050017_OnEnumerate( sceneId, selfId, targetId )
	...
	AddNumText(sceneId, x050017_g_ScriptId, "我要兌換玉兔", 6, x050017_g_eventId_redeem);
	...
	if selEventId then
		if selEventId == x050017_g_eventId_redeem then
			BeginEvent(sceneId);
				...
				AddNumText(sceneId, x050017_g_ScriptId, "兌換攜帶等級5級的玉兔", 6, x050017_g_eventId_redeem_5);
				AddNumText(sceneId, x050017_g_ScriptId, "兌換攜帶等級45級的皓月玉兔", 6, x050017_g_eventId_redeem_45);
				AddNumText(sceneId, x050017_g_ScriptId, "兌換攜帶等級55級的蟾宮玉兔", 6, x050017_g_eventId_redeem_55);
				AddNumText(sceneId, x050017_g_ScriptId, "兌換攜帶等級65級的嫦娥玉兔", 6, x050017_g_eventId_redeem_65);
				AddNumText(sceneId, x050017_g_ScriptId, "暫時不兌換玉兔", 6, x050017_g_eventId_cancel);
			EndEvent(sceneId);
		...
		elseif selEventId == x050017_g_eventId_redeem_5
			...
			if not redeemItem then
				x050017_NotifyBox(sceneId, selfId, targetId, "未開放道具，不能兌換！");
				return 0;
			end
			...
			if not itemCount or itemCount < redeemItem.count then
				x050017_NotifyBox(sceneId, selfId, targetId, "兌換"..GetItemName(sceneId, redeemItem.item).."需要"..redeemItem.count.."個"..redeemItemName.."，您身上的"..redeemItemName.."不夠，不能兌換！");
				return 0;
			end
			...
			if not ret or ret ~= 1 then
				x050017_NotifyBox(sceneId, selfId, targetId, "對不起，您的物品欄已經沒有空間或者你正處於疲勞狀態！");
				return 0;
			end
			
			local delRet = LuaFnDelAvailableItem(sceneId, selfId, x050017_g_redeemItemId, redeemItem.count);
			if not delRet or delRet ~= 1 then
				x050017_NotifyBox(sceneId, selfId, targetId, "無法扣除"..GetItemName(sceneId, redeemItem.item).."！");
				return 0;
			end
			...
			if itemTransfer then
				if selEventId == x050017_g_eventId_redeem_45 then
					msg = "#Y岳常圓：#P各位觀眾，#W#{_INFOUSR"..GetName(sceneId, selfId).."}#l即將用他兌換的#{_INFOMSG"..itemTransfer.."}#P請出我們可愛的#G皓月玉兔#P，大家敬請期待吧！";
				elseif selEventId == x050017_g_eventId_redeem_55 then
					msg = "#Y岳常圓：#P娘子，快出來看#W#{_INFOUSR"..GetName(sceneId, selfId).."}#l又用#{_INFOMSG"..itemTransfer.."}召喚#G蟾宮玉兔#P了！";
				elseif selEventId == x050017_g_eventId_redeem_65 then
					msg = "#Y岳常圓：#P我的老天爺啊！難道是傳說中的玉兔總動員不成，#W#{_INFOUSR"..GetName(sceneId, selfId).."}#l也兌換#{_INFOMSG"..itemTransfer.."}召喚#G嫦娥玉兔#P咧！";
				else
					msg = "#Y岳常圓：#P打雷啦！下雨啦！玉兔又下凡啦！#W#{_INFOUSR"..GetName(sceneId, selfId).."}#l得到了傳說中的#{_INFOMSG"..itemTransfer.."}，終於可以召喚#G玉兔#P啦！";
				end
